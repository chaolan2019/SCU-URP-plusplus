...
<g>${V}</g>
${ct}
</svg>`}a(ea,"trendChartSvg");function aa({bands:p,palette:n,layout:c}){let d=p||[],u=fi(c,"bands",d.length),{width:k,height:P,pad:S,mobile:y}=u,A=k-S.left-S.right,g=P-S.top-S.bottom,w=d.length||1,T=Math.max(1,...d.map(m=>m.count)),C=y?Math.min(32,A/w*.62):Math.min(40,A/w*.52),b=d.map((m,v)=>{let x=S.left+(v+.5)*(A/w),_=m.count?Math.max(8,m.count/T*g):0,q=S.top+g-_,I=(.4+(1-v/(w-1))*.6).toFixed(2),L=m.range||(m.min===0?"<60":`${m.min}-${m.max===100?"100":m.max}`),O=[`${m.level||""}（绩点 ${m.gpa}）`,`百分制 ${L}`,`课程 ${m.count} 门`].join(`
