...
我认为，它是人生的真相</p>`,"    </div>","  </div>","</div>"].join("")}var te="urppp_plugin_",ud="1.0.0";function Bo({GM:o,doc:n,hostInfo:l,uiDeps:c}){let{getValue:m=()=>null,setValue:w=()=>{},xmlHttp:C,addStyle:A}=o||{},g=(typeof c=="function"?c:c&&c.openSubpanel)||null,S=new Map,f=new Map,y=new Map,L=[],_=null;function b(N,G){let tt=y.get(N);tt&&tt.forEach(bt=>{try{bt(G)}catch{}})}function h(N,G){return y.has(N)||y.set(N,new Set),y.get(N).add(G),()=>y.get(N).delete(G)}function k(N,G){return m(`${te}${N}_${G}`)}function x(N,G,tt){w(`${te}${N}_${G}`,tt)}function E(){return N=>({get:G=>k(N,G),set:(G,tt)=>x(N,G,tt),remove:G=>w(`${te}${N}_${G}`,void 0)})}function P(N,G={}){return new Promise((tt,bt)=>{if(typeof C!="function"){bt(new Error("GM_xmlhttpRequest 不可用（未授权跨域？）"));return}C({method:G.method||"GET",url:N,headers:G.headers||{},data:G.data,timeout:G.timeout||8e3,onload:ft=>ft.status>=200&&ft.status<300?tt(ft.responseText):bt(new Error(`HTTP ${ft.status}`)),onerror:()=>bt(new Error("网络错误")),ontimeout:()=>bt(new Error("超时(8s)"))})})}async function T(N,G){let tt=Array.isArray(N)?N:[N],bt=[];for(let ft=0;ft<tt.length;ft+=1){let Et=tt[ft];G&&G({stage:"downloading",index:ft+1,total:tt.length,url:Et});try{let wt=await P(Et);return G&&G({stage:"downloaded",url:Et,size:wt.length}),wt}catch(wt){bt.push(`源${ft+1}(${B(Et)})失败: ${wt&&wt.message?wt.message:wt}`),G&&G({stage:"source_failed",index:ft+1,total:tt.length,error:wt&&wt.message?wt.message:wt})}}throw new Error("所有下载源失败 → "+bt.join(" ｜ "))}function B(N){try{return new URL(N).host}catch{return N}}function M(N){let G=String(N||"").match(/\/\/\s*==UserScript==[\s\S]*?\/\/\s*==\/UserScript==/);return G?N.replace(G[0],""):N}function $(N,G){try{let tt=M(N),bt=["GM_getValue","GM_setValue","GM_xmlhttpRequest","GM_registerMenuCommand","GM_addStyle","unsafeWindow"],ft=[typeof GM_getValue=="function"?GM_getValue:void 0,typeof GM_setValue=="function"?GM_setValue:void 0,typeof GM_xmlhttpRequest=="function"?GM_xmlhttpRequest:void 0,typeof GM_registerMenuCommand=="function"?GM_registerMenuCommand:void 0,typeof GM_addStyle=="function"?GM_addStyle:void 0,typeof unsafeWindow<"u"?unsafeWindow:null];return new Function(...bt,tt)(...ft),!0}catch(tt){return console.warn("[URP++ plugin] 注入失败",G,tt),!1}}function D(N,G){let tt=f.get(N);return tt?(tt.enabled=!!G,w(`${te}${N}_enabled`,tt.enabled),b(G?"enabled":"disabled",N),!0):!1}function W(N){let G=f.get(N);return!!G&&G.enabled}function J(N){if(!N||!N.id)return!1;if(S.has(N.id)&&S.get(N.id).__urpppRegistered)return!0;let G=Object.assign({type:"plugin"},N);G.__urpppRegistered=!0,S.set(N.id,G);let tt=f.get(N.id)||{loaded:!1,enabled:!1,version:N.version||""};return tt.version=G.version||tt.version,f.set(N.id,tt),b("registered",G.id),!0}function et(N){return S.get(N)||null}function at(N){let G=[];for(let tt of S.values())(!N||tt.type===N)&&G.push(tt);return G}function ht(N){let G=f.get(N);return!!G&&G.loaded}async function U(N,G,tt){tt&&tt({stage:"start",id:N});let bt=Array.isArray(G)?G:G?[G]:rt(N),ft=await T(bt,tt);w(`${te}${N}_code`,ft),tt&&tt({stage:"injecting",id:N});let Et=$(ft,N),wt=f.get(N)||{loaded:!1,enabled:!1,version:""};return wt.loaded=Et,wt.enabled=Et,wt.code=ft,wt.version=wt.version||V(ft),f.set(N,wt),w(`${te}${N}_enabled`,Et),b("loaded",N),Et}function V(N){let G=String(N||"").match(/@version\s+(\S+)/);return G?G[1]:""}async function ot(N,G,tt){let bt=Array.isArray(G)?G:G?[G]:rt(N),ft=await T(bt,tt);w(`${te}${N}_code`,ft);let Et=V(ft),wt=f.get(N)||{loaded:!1,enabled:!1,version:""};return wt.version=Et||wt.version,wt.code=ft,f.set(N,wt),b("updated",N),{ok:!0,version:Et||wt.version}}function Q(N){let G=m(`${te}${N}_code`);if(!G)return!1;let tt=f.get(N);if(tt&&tt.loaded)return!0;let bt=$(G,N),ft=f.get(N)||{loaded:!1,enabled:!1,version:V(G)};return ft.loaded=bt,ft.enabled=bt&&m(`${te}${N}_enabled`)!==!1,ft.code=G,f.set(N,ft),b("loaded",N),bt}function it(N){let G=S.get(N);return S.delete(N),f.delete(N),w(`${te}${N}_enabled`,!1),b("unregistered",N),!!G}function rt(N){return N==="assist"?["https://raw.githubusercontent.com/chaolan2019/URP-plusplus-Repository/main/plugins/urpppp.plugin.js","https://cdn.jsdelivr.net/gh/chaolan2019/URP-plusplus-Repository@main/plugins/urpppp.plugin.js","https://gh-proxy.com/https://raw.githubusercontent.com/chaolan2019/URP-plusplus-Repository/main/plugins/urpppp.plugin.js"]:[]}let lt={protocolVersion:ud,register:J,unregister:it,get:et,list:at,loaded:ht,isEnabled:W,enable:(N,G=!0)=>D(N,G),disable:N=>D(N,!1),install:U,update:ot,bootFromCache:Q,storage:()=>m&&{get:N=>m(N),set:(N,G)=>w(N,G)},pluginStorage:N=>E()(N),request:P,addStyle:N=>{try{A&&A(N)}catch{}},log:(...N)=>{console.log("[URP++ plugin]",...N)},on:h,emit:b,hostInfo:Object.assign({name:"SCU URP++"},l||{}),getSubpanel:()=>g};try{window.__urpppPlugin=lt}catch{}try{typeof unsafeWindow<"u"&&unsafeWindow&&(unsafeWindow.__urpppPlugin=lt)}catch{}function xt(N){if(!N||!n||N.querySelector(".urppp-plugin-sec, .urpppp-entry-sec"))return;let G=n.createElement("section");G.className="urppp-set-sec urppp-plugin-sec",G.id="urppp-plugin-sec",G.innerHTML=`
      <h3>辅助插件</h3>
...
#urppp-dl-progress{position:fixed;left:18px;right:auto;bottom:24px;z-index:99999;width:min(420px,calc(100vw - 36px));box-sizing:border-box;padding:12px 16px;border-radius:var(--radius,12px);background:var(--surface,#fff);border:1px solid var(--border,#e6e8ec);box-shadow:var(--shadow,0 -6px 18px rgba(0,0,0,.14));font-family:inherit;opacity:0;transform:translateY(14px);pointer-events:none;transition:opacity .22s,transform .22s}
...
`;function Wi(){return{open:!1,mobileTab:"home",scoreAnalysisTab:"overview",profile:null,schedule:null,scores:null,catalog:null,occupancy:null,currentBuilding:null,loading:{profile:!1,schedule:!1,scores:!1,room:!1},roomError:"",roomDateOffset:0,selected:{passing:new Set,scheme:new Set},activeSchemeIdx:0,_schemeUserSelected:!1,viewWeek:0,weekLocked:!1,_termWeek:0,_termWeekResolved:!1,uiReady:!1}}function Gi(o){o.profile=null,o.schedule=null,o.scores=null,o.catalog=null,o.occupancy=null,o._termWeekResolved=!1,o._schemeUserSelected=!1,o._schemeInited=!1}function Ji({state:o,deps:n}){async function l(m){if(!m&&o.catalog&&o.catalog.length||o.loading.room)return o.catalog;o.loading.room=!0;try{n.render()}catch{}try{o.catalog=await n.loadClassroomCatalog(),o.roomError=""}catch(w){o.catalog=o.catalog||[],o.roomError=String(w&&w.message||w),console.warn("[URP++] room catalog",w)}finally{o.loading.room=!1;try{n.scheduleRender()}catch{}}return o.catalog}async function c(m){m&&Gi(o),o.loading.profile=o.loading.schedule=o.loading.scores=!0;try{let w=await n.ensureTermWeekResolved();!o.weekLocked&&w>=1&&(o.viewWeek=w)}catch{}if(n.render(),await Promise.all([(async()=>{try{o.profile&&!m||(o.profile=await n.loadProfile()),n.reconcileProfileAndScores()}catch(w){o.profile={name:"同学",majorPlan:"主修方案",majorGpa:"—",avatar:""},console.warn(w)}finally{o.loading.profile=!1,n.scheduleRender()}})(),(async()=>{try{o.schedule&&!m||(o.schedule=await n.loadSchedule())}catch(w){o.schedule={courses:[],error:String(w&&w.message||w)}}finally{if(o.loading.schedule=!1,!o.weekLocked){let w=n.getCurrentWeekNumber()||n.readRememberedTermWeek();w>=1&&(o.viewWeek=w)}n.scheduleRender()}})(),(async()=>{let w=null;try{o.scores&&!m||(o.scores=await n.loadScores(m)),w=o.scores,n.reconcileProfileAndScores(),w&&!w.error&&!w.evaluationReady&&n.enrichScoresWithEvaluation(w).then(()=>{o.scores===w&&(n.reconcileProfileAndScores(),n.scheduleRender())}).catch(C=>{console.warn("[URP++] attach evaluation",C)})}catch(C){o.scores={passing:[],schemes:[],error:String(C&&C.message||C)}}finally{o.loading.scores=!1,n.scheduleRender()}})()]),n.reconcileProfileAndScores(),!o.weekLocked){let w=n.getCurrentWeekNumber()||n.readRememberedTermWeek();w>=1&&(o.viewWeek=w)}n.scheduleRender()}return{ensureRoomCatalogLoaded:l,loadAll:c}}var ee={},_a={},Sd=["https://raw.githubusercontent.com/chaolan2019/SCU-URP-plusplus/main/calendar/calendar.json","https://cdn.jsdelivr.net/gh/chaolan2019/SCU-URP-plusplus@main/calendar/calendar.json","https://gh-proxy.com/https://raw.githubusercontent.com/chaolan2019/SCU-URP-plusplus/main/calendar/calendar.json"];function _d(o,n=6e3){return new Promise(l=>{try{if(typeof GM_xmlhttpRequest=="function"){GM_xmlhttpRequest({method:"GET",url:o,timeout:n,cache:"no-store",onload:c=>{try{l(JSON.parse(c&&c.responseText||"null"))}catch{l(null)}},onerror:()=>l(null),ontimeout:()=>l(null)});return}fetch(o,{cache:"no-store"}).then(c=>c&&c.ok?c.json():null).then(l).catch(()=>l(null))}catch{l(null)}})}var Ed=60*1e3;function Cd(){try{let o=GM_getValue("urppp_calendar_cache","");if(!o)return null;let n=JSON.parse(o);return!n||!n.terms?null:n}catch{return null}}function Pd(o,n){try{GM_setValue("urppp_calendar_cache",JSON.stringify({terms:o,lunar:n,ts:Date.now()}))}catch{}}var Nr=!1,Ea=0,$r=null;async function Wo(o,n){if(n&&(Nr=!1,Ea=0),Nr){try{o&&o()}catch{}Vi(o);return}let l=Cd();if(l&&l.terms){ee=l.terms,_a=l.lunar||{},Nr=!0;try{o&&o()}catch{}}Vi(o)}function Vi(o){if($r){$r.then(()=>{try{o&&o()}catch{}});return}let n=Date.now();if(n-Ea<Ed)return;Ea=n;let l=typeof unsafeWindow<"u"&&unsafeWindow.__urpppCalendarSources&&unsafeWindow.__urpppCalendarSources.length?unsafeWindow.__urpppCalendarSources:Sd;return $r=(async()=>{let c=null;for(let m of l)if(c=await _d(m),c&&c.terms)break;if(c&&c.terms){let m=!ee||JSON.stringify(ee)!==JSON.stringify(c.terms);if(ee=c.terms,_a=c.lunar||{},Pd(ee,_a),Nr=!0,m)try{o&&o()}catch{}}$r=null})(),$r}typeof unsafeWindow<"u"&&(unsafeWindow.__urpppCalendarReload=async o=>{Nr=!1,Ea=0,await Wo(o,!0)});var Br={term:{color:"#44616f",label:"教学/开学"},reg:{color:"#8a74bd",label:"报到"},exam:{color:"#c08a3f",label:"考试周"},holiday:{color:"#d0716a",label:"假期"},sport:{color:"#778e63",label:"运动会"}};function Ki(){let o=new Date,n=l=>String(l).padStart(2,"0");return`${o.getFullYear()}-${n(o.getMonth()+1)}-${n(o.getDate())}`}function Ho(o,n){return Math.round((Date.parse(n)-Date.parse(o))/864e5)}function Uo(o,n){let l=ee[o];if(!l||!l.start)return 0;let c=Ho(l.start,n);return c<0?0:Math.floor(c/7)+1}function Oo(o){return _a[o]||""}function Yi(o){return String(o||"").slice(5)}function zd(o){let n=o||Ki(),[l,c]=n.split("-").map(Number);return c===8&&n>="2026-08-15"||c>=9||c<=2?"autumn":"spring"}function Go(o,n){let l=o&&ee[o]?o:ee.autumn?"autumn":"",c=l?ee[l]:null,m=n||Ki();if(!c||!c.events||!c.start||!c.weeks)return{term:null,termId:l,next:null,daysLeft:null,weekNo:0,progress:0,started:!1,today:m,empty:!0};let w=c.events.map(f=>({e:f,d:Ho(m,f.start)})).filter(f=>f.d>=-0).sort((f,y)=>f.d-y.d)[0],C=w?Ho(m,w.e.start):null,A=Uo(l,m),g=Math.max(0,Math.min(100,A/c.weeks*100)),S=m>=c.start;return{term:c,termId:l,next:w,daysLeft:C,weekNo:A,progress:g,started:S,today:m}}function Zi(o,n){let l=Go(o,n);if(l.empty)return'<button type="button" class="uc-cal-summary uc-cal-empty" data-urppp-cal-open aria-label="校历数据加载失败"><span class="cal-s-right"><span class="cal-s-wk">校历数据加载失败</span></span></button>';let c=l.next?Br[l.next.e.t].color:"#c9cdd4",m=l.term;return`<button type="button" class="uc-cal-summary" data-urppp-cal-open aria-label="打开校历时间线">
  ...
  </div>`,A=m.events.slice().sort((y,L)=>y.start<L.start?-1:1),g={};A.forEach(y=>{(g[y.start.slice(0,7)]=g[y.start.slice(0,7)]||[]).push(y)});let S=y=>y===l.today?" cal-today":"",f=Object.keys(g).map(y=>{let[,L]=y.split("-");return`<div class="cal-mon">
      <div class="cal-mon-label">${Number(L)} 月</div>
      <div class="cal-mon-items">${g[y].map(_=>{let b=Br[_.t].color,h=_.end&&_.end!==_.start?"~"+Yi(_.end):"",k=Uo(l.termId,_.start)>0?`第 ${Uo(l.termId,_.start)} 周`:"开学前";return`<div class="cal-ev${S(_.start)}">
          ...
          <span class="cal-ev-tag" style="color:${b};background:${b}1a">${Br[_.t].label}</span>
  ...
  <div class="uc-bd">
    ...
    <div class="uc-sa-pane uc-sa-pane-analysis"${P?"":" hidden"}>${w()}</div>
      ...
      </div>`;return`<div class="uc-mobile"><div class="uc-card">${C(M)}</div></div>`}return o.mobileTab==="room"?`<div class="uc-mobile"><div class="uc-card"><div class="uc-hd">教室查询</div><div class="uc-bd" id="uc-room-panel">${_()}</div></div></div>`:o.mobileTab==="more"?`<div class="uc-mobile"><div class="uc-card"><div class="uc-bd">${f()}</div></div></div>`:`<div class="uc-mobile">
      ...
      <div class="uc-occ"><table class="uc-occ-table">${P}${T}</table></div>`}function h(){let x=n.ensureRoot(),E=x.querySelector("#uc-body");n.getViewWeekNumber();let P=typeof window<"u"&&window.matchMedia?window.matchMedia:null,T=P&&P("(max-width:900px)").matches,B=!o.uiReady;E.innerHTML=T?L():y(),B?(o.uiReady=!0,x.classList.remove("uc-settled"),clearTimeout(x.__ucSettleTimer),x.__ucSettleTimer=setTimeout(()=>{o.open&&x.classList.add("uc-settled")},480)):x.classList.add("uc-settled"),n.bindUI(E),n.applyPersonalDisplay(E)}function k(){if(!o.open||l)return;let x=()=>{l=0,o.open&&h()},E=typeof requestAnimationFrame=="function"?requestAnimationFrame:null;l=E?E(x):setTimeout(x,0)}return{analysisHtml:w,metricHtml:m,occupancyHtml:b,render:h,renderScheduleBoard:g,roomPickerHtml:_,scheduleRender:k,scoreSectionHtml:C}}function ns({state:o,deps:n}){function l(y,L){return!y||(y.__urpppCleanUiBindings||(y.__urpppCleanUiBindings=new Set),y.__urpppCleanUiBindings.has(L))?!1:(y.__urpppCleanUiBindings.add(L),!0)}function c(y){if(!y)return;try{n.bindScheduleExportHosts(y)}catch(_){console.warn("[URP++] schedule export menu",_)}y.querySelectorAll("[data-score]").forEach(_=>{l(_,"score")&&_.addEventListener("click",()=>A(_.getAttribute("data-score")))}),y.querySelectorAll("[data-sa-tab]").forEach(_=>{l(_,"saTab")&&_.addEventListener("click",()=>{o.scoreAnalysisTab=_.getAttribute("data-sa-tab")==="analysis"?"analysis":"overview",n.render()})}),y.querySelectorAll("[data-href]").forEach(_=>{l(_,"href")&&_.addEventListener("click",b=>{let h=_.getAttribute("data-href");h&&(b.preventDefault(),n.closeCleanMode(),location.href=h)})}),y.querySelectorAll("[data-eval-url]").forEach(_=>{l(_,"eval")&&_.addEventListener("click",b=>{let h=_.getAttribute("data-eval-url");h&&(b.preventDefault(),b.stopPropagation(),n.closeCleanMode(),location.href=h)})}),y.querySelectorAll('[data-action="room"]').forEach(_=>{l(_,"room")&&_.addEventListener("click",()=>g())}),y.querySelectorAll("[data-room-reload]").forEach(_=>{l(_,"roomReload")&&_.addEventListener("click",b=>{b.preventDefault(),b.stopPropagation(),n.ensureRoomCatalogLoaded(!0)})}),y.querySelectorAll("[data-build-path]").forEach(_=>{l(_,"building")&&_.addEventListener("click",async()=>{let b=_.getAttribute("data-build-path"),h=(_.textContent||"").trim(),k=_.getAttribute("data-cn")||"",x=_.getAttribute("data-bn")||"",E=_.closest("#uc-room-panel")||_.closest("#uc-modal-body")||null;o.roomDateOffset=0,await f({path:b,name:h,campusNumber:k,buildingNumber:x,dateOffset:0},h,E)})}),y.querySelectorAll("[data-room-day]").forEach(_=>{l(_,"roomDay")&&_.addEventListener("click",async b=>{b.preventDefault(),b.stopPropagation();let h=parseInt(_.getAttribute("data-room-day")||"0",10)||0;if(!o.currentBuilding)return;o.roomDateOffset=h;let k=Object.assign({},o.currentBuilding,{dateOffset:h}),x=_.closest("#uc-room-panel")||_.closest("#uc-modal-body")||null;await f(k,k.name||"",x)})});let L=y.querySelector("#uc-room-back");L&&(L.onclick=()=>{o.occupancy=null,o.currentBuilding=null;let _=L.closest("#uc-room-panel")||document.querySelector("#uc-room-panel")||document.querySelector("#uc-modal-body");_&&_.id==="uc-modal-body"||_&&_.id==="uc-room-panel"?(_.innerHTML=n.roomPickerHtml(),c(_)):n.render()}),y.querySelectorAll(".uc-slot.busy[data-occ]").forEach(_=>{l(_,"occupancy")&&_.addEventListener("click",b=>{b.preventDefault(),b.stopPropagation();try{let h=JSON.parse(_.getAttribute("data-occ")||"{}");w("占用详情",`
          ...
          `,"",{stack:!0})}catch{}})}),y.querySelectorAll(".uc-lesson[data-course]").forEach(_=>{l(_,"course")&&_.addEventListener("click",b=>{b.stopPropagation();try{let h=JSON.parse(_.getAttribute("data-course")||"{}"),k=`第${h.section||"?"}${h.span>1?"-"+(Number(h.section)+Number(h.span)-1):""}节`,x=(h.others||[]).map(E=>`<div class="uc-course-sub ${E.thisWeek?"":"is-fade"}">
      ...
      </tr>`}).join("");w(y==="scheme"?"方案成绩 · "+(b.title||""):"全部及格成绩",`
    ...
    `,C.appendChild(f);let y=C.querySelector("#warningInfo");y&&document.body.appendChild(y),A.forEach(x=>{let E=x.closest('.widget-container-col, [class*="col-"]');E&&(E.style.display="none")}),C.querySelectorAll(":scope > .row").forEach(x=>{x.style.display="none"});let L=f.querySelector("#urppp-stats"),_=Math.max(S.length,5);for(let x=0;x<_;x++){let E=document.createElement("div");E.className="urppp-stat-card urppp-stat-skeleton",E.innerHTML='<div class="value">-</div><div class="label">加载中</div>',L.appendChild(E)}function b(){let x=g?Array.from(g.querySelectorAll(".infobox")):[];x.length!==0&&(L.innerHTML="",x.forEach(E=>{let P=E.innerText.trim().split(/\n+/).map(et=>et.trim()).filter(et=>et),T=P[0]||"",B=P.slice(1).join(" ").replace(/更多\.\.\./g,"").trim(),$=/[\u4e00-\u9fa5]/.test(T)||T.length>5?"value urppp-stat-value-text":"value",D=E.closest("a"),W=document.createElement(D?"a":"div");D&&(W.href=D.href||"javascript:void(0)",W.onclick=D.onclick,W.style.textDecoration="none"),W.className="urppp-stat-card";let J=o.statCardPrivacyMarkup(T,B);W.innerHTML=`<div class="${$}">${J.valueHtml}</div><div class="label">${J.labelHtml}</div>`,L.appendChild(W)}))}if(b(),g){let x=new MutationObserver(()=>b());x.observe(g,{childList:!0,subtree:!0}),setTimeout(()=>x.disconnect(),5e3)}let h=f.querySelector("#urppp-left"),k=f.querySelector("#urppp-right");m(A[5],h,"我的日程安排"),m(A[0],k,"通知公告"),m(A[1],k,"我的待办任务"),m(A[2],k,"可申请业务"),m(A[3],k,"常用下载"),g&&(g.style.display="none"),c(),console.log("[URP++] 首页仪表板已重构")}return{rebuildDashboard:w,refreshHomeFullCalendar:l,scheduleHomeFullCalendarRefresh:c,wrapWidget:m}}function Ee(o){return Math.round((Number(o)||0)*100)/100}var Ld=[{key:"a",level:"A",range:"90-100",gpa:4,min:90,max:100},{key:"am",level:"A-",range:"85-89",gpa:3.7,min:85,max:89.999},{key:"bp",level:"B+",range:"82-84",gpa:3.3,min:82,max:84.999},{key:"b",level:"B",range:"78-81",gpa:3,min:78,max:81.999},{key:"bm",level:"B-",range:"75-77",gpa:2.7,min:75,max:77.999},{key:"cp",level:"C+",range:"72-74",gpa:2.3,min:72,max:74.999},{key:"c",level:"C",range:"68-71",gpa:2,min:68,max:71.999},{key:"cm",level:"C-",range:"64-67",gpa:1.7,min:64,max:67.999},{key:"dp",level:"D+",range:"60-63",gpa:1.3,min:60,max:63.999},{key:"d",level:"D",range:"60-62",gpa:1,min:60,max:62.999},{key:"f",level:"F",range:"<60",gpa:0,min:0,max:59.999}],ss={优秀:95,"A+":98,A:95,"A-":87,良好:85,"B+":83,B:79,"B-":76,中等:73,"C+":73,C:69,"C-":65,及格:62,"D+":62,D:60,不及格:50,F:50},qd=[{key:"required",label:"必修",test:o=>/必修/.test(o)},{key:"elective",label:"任选",test:o=>/任选/.test(o)},{key:"optional",label:"选修",test:o=>/选修/.test(o)},{key:"other",label:"其他",test:()=>!0}];function ls(o){let n=String(o||"").match(/^(\d{4})-(\d{4})-(\d+)/);return n?`${n[1].slice(2)}-${n[2].slice(2)}-${n[3]}`:String(o||"")}function Ca({deps:o}){let n=o.scoreToNumber,l=o.scoreToGpa;function c(b){let h=n(b);if(h!=null)return h;let k=String(b||"").trim().toUpperCase();return ss[k]!=null?ss[k]:null}function m(b){return!b||b.unevaluated?!1:c(b.score)!=null}function w(b){let h=String(b||"").match(/^(\d{4})-(\d{4})-(\d+)/);return h?[Number(h[1]),Number(h[3])]:[9999,9999]}function C(b){let h=b&&b.passing&&b.passing[0];return h&&h.courses||[]}function A(b){let h=b&&b.officialGpa,k=Number(h);return h!=null&&Number.isFinite(k)&&k>=0&&k<=5?k:null}function g(b){let h=A(b);return h??l(b.score)}function S({scorePack:b,profile:h}){let k=C(b),x=h&&h.majorGpa?String(h.majorGpa).trim():"",E=0,P=0,T=0,B=0,M=0,$=0;return k.forEach(D=>{if(!m(D))return;let W=Number(D.credit)||0,J=c(D.score);if(J==null||W<=0)return;E+=W,P+=J*W;let et=g(D);et!=null&&(T+=et*W,B+=W,D.required&&(M+=et*W,$+=W))}),{majorGpa:x,requiredGpa:Ee($?M/$:0),avgGpa:Ee(B?T/B:0),avgScore:Ee(E?P/E:0),totalCredit:Ee(E),courseCount:k.length}}function f(b){let h=new Map;return(b||[]).forEach(k=>{if(!m(k))return;let x=k.term||"未分组",E=h.get(x);E||(E={term:x,count:0,credit:0,scoreW:0,gpaW:0,gpaCredit:0},h.set(x,E));let P=Number(k.credit)||0,T=c(k.score);if(T==null||(E.count+=1,P<=0))return;E.credit+=P,E.scoreW+=T*P;let B=g(k);B!=null&&(E.gpaW+=B*P,E.gpaCredit+=P)}),Array.from(h.values()).map(k=>({term:k.term,label:ls(k.term),count:k.count,credit:Ee(k.credit),avgScore:Ee(k.credit?k.scoreW/k.credit:0),avgGpa:Ee(k.gpaCredit?k.gpaW/k.gpaCredit:0)})).sort((k,x)=>{let E=w(k.term),P=w(x.term);return E[0]-P[0]||E[1]-P[1]})}function y(b){let h=Ld.map(x=>({...x,count:0,credit:0}));(b||[]).forEach(x=>{if(!m(x))return;let E=c(x.score);if(E==null)return;let P=h.find(T=>E>=T.min&&E<=T.max);P&&(P.count+=1,P.credit+=Number(x.credit)||0)});let k=h.reduce((x,E)=>Math.max(x,E.count),1);return h.map(x=>({...x,ratio:Math.round(x.count/k*100)}))}function L(b){let h=qd.map(P=>({...P,credit:0,count:0}));(b||[]).forEach(P=>{if(!m(P))return;let T=String(P.attr||""),B=h.find(M=>M.test(T));B&&(B.credit+=Number(P.credit)||0,B.count+=1)});let k=h.reduce((P,T)=>P+T.credit,0)||1,x=h.filter(P=>P.count>0).map(P=>({key:P.key,label:P.label,credit:Ee(P.credit),count:P.count,ratio:Math.round(P.credit/k*100)})),E=x.find(P=>P.key==="required");return{items:x,requiredCredit:E?E.credit:0,requiredRatio:E?E.ratio:0}}function _({scorePack:b,profile:h}){let k=C(b);return{metrics:S({scorePack:b,profile:h}),trend:f(k),bands:y(k),share:L(k),empty:k.length===0}}return{analyzeScores:_,hasScore:m,officialGpa:A,scoreToNumberWithLevels:c,shortTerm:ls}}var ce="var(--text-secondary)",Yo="var(--border)";function de(o){return X(String(o??""))}function cs(o,n,l){let c=!!(o&&o.variant==="mobile");if(n==="trend"){if(!c)return{mobile:c,width:920,height:330,pad:{top:36,right:30,bottom:46,left:30}};let C={top:58,right:20,bottom:44,left:20},A=Math.max(56,Number(o&&o.slotWidth)||72);return{mobile:c,width:Math.max(300,C.left+C.right+Math.max(1,l)*A),height:286,pad:C}}if(!c)return{mobile:c,width:660,height:236,pad:{top:28,right:14,bottom:44,left:14}};let m={top:28,right:14,bottom:44,left:14},w=Math.max(44,Number(o&&o.slotWidth)||48);return{mobile:c,width:Math.max(320,m.left+m.right+Math.max(1,l)*w),height:236,pad:m}}function Vo({width:o,height:n,mobile:l,kind:c,label:m}){let w=l?` data-urppp-chart-layout="mobile" style="width:max(100%,${o}px);max-width:none;height:auto"`:"";return`<svg viewBox="0 0 ${o} ${n}" class="urppp-sa-chart" role="img" aria-label="${m}" data-urppp-chart-kind="${c}"${w}>`}function Pa({trend:o,palette:n,layout:l}){let c=(o||[]).filter(rt=>rt&&rt.avgScore!=null),m=cs(l,"trend",c.length),{width:w,height:C,pad:A,mobile:g}=m,S=w-A.left-A.right,f=C-A.top-A.bottom;if(!c.length)return`${Vo({...m,kind:"trend",label:"学期成绩趋势"})}</svg>`;let y=c.length,L=rt=>A.left+(rt+.5)*(S/y),_=c.map(rt=>Number(rt.avgGpa)||0),b=c.map(rt=>Number(rt.avgScore)||0),h=c.map(rt=>Number(rt.credit)||0),k=Math.max(0,Math.min(..._)-.2),x=Math.min(5,Math.max(..._)+.2),E=Math.max(0,Math.min(...b)-4),P=Math.min(100,Math.max(...b)+4),T=Math.max(1,...h),B=x-k||1,M=P-E||1,$=rt=>A.top+f-(rt-k)/B*f,D=rt=>A.top+f-(rt-E)/M*f,W=rt=>A.top+f-rt/T*f*.9,J=c.map((rt,lt)=>`${L(lt)},${$(rt.avgGpa)}`).join(" "),et=c.map((rt,lt)=>`${L(lt)},${D(rt.avgScore)}`).join(" "),at=[0,.25,.5,.75,1].map(rt=>{let lt=A.top+f-rt*f;return`<line x1="${A.left}" y1="${lt.toFixed(1)}" x2="${w-A.right}" y2="${lt.toFixed(1)}" stroke="${Yo}" stroke-width="1" stroke-dasharray="3 4"/>`}).join(""),ht=c.map((rt,lt)=>{let xt=L(lt),Z=g?Math.min(30,S/y*.42):Math.min(26,S/y*.32),ut=W(rt.credit);return`<rect x="${(xt-Z/2).toFixed(1)}" y="${ut.toFixed(1)}" width="${Z.toFixed(1)}" height="${(A.top+f-ut).toFixed(1)}" rx="3" fill="${n.credit}" opacity="0.55"/>
...
`);return`<rect class="urppp-sa-hover" x="${Z.toFixed(1)}" y="${A.top}" width="${xt.toFixed(1)}" height="${f.toFixed(1)}" fill="transparent"><title>${de(ut)}</title></rect>`}).join(""),ot=c.map((rt,lt)=>`<circle cx="${L(lt).toFixed(1)}" cy="${$(rt.avgGpa).toFixed(1)}" r="3.5" fill="${n.gpaLine}"/><text x="${L(lt).toFixed(1)}" y="${($(rt.avgGpa)-9).toFixed(1)}" text-anchor="middle" font-size="11.5" font-weight="600" fill="${n.gpaLine}">${de(rt.avgGpa)}</text>`).join(""),Q=c.map((rt,lt)=>`<circle cx="${L(lt).toFixed(1)}" cy="${D(rt.avgScore).toFixed(1)}" r="3" fill="${n.scoreLine}"/><text x="${L(lt).toFixed(1)}" y="${(D(rt.avgScore)+17).toFixed(1)}" text-anchor="middle" font-size="11.5" fill="${n.scoreLine}">${de(rt.avgScore)}</text>`).join(""),it=g?`<g font-size="12">
...
<g>${U}</g>
${it}
</svg>`}function za({bands:o,palette:n,layout:l}){let c=o||[],m=cs(l,"bands",c.length),{width:w,height:C,pad:A,mobile:g}=m,S=w-A.left-A.right,f=C-A.top-A.bottom,y=c.length||1,L=Math.max(1,...c.map(h=>h.count)),_=g?Math.min(32,S/y*.62):Math.min(40,S/y*.52),b=c.map((h,k)=>{let x=A.left+(k+.5)*(S/y),E=h.count?Math.max(8,h.count/L*f):0,P=A.top+f-E,T=(.4+(1-k/(y-1))*.6).toFixed(2),B=h.range||(h.min===0?"<60":`${h.min}-${h.max===100?"100":h.max}`),M=[`${h.level||""}（绩点 ${h.gpa}）`,`百分制 ${B}`,`课程 ${h.count} 门`].join(`
...
<line x1="${A.left}" y1="${(A.top+f).toFixed(1)}" x2="${w-A.right}" y2="${(A.top+f).toFixed(1)}" stroke="${Yo}" stroke-width="1"/>
${b}
</svg>`}function ds({items:o,requiredRatio:n,palette:l}){let A=2*Math.PI*56,g=(o||[]).filter(L=>L&&L.ratio>0),S=Math.max(0,Math.min(100,Math.round(Number(n)||0)));if(!g.length)return'<svg viewBox="0 0 150 150" class="urppp-sa-chart" role="img" aria-label="课程类型构成"></svg>';let f=-90,y=g.map(L=>{let _=L.ratio/100*A,h=`<circle cx="75" cy="75" r="56" fill="none" stroke="${l.share&&l.share[L.key]||l.required}" stroke-width="24"
  stroke-dasharray="${_.toFixed(2)} ${A.toFixed(2)}"
  stroke-linecap="butt" transform="rotate(${f.toFixed(2)} 75 75)"/>`;return f+=L.ratio/100*360,h}).join("");return`<svg viewBox="0 0 150 150" class="urppp-sa-chart" role="img" aria-label="课程类型构成">
