...
(()=>{var Bl=Object.defineProperty;var a=(p,n)=>Bl(p,"name",{value:n,configurable:!0});function Xr(p){let n=String(p).replace("#","").match(/^([a-f\d]{2})([a-f\d]{2})([a-f\d]{2})$/i);return n?{r:parseInt(n[1],16),g:parseInt(n[2],16),b:parseInt(n[3],16)}:{r:30,g:58,b:95}}a(Xr,"hexToRgb");function he(p,n,c){return"#"+[p,n,c].map(d=>Math.max(0,Math.min(255,Math.round(d))).toString(16).padStart(2,"0")).join("")}a(he,"rgbToHex");function Gt(p){let n=String(p||"").trim();return n?(n[0]!=="#"&&(n="#"+n),/^#[0-9a-fA-F]{6}$/.test(n)?n.toUpperCase():""):""}a(Gt,"normalizeHexColor");function Va(p,n){let{r:c,g:d,b:u}=Xr(p),k=1-n;return he(c*k,d*k,u*k)}a(Va,"darken");function gr(p,n){let{r:c,g:d,b:u}=Xr(p);return`rgba(${c},${d},${u},${n})`}a(gr,"alpha");function Bt(p,n,c){let d=Xr(Gt(p)||"#FFFFFF"),u=Xr(Gt(n)||"#FFFFFF"),k=Math.max(0,Math.min(1,Number(c)||0));return he(d.r+(u.r-d.r)*k,d.g+(u.g-d.g)*k,d.b+(u.b-d.b)*k)}a(Bt,"mixHex");function Ya(p,n){if(typeof p!="function")throw new TypeError(`${n} must be a function`)}a(Ya,"assertFunction");function Ue(p){if(!p||typeof p!="object")throw new TypeError("feature definition must be an object");let n=String(p.id||"").trim();if(!n)throw new TypeError("feature id is required");return Ya(p.matches,`${n}.matches`),Ya(p.mount,`${n}.mount`),Ya(p.unmount,`${n}.unmount`),Object.freeze({id:n,matches:p.matches,mount:p.mount,unmount:p.unmount})}a(Ue,"defineFeature");function Yn(p){if(!Array.isArray(p))throw new TypeError("features must be an array");let n=p.map(Ue),c=new Set;n.forEach(S=>{if(c.has(S.id))throw new Error(`duplicate feature id: ${S.id}`);c.add(S.id)});let d=null,u=null;function k(){if(!d)return;let S=d,y=u;d=null,u=null,S.unmount(y)}a(k,"unmount");function P(S={}){let y=n.find(f=>f.matches(S));if(y&&d===y&&S.lifecycleKey!==void 0&&u?.lifecycleKey===S.lifecycleKey)try{return y.mount(S),u=S,y.id}catch(f){throw k(),f}if(k(),!y)return null;try{return y.mount(S),d=y,u=S,y.id}catch(f){try{y.unmount(S)}catch{}throw f}}return a(P,"refresh"),Object.freeze({refresh:P,unmount:k,getActiveFeatureId:a(()=>d?.id||null,"getActiveFeatureId"),listFeatureIds:a(()=>n.map(S=>S.id),"listFeatureIds")})}a(Yn,"createFeatureRuntime");function at(p){return String(p||"").replace(/[&<>"']/g,n=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[n])}a(at,"escapeHtml");function Qa(p){let n=String(p||"").match(/@version\s+([0-9]+(?:\.[0-9]+){0,3}[\w\-]*)/i);return n?n[1]:""}a(Qa,"parseUserscriptVersion");function Qn(p){return String(p||"0").replace(/^v/i,"").split(/[.+\-]/).filter(Boolean).map(n=>/^\d+$/.test(n)?parseInt(n,10):n)}a(Qn,"normalizeVersionParts");function ge(p,n){let c=Qn(p),d=Qn(n),u=Math.max(c.length,d.length);for(let k=0;k<u;k+=1){let P=c[k]==null?0:c[k],S=d[k]==null?0:d[k];if(typeof P=="number"&&typeof S=="number"){if(P>S)return 1;if(P<S)return-1;continue}let f=String(P),w=String(S);if(f>w)return 1;if(f<w)return-1}return 0}a(ge,"compareVersions");var xe={base:{},coursesPath:"courses",schedulePath:"schedule",courseFields:{name:"name",teacher:"teacher",position:"position",day:"day",sections:"sections",weeks:"weeks"},scheduleFields:{morningNum:"morningNum",afternoonNum:"afternoonNum",nightNum:"nightNum",sections:"sections"}},Fl=["name","teacher","position","day","sections","weeks","code","sequence","englishName","attribute","category","credit","status","campus","building","classroom","startSection","endSection","weekList"],Dl=["morningNum","afternoonNum","nightNum","sections","sectionList"];function Ka(p){return JSON.parse(JSON.stringify(p))}a(Ka,"cloneJsonValue");function tp(p,n){return p===n||p.startsWith(`${n}.`)||n.startsWith(`${p}.`)}a(tp,"scheduleJsonPathsOverlap");function We(p,n){let c=String(p??"").trim();if(!c){if(n)return"";throw new Error("课程数组输出路径不能为空")}if(c.length>120)throw new Error("JSON 输出路径不能超过 120 个字符");let d=c.split("."),u=new Set(["__proto__","prototype","constructor"]);if(d.some(P=>!P||/^\d+$/.test(P)||/[\[\]\x00-\x1f]/.test(P)||u.has(P)))throw new Error(`JSON 输出路径包含无效片段：${c}`);return d.join(".")}a(We,"validateScheduleJsonPath");function jl(p,n){for(let c=0;c<p.length;c+=1)for(let d=c+1;d<p.length;d+=1)if(tp(p[c],p[d]))throw new Error(`${n}目标路径不能重叠：${p[c]} / ${p[d]}`)}a(jl,"validateScheduleJsonTargetPaths");function Xn(p,n,c){let d=n.split("."),u=p;for(let k=0;k<d.length;k+=1){let P=d[k];if(!Object.prototype.hasOwnProperty.call(u,P))return;if(k===d.length-1)throw new Error(`${c}输出路径与 base 字段重叠：${n}`);if(u=u[P],!u||typeof u!="object"||Array.isArray(u)){let S=d.slice(0,k+1).join(".");throw new Error(`${c}输出路径无法穿过 base 中的非对象字段：${S}`)}}}a(Xn,"validateScheduleJsonBasePath");function Kn(p,n,c){if(!p||typeof p!="object"||Array.isArray(p))throw new Error(`${c}字段映射必须是对象`);let d={};return Object.entries(p).forEach(([u,k])=>{if(!n.includes(u))throw new Error(`${c}不支持源字段：${u}`);let P=We(k,!0);P&&(d[u]=P)}),jl(Object.values(d),`${c}字段`),d}a(Kn,"validateScheduleJsonFieldMap");function Pr(p){if(!p||typeof p!="object"||Array.isArray(p))throw new Error("自定义 JSON 映射必须是对象");let n=p.base==null?{}:p.base;if(!n||typeof n!="object"||Array.isArray(n))throw new Error("base 必须是 JSON 对象");let c={base:Ka(n),coursesPath:We(p.coursesPath,!1),schedulePath:We(p.schedulePath,!0),courseFields:Kn(p.courseFields,Fl,"课程"),scheduleFields:Kn(p.scheduleFields||{},Dl,"时间表")};if(!Object.keys(c.courseFields).length)throw new Error("至少保留一个课程字段映射");if(c.schedulePath&&tp(c.schedulePath,c.coursesPath))throw new Error("课程与时间表输出路径不能重叠");return Xn(c.base,c.coursesPath,"课程"),c.schedulePath&&Xn(c.base,c.schedulePath,"时间表"),c}a(Pr,"validateScheduleJsonMapping");function fe(p){let n=String(p||"").replace(/\D/g,"").padStart(4,"0").slice(-4),c=`${n.slice(0,2)}:${n.slice(2)}`;return/^(?:[01]\d|2[0-3]):[0-5]\d$/.test(c)?c:""}a(fe,"normalizeSectionTime");function Xa(p,n,c){let d=We(n,!1).split("."),u=p;d.forEach((k,P)=>{if(P===d.length-1){u[k]=c;return}(!u[k]||typeof u[k]!="object"||Array.isArray(u[k]))&&(u[k]={}),u=u[k]})}a(Xa,"setScheduleJsonPath");function Zn(p,n){let c={};return Object.entries(n||{}).forEach(([d,u])=>{!Object.prototype.hasOwnProperty.call(p,d)||p[d]===void 0||Xa(c,u,Ka(p[d]))}),c}a(Zn,"mappedScheduleJsonObject");function Ol(p){return[p.campus,p.building,p.classroom].map(n=>String(n||"").trim()).filter(Boolean).join(" ")}a(Ol,"scheduleJsonPosition");function Hl(p){let n=Number(p.startSection)||0,c=Number(p.endSection)||n;return n<1||c<n?"":Array.from({length:c-n+1},(d,u)=>n+u).join(",")}a(Hl,"scheduleJsonSectionString");function Rl(p,n){let c=Number(n.day)||0,d=Hl(n),u=Array.from(new Set((n.weeks||[]).map(Number).filter(k=>Number.isInteger(k)&&k>=1&&k<=60))).sort((k,P)=>k-P);return c<1||c>7||!d?{error:"invalid"}:u.length?{value:{name:p.name,teacher:p.teacher,position:Ol(n),day:c,sections:d,weeks:u.join(","),code:p.code,sequence:p.sequence,englishName:p.englishName,attribute:p.attribute,category:p.category,credit:p.credit,status:p.status,campus:n.campus,building:n.building,classroom:n.classroom,startSection:n.startSection,endSection:n.endSection,weekList:u}}:{error:"weeks"}}a(Rl,"scheduleJsonCourseRecord");function Ul(p,n){let c=[];return p.courses.forEach(d=>{if(!d.arrangements.length){n.unscheduledCourses+=1;return}d.arrangements.forEach(u=>{let k=Rl(d,u);k.error==="weeks"?n.missingWeeks+=1:k.error?n.invalidArrangements+=1:c.push(k.value)})}),c}a(Ul,"buildScheduleJsonCourses");function Wl(p){let n=new Map;return(p||[]).forEach(c=>{let d=Number(c.section),u=fe(c.start),k=fe(c.end);!Number.isInteger(d)||d<1||d>20||!u||!k||n.set(d,{i:d,s:u,e:k})}),Array.from(n.values()).sort((c,d)=>c.i-d.i)}a(Wl,"buildScheduleJsonSections");function Gl(p){let n=Wl(p);if(!n.length)return{};let c={sections:JSON.stringify(n),sectionList:n};if(!n.every((u,k)=>u.i===k+1))return c;let d={morningNum:0,afternoonNum:0,nightNum:0};return n.forEach(u=>{let[k,P]=u.s.split(":").map(Number),S=k*60+P;S<720?d.morningNum+=1:S>=1080?d.nightNum+=1:d.afternoonNum+=1}),d.morningNum&&d.afternoonNum&&d.nightNum?Object.assign(c,d):c}a(Gl,"buildScheduleJsonSchedule");function Ge(p){let n={unscheduledCourses:0,missingWeeks:0,invalidArrangements:0},c=Ul(p,n);if(!c.length)throw new Error("没有符合导入格式的已排课课程");return{courses:c,schedule:Gl(p.sections),stats:n}}a(Ge,"buildScheduleJsonSource");function Je(p){let n={courses:p.courses.map(d=>({name:d.name,teacher:d.teacher,position:d.position,day:d.day,sections:d.sections,weeks:d.weeks}))},c={};return["morningNum","afternoonNum","nightNum","sections"].forEach(d=>{Object.prototype.hasOwnProperty.call(p.schedule,d)&&(c[d]=p.schedule[d])}),Object.keys(c).length&&(n.schedule=c),n}a(Je,"buildXiaoAiScheduleJson");function Ve(p,n){let c=Ka(n.base||{}),d=p.courses.map(u=>Zn(u,n.courseFields));if(Xa(c,n.coursesPath,d),n.schedulePath&&Object.keys(p.schedule).length){let u=Zn(p.schedule,n.scheduleFields);Object.keys(u).length&&Xa(c,n.schedulePath,u)}return c}a(Ve,"buildCustomScheduleJson");function ye(p){return p.getFullYear()+"-"+String(p.getMonth()+1).padStart(2,"0")+"-"+String(p.getDate()).padStart(2,"0")}a(ye,"localDateIso");function Kr(p){let n=String(p||"").match(/^(\d{4})-(\d{2})-(\d{2})$/);if(!n)return null;let c=new Date(Number(n[1]),Number(n[2])-1,Number(n[3]));return Number.isNaN(c.getTime())||ye(c)!==String(p)?null:c}a(Kr,"parseLocalIsoDate");function Za(p){let n=new Date(p.getFullYear(),p.getMonth(),p.getDate()),c=n.getDay();return n.setDate(n.getDate()-(c===0?6:c-1)),n}a(Za,"mondayOfDate");function ep(p){let n=String(p||"").match(/^(\d{4})-(\d{4})-(\d)/);if(!n)return ye(Za(new Date));let c=n[3]==="1"?Number(n[1]):Number(n[2]),d=n[3]==="1"?8:2,u=new Date(c,d,1);for(;u.getDay()!==1;)u.setDate(u.getDate()+1);return ye(u)}a(ep,"defaultSemesterMonday");function rp(p){return p.getFullYear()+String(p.getMonth()+1).padStart(2,"0")+String(p.getDate()).padStart(2,"0")+"T"+String(p.getHours()).padStart(2,"0")+String(p.getMinutes()).padStart(2,"0")+"00"}a(rp,"formatIcsLocal");function Ye(p){return String(p||"").replace(/\\/g,"\\\\").replace(/;/g,"\\;").replace(/,/g,"\\,").replace(/\r?\n/g,"\\n")}a(Ye,"escapeIcsText");function Jl(p){if(typeof TextEncoder!="function")return p;let n=new TextEncoder,c=[],d="",u=73;for(let k of String(p))n.encode(d+k).length>u&&d?(c.push(d),d=" "+k,u=74):d+=k;return d&&c.push(d),c.join(`\r
`)}a(Jl,"foldIcsLine");function Vl(p){let n=2166136261,c=String(p||"");for(let d=0;d<c.length;d+=1)n=Math.imul(n^c.charCodeAt(d),16777619);return(n>>>0).toString(16)+"@scu-urppp"}a(Vl,"scheduleUid");function ap(p){let n=new Map;return p.sections.forEach(c=>n.set(c.section,c)),n}a(ap,"scheduleSectionMap");function Yl(p){return p.toISOString().replace(/[-:]/g,"").replace(/\.\d{3}/,"")}a(Yl,"formatTimestamp");function op(p,n,c={}){let d=Kr(n);if(!d)throw new Error("第一教学周日期无效");let u=ap(p);if(!u.size)throw new Error("教务接口没有返回节次时间，无法生成 ICS");let k=Yl(c.now instanceof Date?c.now:new Date),P=0,S=["BEGIN:VCALENDAR","VERSION:2.0","PRODID:-//SCU URP++//Schedule Export//CN","CALSCALE:GREGORIAN","METHOD:PUBLISH","X-WR-CALNAME:"+Ye(p.semester.label+"课表"),"X-WR-TIMEZONE:Asia/Shanghai","BEGIN:VTIMEZONE","TZID:Asia/Shanghai","X-LIC-LOCATION:Asia/Shanghai","BEGIN:STANDARD","TZOFFSETFROM:+0800","TZOFFSETTO:+0800","TZNAME:CST","DTSTART:19700101T000000","END:STANDARD","END:VTIMEZONE"];if(p.courses.forEach(y=>y.arrangements.forEach(A=>{let f=u.get(A.startSection),w=u.get(A.endSection);!f||!w||A.weeks.forEach(T=>{let C=new Date(d);C.setDate(d.getDate()+(T-1)*7+A.day-1);let b=new Date(C),m=new Date(C),v=f.start.split(":").map(Number),x=w.end.split(":").map(Number);b.setHours(v[0],v[1],0,0),m.setHours(x[0],x[1],0,0);let _=[A.campus,A.building,A.classroom].filter(Boolean).join(" "),q=["教师："+y.teacher,"周次："+A.weekDescription,"课程号："+y.code+(y.sequence?"_"+y.sequence:""),"学分："+y.credit,"课程属性："+y.attribute].filter(L=>!/[：:]$/.test(L)).join(`
`),I=[p.semester.planCode,y.code,y.sequence,A.day,A.startSection,A.endSection,T,A.campus,A.building,A.classroom].join("|");P+=1,S.push("BEGIN:VEVENT","UID:"+Vl(I),"DTSTAMP:"+k,"SUMMARY:"+Ye(y.name),"LOCATION:"+Ye(_),"DESCRIPTION:"+Ye(q),"DTSTART;TZID=Asia/Shanghai:"+rp(b),"DTEND;TZID=Asia/Shanghai:"+rp(m),"END:VEVENT")})})),!P)throw new Error("课表中没有已安排时间的课程，无法生成 ICS");return S.push("END:VCALENDAR"),S.map(Jl).join(`\r
`)+`\r
`}a(op,"buildScheduleIcs");function np(p){let n=ap(p),c=0,d=0;return p.courses.forEach(u=>u.arrangements.forEach(k=>{k.weeks.length||(c+=1),(!n.has(k.startSection)||!n.has(k.endSection))&&(d+=1)})),{missingWeeks:c,missingTimes:d}}a(np,"scheduleIcsOmissionStats");function Ql(p){let n=String(p||"").replace(/[—–]/g,"-"),c=/单周|单数周|[（(]单[）)]/.test(n)?1:/双周|双数周|[（(]双[）)]/.test(n)?0:-1,d=new Set,u=a(k=>{let P=Number(k);P>=1&&P<=30&&(c<0||P%2===c)&&d.add(P)},"add");return n.replace(/(\d{1,2})\s*[-~至到]\s*(\d{1,2})/g,(k,P,S)=>{let y=Math.min(Number(P),Number(S)),A=Math.max(Number(P),Number(S));for(let f=y;f<=A;f+=1)u(f);return k}),(n.match(/\d{1,2}/g)||[]).forEach(u),Array.from(d).sort((k,P)=>k-P)}a(Ql,"scheduleWeeksFromDescription");function pp(p,n){let c=String(p||"").trim();if(/^[01]+$/.test(c)){let d=[];for(let u=0;u<c.length;u+=1)c.charAt(u)==="1"&&d.push(u+1);return d}return Ql(n||c)}a(pp,"scheduleWeeks");function Xl(p){let n=p&&Array.isArray(p.xkxx)?p.xkxx:[];for(let c of n){let d=Object.values(c||{});if(d.length)return d[0]}return null}a(Xl,"firstScheduleCourse");function Zr(p){let n=Xl(p);if(!n)return"";let c=Array.isArray(n.timeAndPlaceList)?n.timeAndPlaceList[0]:null;return String(n.zxjxjhh||n.executiveEducationPlanNumber||n.id&&(n.id.zxjxjhh||n.id.executiveEducationPlanNumber)||c&&(c.zxjxjhh||c.executiveEducationPlanNumber)||"").trim()}a(Zr,"schedulePlanCodeFromData");function Kl(p){let n=String(p||"").match(/^(\d{4})-(\d{4})-(\d)/);if(!n)return"学生课表";let c=n[3]==="1"?"秋季学期":n[3]==="2"?"春季学期":"学期";return n[1]+"-"+n[2]+"学年"+c}a(Kl,"semesterLabelFromPlanCode");function ip(p,n,c,d={}){let u=n||Zr(p),k=(Array.isArray(p&&p.jcsjbs)?p.jcsjbs:[]).map(y=>({section:Number(y.jc)||0,start:fe(y.kssj),end:fe(y.jssj)})).filter(y=>y.section>=1&&y.section<=20&&y.start&&y.end).sort((y,A)=>y.section-A.section),P=[];(Array.isArray(p&&p.xkxx)?p.xkxx:[]).forEach(y=>{Object.keys(y||{}).forEach(A=>{let f=y[A];if(!f)return;let w=f.id||{},T=(f.timeAndPlaceList||[]).map(C=>({day:Number(C.classDay)||0,startSection:Number(C.classSessions)||1,endSection:Math.min(12,(Number(C.classSessions)||1)+Math.max(1,Number(C.continuingSession)||1)-1),weeks:pp(C.classWeek,C.weekDescription||f.skzcs),weekDescription:String(C.weekDescription||f.skzcs||"").trim(),campus:String(C.campusName||"").trim(),building:String(C.teachingBuildingName||"").trim(),classroom:String(C.classroomName||"").trim()})).filter(C=>C.day>=1&&C.day<=7&&C.startSection>=1&&C.startSection<=12);P.push({code:String(w.coureNumber||f.zkch||"").trim(),sequence:String(w.coureSequenceNumber||f.zkxh||"").trim(),name:String(f.courseName||f.englishCourseName||A).trim(),englishName:String(f.englishCourseName||"").trim(),teacher:String(f.attendClassTeacher||"").trim(),attribute:String(f.coursePropertiesName||"").trim(),category:String(f.courseCategoryName||"").trim(),credit:Number(f.unit)||0,status:String(f.selectCourseStatusName||"").trim(),arrangements:T})})});let S=String(d.firstMonday||"").trim();return{schemaVersion:1,exportedAt:(d.now instanceof Date?d.now:new Date).toISOString(),source:c||"SCU URP++",semester:{planCode:u,label:Kl(u),firstMonday:Kr(S)?S:""},sections:k,courses:P}}a(ip,"normalizeScheduleExportData");function sp(p,n,c,d=0){let u=Math.max(0,Number(p)||0),k=Math.max(1,Math.floor(Number(n)||1)),P=Math.max(0,Math.min(k-1,Math.floor(Number(c)||0))),S=-Math.max(0,Number(d)||0),y=S+u*P/k,A=S+u*(P+1)/k;return{left:y,width:Math.max(0,A-y)}}a(sp,"scheduleCardLaneGeometry");function Qe(p,n,c){let d=[],u=String(p||""),k=Math.max(4,Number(n)||4);for(;u;)d.push({text:u.slice(0,k),kind:c}),u=u.slice(k);return d}a(Qe,"wrapField");function Xe(p,n){let c=p.slice(0,Math.max(0,n)).map(d=>({...d}));if(c.length&&c.length<p.length){let d=c[c.length-1];d.text=d.text.length>1?d.text.slice(0,-1)+"…":"…"}return c}a(Xe,"takeLines");var lp=["#2563EB","#059669","#D97706","#DC2626","#7C3AED","#0891B2","#DB2777","#4D7C0F","#EA580C","#4F46E5"];function dp(p){let n=0,c=String(p||"");for(let d=0;d<c.length;d+=1)n=n*31+c.charCodeAt(d)>>>0;return lp[n%lp.length]}a(dp,"exportCourseColor");function cp(p){let n=[];p.forEach(c=>{let d=n.findIndex(u=>u<c.startSection);d<0&&(d=n.length,n.push(0)),n[d]=c.endSection,c.lane=d}),p.forEach(c=>{c.laneCount=Math.max(1,n.length)})}a(cp,"assignScheduleLanes");function up(p){let n=p.slice().sort((u,k)=>u.startSection-k.startSection||u.endSection-k.endSection||u.course.name.localeCompare(k.course.name)),c=[],d=0;return n.forEach(u=>{c.length&&u.startSection>d&&(cp(c),c=[],d=0),c.push(u),d=Math.max(d,u.endSection)}),c.length&&cp(c),n}a(up,"layoutScheduleDay");function mp(p){let n=[];return p.courses.forEach(c=>c.arrangements.forEach(d=>{n.push({course:c,arrangement:d,startSection:d.startSection,endSection:d.endSection,day:d.day})})),n}a(mp,"scheduleExportEvents");function bp(p,n){let c=[],d=String(p||"");for(;d;)c.push(d.slice(0,n)),d=d.slice(n);return c}a(bp,"wrapScheduleFooter");function hp(p,n,c){let d=p.startSection===p.endSection?p.startSection+"节":p.startSection+"-"+p.endSection+"节",u=Qe(p.name,Math.max(5,n),"title"),k=Qe(p.teacher,Math.max(6,n+2),"teacher"),P=Qe([p.weekDescription,d].filter(Boolean).join(" · "),Math.max(6,n+2),"schedule"),S=Qe([p.campus,p.building,p.classroom].filter(Boolean).join(" "),Math.max(6,n+2),"location"),y=Math.max(1,Number(c)||1),A=S.length&&y>=2?Math.min(2,S.length):0,f=P.length&&y>=3?1:0,w=k.length&&y>=4?1:0,T=Math.max(1,y-A-f-w),C=Xe(u,T),b=y-C.length,m=Math.min(k.length,Math.max(0,b-f-A));C.push(...Xe(k,m)),b=y-C.length;let v=Math.min(P.length,Math.max(0,b-A));return C.push(...Xe(P,v)),b=y-C.length,C.push(...Xe(S,b)),C.slice(0,y)}a(hp,"scheduleImageTextLines");function Zl(p,n){let c=dp(n),d=p.colors,u=p.skin;return u==="brutal"?{fill:Bt(d.surface,c,.48),stroke:"#000000",text:"#111111",secondary:"#242424",stripe:c}:u==="flat"?{fill:Bt(d.surface,c,p.dark?.24:.16),stroke:d.text,text:d.text,secondary:d.secondary,stripe:c}:u==="editorial"?{fill:Bt(d.surface,c,p.dark?.16:.08),stroke:d.border,text:d.text,secondary:d.secondary,stripe:c}:{fill:Bt(d.surface,c,p.dark?.28:u==="organic"?.2:.14),stroke:Bt(d.border,c,p.dark?.52:.42),text:d.text,secondary:d.secondary,stripe:c}}a(Zl,"scheduleImageCourseStyle");function gp(p,n,c={}){if(!n||!n.colors||!n.shape)throw new Error("课表图片主题未解析");let d=n.colors,u=n.shape,k=c.now instanceof Date?c.now:new Date,P=1960,S=40,y=136,A=P-S*2,f=S+24,w=64,T=8,C=f+w+12,b=S+A-24,m=(b-C-T*6)/7,v=y+88,x=108,_=102,I=v+x*12-y+24,L=p.courses.filter(ot=>!ot.arrangements.length).map(ot=>ot.name),O=bp(L.join("、"),92),$=O.length?74+O.length*27:44,H=y+I+$,G=["星期一","星期二","星期三","星期四","星期五","星期六","星期日"],U=u.serif?"Georgia,Noto Serif SC,Songti SC,STSong,SimSun,serif":"Microsoft YaHei,Segoe UI,sans-serif",rt="Microsoft YaHei,Segoe UI,sans-serif",it=["soft","warm","neu"].includes(u.shadow)?' filter="url(#schedule-frame-shadow)"':"",mt=["soft","warm","neu"].includes(u.shadow)?' filter="url(#schedule-card-shadow)"':"",V=[`<svg xmlns="http://www.w3.org/2000/svg" width="${P}" height="${H}" viewBox="0 0 ${P} ${H}">`,"<defs>",`<filter id="schedule-frame-shadow" x="-20%" y="-20%" width="140%" height="150%"><feDropShadow dx="0" dy="${n.dark?10:7}" stdDeviation="${n.dark?16:11}" flood-color="${n.dark?"#000000":d.text}" flood-opacity="${n.dark?.48:.1}"/></filter>`,`<filter id="schedule-card-shadow" x="-20%" y="-20%" width="140%" height="150%"><feDropShadow dx="0" dy="3" stdDeviation="5" flood-color="${n.dark?"#000000":d.text}" flood-opacity="${n.dark?.34:.1}"/></filter>`,"</defs>",`<rect width="100%" height="100%" fill="${d.bg}"/>`,`<rect x="${S}" y="32" width="142" height="36" rx="${u.headerRadius}" fill="${d.primary}"/>`,`<text x="${S+71}" y="56" text-anchor="middle" fill="#FFFFFF" font-size="15" font-weight="700" font-family="${rt}">SCU URP++</text>`,`<text x="${S}" y="106" fill="${d.text}" font-size="36" font-weight="700" font-family="${U}">${at(p.semester.label)}课表</text>`,`<text x="${P-S}" y="54" text-anchor="end" fill="${d.secondary}" font-size="16" font-family="${rt}">${at(n.label)}</text>`,`<text x="${P-S}" y="83" text-anchor="end" fill="${d.muted}" font-size="14" font-family="${rt}">${at(k.toLocaleString("zh-CN",{hour12:!1}))}</text>`];u.shadow==="hard"&&V.push(`<rect x="${S+8}" y="${y+8}" width="${A}" height="${I}" fill="#000000"/>`),V.push(`<rect x="${S}" y="${y}" width="${A}" height="${I}" rx="${u.frameRadius}" fill="${d.surface}" stroke="${u.shadow==="hard"?"#000000":d.border}" stroke-width="${u.frameStroke}"${it}/>`),G.forEach((ot,X)=>{let ct=C+X*(m+T);V.push(`<rect x="${ct}" y="${y+22}" width="${m}" height="48" rx="${u.headerRadius}" fill="${d.input}" stroke="${d.border}" stroke-width="${u.frameStroke?1:0}"/>`,`<text x="${ct+m/2}" y="${y+53}" text-anchor="middle" fill="${d.secondary}" font-size="17" font-weight="600" font-family="${rt}">${ot}</text>`)});for(let ot=1;ot<=12;ot+=1){let X=v+(ot-1)*x;V.push(`<rect x="${f}" y="${X}" width="${w}" height="${_}" rx="${u.gridRadius}" fill="${d.input}" stroke="${d.border}" stroke-width="${u.frameStroke?1:0}"/>`,`<text x="${f+w/2}" y="${X+_/2+6}" text-anchor="middle" fill="${d.muted}" font-size="16" font-weight="600" font-family="${rt}">${ot}</text>`),G.forEach((ct,et)=>{let st=C+et*(m+T);V.push(`<rect x="${st}" y="${X}" width="${m}" height="${_}" rx="${u.gridRadius}" fill="${d.input}" stroke="${d.border}" stroke-width="${u.frameStroke?1:0}"/>`)})}[4,9].forEach(ot=>{let X=v+ot*x-3;V.push(`<line x1="${C}" y1="${X}" x2="${b}" y2="${X}" stroke="${d.primary}" stroke-opacity=".42" stroke-width="2" stroke-dasharray="10 9"/>`)});for(let ot=1;ot<=7;ot+=1)up(mp(p).filter(ct=>ct.day===ot)).forEach((ct,et)=>{let st=m/ct.laneCount,ft=C+(ot-1)*(m+T)+ct.lane*st,Z=v+(ct.startSection-1)*x,dt=st,kt=Math.max(_,(ct.endSection-ct.startSection)*x+_),wt=Zl(n,ct.course.name),Ct="course-clip-"+ot+"-"+et,N=Math.max(1,Math.floor((kt-18)/23)),Y=hp({name:ct.course.name,teacher:ct.course.teacher,weekDescription:ct.arrangement.weekDescription,startSection:ct.startSection,endSection:ct.endSection,campus:ct.arrangement.campus,building:ct.arrangement.building,classroom:ct.arrangement.classroom},Math.floor((dt-22)/16),N);V.push(`<clipPath id="${Ct}"><rect x="${ft+11}" y="${Z+8}" width="${Math.max(10,dt-22)}" height="${Math.max(18,kt-16)}" rx="${Math.max(0,u.cardRadius-5)}"/></clipPath>`,`<rect data-course-card="1" data-day="${ot}" data-start="${ct.startSection}" data-end="${ct.endSection}" x="${ft}" y="${Z}" width="${dt}" height="${kt}" rx="${u.cardRadius}" fill="${wt.fill}" stroke="${wt.stroke}" stroke-width="${u.cardStroke}"${mt}/>`),n.skin==="brutal"&&V.push(`<path d="M ${ft+dt-4} ${Z+4} V ${Z+kt-4} H ${ft+4}" fill="none" stroke="#000000" stroke-opacity=".28" stroke-width="5"/>`),n.skin==="editorial"&&V.push(`<rect x="${ft}" y="${Z}" width="6" height="${kt}" fill="${wt.stripe}"/>`),n.skin==="neu"&&V.push(`<path d="M ${ft+u.cardRadius} ${Z+1} H ${ft+dt-u.cardRadius}" stroke="#FFFFFF" stroke-opacity=".32" stroke-width="2"/>`),V.push('<g clip-path="url(#'+Ct+')">'),Y.forEach((tt,ht)=>{let bt=tt.kind==="title";V.push(`<text data-kind="${tt.kind}" x="${ft+14}" y="${Z+28+ht*23}" fill="${bt?wt.text:wt.secondary}" font-size="${bt?16:13}" font-weight="${bt?700:500}" font-family="${bt&&u.serif?U:rt}">${at(tt.text)}</text>`)}),V.push("</g>")});let Q=y+I+30;return O.length?(V.push(`<text x="${S}" y="${Q}" fill="${d.secondary}" font-size="15" font-weight="700" font-family="${rt}">未排定时间的课程</text>`),O.forEach((ot,X)=>V.push(`<text x="${S}" y="${Q+29+X*27}" fill="${d.muted}" font-size="14" font-family="${rt}">${at(ot)}</text>`))):V.push(`<text x="${S}" y="${Q}" fill="${d.muted}" font-size="14" font-family="${rt}">由 SCU URP++ 基于结构化课表数据生成</text>`),V.push("</svg>"),{svg:V.join(""),width:P,height:H,background:d.bg,theme:n}}a(gp,"buildScheduleSvg");function tc(p,n,c={}){let d=[],u=c.json||null,k=c.ics||null,P=p==="ics"?n.courses.filter(S=>!S.arrangements.length).length:0;return P&&d.push(P+" 门未排定时间的课程未写入日历"),u&&u.unscheduledCourses&&d.push(u.unscheduledCourses+" 门未排定时间的课程未写入 JSON"),u&&u.missingWeeks&&d.push(u.missingWeeks+" 个上课安排缺少周次"),u&&u.invalidArrangements&&d.push(u.invalidArrangements+" 个上课安排缺少日期或节次"),k&&k.missingWeeks&&d.push(k.missingWeeks+" 个上课安排缺少周次"),k&&k.missingTimes&&d.push(k.missingTimes+" 个上课安排缺少节次时间"),d}a(tc,"scheduleExportCompletionNotes");function Ke(p,n,c,d,u){return`<button type="button" class="urppp-export-option" role="menuitem" data-export-type="${p}"${u?" disabled":""}><i class="fa ${n}" aria-hidden="true"></i><span><strong>${c}</strong><small>${d}</small></span></button>`}a(Ke,"exportOptionHtml");function fp(p){let{document:n,window:c,ensureStyles:d,loadData:u,exportJson:k,exportIcs:P,exportPng:S,showToast:y,nativePageUrl:A,navigate:f,logger:w=console}=p;function T(x){x&&(x.classList.remove("open"),x.querySelector(".urppp-export-trigger")?.setAttribute("aria-expanded","false"))}a(T,"closeMenu");function C(){c.__urpppExportDismissBound||(c.__urpppExportDismissBound=!0,n.addEventListener("click",x=>{n.querySelectorAll(".urppp-export-wrap.open").forEach(_=>{_.contains(x.target)||T(_)})},!0),n.addEventListener("keydown",x=>{x.key==="Escape"&&n.querySelectorAll(".urppp-export-wrap.open").forEach(T)}))}a(C,"bindDismiss");async function b(x,_,q,I){if(I&&I.disabled)return;let L=I&&I.innerHTML;try{if(I&&(I.disabled=!0,I.innerHTML='<i class="fa fa-spinner fa-spin"></i> 准备中'),x==="pdf"){if(typeof q!="function")throw new Error("当前页面不提供原生 PDF 导出");await q();return}let O=await u(_),$={};if(x==="json")$.json=await k(O);else if(x==="ics")$.ics=await P(O);else if(x==="png")await S(O);else throw new Error("未知导出格式");let H=tc(x,O,$);y("课表已导出："+x.toUpperCase()+(H.length?"；"+H.join("，"):""))}catch(O){if(O&&O.message==="已取消导出")return;w.warn("[URP++] schedule export",O),y(O&&O.message||String(O),!0)}finally{I&&(I.disabled=!1,I.innerHTML=L)}}a(b,"run");function m(x={}){d();let _=x.source||"native",q=x.pdfHandler,I=typeof q=="function",L=n.createElement("span"),O=_==="native"?"导出课表":"导出";L.className="urppp-export-wrap",L.innerHTML=`<button type="button" class="urppp-export-trigger" aria-haspopup="menu" aria-expanded="false" title="导出课表"><i class="fa fa-cloud-download" aria-hidden="true"></i><span>${O}</span><i class="fa fa-angle-down" aria-hidden="true"></i></button><div class="urppp-export-menu" role="menu">${Ke("ics","fa-calendar","ICS 日历","导入系统日历或日历应用",!1)}${Ke("json","fa-code","JSON 数据","兼容小爱课程导入，可自定义格式",!1)}${Ke("png","fa-image","PNG 图片","完整学期课表高清图片",!1)}${Ke("pdf","fa-file-pdf-o","PDF",I?"使用教务系统原生导出":"仅原教务课表页面可用",!I)}${I?"":'<div class="urppp-export-guide">PDF 依赖原教务课表页面。<button type="button" data-export-native="1">前往本学期课表</button></div>'}</div>`;let $=L.querySelector(".urppp-export-trigger");$.addEventListener("click",G=>{G.preventDefault(),G.stopPropagation();let U=!L.classList.contains("open");n.querySelectorAll(".urppp-export-wrap.open").forEach(T),L.classList.toggle("open",U),$.setAttribute("aria-expanded",U?"true":"false")}),L.querySelectorAll("[data-export-type]:not(:disabled)").forEach(G=>{G.addEventListener("click",()=>{T(L),b(G.getAttribute("data-export-type"),_,q,$)})});let H=L.querySelector("[data-export-native]");return H&&H.addEventListener("click",()=>f(A)),C(),L}a(m,"createMenu");function v(x){(x&&x.querySelectorAll?x:n).querySelectorAll("[data-schedule-export-host]").forEach(q=>{q.querySelector(".urppp-export-wrap")||q.appendChild(m({source:q.getAttribute("data-schedule-export-host")||"clean"}))})}return a(v,"bindHosts"),{bindHosts:v,closeMenu:T,createMenu:m,run:b}}a(fp,"createScheduleExportUi");function xp(p){let n=a(c=>{p.querySelectorAll(".urppp-set-tab").forEach(d=>{let u=d.dataset.tab===c;d.classList.toggle("ac",u),d.setAttribute("aria-selected",u?"true":"false")}),p.querySelectorAll(".urppp-set-pane").forEach(d=>{d.classList.toggle("ac",d.dataset.pane===c)});try{let d=p.querySelector(".urppp-set-body");d&&(d.scrollTop=0)}catch{}},"switchTab");return p.querySelectorAll(".urppp-set-tab").forEach(c=>{c.addEventListener("click",()=>n(c.dataset.tab))}),p.__urpppSwitchTab=n,n}a(xp,"bindSettingsTabs");function yp(p){let{document:n,ensurePanel:c,syncPanel:d,refreshUpdateStatus:u,defaultTab:k="theme"}=p;function P(){c();let y=n.getElementById("urppp-settings-panel"),A=n.getElementById("urppp-settings-mask");if(!y||!A)return!1;d();try{u()}catch{}try{y.__urpppSwitchTab&&y.__urpppSwitchTab(k)}catch{}A.classList.remove("open"),y.classList.remove("open"),y.offsetWidth,A.classList.add("open"),y.classList.add("open");try{let f=y.querySelector(".urppp-set-body");f&&(f.scrollTop=0)}catch{}return!0}a(P,"open");function S(){let y=n.getElementById("urppp-settings-panel"),A=n.getElementById("urppp-settings-mask");y&&y.classList.remove("open"),A&&A.classList.remove("open")}return a(S,"close"),{close:S,open:P}}a(yp,"createSettingsPanelController");function vp(p){let{logoData:n,repositoryUrl:c,version:d}=p;return['<div class="urppp-set-head">','  <div class="urppp-set-title">设置</div>','  <button type="button" class="urppp-set-close" id="urppp-set-close" aria-label="关闭">×</button>',"</div>",'<div class="urppp-set-tabs" role="tablist">','  <button type="button" class="urppp-set-tab ac" data-tab="theme" role="tab" aria-selected="true">主题设置</button>','  <button type="button" class="urppp-set-tab" data-tab="skin" role="tab" aria-selected="false">主题选择</button>','  <button type="button" class="urppp-set-tab" data-tab="system" role="tab" aria-selected="false">系统设置</button>','  <button type="button" class="urppp-set-tab" data-tab="about" role="tab" aria-selected="false">关于</button>',"</div>",'<div class="urppp-set-body">','  <div class="urppp-set-pane ac" data-pane="theme">','    <section class="urppp-set-sec">',"      <h3>主题模式</h3>",'      <div class="urppp-set-modes" id="urppp-set-modes">','        <button type="button" class="urppp-set-mode" data-theme="default">简约白</button>','        <button type="button" class="urppp-set-mode" data-theme="dark">深邃暗</button>','        <button type="button" class="urppp-set-mode" data-theme="scu-red">动态配色</button>',"      </div>",'      <div class="urppp-set-follow-row">','        <button type="button" class="urppp-set-follow" id="urppp-set-follow" aria-pressed="false">跟随系统：关</button>','        <button type="button" class="urppp-set-follow" id="urppp-set-follow-dynamic" aria-pressed="false">浅色用动态配色：关</button>',"      </div>",'      <button type="button" class="urppp-set-follow" id="urppp-set-clean-default" aria-pressed="false" style="margin-top:12px;width:100%">默认进入清爽模式：关</button>','      <button type="button" class="urppp-set-follow" id="urppp-set-clean-analysis" aria-pressed="false" style="margin-top:12px;width:100%">清爽成绩分析展示：选项卡</button>','      <button type="button" class="urppp-set-follow" id="urppp-set-apple-edge" aria-pressed="true" style="margin-top:12px;width:100%">类Apple边缘线条：开</button>',"    </section>",'    <section class="urppp-set-sec" id="urppp-set-dynamic">',"      <h3>种子色</h3>",'      <p class="urppp-set-tip">选一个颜色，自动生成背景、卡片、强调色等多套方案</p>','      <div class="urppp-set-presets" id="urppp-set-presets"></div>','      <div class="urppp-set-custom">','        <input type="color" id="urppp-set-color" value="#B53434" />','        <input type="text" id="urppp-set-hex" maxlength="7" value="#B53434" spellcheck="false" />','        <button type="button" class="urppp-set-btn" id="urppp-set-gen">生成方案</button>','        <button type="button" class="urppp-set-btn ghost" id="urppp-set-save">存为预设</button>',"      </div>",'      <h3 style="margin-top:16px">配色方案</h3>','      <div class="urppp-set-schemes" id="urppp-set-schemes"></div>',"    </section>",'    <section class="urppp-set-sec" id="urppp-set-brutal" style="display:none">',"      <h3>高对比配色</h3>",'      <p class="urppp-set-tip">默认圆点使用高能粉；选择一种备用配色后，可由左上第三个圆点快速切换。</p>','      <div class="urppp-set-schemes" id="urppp-set-brutal-palettes"></div>',"    </section>","  </div>",'  <div class="urppp-set-pane" data-pane="skin">','    <section class="urppp-set-sec">',"      <h3>界面风格</h3>",'      <p class="urppp-set-tip">在同一布局上切换视觉气质。因适配规模较大，仅保证清爽模式的完整适配，如有影响请使用默认类Apple风格并选择性开启边缘线条。</p>','      <div class="urppp-theme-store-bar"><button type="button" class="urppp-set-btn ghost" id="urppp-theme-store">主题商店</button></div>','      <div id="urppp-theme-store-inline" class="urppp-store-inline" style="display:none"></div>','      <div class="urppp-skin-list" id="urppp-skin-list"></div>',"    </section>","  </div>",'  <div class="urppp-set-pane" data-pane="system">','    <section class="urppp-set-sec" id="urppp-set-privacy">',"      <h3>隐私模式</h3>",'      <div class="urppp-set-modes" id="urppp-set-privacy-modes">','        <button type="button" class="urppp-set-follow urppp-privacy-mode" data-privacy-mode="off">关闭</button>','        <button type="button" class="urppp-set-follow urppp-privacy-mode" data-privacy-mode="one">一键隐私</button>','        <button type="button" class="urppp-set-follow urppp-privacy-mode" data-privacy-mode="custom">自定义</button>',"      </div>",'      <div class="urppp-privacy-groups" id="urppp-set-privacy-custom">','        <div class="urppp-privacy-group">','          <div class="urppp-privacy-group-title">身份信息</div>','          <div class="urppp-privacy-group-fields">','            <div class="urppp-privacy-field"><input id="urppp-privacy-identity" type="checkbox" data-privacy-field="identity" aria-label="隐藏学号和证件"><label for="urppp-privacy-identity">学号/证件</label><input class="urppp-feature-input" data-privacy-value="identity" maxlength="40" aria-label="学号和证件替换内容"></div>','            <div class="urppp-privacy-field"><input id="urppp-privacy-organization" type="checkbox" data-privacy-field="organization" aria-label="隐藏学院和专业"><label for="urppp-privacy-organization">学院/专业</label><input class="urppp-feature-input" data-privacy-value="organization" maxlength="40" aria-label="学院和专业替换内容"></div>','            <div class="urppp-privacy-field"><input id="urppp-privacy-contact" type="checkbox" data-privacy-field="contact" aria-label="隐藏联系和个人信息"><label for="urppp-privacy-contact">联系/个人信息</label><input class="urppp-feature-input" data-privacy-value="contact" maxlength="40" aria-label="联系和个人信息替换内容"></div>',"          </div>","        </div>",'        <div class="urppp-privacy-group">','          <div class="urppp-privacy-group-title">学业信息</div>','          <div class="urppp-privacy-group-fields">','            <div class="urppp-privacy-field"><input id="urppp-privacy-grade" type="checkbox" data-privacy-field="grade" aria-label="隐藏成绩"><label for="urppp-privacy-grade">成绩</label><input class="urppp-feature-input" data-privacy-value="grade" maxlength="40" aria-label="成绩替换内容"></div>','            <div class="urppp-privacy-field"><input id="urppp-privacy-gpa" type="checkbox" data-privacy-field="gpa" aria-label="隐藏绩点"><label for="urppp-privacy-gpa">绩点</label><input class="urppp-feature-input" data-privacy-value="gpa" maxlength="40" aria-label="绩点替换内容"></div>','            <div class="urppp-privacy-field"><input id="urppp-privacy-credit" type="checkbox" data-privacy-field="credit" aria-label="隐藏学分"><label for="urppp-privacy-credit">学分</label><input class="urppp-feature-input" data-privacy-value="credit" maxlength="40" aria-label="学分替换内容"></div>',"          </div>","        </div>",'        <div class="urppp-privacy-group">','          <div class="urppp-privacy-group-title">页面内容</div>','          <div class="urppp-privacy-group-fields">','            <div class="urppp-privacy-field"><input id="urppp-privacy-other" type="checkbox" data-privacy-field="other" aria-label="隐藏其他数据"><label for="urppp-privacy-other">其他数据</label><input class="urppp-feature-input" data-privacy-value="other" maxlength="40" aria-label="其他数据替换内容"></div>','            <div class="urppp-privacy-field"><input id="urppp-privacy-schedule" type="checkbox" data-privacy-field="schedule" aria-label="隐藏课表"><label for="urppp-privacy-schedule">课表</label><input class="urppp-feature-input" data-privacy-value="schedule" maxlength="40" aria-label="课表替换内容"></div>','            <div class="urppp-privacy-field urppp-privacy-field-static"><input id="urppp-privacy-avatar" type="checkbox" data-privacy-field="avatar" aria-label="隐藏头像"><label for="urppp-privacy-avatar">头像</label><span class="urppp-privacy-note">使用统一遮罩</span></div>',"          </div>","        </div>","      </div>",'      <div class="urppp-direct-edit-control">',"        <div><strong>自由修改显示数据</strong><span>开启后，直接点击首页或清爽模式中带标记的数据进行修改</span></div>",'        <button type="button" class="urppp-set-follow" id="urppp-set-direct-edit-toggle" aria-pressed="false">页面内修改：关</button>',"      </div>","    </section>",'    <section class="urppp-set-sec" id="urppp-set-identity">',"      <h3>自定义姓名与头像</h3>",'      <div class="urppp-identity-editor">','        <div class="urppp-identity-fields">','          <div class="urppp-feature-row"><label><input type="checkbox" id="urppp-set-name-enabled"> 自定义姓名</label><input class="urppp-feature-input" id="urppp-set-custom-name" maxlength="40" placeholder="输入显示姓名"></div>','          <div class="urppp-feature-row"><label><input type="checkbox" id="urppp-set-avatar-enabled"> 自定义头像</label><input class="urppp-feature-input" id="urppp-set-custom-avatar-url" placeholder="https://... 图片地址"></div>','          <div class="urppp-feature-row"><label for="urppp-set-custom-avatar-file">本地图片</label><input class="urppp-feature-input" type="file" id="urppp-set-custom-avatar-file" accept="image/png,image/jpeg,image/webp,image/gif"></div>',"        </div>",'        <div class="urppp-identity-preview">','          <span class="urppp-identity-preview-label">头像预览</span>','          <div class="urppp-avatar-preview-shell"><span>未设置</span><img class="urppp-avatar-preview" id="urppp-set-avatar-preview" alt="自定义头像预览"></div>',"        </div>","      </div>",'      <div class="urppp-feature-actions"><button type="button" class="urppp-set-btn" id="urppp-set-privacy-save">保存隐私与显示设置</button><button type="button" class="urppp-set-btn ghost" id="urppp-set-avatar-clear">清除自定义头像</button></div>','      <div class="urppp-set-tip" id="urppp-set-privacy-status" style="margin-top:8px"></div>',"    </section>",'    <section class="urppp-set-sec" id="urppp-set-json-export">',"      <h3>JSON 导出格式</h3>",'      <button type="button" class="urppp-set-follow" id="urppp-set-json-custom" aria-pressed="false" style="width:100%">自定义 JSON：关</button>','      <div class="urppp-json-mapping-editor" id="urppp-set-json-editor">','        <label for="urppp-set-json-mapping">字段映射</label>','        <textarea id="urppp-set-json-mapping" spellcheck="false" aria-label="自定义 JSON 字段映射"></textarea>','        <p class="urppp-set-tip">源字段包括 name、teacher、position、day、sections、weeks、code、credit、campus、building、classroom、weekList 等；目标值支持 data.courses 形式的嵌套路径。</p>','        <div class="urppp-feature-actions"><button type="button" class="urppp-set-btn" id="urppp-set-json-save">保存映射</button><button type="button" class="urppp-set-btn ghost" id="urppp-set-json-reset">恢复默认映射</button></div>',"      </div>",'      <div class="urppp-set-tip" id="urppp-set-json-status" style="margin-top:8px"></div>',"    </section>",'    <section class="urppp-set-sec" id="urppp-set-update">',"      <h3>更新</h3>",'      <button type="button" class="urppp-set-follow" id="urppp-set-auto-update" aria-pressed="false" style="width:100%">自动检测更新：关</button>','      <button type="button" class="urppp-set-btn" id="urppp-set-check-update" style="margin-top:12px;width:100%">检查更新</button>','      <div id="urppp-set-update-status" class="urppp-set-tip" style="margin-top:8px"></div>',"    </section>",'    <div id="urppp-set-assist-slot"></div>',"  </div>",'  <div class="urppp-set-pane" data-pane="about">','    <div class="urppp-about">','      <img class="urppp-about-logo" id="urppp-about-logo" src="'+n+'" alt="SCU URP++" referrerpolicy="no-referrer" />','      <a class="urppp-about-ver" id="urppp-about-ver" href="'+c+'" target="_blank" rel="noopener noreferrer">SCU URP++ v'+d+"</a>",'      <p class="urppp-about-author">作者：Chao_Lan · Hanako</p>','      <p class="urppp-about-contact">QQ：2718748334</p>',`      <p class="urppp-about-msg">有任何问题欢迎及时反馈！
半夜Vibe有点爽怎么回事。</p>`,"    </div>","  </div>","</div>"].join("")}a(vp,"buildSettingsPanelHtml");var tr="urppp_plugin_",rc="1.0.0";function to({GM:p,doc:n,hostInfo:c,uiDeps:d}){let{getValue:u=a(()=>null,"getValue"),setValue:k=a(()=>{},"setValue"),xmlHttp:P,addStyle:S}=p||{},y=(typeof d=="function"?d:d&&d.openSubpanel)||null,A=new Map,f=new Map,w=new Map,T=[],C=null;function b(N,Y){let tt=w.get(N);tt&&tt.forEach(ht=>{try{ht(Y)}catch{}})}a(b,"emit");function m(N,Y){return w.has(N)||w.set(N,new Set),w.get(N).add(Y),()=>w.get(N).delete(Y)}a(m,"on");function v(N,Y){return u(`${tr}${N}_${Y}`)}a(v,"storageGet");function x(N,Y,tt){k(`${tr}${N}_${Y}`,tt)}a(x,"storageSet");function _(){return N=>({get:a(Y=>v(N,Y),"get"),set:a((Y,tt)=>x(N,Y,tt),"set"),remove:a(Y=>k(`${tr}${N}_${Y}`,void 0),"remove")})}a(_,"storage");function q(N,Y={}){return new Promise((tt,ht)=>{if(typeof P!="function"){ht(new Error("GM_xmlhttpRequest 不可用（未授权跨域？）"));return}P({method:Y.method||"GET",url:N,headers:Y.headers||{},data:Y.data,timeout:Y.timeout||8e3,onload:a(bt=>bt.status>=200&&bt.status<300?tt(bt.responseText):ht(new Error(`HTTP ${bt.status}`)),"onload"),onerror:a(()=>ht(new Error("网络错误")),"onerror"),ontimeout:a(()=>ht(new Error("超时(8s)")),"ontimeout")})})}a(q,"request");async function I(N,Y){let tt=Array.isArray(N)?N:[N],ht=[];for(let bt=0;bt<tt.length;bt+=1){let zt=tt[bt];Y&&Y({stage:"downloading",index:bt+1,total:tt.length,url:zt});try{let yt=await q(zt);return Y&&Y({stage:"downloaded",url:zt,size:yt.length}),yt}catch(yt){ht.push(`源${bt+1}(${L(zt)})失败: ${yt&&yt.message?yt.message:yt}`),Y&&Y({stage:"source_failed",index:bt+1,total:tt.length,error:yt&&yt.message?yt.message:yt})}}throw new Error("所有下载源失败 → "+ht.join(" ｜ "))}a(I,"fetchWithFallback");function L(N){try{return new URL(N).host}catch{return N}}a(L,"shortHost");function O(N){let Y=String(N||"").match(/\/\/\s*==UserScript==[\s\S]*?\/\/\s*==\/UserScript==/);return Y?N.replace(Y[0],""):N}a(O,"stripMetadata");function $(N,Y){try{let tt=O(N),ht=["GM_getValue","GM_setValue","GM_xmlhttpRequest","GM_registerMenuCommand","GM_addStyle","unsafeWindow"],bt=[typeof GM_getValue=="function"?GM_getValue:void 0,typeof GM_setValue=="function"?GM_setValue:void 0,typeof GM_xmlhttpRequest=="function"?GM_xmlhttpRequest:void 0,typeof GM_registerMenuCommand=="function"?GM_registerMenuCommand:void 0,typeof GM_addStyle=="function"?GM_addStyle:void 0,typeof unsafeWindow<"u"?unsafeWindow:null];return new Function(...ht,tt)(...bt),!0}catch(tt){return console.warn("[URP++ plugin] 注入失败",Y,tt),!1}}a($,"inject");function H(N,Y){let tt=f.get(N);return tt?(tt.enabled=!!Y,k(`${tr}${N}_enabled`,tt.enabled),b(Y?"enabled":"disabled",N),!0):!1}a(H,"setEnabled");function G(N){let Y=f.get(N);return!!Y&&Y.enabled}a(G,"isEnabled");function U(N){if(!N||!N.id)return!1;if(A.has(N.id)&&A.get(N.id).__urpppRegistered)return!0;let Y=Object.assign({type:"plugin"},N);Y.__urpppRegistered=!0,A.set(N.id,Y);let tt=f.get(N.id)||{loaded:!1,enabled:!1,version:N.version||""};return tt.version=Y.version||tt.version,f.set(N.id,tt),b("registered",Y.id),!0}a(U,"register");function rt(N){return A.get(N)||null}a(rt,"get");function it(N){let Y=[];for(let tt of A.values())(!N||tt.type===N)&&Y.push(tt);return Y}a(it,"list");function mt(N){let Y=f.get(N);return!!Y&&Y.loaded}a(mt,"loaded");async function V(N,Y,tt){tt&&tt({stage:"start",id:N});let ht=Array.isArray(Y)?Y:Y?[Y]:et(N),bt=await I(ht,tt);k(`${tr}${N}_code`,bt),tt&&tt({stage:"injecting",id:N});let zt=$(bt,N),yt=f.get(N)||{loaded:!1,enabled:!1,version:""};return yt.loaded=zt,yt.enabled=zt,yt.code=bt,yt.version=yt.version||Q(bt),f.set(N,yt),k(`${tr}${N}_enabled`,zt),b("loaded",N),zt}a(V,"install");function Q(N){let Y=String(N||"").match(/@version\s+(\S+)/);return Y?Y[1]:""}a(Q,"detectVersion");async function ot(N,Y,tt){let ht=Array.isArray(Y)?Y:Y?[Y]:et(N),bt=await I(ht,tt);k(`${tr}${N}_code`,bt);let zt=Q(bt),yt=f.get(N)||{loaded:!1,enabled:!1,version:""};return yt.version=zt||yt.version,yt.code=bt,f.set(N,yt),b("updated",N),{ok:!0,version:zt||yt.version}}a(ot,"update");function X(N){let Y=u(`${tr}${N}_code`);if(!Y)return!1;let tt=f.get(N);if(tt&&tt.loaded)return!0;let ht=$(Y,N),bt=f.get(N)||{loaded:!1,enabled:!1,version:Q(Y)};return bt.loaded=ht,bt.enabled=ht&&u(`${tr}${N}_enabled`)!==!1,bt.code=Y,f.set(N,bt),b("loaded",N),ht}a(X,"bootFromCache");function ct(N){let Y=A.get(N);return A.delete(N),f.delete(N),k(`${tr}${N}_enabled`,!1),b("unregistered",N),!!Y}a(ct,"unregister");function et(N){return N==="assist"?["https://raw.githubusercontent.com/chaolan2019/URP-plusplus-Repository/main/plugins/urpppp.plugin.js","https://cdn.jsdelivr.net/gh/chaolan2019/URP-plusplus-Repository@main/plugins/urpppp.plugin.js","https://gh-proxy.com/https://raw.githubusercontent.com/chaolan2019/URP-plusplus-Repository/main/plugins/urpppp.plugin.js"]:[]}a(et,"pluginSource");let st={protocolVersion:rc,register:U,unregister:ct,get:rt,list:it,loaded:mt,isEnabled:G,enable:a((N,Y=!0)=>H(N,Y),"enable"),disable:a(N=>H(N,!1),"disable"),install:V,update:ot,bootFromCache:X,storage:a(()=>u&&{get:a(N=>u(N),"get"),set:a((N,Y)=>k(N,Y),"set")},"storage"),pluginStorage:a(N=>_()(N),"pluginStorage"),request:q,addStyle:a(N=>{try{S&&S(N)}catch{}},"addStyle"),log:a((...N)=>{console.log("[URP++ plugin]",...N)},"log"),on:m,emit:b,hostInfo:Object.assign({name:"SCU URP++"},c||{}),getSubpanel:a(()=>y,"getSubpanel")};try{window.__urpppPlugin=st}catch{}try{typeof unsafeWindow<"u"&&unsafeWindow&&(unsafeWindow.__urpppPlugin=st)}catch{}function ft(N){if(!N||!n||N.querySelector(".urppp-plugin-sec, .urpppp-entry-sec"))return;let Y=n.createElement("section");Y.className="urppp-set-sec urppp-plugin-sec",Y.id="urppp-plugin-sec",Y.innerHTML=`
      <h3>辅助插件</h3>
    ...
    `,N.appendChild(Y);let tt=Y.querySelector("#urppp-plugin-status"),ht=Y.querySelector("#urppp-plugin-install"),bt=Y.querySelector("#urppp-plugin-store"),zt=Y.querySelector("#urppp-plugin-panels"),yt=Y.querySelector("#urppp-plugin-tip");function Ft(){let Pt=f.get("assist"),It=A.has("assist");Pt&&Pt.loaded||It?(tt.textContent=`辅助插件 v${Pt&&Pt.version?Pt.version:rt("assist")&&rt("assist").version||""} 已装载`,tt.className="urppp-plugin-status ok",ht.textContent="重新装载",ht.dataset.state="loaded",yt.textContent="已装载。下方为扩展入口。"):(tt.textContent=C||"未装载",tt.className=C?"urppp-plugin-status err":"urppp-plugin-status",ht.textContent="装载辅助插件",ht.dataset.state="notloaded",yt.textContent=C?"装载失败，可就近重试或放回本地安装。下方为装载/商店入口。":"点击装载后，主插件会下载并注入辅助插件（登录助手/评教/会话保持/2FA），无需再单独安装。"),zt.innerHTML="";let Xt=Ct();if(Xt&&Object.keys(Xt).length){let zr=n.createElement("div");zr.className="urppp-plugin-sub",Object.keys(Xt).forEach(ur=>{let Yt=n.createElement("button");Yt.type="button",Yt.className="urppp-set-btn ghost",Yt.textContent=Xt[ur].label||ur,Yt.addEventListener("click",()=>{try{Xt[ur]&&typeof Xt[ur].open=="function"?Xt[ur].open():y&&y(ur)}catch{}}),zr.appendChild(Yt)}),zt.appendChild(zr)}}a(Ft,"refresh"),ht.addEventListener("click",async()=>{ht.disabled=!0,ht.textContent="装载中…",tt.className="urppp-plugin-status",tt.textContent="正在开始装载…";try{if(await V("assist",null,It=>{try{It.stage==="downloading"?tt.textContent=`下载中… 源${It.index}/${It.total}（${L(It.url)}）`:It.stage==="downloaded"?tt.textContent=`已下载（${It.size} 字节），注入中…`:It.stage==="source_failed"?tt.textContent=`源${It.index}失败（${It.error||""}），切换下一源…`:It.stage==="injecting"?tt.textContent="注入中…":It.stage==="start"&&(tt.textContent="正在开始装载…"),console.log("[URP++ plugin] assist 装载进度",It)}catch{}}))C=null,tt.textContent="辅助插件已装载 v"+(rt("assist")&&rt("assist").version||""),console.log("[URP++ plugin] assist 装载成功");else throw new Error("注入失败")}catch(Pt){C="装载失败："+(Pt&&Pt.message?Pt.message:Pt),tt.textContent=C,tt.className="urppp-plugin-status err",console.warn("[URP++ plugin] assist 装载失败",Pt)}finally{ht.disabled=!1,Ft()}}),bt.addEventListener("click",()=>{y&&y("plugin-store")}),m("loaded",Pt=>{Pt==="assist"&&Ft()}),m("registered",Pt=>{Pt==="assist"&&Ft()}),m("unregistered",Pt=>{Pt==="assist"&&Ft()}),Ft()}a(ft,"renderAssistUi");function Z(N){return String(N??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}a(Z,"escapeHtml");function dt(N){if(N){if(kt(),N.dataset.rendered==="1"){N.style.display=N.style.display==="none"?"":"none";return}N.dataset.rendered="1",N.innerHTML=`
...
`;function cc(p){p.querySelectorAll("td, th").forEach(n=>{n.style.removeProperty("background"),n.style.removeProperty("background-color")}),p.querySelectorAll("th[rowspan]").forEach(n=>{n.style.removeProperty("width"),n.style.setProperty("white-space","nowrap"),n.style.setProperty("text-align","center")}),p.querySelectorAll("table").forEach(n=>{n.style.setProperty("background","#ffffff","important"),n.style.setProperty("background-color","#ffffff","important"),n.style.setProperty("border","none","important"),n.style.setProperty("color","#000000","important")}),p.querySelectorAll("th").forEach(n=>{if(n.style.setProperty("color","#000000","important"),n.style.setProperty("border","1px solid #dddddd","important"),n.style.setProperty("font-weight","normal","important"),n.childNodes.length===1&&n.firstChild&&n.firstChild.nodeType===3){let c=document.createElement("span");c.textContent=n.textContent,n.textContent="",n.appendChild(c)}}),p.querySelectorAll("thead th").forEach(n=>{n.style.setProperty("background","#dddddd","important"),n.style.setProperty("background-color","#dddddd","important")}),p.querySelectorAll("tbody th").forEach(n=>{n.style.setProperty("background","transparent","important"),n.style.setProperty("background-color","transparent","important")}),p.querySelectorAll("td").forEach(n=>{n.style.setProperty("background","transparent","important"),n.style.setProperty("background-color","transparent","important"),n.style.setProperty("color","#000000","important"),n.style.setProperty("border","1px solid #dddddd","important")})}a(cc,"normalizeNativePdfStage");function Bp(p){let n=sc(),c=document.createElement("div");c.id="urppp-pdf-stage",c.style.cssText="position:fixed;left:-20000px;top:0;z-index:-1;pointer-events:none;width:"+(n||window.innerWidth||1440)+"px;";let d=document.createElement("div");d.id="urppp-pdf-page",d.style.cssText="position:relative;width:100%;box-sizing:border-box;";let u=p.cloneNode(!0);pc(u),ic(u),d.appendChild(u),c.appendChild(d),cc(u);let k=document.createElement("style");k.id="urppp-pdf-reset-style",k.textContent=lc,document.head.appendChild(k),document.body.appendChild(c);let P=c.querySelector("#urppp-pdf-mycoursetable"),S=c.querySelector("#urppp-pdf-page")||c;if(!P)throw c.remove(),new Error("无法建立原生课表捕获节点");return{stage:c,target:P,page:S,sourceHost:p}}a(Bp,"cloneNativePdfStage");var Ze=0;function dr(){return Ze>0}a(dr,"isNativePdfIsolationActive");function dc(p){return!p||p.tagName!=="STYLE"?!1:/^urppp(?:-|$)/.test(p.id||"")||p.hasAttribute("data-urppp-style")?!0:(p.textContent||"").includes("urppp-")}a(dc,"isUrpppOwnedStyle");function Fp(){try{if(typeof unsafeWindow<"u"&&unsafeWindow)return unsafeWindow}catch{}return typeof window<"u"?window:null}a(Fp,"defaultPage");function Dp(p,n){let c=p&&typeof p.requestAnimationFrame=="function"?p.requestAnimationFrame.bind(p):typeof requestAnimationFrame=="function"?requestAnimationFrame:null;return c?c(n):setTimeout(n,0)}a(Dp,"scheduleFrame");function uc(p={}){let n=p.document||(typeof document<"u"?document:null),c=p.page||Fp();if(!n)throw new Error("原生 PDF 隔离缺少 document");let d=n.getElementById("mycoursetable");if(!d)throw new Error("当前页面没有课表节点");Ze+=1;let u=[d,...d.querySelectorAll("*")],k=[],P=n.getElementById("soliderbox");P&&k.push(P);let S=d.parentElement;for(;S&&S!==n.documentElement;){let x=S.classList;(S.id==="page-content-template"||x&&(x.contains("page-content")||x.contains("profile-info-row")||x.contains("profile-info-value")))&&k.push(S),S=S.parentElement}let y=n.getElementById("page-content-template")||n.querySelector(".page-content");y&&!k.includes(y)&&k.push(y);let A=[...u,...k],f=A.map(x=>({element:x,style:x.getAttribute("style")})),w=Array.from(n.querySelectorAll("style")).filter(dc).map(x=>({style:x,disabled:x.sheet?x.sheet.disabled:!1,media:x.getAttribute("media")})),T=Array.from(d.querySelectorAll('[id^="urppp-"], [data-urppp]')),C=c&&c.divBuild,b=c&&c.__urpppOriginalDivBuild,m=!1,v=a(()=>{m||(m=!0,c&&c.divBuild===b&&typeof C=="function"&&(c.divBuild=C),f.forEach(({element:x,style:_})=>{x.isConnected&&(_===null?x.removeAttribute("style"):x.setAttribute("style",_))}),T.forEach(x=>x.removeAttribute("data-urppp-pdf-hidden")),w.forEach(({style:x,disabled:_,media:q})=>{try{q===null?x.removeAttribute("media"):x.setAttribute("media",q),x.sheet&&(x.sheet.disabled=_)}catch{}}),Ze=Math.max(0,Ze-1),Dp(c,()=>{try{typeof p.onAfterRestore=="function"&&p.onAfterRestore()}catch{}}))},"restore");try{return w.forEach(({style:x})=>{try{x.setAttribute("media","not all"),x.sheet&&(x.sheet.disabled=!0)}catch{}}),A.forEach(x=>{!x.style||!x.style.length||Array.from(x.style).forEach(_=>{x.style.getPropertyPriority(_)==="important"&&(_==="height"&&x.matches("td, th")||x.style.removeProperty(_))})}),d.querySelectorAll("td").forEach(x=>{x.style.removeProperty("background"),x.style.removeProperty("background-color")}),y&&y.style.setProperty("position","relative","important"),d.style.setProperty("position","static","important"),d.querySelectorAll("td").forEach(x=>{x.style.setProperty("position","static","important")}),T.forEach(x=>{x.setAttribute("data-urppp-pdf-hidden","1"),x.style.setProperty("display","none","important")}),c&&typeof b=="function"&&(c.divBuild=b),v}catch(x){throw v(),x}}a(uc,"isolateScheduleForNativeExport");function jp(p,n={}){return new Promise((c,d)=>{let u=n.page||Fp(),k=u&&u.back,P=u&&u.html2canvas;if(!p||typeof k!="function"){d(new Error("教务原生导出依赖未就绪"));return}let S=null;try{S=uc(n)}catch(b){d(b);return}let y=0,A=!1,f=null,w=null,T=a(b=>{if(!A){A=!0,y&&clearTimeout(y),u&&f&&u.back===f&&(u.back=k),w&&u.html2canvas===w&&(u.html2canvas=P);try{S&&S()}catch{}b?d(b):c()}},"settle"),C=a(b=>T(b instanceof Error?b:new Error(String(b))),"fail");typeof P=="function"&&(w=a(function(){let b=P.apply(this,arguments);return b&&typeof b.catch=="function"&&b.catch(C),b},"scopedCanvas"),u.html2canvas=w),f=a(function(){try{return k.apply(this,arguments)}finally{setTimeout(()=>T(),0)}},"wrappedBack"),u.back=f,y=setTimeout(()=>{try{k.call(u)}catch{}C(new Error("原生 PDF 生成超时"))},n.timeoutMs||60*1e3),Dp(u,()=>{try{p.click()}catch(b){C(b)}})})}a(jp,"exportNativePdfIsolated");var Op=`.urppp-private-value{font-family:inherit!important;font-size:inherit!important;font-weight:inherit!important;font-style:inherit!important;line-height:inherit!important;letter-spacing:0!important;color:inherit!important}
...
`;var Rp=`/* Personal and resource schedule course cards. Keep table cells and table surfaces untouched. */
#courseTable .class_div.box_font {
  box-sizing: border-box !important;
...
`;var Up=`.urppp-export-wrap{position:relative!important;display:inline-flex!important;align-items:center!important;flex:0 0 auto!important;margin-left:7px!important;font-weight:400!important;vertical-align:middle!important;white-space:nowrap!important}
...
html[data-urppp-skin] #urppp-settings-panel .urppp-skin-card[data-skin="apple"] .urppp-set-btn.ghost:hover { background: #0071e3 !important; color: #fff !important; }
...
/* 暗色：flat 卡（旧块补缺）+ 各主题卡内次要按钮(仓库/删除)随卡片主题 */
html.urppp-theme-dark #urppp-settings-panel .urppp-skin-card[data-skin="flat"] {
  background: #111 !important; color: #fff !important; border-color: #fff !important; box-shadow: none !important;
}
html.urppp-theme-dark #urppp-settings-panel .urppp-skin-card[data-skin="flat"] .urppp-skin-name, html.urppp-theme-dark #urppp-settings-panel .urppp-skin-card[data-skin="flat"] .urppp-skin-desc { color: #fff !important; }
html.urppp-theme-dark #urppp-settings-panel .urppp-skin-card[data-skin="flat"] .urppp-skin-apply {
  background: #fff !important; color: #000 !important; border: 2px solid #fff !important;
}

/* 暗色 hover：背景与文字同色系，避免悬停变白但文字不变色 */
html.urppp-theme-dark #urppp-settings-panel .urppp-skin-card[data-skin="flat"]:hover {
  background: #fff !important; color: #000 !important; border-color: #000 !important;
}
html.urppp-theme-dark #urppp-settings-panel .urppp-skin-card[data-skin="flat"]:hover .urppp-skin-name,
html.urppp-theme-dark #urppp-settings-panel .urppp-skin-card[data-skin="flat"]:hover .urppp-skin-desc,
html.urppp-theme-dark #urppp-settings-panel .urppp-skin-card[data-skin="flat"]:hover .urppp-skin-meta { color: #000 !important; }
...
html.urppp-theme-dark #urppp-settings-panel .urppp-skin-card[data-skin="brutal"]:hover {
  background: #1a1a1a !important; transform: translate(-2px,-2px) !important; box-shadow: 8px 8px 0 #fff !important;
}
...
/* 兜底：新野兽派暗色卡 + 卡内按钮（最高 specificity，最后声明，确保暗色下生效） */
html.urppp-theme-dark #urppp-settings-panel .urppp-skin-card[data-skin="brutal"] {
  background: #000 !important; color: #fff !important; border: 3px solid #fff !important; box-shadow: 5px 5px 0 #fff !important;
}
html.urppp-theme-dark #urppp-settings-panel .urppp-skin-card[data-skin="brutal"] .urppp-skin-name,
html.urppp-theme-dark #urppp-settings-panel .urppp-skin-card[data-skin="brutal"] .urppp-skin-desc { color: #fff !important; }
html.urppp-theme-dark #urppp-settings-panel .urppp-skin-card[data-skin="brutal"] .urppp-skin-apply {
  background: #ff006e !important; color: #fff !important; border-color: #fff !important; box-shadow: 3px 3px 0 #fff !important;
}


/* 各主题卡次要按钮（仓库/删除）：随卡片主题色，但克制（细边 + 透明底 + 圆角），不做大面积色块/硬阴影 */
html[data-urppp-skin] #urppp-settings-panel .urppp-skin-card[data-skin="apple"] .urppp-set-btn.ghost { color:#0071e3!important; border:1px solid rgba(0,113,227,.4)!important; border-radius:999px!important; background:transparent!important; box-shadow:none!important; }
html[data-urppp-skin] #urppp-settings-panel .urppp-skin-card[data-skin="editorial"] .urppp-set-btn.ghost { color:#1c1c1c!important; border:none!important; background:transparent!important; border-radius:0!important; box-shadow:none!important; }
html[data-urppp-skin] #urppp-settings-panel .urppp-skin-card[data-skin="editorial"] .urppp-set-btn.ghost:hover { text-decoration-line:underline!important; text-underline-offset:3px!important; }
html[data-urppp-skin].urppp-theme-dark #urppp-settings-panel .urppp-skin-card[data-skin="editorial"] .urppp-set-btn.ghost:hover { text-decoration-line:underline!important; }
html[data-urppp-skin] #urppp-settings-panel .urppp-skin-card[data-skin="flat"] .urppp-set-btn.ghost { color:#000!important; border:1px solid rgba(0,0,0,.5)!important; background:transparent!important; border-radius:0!important; box-shadow:none!important; }
html[data-urppp-skin] #urppp-settings-panel .urppp-skin-card[data-skin="organic"] .urppp-set-btn.ghost { color:#5c4033!important; border:1px solid rgba(92,64,51,.35)!important; background:transparent!important; border-radius:999px!important; box-shadow:none!important; }
html[data-urppp-skin] #urppp-settings-panel .urppp-skin-card[data-skin="brutal"] .urppp-set-btn.ghost { color:#000!important; border:1px solid rgba(0,0,0,.55)!important; background:transparent!important; border-radius:0!important; box-shadow:none!important; }
html[data-urppp-skin] #urppp-settings-panel .urppp-skin-card[data-skin="neu"] .urppp-set-btn.ghost { color:#4f5258!important; background:#e0e5ec!important; border:none!important; border-radius:12px!important; box-shadow:3px 3px 6px #bec3ca,-3px -3px 6px #f7f9fc!important; }
/* 暗色下同样随卡片主题色（克制，background 必须透明，否则继承全局 --input-bg 变黑块） */
html[data-urppp-skin].urppp-theme-dark #urppp-settings-panel .urppp-skin-card[data-skin="apple"] .urppp-set-btn.ghost { color:#7aa2ff!important; border-color:rgba(122,162,255,.4)!important; background:transparent!important; box-shadow:none!important; }
html[data-urppp-skin].urppp-theme-dark #urppp-settings-panel .urppp-skin-card[data-skin="editorial"] .urppp-set-btn.ghost { color:#e8e8e4!important; border:none!important; background:transparent!important; box-shadow:none!important; }
html[data-urppp-skin].urppp-theme-dark #urppp-settings-panel .urppp-skin-card[data-skin="flat"] .urppp-set-btn.ghost { color:#fff!important; border-color:rgba(255,255,255,.4)!important; background:transparent!important; box-shadow:none!important; }
html[data-urppp-skin].urppp-theme-dark #urppp-settings-panel .urppp-skin-card[data-skin="organic"] .urppp-set-btn.ghost { color:#eadbc8!important; border-color:rgba(234,219,200,.35)!important; background:transparent!important; box-shadow:none!important; }
html[data-urppp-skin].urppp-theme-dark #urppp-settings-panel .urppp-skin-card[data-skin="brutal"] .urppp-set-btn.ghost { color:#fff!important; border-color:rgba(255,255,255,.4)!important; background:transparent!important; box-shadow:none!important; }
html[data-urppp-skin].urppp-theme-dark #urppp-settings-panel .urppp-skin-card[data-skin="neu"] .urppp-set-btn.ghost { color:#dfe3e9!important; background:#2a2f36!important; border:none!important; box-shadow:3px 3px 6px #1e2228,-3px -3px 6px #363d47!important; }
...
`;var Jp=`      /* 导航：顶栏、侧栏、面包屑（navigation） */
      /* 顶栏 —— 基于真实 DOM */
...
`;var Vp=`/* ===== 插件弹窗统一进入动画：淡入+缩放 + 内容逐条浮现 ===== */
...
`;function Kp(){return{open:!1,mobileTab:"home",scoreAnalysisTab:"overview",profile:null,schedule:null,scores:null,catalog:null,occupancy:null,currentBuilding:null,loading:{profile:!1,schedule:!1,scores:!1,room:!1},roomError:"",roomDateOffset:0,selected:{passing:new Set,scheme:new Set},activeSchemeIdx:0,_schemeUserSelected:!1,viewWeek:0,weekLocked:!1,_termWeek:0,_termWeekResolved:!1,uiReady:!1}}a(Kp,"createCleanModeState");function Zp(p){p.profile=null,p.schedule=null,p.scores=null,p.catalog=null,p.occupancy=null,p._termWeekResolved=!1,p._schemeUserSelected=!1,p._schemeInited=!1}a(Zp,"resetCleanModeData");function ti({state:p,deps:n}){async function c(u){if(!u&&p.catalog&&p.catalog.length||p.loading.room)return p.catalog;p.loading.room=!0;try{n.render()}catch{}try{p.catalog=await n.loadClassroomCatalog(),p.roomError=""}catch(k){p.catalog=p.catalog||[],p.roomError=String(k&&k.message||k),console.warn("[URP++] room catalog",k)}finally{p.loading.room=!1;try{n.scheduleRender()}catch{}}return p.catalog}a(c,"ensureRoomCatalogLoaded");async function d(u){u&&Zp(p),p.loading.profile=p.loading.schedule=p.loading.scores=!0;try{let k=await n.ensureTermWeekResolved();!p.weekLocked&&k>=1&&(p.viewWeek=k)}catch{}if(n.render(),await Promise.all([(async()=>{try{p.profile&&!u||(p.profile=await n.loadProfile()),n.reconcileProfileAndScores()}catch(k){p.profile={name:"同学",majorPlan:"主修方案",majorGpa:"—",avatar:""},console.warn(k)}finally{p.loading.profile=!1,n.scheduleRender()}})(),(async()=>{try{p.schedule&&!u||(p.schedule=await n.loadSchedule())}catch(k){p.schedule={courses:[],error:String(k&&k.message||k)}}finally{if(p.loading.schedule=!1,!p.weekLocked){let k=n.getCurrentWeekNumber()||n.readRememberedTermWeek();k>=1&&(p.viewWeek=k)}n.scheduleRender()}})(),(async()=>{let k=null;try{p.scores&&!u||(p.scores=await n.loadScores(u)),k=p.scores,n.reconcileProfileAndScores(),k&&!k.error&&!k.evaluationReady&&n.enrichScoresWithEvaluation(k).then(()=>{p.scores===k&&(n.reconcileProfileAndScores(),n.scheduleRender())}).catch(P=>{console.warn("[URP++] attach evaluation",P)})}catch(P){p.scores={passing:[],schemes:[],error:String(P&&P.message||P)}}finally{p.loading.scores=!1,n.scheduleRender()}})()]),n.reconcileProfileAndScores(),!p.weekLocked){let k=n.getCurrentWeekNumber()||n.readRememberedTermWeek();k>=1&&(p.viewWeek=k)}n.scheduleRender()}return a(d,"loadAll"),{ensureRoomCatalogLoaded:c,loadAll:d}}a(ti,"createCleanModeDataLoader");var re={autumn:{name:"秋季学期",weeks:20,start:"2026-08-31",end:"2027-02-20",events:[{t:"reg",name:"本科生新生报到",start:"2026-08-24",end:"2026-08-25"},{t:"reg",name:"在校生报到",start:"2026-08-27",end:"2026-08-28"},{t:"reg",name:"研究生新生报到",start:"2026-08-27",end:"2026-08-28"},{t:"reg",name:"在校本科补缓考",start:"2026-08-28",end:"2026-08-30"},{t:"term",name:"本科生开学典礼",start:"2026-09-01"},{t:"term",name:"研究生开学典礼",start:"2026-09-04"},{t:"term",name:"在校生正式行课",start:"2026-08-31",end:"2026-09-06"},{t:"holiday",name:"中秋节",start:"2026-09-25"},{t:"holiday",name:"国庆节假期",start:"2026-10-01",end:"2026-10-07"},{t:"sport",name:"校秋季田径运动会",start:"2026-10-23",end:"2026-10-24"},{t:"exam",name:"本科生期末集中考试周",start:"2027-01-04",end:"2027-01-15"},{t:"holiday",name:"寒假",start:"2027-01-18",end:"2027-02-20"},{t:"holiday",name:"春节",start:"2027-02-06"}]},spring:{name:"春季学期",weeks:18,start:"2027-03-01",end:"2027-07-03",events:[{t:"reg",name:"在校生报到",start:"2027-02-25",end:"2027-02-26"},{t:"term",name:"正式行课",start:"2027-03-01",end:"2027-03-07"},{t:"holiday",name:"清明节",start:"2027-04-05"},{t:"holiday",name:"劳动节假期",start:"2027-05-01",end:"2027-05-05"},{t:"holiday",name:"端午节",start:"2027-06-09"},{t:"exam",name:"期末集中考试",start:"2027-06-21",end:"2027-06-27"},{t:"term",name:"毕业典礼",start:"2027-06-25"},{t:"holiday",name:"暑假开始",start:"2027-07-04"}]}},mc={"2026-08-24":"农历七月十二","2026-08-25":"农历七月十三","2026-08-27":"农历七月十五","2026-08-28":"农历七月十六","2026-08-30":"农历七月十八","2026-08-31":"农历七月十九","2026-09-01":"农历七月二十","2026-09-04":"农历七月廿三","2026-09-25":"农历八月十五","2026-10-01":"农历八月廿一","2026-10-07":"农历八月廿七","2026-10-23":"农历九月十四","2026-10-24":"农历九月十五","2027-01-04":"农历冬月廿七","2027-01-15":"农历腊月初八","2027-01-18":"农历腊月十一","2027-02-06":"农历正月初一","2027-02-20":"农历正月十五","2027-02-25":"农历正月二十","2027-02-26":"农历正月廿一","2027-03-01":"农历正月廿四","2027-04-05":"农历二月廿九","2027-05-01":"农历三月廿五","2027-05-05":"农历三月廿九","2027-06-09":"农历五月初五","2027-06-21":"农历五月十七","2027-06-25":"农历五月廿一","2027-06-27":"农历五月廿三","2027-07-03":"农历五月廿九","2027-07-04":"农历六月初一"},ke={term:{color:"#44616f",label:"教学/开学"},reg:{color:"#8a74bd",label:"报到"},exam:{color:"#c08a3f",label:"考试周"},holiday:{color:"#d0716a",label:"假期"},sport:{color:"#778e63",label:"运动会"}};function ai(){let p=new Date,n=a(c=>String(c).padStart(2,"0"),"p");return`${p.getFullYear()}-${n(p.getMonth()+1)}-${n(p.getDate())}`}a(ai,"calToday");function po(p,n){return Math.round((Date.parse(n)-Date.parse(p))/864e5)}a(po,"calDayDiff");function io(p,n){let c=po(re[p].start,n);return c<0?0:Math.floor(c/7)+1}a(io,"calWeekNo");function oo(p){return mc[p]||""}a(oo,"calLunar");function ri(p){return String(p||"").slice(5)}a(ri,"calYY");function bc(p){let n=p||ai(),[c,d]=n.split("-").map(Number);return d===8&&n>="2026-08-15"||d>=9||d<=2?"autumn":"spring"}a(bc,"calActiveTerm");function so(p,n){let c=p&&re[p]?p:"autumn",d=re[c],u=n||ai(),k=d.events.map(f=>({e:f,d:po(u,f.start)})).filter(f=>f.d>=-0).sort((f,w)=>f.d-w.d)[0],P=k?po(u,k.e.start):null,S=io(c,u),y=Math.max(0,Math.min(100,S/d.weeks*100)),A=u>=d.start;return{term:d,termId:c,next:k,daysLeft:P,weekNo:S,progress:y,started:A,today:u}}a(so,"calStatus");function oi(p,n){let c=so(p,n),d=c.next?ke[c.next.e.t].color:"#c9cdd4",u=c.term;return`<button type="button" class="uc-cal-summary" data-urppp-cal-open aria-label="打开校历时间线">
  ...
  </button>`}a(oi,"calendarSummaryHtml");function ni(p,n){let c=so(p,n);return`<button type="button" class="uc-cal-summary uc-cal-summary-compact" data-urppp-cal-open aria-label="打开校历时间线">
    <span class="cal-c-dot" style="background:${c.next?ke[c.next.e.t].color:"#c9cdd4"}"></span>
  ...
  </button>`}a(ni,"calendarSummaryCompactHtml");function ei(p,n){let c=so(p,n),d=c.next?ke[c.next.e.t].color:"#c9cdd4",u=c.term,k=Object.keys(re).map(w=>`<button type="button" class="cal-term${w===c.termId?" ac":""}" data-cal-term="${w}">${re[w].name}</button>`).join(""),P=`<div class="cal-widget">
    <div class="cal-w-left">
      ...
      <div class="cal-w-sub">${c.next?c.next.e.start+(c.next.e.end&&c.next.e.end!==c.next.e.start?" ~ "+c.next.e.end:""):""}${c.next&&oo(c.next.e.start)?" · "+oo(c.next.e.start):""}</div>
    </div>
    <div class="cal-w-mid">
    ...
    <div class="cal-w-right">
      ...
      <div class="cal-mon-items">${y[w].map(C=>{let b=ke[C.t].color,m=C.end&&C.end!==C.start?"~"+ri(C.end):"",v=io(c.termId,C.start)>0?`第 ${io(c.termId,C.start)} 周`:"开学前";return`<div class="cal-ev${A(C.start)}">
          <span class="cal-ev-dot" style="background:${b}"></span>
          <span class="cal-ev-date">${ri(C.start)}${m||""}<em>${oo(C.start)||"&nbsp;"}</em></span>
          <span class="cal-ev-name">${C.name}</span>
          <span class="cal-ev-tag" style="color:${b};background:${b}1a">${ke[C.t].label}</span>
  ...
  </div>`}a(ei,"calendarModalHtml");function pi(p,n){let c=typeof document<"u"?document:null;if(!c)return;no();let d=p&&re[p]?p:bc(n),u=c.createElement("div");u.id="urppp-cal-modal",u.innerHTML=`<div class="cal-overlay"></div>
    <div class="cal-dialog"><div class="cal-body">${ei(d,n)}</div></div>`,c.documentElement.appendChild(u),setTimeout(()=>u.classList.add("open"),20),u.querySelector(".cal-overlay").addEventListener("click",()=>no()),u.addEventListener("click",k=>{let P=k.target;if(P&&P.closest&&P.closest(".cal-close")){no();return}let S=P&&P.closest?P.closest("[data-cal-term]"):null;if(S){let y=u.querySelector(".cal-body");y&&(y.innerHTML=ei(S.dataset.calTerm,n)),u.querySelectorAll("[data-cal-term]").forEach(A=>A.classList.toggle("ac",A.dataset.calTerm===S.dataset.calTerm))}})}a(pi,"openCalendarModal");function no(){let p=typeof document<"u"?document:null;if(!p)return;let n=p.getElementById("urppp-cal-modal");n&&(n.classList.remove("open"),n.classList.add("closing"),setTimeout(()=>{n.remove()},200))}a(no,"closeCalendarModal");function ii(p,n){let c=p||(typeof document<"u"?document:null);c&&c.addEventListener("click",d=>{let u=d.target;u&&u.closest&&u.closest("[data-urppp-cal-open]")&&(d.preventDefault(),d.stopPropagation(),pi())})}a(ii,"bindCalendarOpen");var ta=!1;function lo(){let p=typeof document<"u"?document:null;if(!p||ta)return ta;try{let n=p.createElement("style");if(n&&n.id!==void 0){n.id="urppp-cal-style",n.textContent=`
  ...
  <div class="uc-bd">
    ...
    <div class="uc-sa-pane uc-sa-pane-analysis"${q?"":" hidden"}>${k()}</div>
      ...
      </button>`).join("")}</div>`}a(f,"servicesHtml");function w(){let x=n.personalizedProfile(p.profile||{}),_=p.schedule&&p.schedule.courses||[],q=p.scores&&p.scores.passing&&p.scores.passing[0]||{summary:n.summarizeCourses([])},I=p.scores&&p.scores.schemes||[];p.scores&&p.scores.majorIdx!=null&&p._schemeInited!==!0&&(p.activeSchemeIdx=p.scores.majorIdx||0,p._schemeInited=!0);let L=I[p.activeSchemeIdx]||I[0]||{summary:n.summarizeCourses([]),title:"方案成绩"},O=x.avatar?`<img src="${n.escapeHtml(x.avatar)}" alt="">`:`<span>${n.escapeHtml((x.name||"同")[0])}</span>`,$=p.loading.scores?'<div class="uc-loading">成绩加载中</div>':p.scores&&p.scores.error?`<div class="uc-empty">${n.escapeHtml(p.scores.error)}</div>`:`<div class="uc-score-grid">
      ...
      <div class="uc-col">
        ...
        </div>${(()=>{try{return oi()}catch{return""}})()}</div></div>
      ...
      </div>`).join(""):`<div class="uc-empty">${n.escapeHtml(p.roomError||"未读到教学楼列表")}<div style="margin-top:10px"><button type="button" class="uc-btn" data-room-reload="1">重新加载</button></div></div>`}a(C,"roomPickerHtml");function b(x,_){if(!x||!x.rooms||!x.rooms.length)return'<div class="uc-empty">该楼暂无教室占用数据</div>';let q='<tr><th class="sticky">教室</th><th class="sticky2">座位</th>';for(let $=1;$<=12;$++)q+=`<th class="sec">${$}</th>`;q+="</tr>";let I=x.rooms.map($=>{let H=`<tr><th class="sticky">${n.escapeHtml($.name)}</th><th class="sticky2">${n.escapeHtml($.seats)}</th>`;for(let G=1;G<=12;G++){let U=($.slots||[]).find(rt=>rt.section===G)||{busy:!1};if(U.busy){let rt=U.reason||U.typeLabel||"占用",it=U.typeLabel||n.occupancyTypeLabel({occupancymoduleId:U.module}),mt=U.displayChar||n.firstContentChar(rt)||n.firstContentChar(it)||"占",V=Object.assign({},U.detail||{room:$.name,section:G,reason:rt},{reason:rt,typeLabel:it,contentName:U.contentName||U.detail&&U.detail.contentName||""}),Q=n.escapeHtml(JSON.stringify(V));H+=`<td><button type="button" class="uc-slot busy ${n.occupancyKindClass(it)}" data-occ='${Q}' title="${n.escapeHtml($.name)} 第${G}节 · ${n.escapeHtml(rt)}">${n.escapeHtml(mt)}</button></td>`}else H+=`<td><div class="uc-slot free" title="${n.escapeHtml($.name)} 第${G}节 · 空闲"></div></td>`}return H+"</tr>"}).join(""),L=Number(x.dateOffset!=null?x.dateOffset:p.roomDateOffset)||0,O=a(($,H)=>`<button type="button" class="uc-btn${L===$?" primary":""}" data-room-day="${$}">${H}</button>`,"dayBtn");return`
    ...
    `,P.appendChild(f);let w=P.querySelector("#warningInfo");w&&document.body.appendChild(w),S.forEach(x=>{let _=x.closest('.widget-container-col, [class*="col-"]');_&&(_.style.display="none")}),P.querySelectorAll(":scope > .row").forEach(x=>{x.style.display="none"});let T=f.querySelector("#urppp-stats"),C=Math.max(A.length,5);for(let x=0;x<C;x++){let _=document.createElement("div");_.className="urppp-stat-card urppp-stat-skeleton",_.innerHTML='<div class="value">-</div><div class="label">加载中</div>',T.appendChild(_)}function b(){let x=y?Array.from(y.querySelectorAll(".infobox")):[];x.length!==0&&(T.innerHTML="",x.forEach(_=>{let q=_.innerText.trim().split(/\n+/).map(rt=>rt.trim()).filter(rt=>rt),I=q[0]||"",L=q.slice(1).join(" ").replace(/更多\.\.\./g,"").trim(),$=/[\u4e00-\u9fa5]/.test(I)||I.length>5?"value urppp-stat-value-text":"value",H=_.closest("a"),G=document.createElement(H?"a":"div");H&&(G.href=H.href||"javascript:void(0)",G.onclick=H.onclick,G.style.textDecoration="none"),G.className="urppp-stat-card";let U=p.statCardPrivacyMarkup(I,L);G.innerHTML=`<div class="${$}">${U.valueHtml}</div><div class="label">${U.labelHtml}</div>`,T.appendChild(G)}))}if(a(b,"updateStats"),b(),y){let x=new MutationObserver(()=>b());x.observe(y,{childList:!0,subtree:!0}),setTimeout(()=>x.disconnect(),5e3)}let m=f.querySelector("#urppp-left"),v=f.querySelector("#urppp-right");u(S[5],m,"我的日程安排"),u(S[0],v,"通知公告"),u(S[1],v,"我的待办任务"),u(S[2],v,"可申请业务"),u(S[3],v,"常用下载"),y&&(y.style.display="none"),d(),console.log("[URP++] 首页仪表板已重构")}return a(k,"rebuildDashboard"),{rebuildDashboard:k,refreshHomeFullCalendar:c,scheduleHomeFullCalendarRefresh:d,wrapWidget:u}}a(mi,"createDashboardController");function fr(p){return Math.round((Number(p)||0)*100)/100}a(fr,"round2");var hc=[{key:"a",level:"A",range:"90-100",gpa:4,min:90,max:100},{key:"am",level:"A-",range:"85-89",gpa:3.7,min:85,max:89.999},{key:"bp",level:"B+",range:"82-84",gpa:3.3,min:82,max:84.999},{key:"b",level:"B",range:"78-81",gpa:3,min:78,max:81.999},{key:"bm",level:"B-",range:"75-77",gpa:2.7,min:75,max:77.999},{key:"cp",level:"C+",range:"72-74",gpa:2.3,min:72,max:74.999},{key:"c",level:"C",range:"68-71",gpa:2,min:68,max:71.999},{key:"cm",level:"C-",range:"64-67",gpa:1.7,min:64,max:67.999},{key:"dp",level:"D+",range:"60-63",gpa:1.3,min:60,max:63.999},{key:"d",level:"D",range:"60-62",gpa:1,min:60,max:62.999},{key:"f",level:"F",range:"<60",gpa:0,min:0,max:59.999}],bi={优秀:95,"A+":98,A:95,"A-":87,良好:85,"B+":83,B:79,"B-":76,中等:73,"C+":73,C:69,"C-":65,及格:62,"D+":62,D:60,不及格:50,F:50},gc=[{key:"required",label:"必修",test:a(p=>/必修/.test(p),"test")},{key:"elective",label:"任选",test:a(p=>/任选/.test(p),"test")},{key:"optional",label:"选修",test:a(p=>/选修/.test(p),"test")},{key:"other",label:"其他",test:a(()=>!0,"test")}];function hi(p){let n=String(p||"").match(/^(\d{4})-(\d{4})-(\d+)/);return n?`${n[1].slice(2)}-${n[2].slice(2)}-${n[3]}`:String(p||"")}a(hi,"shortTerm");function ra({deps:p}){let n=p.scoreToNumber,c=p.scoreToGpa;function d(b){let m=n(b);if(m!=null)return m;let v=String(b||"").trim().toUpperCase();return bi[v]!=null?bi[v]:null}a(d,"scoreToNumberWithLevels");function u(b){return!b||b.unevaluated?!1:d(b.score)!=null}a(u,"hasScore");function k(b){let m=String(b||"").match(/^(\d{4})-(\d{4})-(\d+)/);return m?[Number(m[1]),Number(m[3])]:[9999,9999]}a(k,"termOrderKey");function P(b){let m=b&&b.passing&&b.passing[0];return m&&m.courses||[]}a(P,"allCourses");function S(b){let m=b&&b.officialGpa,v=Number(m);return m!=null&&Number.isFinite(v)&&v>=0&&v<=5?v:null}a(S,"officialGpa");function y(b){let m=S(b);return m??c(b.score)}a(y,"courseGpa");function A({scorePack:b,profile:m}){let v=P(b),x=m&&m.majorGpa?String(m.majorGpa).trim():"",_=0,q=0,I=0,L=0,O=0,$=0;return v.forEach(H=>{if(!u(H))return;let G=Number(H.credit)||0,U=d(H.score);if(U==null||G<=0)return;_+=G,q+=U*G;let rt=y(H);rt!=null&&(I+=rt*G,L+=G,H.required&&(O+=rt*G,$+=G))}),{majorGpa:x,requiredGpa:fr($?O/$:0),avgGpa:fr(L?I/L:0),avgScore:fr(_?q/_:0),totalCredit:fr(_),courseCount:v.length}}a(A,"computeMetrics");function f(b){let m=new Map;return(b||[]).forEach(v=>{if(!u(v))return;let x=v.term||"未分组",_=m.get(x);_||(_={term:x,count:0,credit:0,scoreW:0,gpaW:0,gpaCredit:0},m.set(x,_));let q=Number(v.credit)||0,I=d(v.score);if(I==null||(_.count+=1,q<=0))return;_.credit+=q,_.scoreW+=I*q;let L=y(v);L!=null&&(_.gpaW+=L*q,_.gpaCredit+=q)}),Array.from(m.values()).map(v=>({term:v.term,label:hi(v.term),count:v.count,credit:fr(v.credit),avgScore:fr(v.credit?v.scoreW/v.credit:0),avgGpa:fr(v.gpaCredit?v.gpaW/v.gpaCredit:0)})).sort((v,x)=>{let _=k(v.term),q=k(x.term);return _[0]-q[0]||_[1]-q[1]})}a(f,"computeTrend");function w(b){let m=hc.map(x=>({...x,count:0,credit:0}));(b||[]).forEach(x=>{if(!u(x))return;let _=d(x.score);if(_==null)return;let q=m.find(I=>_>=I.min&&_<=I.max);q&&(q.count+=1,q.credit+=Number(x.credit)||0)});let v=m.reduce((x,_)=>Math.max(x,_.count),1);return m.map(x=>({...x,ratio:Math.round(x.count/v*100)}))}a(w,"computeBands");function T(b){let m=gc.map(q=>({...q,credit:0,count:0}));(b||[]).forEach(q=>{if(!u(q))return;let I=String(q.attr||""),L=m.find(O=>O.test(I));L&&(L.credit+=Number(q.credit)||0,L.count+=1)});let v=m.reduce((q,I)=>q+I.credit,0)||1,x=m.filter(q=>q.count>0).map(q=>({key:q.key,label:q.label,credit:fr(q.credit),count:q.count,ratio:Math.round(q.credit/v*100)})),_=x.find(q=>q.key==="required");return{items:x,requiredCredit:_?_.credit:0,requiredRatio:_?_.ratio:0}}a(T,"computeShare");function C({scorePack:b,profile:m}){let v=P(b);return{metrics:A({scorePack:b,profile:m}),trend:f(v),bands:w(v),share:T(v),empty:v.length===0}}return a(C,"analyzeScores"),{analyzeScores:C,hasScore:u,officialGpa:S,scoreToNumberWithLevels:d,shortTerm:hi}}a(ra,"createScoreAnalysisData");var nr="var(--text-secondary)",uo="var(--border)";function pr(p){return at(String(p??""))}a(pr,"escapeLabel");function gi(p,n,c){let d=!!(p&&p.variant==="mobile");if(n==="trend"){if(!d)return{mobile:d,width:920,height:330,pad:{top:36,right:30,bottom:46,left:30}};let P={top:58,right:20,bottom:44,left:20},S=Math.max(56,Number(p&&p.slotWidth)||72);return{mobile:d,width:Math.max(300,P.left+P.right+Math.max(1,c)*S),height:286,pad:P}}if(!d)return{mobile:d,width:660,height:236,pad:{top:28,right:14,bottom:44,left:14}};let u={top:28,right:14,bottom:44,left:14},k=Math.max(44,Number(p&&p.slotWidth)||48);return{mobile:d,width:Math.max(320,u.left+u.right+Math.max(1,c)*k),height:236,pad:u}}a(gi,"resolveChartLayout");function co({width:p,height:n,mobile:c,kind:d,label:u}){let k=c?` data-urppp-chart-layout="mobile" style="width:max(100%,${p}px);max-width:none;height:auto"`:"";return`<svg viewBox="0 0 ${p} ${n}" class="urppp-sa-chart" role="img" aria-label="${u}" data-urppp-chart-kind="${d}"${k}>`}a(co,"openSvg");function ea({trend:p,palette:n,layout:c}){let d=(p||[]).filter(et=>et&&et.avgScore!=null),u=gi(c,"trend",d.length),{width:k,height:P,pad:S,mobile:y}=u,A=k-S.left-S.right,f=P-S.top-S.bottom;if(!d.length)return`${co({...u,kind:"trend",label:"学期成绩趋势"})}</svg>`;let w=d.length,T=a(et=>S.left+(et+.5)*(A/w),"xAt"),C=d.map(et=>Number(et.avgGpa)||0),b=d.map(et=>Number(et.avgScore)||0),m=d.map(et=>Number(et.credit)||0),v=Math.max(0,Math.min(...C)-.2),x=Math.min(5,Math.max(...C)+.2),_=Math.max(0,Math.min(...b)-4),q=Math.min(100,Math.max(...b)+4),I=Math.max(1,...m),L=x-v||1,O=q-_||1,$=a(et=>S.top+f-(et-v)/L*f,"yGpa"),H=a(et=>S.top+f-(et-_)/O*f,"yScore"),G=a(et=>S.top+f-et/I*f*.9,"yCredit"),U=d.map((et,st)=>`${T(st)},${$(et.avgGpa)}`).join(" "),rt=d.map((et,st)=>`${T(st)},${H(et.avgScore)}`).join(" "),it=[0,.25,.5,.75,1].map(et=>{let st=S.top+f-et*f;return`<line x1="${S.left}" y1="${st.toFixed(1)}" x2="${k-S.right}" y2="${st.toFixed(1)}" stroke="${uo}" stroke-width="1" stroke-dasharray="3 4"/>`}).join(""),mt=d.map((et,st)=>{let ft=T(st),Z=y?Math.min(30,A/w*.42):Math.min(26,A/w*.32),dt=G(et.credit);return`<rect x="${(ft-Z/2).toFixed(1)}" y="${dt.toFixed(1)}" width="${Z.toFixed(1)}" height="${(S.top+f-dt).toFixed(1)}" rx="3" fill="${n.credit}" opacity="0.55"/>
<text x="${ft.toFixed(1)}" y="${(dt-4).toFixed(1)}" text-anchor="middle" font-size="12" fill="${nr}">${pr(et.credit)}</text>`}).join(""),V=d.map((et,st)=>`<text x="${T(st).toFixed(1)}" y="${P-16}" text-anchor="middle" font-size="12" fill="${nr}">${pr(et.label)}</text>`).join(""),Q=d.map((et,st)=>{let ft=A/w,Z=T(st)-ft/2,dt=[`学期 ${et.label}`,`课程 ${et.count} 门`,`修读学分 ${et.credit}`,`加权均分 ${et.avgScore}`,`平均绩点 ${et.avgGpa}`].join(`
`);return`<rect class="urppp-sa-hover" x="${Z.toFixed(1)}" y="${S.top}" width="${ft.toFixed(1)}" height="${f.toFixed(1)}" fill="transparent"><title>${pr(dt)}</title></rect>`}).join(""),ot=d.map((et,st)=>`<circle cx="${T(st).toFixed(1)}" cy="${$(et.avgGpa).toFixed(1)}" r="3.5" fill="${n.gpaLine}"/><text x="${T(st).toFixed(1)}" y="${($(et.avgGpa)-9).toFixed(1)}" text-anchor="middle" font-size="11.5" font-weight="600" fill="${n.gpaLine}">${pr(et.avgGpa)}</text>`).join(""),X=d.map((et,st)=>`<circle cx="${T(st).toFixed(1)}" cy="${H(et.avgScore).toFixed(1)}" r="3" fill="${n.scoreLine}"/><text x="${T(st).toFixed(1)}" y="${(H(et.avgScore)+17).toFixed(1)}" text-anchor="middle" font-size="11.5" fill="${n.scoreLine}">${pr(et.avgScore)}</text>`).join(""),ct=y?`<g font-size="12">
  <rect x="${S.left}" y="30" width="12" height="12" rx="3" fill="${n.gpaLine}"/><text x="${S.left+18}" y="40" fill="${nr}">学期平均绩点</text>
  <rect x="${S.left+132}" y="30" width="12" height="12" rx="3" fill="${n.scoreLine}"/><text x="${S.left+150}" y="40" fill="${nr}">加权均分</text>
</g>`:`<g font-size="12">
  <rect x="${k-S.right-176}" y="8" width="12" height="12" rx="3" fill="${n.gpaLine}"/><text x="${k-S.right-158}" y="18" fill="${nr}">学期平均绩点</text>
  <rect x="${k-S.right-82}" y="8" width="12" height="12" rx="3" fill="${n.scoreLine}"/><text x="${k-S.right-64}" y="18" fill="${nr}">加权均分</text>
</g>`;return`${co({...u,kind:"trend",label:"学期成绩趋势"})}
${it}
${mt}
<g>${Q}</g>
<text x="${S.left}" y="18" font-size="12" fill="${nr}">每学期修读学分（柱）</text>
...
<g>${V}</g>
${ct}
</svg>`}a(ea,"trendChartSvg");function aa({bands:p,palette:n,layout:c}){let d=p||[],u=gi(c,"bands",d.length),{width:k,height:P,pad:S,mobile:y}=u,A=k-S.left-S.right,f=P-S.top-S.bottom,w=d.length||1,T=Math.max(1,...d.map(m=>m.count)),C=y?Math.min(32,A/w*.62):Math.min(40,A/w*.52),b=d.map((m,v)=>{let x=S.left+(v+.5)*(A/w),_=m.count?Math.max(8,m.count/T*f):0,q=S.top+f-_,I=(.4+(1-v/(w-1))*.6).toFixed(2),L=m.range||(m.min===0?"<60":`${m.min}-${m.max===100?"100":m.max}`),O=[`${m.level||""}（绩点 ${m.gpa}）`,`百分制 ${L}`,`课程 ${m.count} 门`].join(`
`);return`<rect class="urppp-sa-band" x="${(x-C/2).toFixed(1)}" y="${q.toFixed(1)}" width="${C.toFixed(1)}" height="${_.toFixed(1)}" rx="4" fill="${n.primary}" opacity="${I}"><title>${pr(O)}</title></rect>
<text x="${x.toFixed(1)}" y="${(q-6).toFixed(1)}" text-anchor="middle" font-size="12.5" font-weight="600" fill="var(--text)">${pr(m.count)}</text>
<text x="${x.toFixed(1)}" y="${P-26}" text-anchor="middle" font-size="11" font-weight="600" fill="${nr}">${pr(L)}</text>
<text x="${x.toFixed(1)}" y="${P-12}" text-anchor="middle" font-size="12" fill="${nr}">${pr(m.gpa)}</text>`}).join("");return`${co({...u,kind:"bands",label:"成绩分段分布"})}
<line x1="${S.left}" y1="${(S.top+f).toFixed(1)}" x2="${k-S.right}" y2="${(S.top+f).toFixed(1)}" stroke="${uo}" stroke-width="1"/>
${b}
</svg>`}a(aa,"bandsChartSvg");function fi({items:p,requiredRatio:n,palette:c}){let S=2*Math.PI*56,y=(p||[]).filter(T=>T&&T.ratio>0),A=Math.max(0,Math.min(100,Math.round(Number(n)||0)));if(!y.length)return'<svg viewBox="0 0 150 150" class="urppp-sa-chart" role="img" aria-label="课程类型构成"></svg>';let f=-90,w=y.map(T=>{let C=T.ratio/100*S,m=`<circle cx="75" cy="75" r="56" fill="none" stroke="${c.share&&c.share[T.key]||c.required}" stroke-width="24"
...
${w}
<text x="75" y="69" text-anchor="middle" font-size="22" font-weight="700" fill="var(--text)">${pr(A)}%</text>
<text x="75" y="91" text-anchor="middle" font-size="11.5" fill="${nr}">必修学分占比</text>
</svg>`}a(fi,"donutSvg");var fc=Object.freeze({gpaLine:"var(--primary)",scoreLine:"var(--text-secondary)",credit:"var(--primary)",primary:"var(--primary)",share:Object.freeze({required:"var(--primary)",elective:"var(--text-muted)",optional:"var(--text-secondary)",other:"var(--border)"})}),xc='<svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M4 20V10"/><path d="M10 20V4"/><path d="M16 20v-8"/><path d="M22 20H2"/></svg>',yc='<svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M6 9l6 6 6-6"/></svg>';function xi({deps:p}){let n=p&&p.palette||fc;function c(){return`<div id="urppp-score-analysis" class="urppp-sa" data-urppp-sa-state="collapsed">
  <button type="button" class="urppp-sa-toggle" aria-expanded="false">
    <span class="urppp-sa-icon">${xc}</span>
    <span class="urppp-sa-title">成绩分析</span>
    <span class="urppp-sa-summary" data-urppp-sa-summary>点击展开，查看成绩指标与学期变化</span>
    <span class="urppp-sa-chevron">${yc}</span>
  </button>
...
<div class="urppp-sa-grid">
  <section class="urppp-sa-card urppp-sa-trend">
    ...
    <div class="urppp-sa-chart-scroll">${ea({trend:A.trend,palette:n,layout:T})}</div>
  </section>
  <section class="urppp-sa-card urppp-sa-share">
    <h5 class="urppp-sa-card-title">课程类型构成</h5>
    <div class="urppp-sa-share-body">
      <div class="urppp-sa-donut">${fi({items:w.items,requiredRatio:w.requiredRatio,palette:n})}</div>
...
<div class="urppp-sa-grid">
  ...
  `;function S(t){let r=document.createElement("div");return r.className="urppp-inline-loader",r.innerHTML=P+(t?`<div>${t}</div>`:""),r}a(S,"makeInlineLoader");function y(t){return!t||!t.closest?!1:!!t.closest('[id^="div_page_loading"], [id*="page_loading"], [id*="PageLoading"]')}a(y,"isPageLoadingOverlay");function A(t){try{(t&&t.querySelectorAll?t:document).querySelectorAll('[id^="div_page_loading"], [id*="page_loading"]').forEach(e=>{e.querySelectorAll(".urppp-inline-loader").forEach(o=>{try{o.remove()}catch{}}),e.classList.remove("urppp-loading-active")})}catch{}}a(A,"cleanupPageLoadingOverlays");function f(t){try{let r=t&&t.querySelectorAll?t:document;A(r),r.querySelectorAll("img").forEach(e=>{try{if(!e||e.dataset.urpppReplaced==="1"||y(e))return;let o=(e.getAttribute("src")||e.src||"").toLowerCase();if(!o||!(o.includes("pageloading")||o.includes("page-loading")||o.includes("loading.gif")||o.includes("loading-0")||o.includes("loading-1"))||o.includes("/loading")&&!o.includes("pageloading")&&!o.includes("loading.gif")&&!o.includes("loading-0"))return;e.dataset.urpppReplaced="1";let i=S("");i.style.minHeight="0",i.style.padding="0",e.parentElement&&e.parentElement.replaceChild(i,e)}catch{}}),r.querySelectorAll(".layui-layer-content.layui-layer-loading0, .layui-layer-content.layui-layer-loading1, .layui-layer-content.layui-layer-loading2, .layui-layer-loading .layui-layer-content").forEach(e=>{try{if(!e||e.dataset.urpppReplaced==="1")return;if(e.dataset.urpppReplaced="1",e.style.setProperty("background","transparent","important"),e.style.setProperty("background-image","none","important"),!e.querySelector(".urppp-inline-loader")){let o=S("");o.style.minHeight="0",o.style.padding="0",e.appendChild(o)}}catch{}})}catch{}}if(a(f,"replaceNativeLoaders"),!window.__urpppLoaderObs){window.__urpppLoaderObs=!0;let t=!1,r=a(()=>{if(!t){t=!0;try{f(document)}catch{}t=!1}},"run");document.body&&setTimeout(r,0),document.addEventListener("DOMContentLoaded",()=>setTimeout(r,0),{once:!0});let e=a(()=>{new MutationObserver(()=>{clearTimeout(window.__urpppLoaderTimer),window.__urpppLoaderTimer=setTimeout(r,200)}).observe(document.documentElement,{childList:!0,subtree:!0})},"startObs");document.body?e():document.addEventListener("DOMContentLoaded",e,{once:!0})}let w="urppp_theme_v3",T="urppp_accent_v1",C="urppp_accent_presets_v1",b="urppp_scheme_v1",m="urppp_theme_follow_system_v1",v="urppp_clean_default_v1",x="urppp_clean_analysis_v1",_="urppp_apple_edge_line_v1",q="urppp_follow_use_dynamic_v1",I="urppp_brutal_palette_v1",L="urppp_brutal_active_palette_v1",O="urppp_privacy_v1",$="urppp_custom_identity_v1",H="urppp_schedule_first_monday_v1",G="urppp_schedule_json_format_v1",U={completedCourses:"已修课程",failedCourses:"未及格课程",majorGpa:"主修绩点",majorPlan:"主修方案",remainingCourses:"待修课程",passingTotalCredit:"全部及格总学分",passingAvgScore:"全部及格平均成绩",passingAvgGpa:"全部及格平均绩点",passingRequiredCredit:"全部及格必修学分",passingRequiredAvg:"全部及格必修平均",passingRequiredGpa:"全部及格必修绩点",schemeTotalCredit:"方案总学分",schemeAvgScore:"方案平均成绩",schemeAvgGpa:"方案平均绩点",schemeRequiredCredit:"方案必修学分",schemeRequiredAvg:"方案必修平均",schemeRequiredGpa:"方案必修绩点"},rt="",it=["#1E3A5F","#B53434","#0F766E","#7C3AED","#C2410C","#0369A1","#BE185D","#365314"],mt="#B53434",V="pink",Q=[{id:"pink",name:"高能粉",desc:"默认配色，热粉强调与酸性绿辅助",accent:"#FF006E",secondary:"#CCFF00",info:"#00D9FF",warning:"#FF9500"},{id:"acid",name:"酸性绿",desc:"酸性绿强调与热粉辅助",accent:"#CCFF00",secondary:"#FF006E",info:"#00D9FF",warning:"#FF9500"},{id:"cyan",name:"电子蓝",desc:"电子蓝强调与亮橙辅助",accent:"#00D9FF",secondary:"#FF9500",info:"#CCFF00",warning:"#FF006E"},{id:"orange",name:"亮橙",desc:"亮橙强调与电子蓝辅助",accent:"#FF9500",secondary:"#00D9FF",info:"#CCFF00",warning:"#FF006E"}],ot="tonal",X=[{id:"paper",name:"纯白卡片",desc:"卡片保持白，仅强调色跟种子"},{id:"tonal",name:"色调点缀",desc:"背景轻染，卡片带同色相浅底"},{id:"soft",name:"柔和粉彩",desc:"卡片明显粉彩/浅色，低对比"},{id:"vibrant",name:"鲜艳",desc:"背景与卡片都更有色，主色更饱和"},{id:"expressive",name:"表现力",desc:"双色拼色：卡片跟主色，背景走协调次色"}],{handleThemeDotClick:ct,injectNavbarThemeSwitch:et,rebuildNavbar:st,syncNavbarThemeUI:ft,syncThemeDotGroup:Z}=Si({theme:{BRUTAL_DEFAULT_PALETTE:V,DEFAULT_SEED:mt,applyTheme:Wt,buildSchemePreview:Xt,getAccent:Yt,getBrutalActivePalette:wo,getBrutalPaletteById:ae,getBrutalSelectedPalette:vo,getCurrent:Qt,getScheme:Lr,getSkin:Zt,isThemeModeAvailable:ee,setBrutalPalette:ko,skinSupportsDark:$r,skinSupportsDynamic:Mr,skinSupportsFixedPalettes:yo},settings:{ensureSettingsPanel:Yo,openSettingsPanel:Go,syncSettingsPanelUI:Dt}});function dt(){try{if(!!(window.matchMedia&&window.matchMedia("(max-width: 640px)").matches))return;let r=document.getElementById("navbar"),e=r?.querySelector(".ace-nav");if(!r||!e)return;let o=document.getElementById("intellegenceUDiv"),i=document.getElementById("clickdiv"),s=document.getElementById("form-search");if(!o){let D=document.createElement("li");D.className="green urppp-search-item",o=document.createElement("div"),o.id="intellegenceUDiv",D.appendChild(o),e.appendChild(D)}let l=o.closest("li")||o.parentElement,h=Array.from(e.children).find(D=>{let R=D.querySelector(":scope > a");if(!R)return!1;let B=R.getAttribute("href")||"",K=(R.getAttribute("title")||"")+" "+(R.textContent||"");return B.includes("customerServiceCenter")||/help|service|support/i.test(B)||!!R.querySelector(".glyphicon-headphones, .fa-headphones, .fa-question-circle, .fa-life-ring")||/帮助|客服|服务|帮助中心/i.test(K)}),g=Array.from(e.children).find(D=>D.classList.contains("light-blue")),E=h||g||null;E&&l&&E!==l&&((l.compareDocumentPosition(E)&Node.DOCUMENT_POSITION_FOLLOWING)!==0||e.insertBefore(l,E)),l&&!l.classList.contains("urppp-search-item")&&l.classList.add("urppp-search-item");let M=l;i?(i.removeAttribute("onclick"),i.setAttribute("role","button"),i.setAttribute("aria-label","搜索功能")):(i=document.createElement("button"),i.type="button",i.id="clickdiv",i.setAttribute("aria-label","搜索功能"),i.innerHTML='<i class="fa fa-search" id="clicki" aria-hidden="true"></i>',o.appendChild(i)),i.style.setProperty("left","8px","important"),i.style.setProperty("position","relative","important"),i.style.setProperty("z-index","31","important"),s||(s=document.createElement("div"),s.id="form-search",s.className="nav-search",s.innerHTML='<form class="form-search"><span class="input-icon"><input type="text" placeholder="查找功能..." class="nav-search-input" id="search-input" autocomplete="off"><i class="ace-icon fa fa-search" aria-hidden="true"></i></span></form>'),M&&s.parentElement!==M&&M.appendChild(s),M&&M.style.setProperty("position","relative","important"),s.classList.add("urppp-desktop-search"),s.style.setProperty("position","absolute","important"),s.style.setProperty("top","50%","important"),s.style.setProperty("right","24px","important"),s.style.setProperty("left","auto","important"),s.style.setProperty("transform","translateY(-50%)","important"),s.style.setProperty("width",s.dataset.open==="1"?"min(240px, calc(100vw - 24px))":"0px","important"),s.style.setProperty("max-width","calc(100vw - 24px)","important"),s.style.setProperty("opacity",s.dataset.open==="1"?"1":"0","important"),s.style.setProperty("pointer-events",s.dataset.open==="1"?"auto":"none","important"),s.style.setProperty("z-index","1200","important"),s.style.setProperty("margin","0","important"),s.style.setProperty("background","transparent","important"),s.style.setProperty("border","0 solid transparent","important"),s.style.setProperty("box-shadow","none","important"),s.style.setProperty("overflow","visible","important"),s.style.setProperty("transition","width .2s ease, opacity .2s ease","important");let j=s.querySelector("#search-input"),z=s.querySelector("form");if(!j||!z)return;z.style.setProperty("display","block","important"),z.style.setProperty("margin","0","important"),z.style.setProperty("padding","10px","important");let F=s.querySelector(".input-icon");F&&(F.style.setProperty("display","block","important"),F.style.setProperty("position","relative","important")),j.style.setProperty("display","block","important"),j.style.setProperty("width","100%","important"),j.style.setProperty("height","36px","important"),j.style.setProperty("box-sizing","border-box","important"),j.style.setProperty("padding","0 12px","important"),j.style.setProperty("border","1px solid var(--border)","important"),j.style.setProperty("border-radius","var(--radius-sm)","important"),j.style.setProperty("background","var(--input-bg)","important"),j.style.setProperty("color","var(--text)","important");let J=a(D=>{s.dataset.open=D?"1":"0",s.style.setProperty("width",D?"min(240px, calc(100vw - 24px))":"0px","important"),s.style.setProperty("opacity",D?"1":"0","important"),s.style.setProperty("pointer-events",D?"auto":"none","important"),i.setAttribute("aria-expanded",D?"true":"false"),D&&setTimeout(()=>j.focus(),30)},"setOpen");i.__urpppSearchBound||(i.__urpppSearchBound=!0,i.addEventListener("click",D=>{D.preventDefault(),D.stopImmediatePropagation(),J(s.dataset.open!=="1")},!0)),document.__urpppDesktopSearchOutsideBound||(document.__urpppDesktopSearchOutsideBound=!0,document.addEventListener("click",D=>{let R=document.getElementById("form-search"),B=document.getElementById("clickdiv");!R||R.dataset.open!=="1"||R.classList.contains("urppp-mobile-form-search")||R.closest("#urppp-mobile-search-panel")||R.contains(D.target)||B?.contains(D.target)||J(!1)},!0))}catch(t){console.warn("[URP++] desktop search bind failed",t)}}a(dt,"bindDesktopNavbarSearch");function kt(){if(document.getElementById("urppp-boot-loader"))return;let t=document.createElement("div");t.id="urppp-boot-loader",t.setAttribute("aria-busy","true"),t.innerHTML=`
    ...
    `;let r=document.documentElement||document.body;r&&r.appendChild(t)}a(kt,"ensureBootLoader");function wt(){try{document.documentElement.classList.add("urppp-ready"),document.body&&(document.body.classList.add("urppp-ready"),document.body.style.removeProperty("opacity"));let t=document.getElementById("urppp-boot-loader");if(!t)return;t.classList.add("urppp-boot-hide"),setTimeout(()=>{try{t.remove()}catch{}},280)}catch{}}a(wt,"hideBootLoader");try{kt()}catch{}window.__urpppBootSafety||(window.__urpppBootSafety=setTimeout(()=>{try{wt()}catch{}},2500));let Ct={default:{name:"简约白",vars:{"--bg":"#F5F5F7","--surface":"#FFFFFF","--text":"#1D1D1F","--text-secondary":"#6E6E73","--text-muted":"#86868B","--border":"#D2D2D7","--border-focus":"#0071E3","--input-bg":"#F5F5F7","--primary":"#0071E3","--primary-hover":"#0077ED","--ring":"rgba(0,113,227,0.28)","--shadow":"0 4px 16px rgba(0,0,0,0.08), 0 1px 3px rgba(0,0,0,0.04)","--radius":"18px","--radius-sm":"12px","--border-w":"0px"},font:'-apple-system, BlinkMacSystemFont, "SF Pro Text", "SF Pro Display", "Segoe UI", "PingFang SC", "Helvetica Neue", "Microsoft YaHei", sans-serif'},dark:{name:"深邃暗",vars:{"--bg":"#000000","--surface":"#1C1C1E","--text":"#F5F5F7","--text-secondary":"#A1A1A6","--text-muted":"#8E8E93","--border":"#38383A","--border-focus":"#0A84FF","--input-bg":"#2C2C2E","--primary":"#0A84FF","--primary-hover":"#409CFF","--ring":"rgba(10,132,255,0.32)","--shadow":"0 8px 24px rgba(0,0,0,0.45), 0 0 0 1px rgba(255,255,255,0.06)","--radius":"18px","--radius-sm":"12px"},font:'-apple-system, BlinkMacSystemFont, "SF Pro Text", "SF Pro Display", "Segoe UI", "PingFang SC", "Helvetica Neue", "Microsoft YaHei", sans-serif'},"scu-red":{name:"动态配色",vars:{"--bg":"#F5F5F7","--surface":"#FFFFFF","--text":"#1D1D1F","--text-secondary":"#6E6E73","--text-muted":"#86868B","--border":"#D2D2D7","--border-focus":"var(--urppp-accent, #B53434)","--input-bg":"#F5F5F7","--primary":"var(--urppp-accent, #B53434)","--primary-hover":"var(--urppp-accent-hover, #962929)","--ring":"var(--urppp-accent-ring, rgba(181,52,52,0.18))","--shadow":"0 4px 12px rgba(0,0,0,0.08), 0 1px 2px rgba(0,0,0,0.04)","--radius":"18px","--radius-sm":"12px"},font:'-apple-system, BlinkMacSystemFont, "SF Pro Text", "SF Pro Display", "Segoe UI", "PingFang SC", "Helvetica Neue", "Microsoft YaHei", sans-serif'}};function N(t,r,e){t/=255,r/=255,e/=255;let o=Math.max(t,r,e),i=Math.min(t,r,e),s=0,l=0,h=(o+i)/2;if(o!==i){let g=o-i;switch(l=h>.5?g/(2-o-i):g/(o+i),o){case t:s=(r-e)/g+(r<e?6:0);break;case r:s=(e-t)/g+2;break;default:s=(t-r)/g+4;break}s/=6}return{h:s*360,s:l,l:h}}a(N,"rgbToHsl");function Y(t,r,e){t=(t%360+360)%360,r=Math.max(0,Math.min(1,r)),e=Math.max(0,Math.min(1,e));let o=(1-Math.abs(2*e-1))*r,i=o*(1-Math.abs(t/60%2-1)),s=e-o/2,l=0,h=0,g=0;return t<60?(l=o,h=i):t<120?(l=i,h=o):t<180?(h=o,g=i):t<240?(h=i,g=o):t<300?(l=i,g=o):(l=o,g=i),{r:Math.round((l+s)*255),g:Math.round((h+s)*255),b:Math.round((g+s)*255)}}a(Y,"hslToRgb");function tt(t,r,e){let{r:o,g:i,b:s}=Y(t,r,e);return he(o,i,s)}a(tt,"hslHex");function ht(t){let{r,g:e,b:o}=Xr(Gt(t)||mt),i=N(r,e,o);return i.s<.12&&(i.s=.18),i}a(ht,"seedHsl");function bt(t,r,e){let o=Math.max(0,Math.min(100,e))/100,i=Math.max(0,Math.min(.95,r));return tt(t,i,o)}a(bt,"tone");function zt(t){switch(t){case"paper":case"neutral":return{chroma:1,secShift:0,primaryTone:38,whiteCard:!0,bgSeed:.05,surfaceSeed:0,borderSeed:.08};case"soft":return{chroma:1,secShift:10,primaryTone:42,bgSeed:.14,surfaceSeed:.16,borderSeed:.18};case"vibrant":return{chroma:1.15,secShift:14,primaryTone:36,bgSeed:.2,surfaceSeed:.22,borderSeed:.26};case"expressive":return{chroma:1.08,secShift:0,primaryTone:36,duo:!0,bgSeed:.12,surfaceSeed:.15,borderSeed:.18};default:return{chroma:1,secShift:18,primaryTone:40,bgSeed:.12,surfaceSeed:.13,borderSeed:.16}}}a(zt,"schemeProfile");function yt(t,r){let e=Gt(t)||mt,o=Math.max(0,Math.min(.45,Number(r)||0));return o<=.001?"#FFFFFF":Bt("#FFFFFF",e,o)}a(yt,"tintFromHex");function Ft(t){return t<25||t>=345?(t+28)%360:t<55?(t+22)%360:t<90?(t+160)%360:t<160?(t+40)%360:t<210?(t+35)%360:t<265?(t+48)%360:t<310?(t+40)%360:(t+24)%360}a(Ft,"companionHue");function Pt(t){let r=Gt(t)||mt,{h:e,s:o}=ht(r),i=Ft(e),s=Math.min(.72,Math.max(.28,o*.78));return bt(i,s,42)}a(Pt,"companionColor");function It(t,r){let e=Gt(t)||mt,{h:o,s:i}=ht(e),l=zt(r||ot),h=Math.min(.92,Math.max(.35,i*l.chroma)),g=Pt(e),{h:E}=ht(g),M=bt(o,h,l.primaryTone),j=bt(o,h,Math.max(24,l.primaryTone-10)),z=Bt("#FFFFFF",e,.18),F,J,D;l.whiteCard?(F=Bt("#F1F5F9",Bt("#FFFFFF",e,.08),.5),J="#FFFFFF",D="#E5E7EB"):l.duo?(F=Bt(yt(g,l.bgSeed+.04),"#EEF1F4",.1),J=Bt(yt(e,l.surfaceSeed),"#FFFFFF",.1),D=Bt("#E5E7EB",g,.16)):(F=Bt(yt(e,l.bgSeed),"#E8EBEF",.12),J=Bt(yt(e,l.surfaceSeed),"#FFFFFF",.12),D=Bt("#E5E7EB",e,Math.max(.08,l.borderSeed*.7)));let R=l.whiteCard?"#F8FAFC":Bt(J,yt(l.duo?g:e,Math.max(.05,(l.surfaceSeed||.1)*.55)),.35),B=bt(o,Math.min(.45,h*.55),14),K=gr(bt(o,h*.3,34),.88),ut=gr(bt(o,h*.22,46),.76),vt=gr(M,.18),Et="0 4px 12px "+gr(M,.1)+", 0 1px 2px "+gr(M,.05);return{"--bg":F,"--surface":J,"--text":B,"--text-secondary":K,"--text-muted":ut,"--border":D,"--border-focus":M,"--input-bg":R,"--primary":M,"--primary-hover":j,"--ring":vt,"--shadow":Et,"--radius":"18px","--radius-sm":"12px","--primary-container":z,"--secondary":g}}a(It,"buildMaterialSchemeVars");function Xt(t,r){let e=It(t,r);return{id:r,primary:e["--primary"],bg:e["--bg"],surface:e["--surface"],border:e["--border"],text:e["--text"]}}a(Xt,"buildSchemePreview");function zr(t){let r=Gt(t)||Yt()||mt;return X.map(e=>Object.assign({},e,Xt(r,e.id)))}a(zr,"listSchemePreviews");function ur(){let t=document.documentElement;["--primary","--primary-hover","--border-focus","--ring","--bg","--surface","--text","--text-secondary","--text-muted","--border","--input-bg","--shadow","--primary-container","--secondary"].forEach(r=>t.style.removeProperty(r))}a(ur,"clearInlinePrimaryOverrides");function Yt(){return Gt(GM_getValue(T,""))||""}a(Yt,"getAccent");function Lr(){let t=String(GM_getValue(b,ot)||ot);return X.some(r=>r.id===t)?t:ot}a(Lr,"getScheme");function oa(t){let r=X.some(e=>e.id===t)?t:ot;return GM_setValue(b,r),r}a(oa,"setScheme");function _i(t,r){if(!t)return;let e=Gt(t);if(e){if(GM_setValue(T,e),r&&r.scheme&&oa(r.scheme),r&&r.skipTheme){let o=Va(e,.15),i=gr(e,.15);document.documentElement.style.setProperty("--urppp-accent",e),document.documentElement.style.setProperty("--urppp-accent-hover",o),document.documentElement.style.setProperty("--urppp-accent-ring",i);try{ft()}catch{}try{Dt()}catch{}return}Wt("scu-red");try{ft()}catch{}try{Dt()}catch{}}}a(_i,"applyAccent");function na(){try{let t=GM_getValue(C,"");if(!t)return it.slice();let r=JSON.parse(t);return Array.isArray(r)?r.filter(e=>typeof e=="string"&&/^#?[0-9a-fA-F]{6}$/i.test(e.replace("#",""))).map(e=>e.startsWith("#")?e.toUpperCase():"#"+e.toUpperCase()):it.slice()}catch{return it.slice()}}a(na,"getAccentPresets");function Ei(t){let r=Gt(t||Yt()||mt);if(!r)return na();let e=na();return e=[r].concat(e.filter(o=>o.toLowerCase()!==r.toLowerCase())),e=e.slice(0,12),GM_setValue(C,JSON.stringify(e)),e}a(Ei,"saveAccentPreset");function Kt(){try{return!!GM_getValue(m,!1)}catch{return!1}}a(Kt,"isThemeFollowSystem");function Ae(t){return GM_setValue(m,!!t),!!t}a(Ae,"setThemeFollowSystem");function pa(){try{return!!GM_getValue(v,!1)}catch{return!1}}a(pa,"isCleanDefault");function Ci(t){return GM_setValue(v,!!t),!!t}a(Ci,"setCleanDefault");function ia(){try{return GM_getValue(x,"tab")==="direct"}catch{return!1}}a(ia,"isCleanAnalysisDirect");function Pi(t){return GM_setValue(x,t==="direct"?"direct":"tab"),t==="direct"?"direct":"tab"}a(Pi,"setCleanAnalysis");function xr(){try{let t=GM_getValue(_,!0);return t!==!1&&t!==0&&t!=="0"}catch{return!0}}a(xr,"isAppleEdgeLine");function zi(t){return GM_setValue(_,!!t),!!t}a(zi,"setAppleEdgeLine");function sa(){try{return!!GM_getValue(c,!1)}catch{return!1}}a(sa,"isAutoUpdateCheck");function Li(t){return GM_setValue(c,!!t),!!t}a(Li,"setAutoUpdateCheck");function Se(t,r){try{let e=GM_getValue(t,"");if(e&&typeof e=="object")return e;if(typeof e=="string"&&e.trim())return JSON.parse(e)}catch{}return r}a(Se,"readJsonSetting");function _e(t,r){return GM_setValue(t,JSON.stringify(r)),r}a(_e,"writeJsonSetting");function yr(){return ro(Se(O,null))}a(yr,"getPrivacySettings");function la(t){return _e(O,ro(t))}a(la,"setPrivacySettings");function qr(){return we(Se($,null))}a(qr,"getCustomIdentity");function mo(t){return _e($,we(t))}a(mo,"setCustomIdentity");function ca(){let t=Se(H,{});return t&&typeof t=="object"&&!Array.isArray(t)?t:{}}a(ca,"getScheduleFirstMondayMap");function bo(t,r){if(!t||!/^\d{4}-\d{2}-\d{2}$/.test(String(r||"")))return;let e=ca();e[String(t)]=String(r),_e(H,e)}a(bo,"rememberScheduleFirstMonday");function Ee(){let t="";try{t=GM_getValue(G,"")}catch{}let r=!!(t&&(typeof t!="string"||t.trim())),e=Se(G,null);try{if(r&&(!e||typeof e!="object"||Array.isArray(e)))throw new Error("配置不是 JSON 对象");let o=e&&typeof e=="object"?e:{},i={enabled:!!o.enabled,mapping:Pr(o.mapping||xe)};return rt="",i}catch{return rt=r?"JSON 映射配置损坏，已回退小爱课程兼容格式":"",{enabled:!1,mapping:Pr(xe)}}}a(Ee,"getScheduleJsonFormatSettings");function ho(t){let r=t&&typeof t=="object"?t:{},e={enabled:!!r.enabled,mapping:Pr(r.mapping||xe)};return rt="",_e(G,e)}a(ho,"setScheduleJsonFormatSettings");function go(){try{let t=String(location.pathname||"").replace(/\/+$/,"")||"/";return t==="/"||t==="/index"||/\/index\.html?$/i.test(t)}catch{return!1}}a(go,"isHomePage");function Ce(){try{return!!GM_getValue(q,!1)}catch{return!1}}a(Ce,"isFollowUseDynamic");function da(t){return GM_setValue(q,!!t),!!t}a(da,"setFollowUseDynamic");function qi(){try{return!!(window.matchMedia&&window.matchMedia("(prefers-color-scheme: dark)").matches)}catch{return!1}}a(qi,"systemPrefersDark");function vr(){return qi()&&$r()?"dark":Ce()&&Mr()?"scu-red":"default"}a(vr,"resolveFollowThemeName");function ee(t,r){return t==="dark"?$r(r):t==="scu-red"?Mr(r):t==="default"}a(ee,"isThemeModeAvailable");function Wt(t,r){r=r||{},!$r()&&Kt()&&Ae(!1),!Mr()&&Ce()&&da(!1),r.manual&&Ae(!1);let e;r.system||Kt()&&!r.manual?e=vr():(e=Ct[t]?t:Qt()||"default",Ct[e]||(e="default")),ee(e)||(e="default");let o=Ct[e]||Ct.default;r.skipPersist||GM_setValue(w,e),ur();let i=document.getElementById("urppp-theme-vars")||(()=>{let E=document.createElement("style");return E.id="urppp-theme-vars",(document.head||document.documentElement).appendChild(E),E})(),s=Yt(),l=Object.assign({},o.vars);if(e==="scu-red"){let E=s||mt,M=Lr();l=Object.assign(l,It(E,M));let j=l["--primary"]||E,z=l["--primary-hover"]||Va(j,.12);document.documentElement.style.setProperty("--urppp-accent",j),document.documentElement.style.setProperty("--urppp-accent-hover",z),document.documentElement.style.setProperty("--urppp-accent-ring",l["--ring"]||gr(j,.15)),document.documentElement.style.setProperty("--urppp-seed",E),document.documentElement.style.setProperty("--urppp-scheme",M)}else e==="default"?(document.documentElement.style.setProperty("--urppp-accent","#0071E3"),document.documentElement.style.setProperty("--urppp-accent-hover","#0077ED"),document.documentElement.style.setProperty("--urppp-accent-ring","rgba(0,113,227,0.28)"),document.documentElement.style.removeProperty("--urppp-seed"),document.documentElement.style.removeProperty("--urppp-scheme")):(document.documentElement.style.removeProperty("--urppp-accent"),document.documentElement.style.removeProperty("--urppp-accent-hover"),document.documentElement.style.removeProperty("--urppp-accent-ring"),document.documentElement.style.removeProperty("--urppp-seed"),document.documentElement.style.removeProperty("--urppp-scheme"));let h=":root {";for(let[E,M]of Object.entries(l))h+=`${E}:${M};`;h+="}",i.textContent=h,document.body&&(document.body.style.fontFamily=o.font);try{let E=document.documentElement;E.dataset.urpppTheme=e,E.classList.remove("urppp-theme-default","urppp-theme-dark","urppp-theme-scu-red"),E.classList.add("urppp-theme-"+e),E.classList.toggle("urppp-theme-follow",Kt()),document.body&&(document.body.dataset.urpppTheme=e,document.body.classList.toggle("urppp-dark",e==="dark"),document.body.classList.toggle("urppp-theme-follow",Kt()))}catch{}try{rr()}catch{}try{ft()}catch{}try{Dt()}catch{}try{Fo()}catch{}try{Di()}catch{}let g=document.getElementById("urppp-boot-loader");g&&(g.style.fontFamily=o.font)}a(Wt,"applyTheme");function Qt(){return GM_getValue(w,"default")}a(Qt,"getCurrent");function Tr(t){try{return!!GM_getValue("urppp_theme_css_"+t,"")}catch{return!1}}a(Tr,"themeDownloaded");function fo(t){let r=document.getElementById("urppp-store-theme-"+t);return r||(r=document.createElement("style"),r.id="urppp-store-theme-"+t,r.dataset.urpppStoreTheme=t,(document.head||document.documentElement).appendChild(r)),r}a(fo,"storeThemeStyleEl");function Ti(t){let r=document.getElementById("urppp-store-theme-"+t);r&&r.remove()}a(Ti,"removeStoreThemeStyle");function xo(){u.forEach(t=>{let r="urppp_theme_css_"+t.id,e="";try{e=GM_getValue(r,"")||""}catch{}e&&(fo(t.id).textContent=e)});try{rr()}catch{}}a(xo,"injectAllStoreThemeStyles");function Zt(){let t=GM_getValue(d,"apple"),r=u.find(e=>e.id===t);return r&&r.ready&&(r.installed!==!1||Tr(r.id))?t:"apple"}a(Zt,"getSkin");function ua(t,r){let e=t||Zt(),o=u.find(i=>i.id===e);return!!(o&&o[r])}a(ua,"getSkinCapability");function $r(t){return ua(t,"dark")}a($r,"skinSupportsDark");function Mr(t){return ua(t,"dynamic")}a(Mr,"skinSupportsDynamic");function yo(t){return ua(t,"palettes")}a(yo,"skinSupportsFixedPalettes");function ae(t){return Q.find(r=>r.id===t)||Q[0]}a(ae,"getBrutalPaletteById");function vo(){let t=String(GM_getValue(I,"acid")||"acid"),r=ae(t);return r.id===V?ae("acid"):r}a(vo,"getBrutalSelectedPalette");function wo(){let t=String(GM_getValue(L,V)||V);return ae(t)}a(wo,"getBrutalActivePalette");function ko(t,r){let e=r||{},o=ae(t);e.select&&o.id!==V&&GM_setValue(I,o.id),GM_setValue(L,o.id);try{rr()}catch{}try{ft()}catch{}try{Dt()}catch{}try{let i=document.getElementById("urppp-clean-root");i&&typeof i.__syncCleanThemeDots=="function"&&i.__syncCleanThemeDots()}catch{}}a(ko,"setBrutalPalette");function $i(t){let r=t||Zt();return r==="flat"?{"--radius":"0px","--radius-sm":"0px","--shadow":"none","--border-w":"2px","--urppp-card-border":"2px solid var(--text)","--urppp-input-border":"2px solid var(--text)","--urppp-action-radius":"0px","--urppp-action-border":"2px solid var(--text)","--urppp-action-shadow":"none","--urppp-action-bg":"var(--surface)","--urppp-action-color":"var(--text)","--urppp-menu-radius":"0px","--urppp-menu-border":"2px solid var(--text)","--urppp-menu-shadow":"none","--urppp-menu-bg":"var(--surface)","--urppp-menu-color":"var(--text)"}:r==="organic"?{"--radius":"22px","--radius-sm":"14px","--shadow":"0 2px 10px rgba(92,64,51,0.06)","--border-w":"1px","--urppp-card-border":"1px solid #E7E0D6","--urppp-input-border":"1px solid var(--border)","--urppp-action-radius":"999px","--urppp-action-border":"1px solid var(--border)","--urppp-action-shadow":"none","--urppp-action-bg":"var(--input-bg)","--urppp-action-color":"var(--primary)","--urppp-menu-radius":"14px","--urppp-menu-border":"1px solid var(--border)","--urppp-menu-shadow":"none","--urppp-menu-bg":"var(--input-bg)","--urppp-menu-color":"var(--text)"}:r==="editorial"?{"--radius":"0px","--radius-sm":"0px","--shadow":"none","--border-w":"0px","--urppp-card-border":"none","--urppp-input-border":"none","--urppp-action-radius":"0px","--urppp-action-border":"none","--urppp-action-shadow":"none","--urppp-action-bg":"transparent","--urppp-action-color":"var(--text)","--urppp-menu-radius":"0px","--urppp-menu-border":"1px solid var(--text)","--urppp-menu-shadow":"none","--urppp-menu-bg":"transparent","--urppp-menu-color":"var(--text)"}:r==="brutal"?{"--radius":"0px","--radius-sm":"0px","--shadow":"6px 6px 0 #000","--border-w":"3px","--urppp-card-border":"3px solid #000","--urppp-input-border":"2px solid #000","--urppp-action-radius":"0px","--urppp-action-border":"2px solid var(--text)","--urppp-action-shadow":"3px 3px 0 var(--text)","--urppp-action-bg":"var(--surface)","--urppp-action-color":"var(--text)","--urppp-menu-radius":"0px","--urppp-menu-border":"2px solid var(--text)","--urppp-menu-shadow":"3px 3px 0 var(--text)","--urppp-menu-bg":"var(--surface)","--urppp-menu-color":"var(--text)"}:r==="neu"?{"--radius":"16px","--radius-sm":"12px","--shadow":"5px 5px 10px #BEC3CA, -5px -5px 10px #F7F9FC","--border-w":"0px","--urppp-card-border":"none","--urppp-input-border":"1px solid rgba(38,49,66,.16)","--urppp-input-shadow":"inset 2px 2px 4px rgba(38,49,66,.16), inset -2px -2px 4px rgba(255,255,255,.72)","--urppp-action-radius":"12px","--urppp-action-border":"none","--urppp-action-shadow":"var(--shadow)","--urppp-action-bg":"var(--bg)","--urppp-action-color":"var(--text)","--urppp-menu-radius":"12px","--urppp-menu-border":"none","--urppp-menu-shadow":"var(--shadow)","--urppp-menu-bg":"var(--bg)","--urppp-menu-color":"var(--text)"}:{"--radius":"18px","--radius-sm":"12px","--shadow":"0 4px 16px rgba(0,0,0,0.08), 0 1px 3px rgba(0,0,0,0.04)","--border-w":"0px","--urppp-card-border":r==="apple"&&xr()?"1px solid rgba(0,0,0,0.08)":"none","--urppp-input-border":"1px solid var(--border)","--urppp-action-radius":"999px","--urppp-action-border":"none","--urppp-action-shadow":"0 2px 6px var(--ring)","--urppp-action-bg":"var(--primary)","--urppp-action-color":"var(--surface)","--urppp-menu-radius":"12px","--urppp-menu-border":r==="apple"&&xr()?"1px solid var(--border)":"none","--urppp-menu-shadow":"0 1px 3px rgba(0,0,0,.08)","--urppp-menu-bg":"var(--input-bg)","--urppp-menu-color":"var(--text)"}}a($i,"getSkinShapeOverrides");function Ir(){try{let t=Zt();if(t==="apple")return xr()?"1px solid rgba(0,0,0,0.08)":"none";if(t==="flat")return"2px solid var(--text)";if(t==="organic")return"1px solid #E7E0D6";if(t==="brutal")return"3px solid var(--text)";if(t==="editorial"||t==="neu")return"none"}catch{}return"1px solid var(--border)"}a(Ir,"urpppCardBorderValue");function rr(){let t=Zt();try{document.documentElement.setAttribute("data-urppp-skin",t)}catch{}try{document.body&&document.body.setAttribute("data-urppp-skin",t)}catch{}try{let r=t==="apple"&&xr();document.documentElement.setAttribute("data-urppp-apple-edge",r?"1":"0"),document.body&&document.body.setAttribute("data-urppp-apple-edge",r?"1":"0")}catch{}try{let r=document.getElementById("urppp-skin-vars")||(()=>{let s=document.createElement("style");return s.id="urppp-skin-vars",(document.head||document.documentElement).appendChild(s),s})(),e=$i(t),o=":root, html[data-urppp-skin] {";if(Object.keys(e).forEach(s=>{o+=s+":"+e[s]+";"}),o+="}",o+=".urppp-nav-dot.urppp-theme-disabled{opacity:.42!important;cursor:not-allowed!important;box-shadow:none!important;filter:grayscale(1)!important;transform:none!important;}",t==="flat"||t==="organic"||t==="brutal"||t==="neu"){if(t==="brutal"){let s=wo();o+='html[data-urppp-skin="brutal"]{--brutal-accent:'+s.accent+";--brutal-secondary:"+s.secondary+";--brutal-info:"+s.info+";--brutal-warning:"+s.warning+";}"}r.textContent=o;return}if(t==="apple"){let s=xr(),l=s?"1px solid rgba(0,0,0,0.08)":"none",h=s?"1px solid rgba(255,255,255,0.10)":"none",g=s?"1px solid rgba(0,0,0,0.06)":"none";o+=['html[data-urppp-skin="apple"]{--shadow:0 6px 20px rgba(0,0,0,.07),0 1px 3px rgba(0,0,0,.04);--border:'+(s?"rgba(0,0,0,0.08)":"rgba(0,0,0,0.04)")+";}",'html[data-urppp-skin="apple"].urppp-theme-dark,html.urppp-theme-dark[data-urppp-skin="apple"]{--shadow:0 10px 28px rgba(0,0,0,.45),0 0 0 1px rgba(255,255,255,.04);--border:'+(s?"rgba(255,255,255,0.10)":"rgba(255,255,255,0.06)")+";}",'html[data-urppp-skin="apple"] .widget-box,html[data-urppp-skin="apple"] .widget-box.transparent,html[data-urppp-skin="apple"] .panel,html[data-urppp-skin="apple"] .panel-default,html[data-urppp-skin="apple"] .well,html[data-urppp-skin="apple"] .thumbnail,html[data-urppp-skin="apple"] .infobox,html[data-urppp-skin="apple"] .profile-user-info,html[data-urppp-skin="apple"] .profile-user-info-striped,html[data-urppp-skin="apple"] .modal-content,html[data-urppp-skin="apple"] fieldset,html[data-urppp-skin="apple"] .urppp-stat-card,html[data-urppp-skin="apple"] .urppp-db-card,html[data-urppp-skin="apple"] .urppp-db-panel,html[data-urppp-skin="apple"] #urppp-dashboard .widget-box,html[data-urppp-skin="apple"] #urppp-root .uc,html[data-urppp-skin="apple"] #urppp-clean-root .uc-card,html[data-urppp-skin="apple"] #urppp-clean-root .uc-modal,html[data-urppp-skin="apple"] #urppp-clean-root .uc-top,html[data-urppp-skin="apple"] #urppp-clean-root .uc-tabbar,html[data-urppp-skin="apple"] .urppp-card,html[data-urppp-skin="apple"] #urppp-dashboard .urppp-card,html[data-urppp-skin="apple"] #urppp-clean-root .uc-score-pane,html[data-urppp-skin="apple"] #urppp-clean-root .uc-svc{border:'+l+"!important;box-shadow:var(--shadow)!important;}",'html[data-urppp-skin="apple"].urppp-theme-dark .widget-box,html[data-urppp-skin="apple"].urppp-theme-dark .panel,html[data-urppp-skin="apple"].urppp-theme-dark .profile-user-info,html[data-urppp-skin="apple"].urppp-theme-dark .modal-content,html[data-urppp-skin="apple"].urppp-theme-dark .urppp-card,html[data-urppp-skin="apple"].urppp-theme-dark #urppp-clean-root .uc-card,html[data-urppp-skin="apple"].urppp-theme-dark #urppp-clean-root .uc-score-pane,html[data-urppp-skin="apple"].urppp-theme-dark #urppp-root .uc{border:'+h+"!important;}",'html[data-urppp-skin="apple"] .page-content .widget-box,html[data-urppp-skin="apple"] #page-content-template .widget-box,html[data-urppp-skin="apple"] html body .page-content .profile-user-info.setLabelWidth{border:'+l+"!important;box-shadow:var(--shadow)!important;}",'html[data-urppp-skin="apple"] .btn,html[data-urppp-skin="apple"] .btn-default,html[data-urppp-skin="apple"] .btn-white,html[data-urppp-skin="apple"] .btn-primary,html[data-urppp-skin="apple"] .btn-info,html[data-urppp-skin="apple"] .btn-success,html[data-urppp-skin="apple"] .btn-warning,html[data-urppp-skin="apple"] .btn-danger,html[data-urppp-skin="apple"] a.btn,html[data-urppp-skin="apple"] #urppp-clean-root .uc-btn{border-color:transparent!important;box-shadow:0 1px 2px rgba(0,0,0,.05)!important;}','html[data-urppp-skin="apple"] .btn-primary,html[data-urppp-skin="apple"] .btn-info,html[data-urppp-skin="apple"] #urppp-clean-root .uc-btn.primary{border:none!important;}','html[data-urppp-skin="apple"] .table,html[data-urppp-skin="apple"] table,html[data-urppp-skin="apple"] .table-bordered,html[data-urppp-skin="apple"] .table-bordered>thead>tr>th,html[data-urppp-skin="apple"] .table-bordered>tbody>tr>td{border-color:rgba(0,0,0,.05)!important;}','html[data-urppp-skin="apple"].urppp-theme-dark .table,html[data-urppp-skin="apple"].urppp-theme-dark .table-bordered,html[data-urppp-skin="apple"].urppp-theme-dark .table-bordered>thead>tr>th,html[data-urppp-skin="apple"].urppp-theme-dark .table-bordered>tbody>tr>td{border-color:rgba(255,255,255,.06)!important;}','html[data-urppp-skin="apple"] .nav-tabs>li>a,html[data-urppp-skin="apple"] .nav-tabs{border-color:transparent!important;}','html[data-urppp-skin="apple"] .urppp-nav-link{border:none!important;}','html[data-urppp-skin="apple"] #urppp-clean-root .uc-lesson,html[data-urppp-skin="apple"] #urppp-clean-root .uc-grid-cell{border-color:'+(s?"rgba(0,0,0,0.06)":"transparent")+"!important;box-shadow:0 2px 8px rgba(0,0,0,.05)!important;}",'html[data-urppp-skin="apple"] #urppp-clean-root .uc-svc{border:'+g+"!important;box-shadow:0 2px 8px rgba(0,0,0,.05)!important;}",'html[data-urppp-skin="apple"] .urppp-nav-dot,html[data-urppp-skin="apple"] #urppp-nav-theme .urppp-nav-dot,html[data-urppp-skin="apple"] #urppp-clean-root .uc-top-theme .urppp-nav-dot,html[data-urppp-skin="apple"] #urppp-dots span{border-radius:50%!important;border:2px solid var(--border)!important;box-shadow:none!important;width:18px!important;height:18px!important;min-width:18px!important;min-height:18px!important;padding:0!important;overflow:hidden!important;background-clip:padding-box!important;flex:0 0 auto!important;}','html[data-urppp-skin="apple"] .urppp-nav-dot.ac,html[data-urppp-skin="apple"] #urppp-nav-theme .urppp-nav-dot.ac,html[data-urppp-skin="apple"] #urppp-clean-root .uc-top-theme .urppp-nav-dot.ac{border-color:var(--primary)!important;box-shadow:0 0 0 3px var(--ring)!important;}','html[data-urppp-skin="apple"] #urppp-nav-theme .urppp-nav-dot[data-theme="scu-red"],html[data-urppp-skin="apple"] #urppp-clean-root .uc-top-theme .urppp-nav-dot[data-theme="scu-red"],html[data-urppp-skin="apple"] #urppp-dots span[data-theme="scu-red"]{border-radius:50%!important;border:2px solid var(--border)!important;}'].join("")}else t==="editorial"&&(o+=`
        ...
        `);r.textContent=o;let i=document.head||document.documentElement;r.parentNode===i&&i.lastElementChild!==r&&i.appendChild(r)}catch(r){try{console.warn("[URP++] applySkinAttr",r)}catch{}}setTimeout(()=>{try{Vt(document)}catch{}},0)}a(rr,"applySkinAttr");function Ao(t){let r=u.find(e=>e.id===t&&e.ready&&(e.installed!==!1||Tr(e.id)));if(!r)return!1;GM_setValue(d,r.id);try{r.dynamic||da(!1),!r.dark&&Kt()&&Ae(!1);let e=Kt(),o=e?vr():Qt(),i=ee(o,r.id)?o:"default";rr(),Wt(i,{system:e})}catch{try{rr()}catch{}}try{Dt()}catch{}try{ft()}catch{}try{let e=document.getElementById("urppp-clean-root");e&&typeof e.__syncCleanThemeDots=="function"&&e.__syncCleanThemeDots()}catch{}return!0}a(Ao,"setSkin");function Mi(){if(!window.__urpppSystemThemeBound&&window.matchMedia){window.__urpppSystemThemeBound=!0;try{let t=window.matchMedia("(prefers-color-scheme: dark)"),r=a(()=>{if(Kt())try{Wt(vr(),{system:!0})}catch{}},"onChange");t.addEventListener?t.addEventListener("change",r):t.addListener&&t.addListener(r)}catch{}}}a(Mi,"bindSystemThemeListener");try{Kt()?Wt(vr(),{system:!0}):Wt(Qt())}catch{}try{rr()}catch{}try{Mi()}catch{}function Ii(t){let r=String(document.body&&document.body.innerText||t&&t.innerText||"").replace(/\s+/g," ").trim(),e=[/token\s*校验失败[！!]?/i,/令牌\s*校验失败[！!]?/i,/验证码.{0,12}(?:错误|失败|过期)[！!]?/i,/(?:用户名|账号|学号).{0,12}(?:密码).{0,12}(?:错误|失败)[！!]?/i,/登录.{0,12}(?:错误|失败)[！!]?/i];for(let o of e){let i=r.match(o);if(i)return i[0].trim()}return""}a(Ii,"extractLoginErrorMessage");function So(){let t=location.pathname,r=document.getElementById("formContent"),e=document.querySelector(".form-signin");if(!r||!e){setTimeout(So,50);return}if(r.querySelector(":scope > #urppp-root"))return;let o=Ii(r),i=e.querySelector('a[onclick*="toModifyPwd"]'),s=(()=>{let R=r.querySelector(".fadeIn.first svg");return R?R.outerHTML:""})(),l=(()=>{let R=document.querySelector("#tocas a");return R?R.href:"https://id.scu.edu.cn/"})();for(let R of r.children)R.style.display="none";r.style.cssText="max-width:420px;width:90%;margin:0 auto;background:transparent;box-shadow:none;border-radius:0;position:relative;z-index:1;";let h=location.pathname==="/loginEn",g=a((R,B)=>h?B:R,"t");r.insertAdjacentHTML("afterbegin",`
    <div id="urppp-root">
      <style>
      ...
      <div class="uc">
        <div class="ub" id="urppp-brand">
          ...
          <h1>${g("四川大学教务管理系统","SCU Academic System")}</h1>
          <p>${g("学生端 · 欢迎登录","Student Portal · Welcome")}</p>
        </div>

        <div class="ut" id="urppp-tabs">
          <button class="ac" data-mode="account">${g("账号登录","Account")}</button>
          <button data-mode="sso">${g("统一认证","SSO")}</button>
        </div>
        ...
        <div class="ufb" id="urppp-form">
          <div class="ufg">
            <label class="ufl" for="urppp-user">${g("学号","Student ID")}</label>
            <input class="ui" id="urppp-user" type="text" placeholder="${g("请输入学号","Enter student ID")}" autocomplete="username">
          </div>
          <div class="ufg">
            <label class="ufl" for="urppp-pass">${g("密码","Password")}</label>
            <input class="ui" id="urppp-pass" type="password" placeholder="${g("请输入密码","Enter password")}" autocomplete="current-password">
          </div>
          <div class="ucr">
            <div class="ufg ufg-cap">
              <label class="ufl" for="urppp-cap">${g("验证码","Captcha")}</label>
              <div class="ucap-input-wrap">
                <input class="ui" id="urppp-cap" type="text" placeholder="${g("请输入","Enter")}" maxlength="4" autocomplete="off">
                <div class="uci-wrap" id="urppp-capwrap" title="${g("点击刷新","Refresh")}">
                  <img class="uci" id="urppp-capimg" src="" alt="Captcha">
                </div>
              </div>
            </div>
          </div>
          <button class="ubtn" id="urppp-submit">${g("登 录","Sign In")}</button>
        </div>

        <div class="uft">
          <a href="javascript:void(0)" id="urppp-forgot">${g("忘记密码？","Forgot password?")}</a>
    ...
    </div>`);let E=r.querySelector("#urppp-root");[["#urppp-user","#input_username"],["#urppp-pass","#input_password"],["#urppp-cap","#input_checkcode"]].forEach(([R,B])=>{let K=E.querySelector(R),ut=document.querySelector(B);K&&ut&&(ut.value&&(K.value=ut.value),K.addEventListener("input",()=>{ut.value=K.value}))});let M=E.querySelector("#urppp-capimg"),j=E.querySelector("#urppp-capwrap"),z=document.querySelector(".form-signin img");if(M&&z){M.src=z.src;let R=a(()=>{let B=z.src.replace(/\?.*/,"")+"?"+Date.now();z.src=B,M.src=B},"refreshCap");j?j.addEventListener("click",R):M.addEventListener("click",R)}E.querySelectorAll(".ut button").forEach(R=>{R.addEventListener("click",()=>{if(R.dataset.mode==="sso"){location.href=l;return}E.querySelectorAll(".ut button").forEach(ut=>ut.classList.remove("ac")),R.classList.add("ac");let B=E.querySelector("#urppp-form"),K=E.querySelector("#urppp-sso");B&&(B.style.display="block"),K&&(K.style.display="none")})});let F=E.querySelector("#urppp-submit");F.addEventListener("click",()=>{if(F.dataset.submitting==="1")return;F.dataset.submitting="1",F.disabled=!0;let R=document.getElementById("loginButton");R?R.click():typeof e.requestSubmit=="function"?e.requestSubmit():e.submit(),setTimeout(()=>{F.dataset.submitting="0",F.disabled=!1},1500)}),E.querySelectorAll(".ui").forEach(R=>{R.addEventListener("keydown",B=>{B.key==="Enter"&&F.click()})}),E.querySelector("#urppp-forgot").addEventListener("click",R=>{R.preventDefault(),i&&i.click()});let J=E.querySelector("#urppp-dots"),D=a(()=>{if(!J)return;let R=Qt();J.querySelectorAll("span").forEach(K=>{K.classList.toggle("ac",K.dataset.theme===R)});let B=J.querySelector('span[data-theme="scu-red"]');if(B){let K=Yt()||mt;try{let ut=Xt(K,Lr());B.style.background="linear-gradient(135deg, "+ut.primary+" 0 55%, "+ut.surface+" 55% 100%)"}catch{B.style.background=K}}},"syncLoginDots");J&&(J.querySelectorAll("span").forEach(R=>{R.addEventListener("click",()=>{Wt(R.dataset.theme,{manual:!0}),D()})}),D()),console.log("[URP++] 登录界面已重建"),setTimeout(()=>{document.body.classList.add("urppp-ready"),wt()},100)}a(So,"rebuild");let{beautifyBreadcrumbs:Pe}=ki({});function ma(){try{document.querySelectorAll(".profile-user-info, .profile-user-info-striped").forEach(t=>{if(t.classList.contains("setLabelWidth")||t.classList.contains("urppp-query-form")||t.querySelector(".urppp-query-pair"))return;let r=Array.from(t.querySelectorAll(":scope > .profile-info-row, .profile-info-row"));!r.length||r.some(o=>Array.from(o.children).filter(i=>i.classList&&i.classList.contains("profile-info-name")).length>=2)||(t.classList.remove("urppp-query-form"),t.style.setProperty("width","100%","important"),t.style.setProperty("max-width","100%","important"),t.style.setProperty("display","block","important"),qe(t),r.forEach(o=>{o.classList.remove("urppp-query-row","urppp-dual-pair"),delete o.dataset.urpppQueryDone,delete o.dataset.urpppQueryCols;let i=Array.from(o.querySelectorAll(":scope > .urppp-query-pair"));if(i.length){let s=[];for(i.forEach(l=>Array.from(l.children).forEach(h=>s.push(h)));o.firstChild;)o.removeChild(o.firstChild);s.forEach(l=>o.appendChild(l))}o.style.setProperty("display","grid","important"),o.style.setProperty("grid-template-columns","140px minmax(0,1fr)","important"),o.style.setProperty("align-items","stretch","important"),o.style.setProperty("width","100%","important"),Array.from(o.children).forEach(s=>{s.classList&&(s.style.setProperty("float","none","important"),s.style.setProperty("margin-left","0","important"),s.style.setProperty("width","auto","important"),s.style.setProperty("max-width","none","important"),s.style.setProperty("display","flex","important"),s.style.setProperty("align-items","center","important"),s.style.setProperty("box-sizing","border-box","important"))})}))})}catch(t){console.warn("[URP++] single pair profile fix failed",t)}}a(ma,"fixSinglePairProfileForms");function ze(){let t=document.querySelector(".page-content")||document.getElementById("page-content-template");t&&(t.querySelectorAll(".col-xs-4, .col-xs-8, .col-sm-4, .col-sm-8, .col-md-4, .col-md-8").forEach(r=>{if(!r.querySelector(".setLabelWidth"))return;let e=r.querySelector(".setLabelWidth");e&&(r.querySelectorAll("h4.header, h3.header, .header.smaller, .header").forEach(o=>{e.contains(o)||o.compareDocumentPosition(e)&Node.DOCUMENT_POSITION_FOLLOWING&&(o.classList.add("urppp-section-label"),["background","background-color","background-image","border","box-shadow","border-radius","padding","margin","min-height"].forEach(i=>{o.style.removeProperty(i)}),o.style.setProperty("background","transparent","important"),o.style.setProperty("background-color","transparent","important"),o.style.setProperty("background-image","none","important"),o.style.setProperty("border","0 none transparent","important"),o.style.setProperty("box-shadow","none","important"),o.style.setProperty("border-radius","0","important"),o.style.setProperty("padding","4px 2px 10px","important"),o.style.setProperty("margin","0 0 8px 0","important"),o.style.setProperty("min-height","0","important"))}),e.classList.remove("urppp-query-form"),e.style.setProperty("padding","0","important"),e.style.setProperty("overflow","hidden","important"),e.style.setProperty("background","var(--surface)","important"),e.style.setProperty("border",Ir(),"important"),e.style.setProperty("border-radius","12px","important"),e.style.setProperty("box-shadow","none","important"))}),t.querySelectorAll(".profile-user-info.setLabelWidth, .profile-user-info-striped.setLabelWidth").forEach(r=>{r.classList.remove("urppp-query-form"),r.querySelectorAll(".profile-info-row").forEach(e=>{e.classList.remove("urppp-query-row"),delete e.dataset.urpppQueryDone,delete e.dataset.urpppQueryCols;let o=Array.from(e.querySelectorAll(":scope > .urppp-query-pair"));if(o.length){let i=[];for(o.forEach(s=>{Array.from(s.children).forEach(l=>i.push(l))});e.firstChild;)e.removeChild(e.firstChild);i.forEach(s=>e.appendChild(s))}})}),t.querySelectorAll(".setLabelWidth .profile-info-row, .profile-user-info.setLabelWidth .profile-info-row, .profile-user-info-striped.setLabelWidth .profile-info-row").forEach(r=>{let e=Array.from(r.querySelectorAll(":scope > .urppp-query-pair"));if(e.length){let s=[];for(e.forEach(l=>{Array.from(l.children).forEach(h=>s.push(h))});r.firstChild;)r.removeChild(r.firstChild);s.forEach(l=>r.appendChild(l))}r.classList.remove("urppp-query-row"),delete r.dataset.urpppQueryDone,delete r.dataset.urpppQueryCols;let o=Array.from(r.children).filter(s=>s.classList&&(s.classList.contains("profile-info-name")||s.classList.contains("profile-info-value")));o.filter(s=>s.classList.contains("profile-info-name")).length>=2?(r.classList.add("urppp-dual-pair"),r.style.setProperty("display","grid","important"),r.style.setProperty("grid-template-columns","112px minmax(140px,1fr) 112px minmax(140px,1fr)","important"),r.style.setProperty("align-items","stretch","important"),r.style.setProperty("width","100%","important"),r.style.setProperty("max-width","100%","important"),r.style.setProperty("float","none","important"),o.forEach(s=>{s.style.setProperty("float","none","important"),s.style.setProperty("clear","none","important"),s.style.setProperty("margin","0","important"),s.style.setProperty("margin-left","0","important"),s.style.setProperty("width","auto","important"),s.style.setProperty("max-width","none","important"),s.style.setProperty("min-width","0","important"),s.style.setProperty("box-sizing","border-box","important"),s.style.setProperty("display","flex","important"),s.style.setProperty("align-items","center","important"),s.classList.contains("profile-info-value")?(s.style.removeProperty("width"),s.style.setProperty("width","auto","important"),s.style.setProperty("justify-content","flex-start","important"),s.style.setProperty("white-space","normal","important"),s.style.setProperty("word-break","normal","important")):(s.style.setProperty("justify-content","flex-end","important"),s.style.setProperty("white-space","nowrap","important"))})):r.classList.remove("urppp-dual-pair")}),t.querySelectorAll(".profile-user-info.setLabelWidth, .profile-user-info-striped.setLabelWidth").forEach(r=>{r.classList.remove("urppp-query-form"),r.style.cssText=(r.getAttribute("style")||"").replace(/padding\s*:[^;]+;?/gi,""),r.style.setProperty("background","var(--surface)","important"),r.style.setProperty("border-radius","12px","important"),r.style.setProperty("overflow","hidden","important"),r.style.setProperty("border",Ir(),"important"),r.style.setProperty("box-shadow","none","important"),r.style.setProperty("width","100%","important"),r.style.setProperty("max-width","100%","important"),r.style.setProperty("box-sizing","border-box","important"),r.style.setProperty("margin","0 0 16px 0","important"),r.style.setProperty("padding","0","important");let e=r.closest(".col-xs-4, .col-xs-8, .col-sm-4, .col-sm-8, .col-md-4, .col-md-8")||r.parentElement;e&&Array.from(e.querySelectorAll("h4.header, h3.header, .header.smaller")).forEach(o=>{r.contains(o)||o.compareDocumentPosition(r)&Node.DOCUMENT_POSITION_FOLLOWING&&(o.classList.add("urppp-section-label"),o.style.setProperty("background","transparent","important"),o.style.setProperty("background-color","transparent","important"),o.style.setProperty("background-image","none","important"),o.style.setProperty("border","0 none transparent","important"),o.style.setProperty("box-shadow","none","important"),o.style.setProperty("border-radius","0","important"),o.style.setProperty("padding","4px 2px 10px","important"),o.style.setProperty("margin","0 0 8px 0","important"),o.style.setProperty("min-height","0","important"))})}),t.querySelectorAll(".urppp-col-row").forEach(r=>{r.classList.remove("urppp-col-row"),["display","flex-wrap","gap","align-items","width","box-sizing"].forEach(e=>r.style.removeProperty(e))}),t.querySelectorAll('[class*="col-xs-"], [class*="col-sm-"], [class*="col-md-"], [class*="col-lg-"]').forEach(r=>{["float","flex","width","max-width","padding-left","padding-right","box-sizing"].forEach(e=>{r.style.getPropertyPriority(e)==="important"&&r.style.removeProperty(e)}),r.style.setProperty("padding-left","0","important"),r.style.setProperty("box-sizing","border-box","important")}),t.querySelectorAll(".col-xs-4, .col-sm-4, .col-md-4").forEach(r=>{r.style.setProperty("padding-right","16px","important")}),t.querySelectorAll(".col-xs-8, .col-sm-8, .col-md-8").forEach(r=>{r.style.setProperty("padding-left","0","important"),r.style.setProperty("padding-right","0","important")}),t.querySelectorAll(".col-xs-4, .col-xs-8, .col-sm-4, .col-sm-8, .col-md-4, .col-md-8").forEach(r=>{r.querySelector(".setLabelWidth")&&r.querySelectorAll(":scope > h4.header, :scope > .header, :scope > .header.smaller").forEach(e=>{e.style.cssText+=";background:transparent!important;background-color:transparent!important;border:none!important;box-shadow:none!important;border-radius:0!important;padding:4px 2px 10px!important;margin:0 0 8px 0!important;min-height:0!important;"})}),t.querySelectorAll(".urppp-section-title-wrap").forEach(r=>{let e=r.querySelector("h4.header, h3.header, h5.header, .header.smaller");if(!e){r.remove();return}let o=r.nextElementSibling;for(;o&&!o.querySelector?.('.col-xs-4, .col-sm-4, .col-md-4, [class*="col-xs-"], [class*="col-sm-"]');)o=o.nextElementSibling;let i=o&&(o.querySelector(".col-xs-4, .col-sm-4, .col-md-4")||Array.from(o.children).find(s=>/col-(?:xs|sm|md|lg)-([1-9]|1[01])\b/.test(s.className||"")));i&&(i.insertBefore(e,i.firstChild),delete e.dataset.urpppHoisted,e.style.removeProperty("width"),e.style.removeProperty("max-width"),e.style.removeProperty("margin-left"),e.style.removeProperty("margin-right"),e.style.removeProperty("box-sizing"),e.style.removeProperty("position"),e.style.removeProperty("left")),r.remove()}))}a(ze,"alignRollInfoLayout");function Le(){let t=typeof unsafeWindow<"u"?unsafeWindow:window;return t.jQuery||t.$||window.jQuery||window.$||null}a(Le,"pageJQuery");function Ni(t){return t?t.id&&String(t.id).indexOf("pagination_pageSize_")===0?!0:!!(t.closest&&t.closest('#urppagebar, .urppagebreak, .dataTables_paginate, [id^="sample-table-2_paginate_"]')):!1}a(Ni,"isPagebarSelect");function _o(t){if(t){try{let r=Le();r&&r.fn&&r(t).data("chosen")&&r(t).chosen("destroy")}catch{}try{if(t.parentElement&&t.parentElement.querySelectorAll(":scope > .chosen-container").forEach(r=>{try{r.remove()}catch{}}),t.nextElementSibling&&t.nextElementSibling.classList.contains("chosen-container"))try{t.nextElementSibling.remove()}catch{}}catch{}t.classList.remove("urppp-chosen-hidden","chzn-done","chosen");try{delete t.dataset.urpppChosen}catch{}t.style.setProperty("display","inline-block","important")}}a(_o,"destroyPagebarChosen");let Eo=0,Co=!1;function Bi(){if(Co)return;Co=!0;let t=a(r=>{if(Date.now()<Eo){try{r.preventDefault()}catch{}try{r.stopPropagation()}catch{}}},"guard");document.addEventListener("mousedown",t,!0),document.addEventListener("mouseup",t,!0),document.addEventListener("click",t,!0)}a(Bi,"bindChosenPickGuard");function ba(t){if(!t||t.__urpppChosenNoPierce)return;t.__urpppChosenNoPierce=!0,Bi();let r=t.querySelector(".chosen-drop"),e=a(o=>{let i=o.target;!i||!i.closest||!i.closest(".chosen-results li")||(Eo=Date.now()+350)},"onPick");t.addEventListener("mouseup",e,!1),t.addEventListener("touchend",e,!1),r&&(r.addEventListener("mouseup",e,!1),r.addEventListener("touchend",e,!1))}a(ba,"bindChosenNoPierce");function ha(t=document){try{t.querySelectorAll(".chosen-container").forEach(ba)}catch{}}a(ha,"bindAllChosenNoPierce");function mr(){try{let t=Le();if(!t||!t.fn||typeof t.fn.chosen!="function")return!1;let r=document.querySelectorAll(".profile-user-info, .urppp-query-form, .profile-info-row, form"),e=new Set,o=[];if(r.forEach(i=>{i.querySelectorAll("select").forEach(s=>{e.has(s)||(e.add(s),o.push(s))})}),document.querySelectorAll("select.value_element, .profile-info-value > select").forEach(i=>{e.has(i)||(e.add(i),o.push(i))}),o.forEach(i=>{if(!i||i.multiple||i.disabled||i.size&&i.size>1)return;if(Ni(i)){_o(i);return}let s=t(i);if(!!s.data("chosen")||i.classList.contains("chzn-done")||!!(i.nextElementSibling&&i.nextElementSibling.classList.contains("chosen-container"))||!!(i.parentElement&&i.parentElement.querySelector(":scope > .chosen-container"))){i.dataset.urpppChosen="1",i.classList.add("urppp-chosen-hidden"),i.style.setProperty("display","none","important");let h=i.nextElementSibling&&i.nextElementSibling.classList.contains("chosen-container")?i.nextElementSibling:i.parentElement&&i.parentElement.querySelector(":scope > .chosen-container");h&&ba(h);return}try{i.classList.contains("select")||i.classList.add("select");try{s.data("chosen")&&s.chosen("destroy")}catch{}s.chosen({allow_single_deselect:!0,search_contains:!0,width:"100%",no_results_text:"无匹配项",disable_search_threshold:0}),i.dataset.urpppChosen="1",i.classList.add("urppp-chosen-hidden"),i.style.setProperty("display","none","important");let h=i.nextElementSibling&&i.nextElementSibling.classList.contains("chosen-container")?i.nextElementSibling:i.parentElement&&i.parentElement.querySelector(".chosen-container");h&&(h.style.setProperty("width","100%","important"),h.style.setProperty("min-width","0","important"),h.style.setProperty("display","block","important")),h&&ba(h)}catch(h){console.warn("[URP++] chosen init failed",i,h)}}),!window.__urpppChosenHtmlPatch){window.__urpppChosenHtmlPatch=!0;let i=t.fn.html;t.fn.html=function(){let s=i.apply(this,arguments);if(arguments.length)try{this.filter("select").add(this.find("select")).each(function(){let l=t(this);if(l.data("chosen")||l.next(".chosen-container").length)try{l.trigger("chosen:updated")}catch{}})}catch{}return s}}return!0}catch(t){return console.warn("[URP++] ensureQueryChosen failed",t),!1}}a(mr,"ensureQueryChosen");function Po(){if(window.__urpppChosenScheduleBound)return;window.__urpppChosenScheduleBound=!0,[0,200,600,1500,3e3].forEach(o=>setTimeout(()=>{mr(),ha()},o));let r=0,e=setInterval(()=>{r+=1;let o=mr();ha(),(o&&r>3||r>15)&&clearInterval(e)},500)}a(Po,"scheduleEnsureQueryChosen");let{beautifyPagebar:zo}=Tp({destroyPagebarChosen:_o}),{scheduleBeautifyPagebar:Lo}=qp({beautifyPagebar:zo});function ga(){try{document.querySelectorAll("#drag-ul, ul#drag-ul").forEach(t=>{if(!t)return;let r=Array.from(t.children).filter(e=>e.tagName==="LI");if(!r.length){t.classList.add("urppp-empty"),t.style.setProperty("display","none","important");let e=t.closest("#xq-section, .widget-main, .widget-body");e&&!e.querySelector("li")&&(e.classList.add("urppp-empty"),e.style.setProperty("display","none","important"));return}t.classList.remove("urppp-empty"),t.classList.add("urppp-drag-ul"),t.style.removeProperty("display"),t.style.setProperty("height","auto","important"),t.style.setProperty("min-height","0","important"),r.forEach(e=>{let o=(e.textContent||"").replace(/\s+/g," ").trim(),i=(e.getAttribute("onclick")||"").includes("goDetail")||e.classList.contains("ui-selectee")||e.classList.contains("jc-future")||!!e.querySelector("a");!i&&/校区/.test(o)&&o.length<=12?(e.classList.add("xq-section"),e.classList.remove("ui-selectee","jc-future","urppp-building-active")):i&&!e.classList.contains("jc-future")&&e.classList.add("ui-selectee")})}),window.__urpppBuildingActiveBound||(window.__urpppBuildingActiveBound=!0,document.addEventListener("click",t=>{let r=t.target&&t.target.closest?t.target.closest("#drag-ul > li"):null;if(!r||r.classList.contains("xq-section")||r.classList.contains("jc-future"))return;let e=r.parentElement;e&&(e.querySelectorAll("li.urppp-building-active, li.ui-selected").forEach(o=>{o.classList.remove("urppp-building-active","ui-selected")}),r.classList.add("urppp-building-active","ui-selected"))},!0))}catch(t){console.warn("[URP++] free classroom list beautify failed",t)}}a(ga,"beautifyFreeClassroomList");function qe(t){if(!t||!t.style)return;if(t.classList.contains("setLabelWidth")){t.classList.remove("urppp-query-form"),t.style.setProperty("width","100%","important"),t.style.setProperty("max-width","100%","important"),t.style.setProperty("box-sizing","border-box","important"),t.style.setProperty("float","none","important"),t.style.setProperty("display","block","important"),t.style.setProperty("background","var(--surface)","important"),t.style.setProperty("border",Ir(),"important"),t.style.setProperty("border-radius","12px","important"),t.style.setProperty("box-shadow","none","important"),t.style.setProperty("margin","0 0 16px 0","important"),t.style.setProperty("padding","0","important"),t.style.setProperty("overflow","hidden","important");return}let r=!!(t.closest&&t.closest(".widget-box, .widget-main, .widget-body, .panel"));t.style.setProperty("width","100%","important"),t.style.setProperty("max-width","100%","important"),t.style.setProperty("min-width","0","important"),t.style.setProperty("box-sizing","border-box","important"),t.style.setProperty("float","none","important"),t.style.setProperty("display","block","important"),t.style.setProperty("clear","both","important");let e=t.parentElement&&t.parentElement.tagName==="FORM"?t.parentElement:null;e&&(e.style.setProperty("width","100%","important"),e.style.setProperty("max-width","100%","important"),e.style.setProperty("display","block","important"),e.style.setProperty("float","none","important"),e.style.setProperty("box-sizing","border-box","important"),e.style.setProperty("margin","0","important"));let o=t.closest&&t.closest(".tab-pane, .tab-content");if(o&&(o.style.setProperty("width","100%","important"),o.style.setProperty("max-width","100%","important"),o.style.setProperty("box-sizing","border-box","important")),r){t.style.setProperty("background","transparent","important"),t.style.setProperty("border","none","important"),t.style.setProperty("border-radius","0","important"),t.style.setProperty("padding","0","important"),t.style.setProperty("margin","0","important"),t.style.setProperty("box-shadow","none","important");return}t.style.setProperty("background","var(--surface)","important"),t.style.setProperty("border",Ir(),"important"),t.style.setProperty("border-radius","12px","important"),t.style.setProperty("box-shadow","none","important"),t.style.setProperty("margin","0 0 18px 0","important"),!t.classList.contains("setLabelWidth")&&(t.classList.contains("urppp-query-form")||!!t.querySelector(".urppp-query-pair, .chosen-container"))?(t.style.setProperty("padding","14px 16px","important"),t.style.setProperty("overflow","visible","important")):(t.style.setProperty("padding","0","important"),t.style.setProperty("overflow","hidden","important"))}a(qe,"ensureProfileCardShell");function oe(){try{mr(),document.querySelectorAll(".page-content .profile-user-info, #page-content-template .profile-user-info").forEach(o=>{qe(o)});let t=a(o=>{let i=o.closest(".profile-user-info, .urppp-query-form")||o.parentElement;if(!i)return Math.min(Math.max(o.querySelectorAll(":scope > .urppp-query-pair").length,1),4);let s=0;return i.querySelectorAll(":scope > .profile-info-row, .profile-info-row").forEach(l=>{let h=l.querySelectorAll(":scope > .urppp-query-pair").length;h>s&&(s=h)}),Math.min(Math.max(s,1),4)},"getFormQueryCols"),r=a(o=>{let i=Array.from(o.querySelectorAll(":scope > .urppp-query-pair")),s=t(o);o.classList.add("urppp-query-row"),o.style.setProperty("display","grid","important"),o.style.removeProperty("grid-template-columns"),o.style.setProperty("column-gap","14px","important"),o.style.setProperty("row-gap","10px","important"),o.style.setProperty("align-items","center","important"),o.style.setProperty("width","100%","important"),o.style.setProperty("max-width","100%","important"),o.style.setProperty("box-sizing","border-box","important"),o.dataset.urpppQueryCols=String(s),i.forEach(l=>{l.style.removeProperty("grid-column")}),i.forEach(l=>{l.style.setProperty("display","flex","important"),l.style.setProperty("align-items","center","important"),l.style.setProperty("width","100%","important"),l.style.setProperty("min-width","0","important"),l.style.setProperty("max-width","100%","important"),l.style.setProperty("box-sizing","border-box","important"),l.style.removeProperty("flex");let h=l.querySelector(".profile-info-name"),g=l.querySelector(".profile-info-value");h&&(h.style.setProperty("float","none","important"),h.style.setProperty("display","flex","important"),h.style.setProperty("align-items","center","important"),h.style.setProperty("justify-content","flex-end","important"),h.style.setProperty("flex","0 0 var(--urppp-qlabel, 84px)","important"),h.style.setProperty("width","var(--urppp-qlabel, 84px)","important"),h.style.setProperty("min-width","var(--urppp-qlabel, 84px)","important"),h.style.setProperty("max-width","var(--urppp-qlabel-max, 96px)","important"),h.style.setProperty("margin","0","important"),h.style.setProperty("margin-left","0","important"),h.style.setProperty("padding","0 8px 0 0","important"),h.style.setProperty("background","transparent","important"),h.style.setProperty("border","none","important"),h.style.setProperty("border-right","none","important")),g&&(g.style.setProperty("float","none","important"),g.style.setProperty("display","flex","important"),g.style.setProperty("align-items","center","important"),g.style.setProperty("flex","1 1 auto","important"),g.style.setProperty("width","auto","important"),g.style.setProperty("min-width","0","important"),g.style.setProperty("max-width","none","important"),g.style.setProperty("margin","0","important"),g.style.setProperty("margin-left","0","important"),g.style.setProperty("padding","0","important"),g.style.setProperty("background","transparent","important"),g.style.setProperty("border","none","important"),g.querySelectorAll("input, select, .chosen-container, .form-control").forEach(E=>{E.style.setProperty("width","100%","important"),E.style.setProperty("min-width","0","important"),E.style.setProperty("max-width","none","important")})),l.querySelectorAll(".chosen-container").forEach(E=>{let M=E.previousElementSibling;M&&M.tagName==="SELECT"&&(M.style.setProperty("display","none","important"),M.classList.add("urppp-chosen-hidden"));let j=E.parentElement&&E.parentElement.querySelector("select");j&&(j.style.setProperty("display","none","important"),j.classList.add("urppp-chosen-hidden")),E.style.setProperty("width","100%","important"),E.style.setProperty("min-width","0","important"),E.style.setProperty("max-width","none","important");let z=E.querySelector(".chosen-single");if(z){z.style.setProperty("width","100%","important"),z.style.setProperty("max-width","none","important"),z.style.setProperty("display","flex","important"),z.style.setProperty("align-items","center","important"),z.style.setProperty("height","34px","important"),z.style.setProperty("line-height","normal","important");let F=z.querySelector(":scope > span, span");F&&(F.style.setProperty("line-height","normal","important"),F.style.setProperty("height","auto","important"),F.style.setProperty("margin-top","0","important"),F.style.setProperty("padding-top","0","important"));let J=z.querySelector("div");if(J){J.style.setProperty("display","flex","important"),J.style.setProperty("align-items","center","important"),J.style.setProperty("justify-content","center","important"),J.style.setProperty("top","0","important"),J.style.setProperty("bottom","0","important"),J.style.setProperty("height","auto","important");let D=J.querySelector("b");D&&(D.style.setProperty("margin","0","important"),D.style.setProperty("background-position","center center","important"),D.style.setProperty("background-size","12px 12px","important"),D.style.setProperty("width","14px","important"),D.style.setProperty("height","14px","important"))}}})})},"applyRowLayout");document.querySelectorAll(".profile-user-info.self, .profile-user-info-striped.self, .profile-user-info:has(.value_element)").forEach(o=>{if(o.classList.contains("setLabelWidth")||o.closest&&o.closest("#curriculumInfo-divcon, #curriculumInfo-divcon1, #curriculumInfo-divcon2, #fajh, #xnxq, #kz, #kc, #kcfa"))return;let i=Array.from(o.querySelectorAll(".profile-info-row")).some(l=>Array.from(l.children).filter(h=>h.classList&&h.classList.contains("profile-info-name")).length>=2),s=!!o.querySelector("select.chosen, select.select, .chosen-container");if(!i&&!s){o.classList.remove("urppp-query-form");return}o.querySelector('select, input:not([type="hidden"]), .chosen-container, .value_element, textarea')&&(o.classList.add("urppp-query-form"),qe(o),o.querySelectorAll(".profile-info-row").forEach(l=>{if(l.dataset.urpppQueryDone==="1"){l.querySelector(":scope > .urppp-query-pair")&&r(l);return}let h=Array.from(l.children).filter(j=>j.classList&&(j.classList.contains("profile-info-name")||j.classList.contains("profile-info-value"))),g=[];for(let j=0;j<h.length;){let z=h[j],F=h[j+1];z&&F&&z.classList.contains("profile-info-name")&&F.classList.contains("profile-info-value")?(g.push([z,F]),j+=2):j+=1}if(!g.length){l.dataset.urpppQueryDone="1";return}let E=document.createDocumentFragment(),M=new Set;for(g.forEach(([j,z])=>{let F=document.createElement("div");F.className="urppp-query-pair",F.appendChild(j),F.appendChild(z),M.add(j),M.add(z),E.appendChild(F)}),h.forEach(j=>{M.has(j)||E.appendChild(j)});l.firstChild;)l.removeChild(l.firstChild);l.appendChild(E),l.dataset.urpppQueryDone="1",r(l)}))}),mr()}catch(t){console.warn("[URP++] query form beautify failed",t)}}a(oe,"beautifyQueryForms");function qo(){if(window.__urpppChosenAlignBound)return;window.__urpppChosenAlignBound=!0;let t=!1,r=a(e=>{if(!t){t=!0;try{let o=e&&e.querySelectorAll?e:document,i=document.getElementById("urppp-chosen-li-style");i||(i=document.createElement("style"),i.id="urppp-chosen-li-style",document.documentElement.appendChild(i)),i.textContent=[".self div.profile-info-value a.chosen-single > span,","body .self div.profile-info-value a.chosen-single > span {","  line-height: normal !important;","  height: auto !important;","  margin-top: 0 !important;","  padding-top: 0 !important;","}",".self div.profile-info-value a.chosen-single,","body .self div.profile-info-value a.chosen-single {","  display: flex !important;","  align-items: center !important;","  height: 34px !important;","  line-height: normal !important;","}","body .chosen-container .chosen-results li,","body .chosen-with-drop .chosen-results li,","html body .chosen-container .chosen-results li.active-result {","  display:flex !important;","  align-items:center !important;","  justify-content:flex-start !important;","  height:36px !important;","  min-height:36px !important;","  max-height:36px !important;","  line-height:1 !important;","  padding:0 12px !important;","  margin:0 !important;","  box-sizing:border-box !important;","}","body .chosen-container .chosen-results li.highlighted,","body .chosen-container .chosen-results li.result-selected {","  display:flex !important;","  align-items:center !important;","  height:36px !important;","  line-height:1 !important;","  padding:0 12px !important;","}"].join(""),o.querySelectorAll(".chosen-results li").forEach(s=>{s.style.cssText=["display:flex !important","align-items:center !important","justify-content:flex-start !important","height:36px !important","min-height:36px !important","max-height:36px !important","line-height:1 !important","padding:0 12px !important","margin:0 !important","box-sizing:border-box !important"].join(";")}),o.querySelectorAll("a.chosen-single").forEach(s=>{s.style.setProperty("display","flex","important"),s.style.setProperty("align-items","center","important"),s.style.setProperty("height","34px","important"),s.style.setProperty("min-height","34px","important"),s.style.setProperty("line-height","normal","important"),s.style.setProperty("padding-top","0","important"),s.style.setProperty("padding-bottom","0","important");let l=s.querySelector(":scope > span");l&&(l.style.setProperty("line-height","normal","important"),l.style.setProperty("height","auto","important"),l.style.setProperty("margin-top","0","important"),l.style.setProperty("margin-bottom","0","important"),l.style.setProperty("padding-top","0","important"),l.style.setProperty("padding-bottom","0","important"))}),o.querySelectorAll(".chosen-search").forEach(s=>{if(!s.querySelector(".urppp-chosen-search-icon")){let l=document.createElement("i");l.className="fa fa-search urppp-chosen-search-icon",l.setAttribute("aria-hidden","true"),s.appendChild(l)}})}finally{setTimeout(()=>{t=!1},0)}}},"apply");document.addEventListener("mousedown",e=>{let o=e.target&&e.target.closest?e.target.closest(".chosen-container"):null;o&&(setTimeout(()=>r(o),0),setTimeout(()=>r(o),30),setTimeout(()=>r(o),100),setTimeout(()=>r(o),200))},!0);try{let e=window.jQuery||window.$;e&&e.fn&&e(document).off("chosen:showing_dropdown.urppp chosen:updated.urppp").on("chosen:showing_dropdown.urppp chosen:updated.urppp",o=>{let i=o.target&&o.target.parentElement?o.target.parentElement:document;setTimeout(()=>r(i),0),setTimeout(()=>r(i),60)})}catch{}}a(qo,"patchChosenDropdownAlign");function fa(){try{let t=document.getElementById("work_rest_schedule_modal");if(!t)return;(t.classList.contains("in")||t.classList.contains("show"))&&t.style.setProperty("display","block","important");let r=t.querySelector(".modal-body")||t,e=Array.from(r.querySelectorAll("table"));if(!e.length)return;let o=a(l=>(l||"").replace(/\s+/g," ").trim(),"norm"),i=a(l=>String(l??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;"),"esc");if(r.dataset.urpppWrsDone==="1")return;r.dataset.urpppWrsDone="1",e.forEach(l=>{let h=l.closest(".urppp-table-wrap");h&&t.contains(h)&&h.parentElement&&(h.parentElement.insertBefore(l,h),h.remove()),l.classList.add("urppp-wrs-table"),l.style.setProperty("width","100%","important");let g=Array.from(l.rows||[]);if(!g.length)return;let E=0;g.forEach(M=>{let j=o(M.textContent);if(!/\d{1,2}:\d{2}/.test(j))return;let z=0;Array.from(M.cells||[]).forEach(F=>{z+=F.colSpan||1}),z>E&&(E=z)}),E<4&&g.forEach(M=>{let j=0;Array.from(M.cells||[]).forEach(z=>{j+=z.colSpan||1}),j>E&&(E=j)}),E<1&&(E=1),Array.from(l.rows||[]).forEach(M=>{let j=Array.from(M.cells||[]);if(!j.length)return;let z=o(M.textContent);if(!/\d{1,2}:\d{2}/.test(z)&&(/作息时间|学年/.test(z)||/(望江|华西|江安)/.test(z)&&/校区|时间|安排|作息/.test(z))){let D=z;M.className="urppp-wrs-title-row",M.innerHTML='<td class="urppp-wrs-title" colspan="'+E+'" align="center">'+i(D)+"</td>";return}j.forEach(D=>{["border","borderTop","borderRight","borderBottom","borderLeft","textAlign","verticalAlign","width"].forEach(B=>{try{D.style[B]=""}catch{}}),D.classList.remove("urppp-wrs-title","urppp-wrs-period","urppp-wrs-time","urppp-wrs-head");let R=o(D.textContent);R&&(/^(上午|下午|晚上|中午)$/.test(R)||(D.rowSpan||1)>1&&/上午|下午|晚上|中午/.test(R)?D.classList.add("urppp-wrs-period"):/节次|大节|时间|校区/.test(R)&&!/\d{1,2}:\d{2}/.test(R)&&!/第\d/.test(R)?/节次|时间|大节|校区/.test(z)&&!/\d{1,2}:\d{2}/.test(z)&&D.classList.add("urppp-wrs-head"):/\d{1,2}:\d{2}/.test(R)&&D.classList.add("urppp-wrs-time"),D.style.setProperty("text-align","center","important"),D.style.setProperty("vertical-align","middle","important"))})})});let s=t.querySelector(".modal-title");s&&(s.style.setProperty("text-align","center","important"),s.style.setProperty("width","100%","important")),r.dataset.urpppWrsDone="1"}catch{}}a(fa,"beautifyWorkRestSchedule");let To="https://jwc.scu.edu.cn/cdxl.htm";function xa(){let t=['a[onclick*="jwc.scu.edu.cn/article/206"]','a[href*="jwc.scu.edu.cn/article/206"]',".cdsj a",".ace-nav a"],r=new Set;t.forEach(e=>{document.querySelectorAll(e).forEach(o=>{if(r.has(o))return;r.add(o);let i=(o.textContent||"").replace(/\s+/g,""),s=o.getAttribute("onclick")||"",l=o.getAttribute("href")||"";(i.includes("学校校历")||s.includes("article/206")||l.includes("article/206")||s.includes("jwc.scu.edu.cn")&&i.includes("校历"))&&(o.setAttribute("href",To),o.setAttribute("target","_blank"),o.setAttribute("rel","noopener noreferrer"),o.setAttribute("onclick",`window.open('${To}');return false;`))})})}a(xa,"patchSchoolCalendarLink");function Te(){document.querySelectorAll("#navbar-example, .page-content .navbar.navbar-static, #page-content-template .navbar.navbar-static").forEach(t=>{if(!t.querySelector(".nav-tabs"))return;["background","background-color","background-image","border","border-radius","box-shadow"].forEach(o=>{t.style.setProperty(o,o.startsWith("background")||o==="box-shadow"?o==="box-shadow"?"none":"transparent":o==="border"?"none":"0","important")}),t.style.setProperty("background","transparent","important"),t.style.setProperty("background-color","transparent","important"),t.style.setProperty("border","none","important"),t.style.setProperty("border-radius","0","important"),t.style.setProperty("box-shadow","none","important"),t.style.setProperty("width","100%","important"),t.style.setProperty("margin","0 0 14px 0","important"),t.style.setProperty("padding","0","important"),t.style.setProperty("min-height","0","important"),t.style.setProperty("box-sizing","border-box","important");let r=t.querySelector(".navbar-inner");r&&(r.style.setProperty("background","transparent","important"),r.style.setProperty("border","none","important"),r.style.setProperty("box-shadow","none","important"),r.style.setProperty("padding","0","important"),r.style.setProperty("min-height","0","important"),r.style.setProperty("filter","none","important"),r.style.setProperty("width","100%","important")),t.querySelectorAll(".container, .container-fluid").forEach(o=>{o.style.setProperty("width","100%","important"),o.style.setProperty("max-width","100%","important"),o.style.setProperty("margin","0","important"),o.style.setProperty("margin-left","0","important"),o.style.setProperty("padding","0","important"),o.style.setProperty("background","transparent","important"),o.style.setProperty("box-sizing","border-box","important")});let e=t.querySelector(".nav-tabs");e&&(e.style.setProperty("width","100%","important"),e.style.setProperty("margin","0","important"),e.style.setProperty("padding","8px 10px","important"),e.style.setProperty("background","var(--surface)","important"),e.style.setProperty("background-color","var(--surface)","important"),e.style.setProperty("border",Ir(),"important"),e.style.setProperty("border-radius","12px","important"),e.style.setProperty("box-sizing","border-box","important"))})}a(Te,"patchAceTabNavbars");function Nr(){let t=a(r=>{let e=NaN,o=[r.getAttribute("data-percent"),r.querySelector("[data-percent]")?.getAttribute("data-percent"),r.querySelector(".percent")?.textContent,r.querySelector(".urppp-pct-text")?.textContent];for(let i of o){if(i==null||i==="")continue;let s=parseFloat(String(i).replace(/[^\d.]/g,""));if(!Number.isNaN(s)){e=s;break}}if(Number.isNaN(e)){let i=(r.textContent||"").match(/(\d+(?:\.\d+)?)\s*%/);i&&(e=parseFloat(i[1]))}if(Number.isNaN(e)){let i=r.querySelector('.progress-bar, .infobox-progress [style*="width"], .urppp-pct-fill');if(i){let s=String(i.style.width||"").match(/([\d.]+)%/);s&&(e=parseFloat(s[1]))}}return Number.isNaN(e)?null:Math.max(0,Math.min(100,e))},"readPct");document.querySelectorAll(".infobox").forEach(r=>{let e=t(r);if(e==null)return;r.querySelectorAll("canvas").forEach(l=>l.remove()),r.querySelectorAll(".easy-pie-chart, .percentage, .infobox-progress").forEach(l=>{l.classList.contains("urppp-pct-bar")||l.remove()}),r.querySelectorAll(".urppp-pct-text, .urppp-pct-bar").forEach(l=>l.remove());let o=r.querySelector(".infobox-data")||r,i=document.createElement("div");i.className="urppp-pct-text",i.textContent=Math.round(e)+"%";let s=document.createElement("div");if(s.className="urppp-pct-bar"+(e<=0?" is-empty":""),e>0){let l=document.createElement("span");l.className="urppp-pct-fill",l.style.width=e+"%",s.appendChild(l)}o.insertBefore(s,o.firstChild),o.insertBefore(i,o.firstChild),r.dataset.urpppPctDone="1"})}a(Nr,"restyleInfoboxPercentages");function ne(t){let r=document.getElementById("treeDemo");if(!r)return;let e=!!(t&&t.force);if(r.dataset.urpppBusy==="1"&&!(t&&t.ignoreBusy))return;let o=r.closest('div[style*="border"]')||r.closest("#tree_div")?.parentElement||r.parentElement;o&&o.classList.add("urppp-plan-tree-shell"),r.classList.add("urppp-ztree");let i=typeof unsafeWindow<"u"?unsafeWindow:window,s=a(()=>{try{return(i.jQuery||i.$||window.jQuery||window.$)?.fn?.zTree?.getZTreeObj?.("treeDemo")||null}catch{return null}},"getZTree"),l=a(()=>{let R=Array.from(r.querySelectorAll('span.button.switch[class*="_open"]')).filter(B=>!/_docu\b/.test(B.className));return R.reverse().forEach(B=>{try{B.click()}catch{}}),R.length>0},"collapseAllDom"),h=a(()=>{let R=s();if(R)try{R.expandAll(!1)}catch{}return r.querySelector('span.button.switch[class*="_open"]:not([class*="_docu"])')&&l(),!0},"collapseAll");if(!window.__urpppExpandKzPatched){window.__urpppExpandKzPatched=!0;let R=a(()=>{let B=typeof unsafeWindow<"u"?unsafeWindow:window;try{B.expandKzByRule=function(){r.dataset.urpppUserExpanded||h()}}catch{}},"patch");R(),setTimeout(R,0),setTimeout(R,200)}r.dataset.urpppCollapsedOnce||(r.dataset.urpppCollapsedOnce="1",[0,80,200,500,1e3].forEach(R=>setTimeout(()=>{r.dataset.urpppUserExpanded||h()},R)));let g=document.querySelector("#two h4.header, #two .header");if(g&&!g.dataset.urpppLegendDone){let R=g.querySelector("font");if(R){let B=document.createElement("div");B.className="urppp-plan-legend",B.innerHTML=['<span class="urppp-lg done"><i class="ace-icon fa fa-check-square-o"></i>已完成课组</span>','<span class="urppp-lg todo"><i class="ace-icon fa fa-folder-o"></i>尚未完成课组</span>','<span class="urppp-lg pass"><i class="ace-icon fa fa-smile-o"></i>已修读及格</span>','<span class="urppp-lg fail"><i class="ace-icon fa fa-frown-o"></i>已修读未及格</span>','<span class="urppp-lg pending"><i class="ace-icon fa fa-meh-o"></i>尚未修读</span>'].join(""),R.replaceWith(B)}g.classList.add("urppp-plan-header"),g.dataset.urpppLegendDone="1"}let E=a(()=>{if(r.dataset.urpppBusy="1",window.__urpppPlanTreeObs)try{window.__urpppPlanTreeObs.disconnect()}catch{}},"pauseObs"),M=a(()=>{r.dataset.urpppBusy="0";let R=document.getElementById("tree_div")||r;if(window.__urpppPlanTreeObs&&R)try{window.__urpppPlanTreeObs.observe(R,{childList:!0,subtree:!0})}catch{}},"resumeObs"),j=a(R=>{let B=R;return B=B.replace(/\((最低修读学分:[^)]+)\)/g,(K,ut)=>{let vt=ut.split(",").map(qt=>qt.trim()).filter(Boolean),Et=[];return vt.forEach(qt=>{/最低修读学分|通过学分|必修课未修读|已及格课程门数/.test(qt)&&Et.push(qt)}),`<span class="urppp-sub">${(Et.length?Et:vt).map(qt=>{let Ot=qt.match(/^([^:：]+)[:：]\s*(.+)$/);if(!Ot)return qt;let Ht=Ot[1].trim(),$t=Ot[2].trim(),ar="neutral";return/通过|已及格/.test(Ht)?ar="ok":/未修读|未及格/.test(Ht)?ar=Number($t)>0?"warn":"muted":/最低/.test(Ht)&&(ar="req"),`<span class="urppp-kv ${ar}"><em>${Ht}</em><b>${$t}</b></span>`}).join("")}</span>`}),B=B.replace(/\[(\d{6,})\]/g,'<span class="urppp-code">$1</span>'),B=B.replace(/\[(\d+(?:\.\d+)?学分(?:,[^\]\[]*)?)\]/g,'<span class="urppp-meta">$1</span>'),B=B.replace(/\((必修|任选|限选),((?:[^()]|\([^()]*\))*)\)/g,(K,ut,vt)=>{let Et=String(vt).trim(),Lt=Et.match(/^(.+?)(?:\((\d{6,8})\))?$/),qt=(Lt?Lt[1]:Et).trim(),Ot=Lt&&Lt[2]?Lt[2]:"",Ht=parseFloat(qt),$t=!1;Number.isNaN(Ht)?/不及格|未通过|不通过/.test(qt)?$t=!1:(/^(?:[A-D][+]?|优秀|良好|中等|及格|通过)/.test(qt),$t=!0):$t=Ht>=60;let ar=Ot?`<i>${Ot}</i>`:"";return`<span class="urppp-score ${$t?"pass":"fail"}"><b>${ut}</b><em>${qt}</em>${ar}</span>`}),B=B.replace(/(<span class="urppp-code">[^<]*<\/span>)\s*([^<]+?)(?=\s*(?:<span class="urppp-meta"|<span class="urppp-score"|$))/g,'$1<span class="urppp-title">$2</span>'),B=B.replace(/(<\/i>)(?:&nbsp;|\s)*([^<]+?)(?=<span class="urppp-sub")/g,'$1 <span class="urppp-gname">$2</span>'),B=B.replace(/(<\/i>)(?:&nbsp;|\s)+(?=<span class="urppp-gname")/g,"$1 "),B},"formatNodeHtml"),z=a(R=>{let B=R.querySelector("i.fa, i.ace-icon"),K=R.closest("li");K&&(K.classList.remove("urppp-node-done","urppp-node-todo","urppp-node-pass","urppp-node-fail","urppp-node-pending"),B&&(B.classList.contains("fa-check-square-o")?K.classList.add("urppp-node-done"):B.classList.contains("fa-smile-o")?K.classList.add("urppp-node-pass"):B.classList.contains("fa-frown-o")?K.classList.add("urppp-node-fail"):B.classList.contains("fa-meh-o")?K.classList.add("urppp-node-pending"):B.classList.contains("fa-kz")&&K.classList.add("urppp-node-todo")))},"markStatus"),F=a(R=>{if(!R||!e&&R.dataset.urpppNodeDone==="1")return!1;z(R);let B=R.querySelector("span.node_name")||R;if(!B)return!1;if(!e&&B.querySelector(".urppp-score, .urppp-code, .urppp-sub, .urppp-title, .urppp-gname"))R.dataset.urpppNodeDone="1";else{let ut=B.dataset.urpppRaw;ut||(B.querySelector(".urppp-score, .urppp-code, .urppp-sub")?(R.dataset.urpppNodeDone="1",ut=null):(ut=B.innerHTML,ut&&(B.dataset.urpppRaw=ut))),ut&&(B.innerHTML=j(ut),R.dataset.urpppNodeDone="1")}let K=R.parentElement&&R.parentElement.querySelector(":scope > span.button.switch");return K&&(K.dataset.urpppSw||(K.dataset.urpppSw="1",/_docu\b/.test(K.className)&&(K.classList.add("urppp-switch-leaf"),K.style.setProperty("display","none","important"))),/_docu\b/.test(K.className)||K.classList.contains("urppp-switch-leaf")?R.classList.remove("urppp-expandable"):R.classList.add("urppp-expandable")),!0},"paintOne"),J=a((R,B)=>{let K=Array.from(R||[]),ut=0,vt=a(()=>{let Et=Math.min(ut+48,K.length);for(;ut<Et;ut++)F(K[ut]);ut<K.length?window.requestIdleCallback?requestIdleCallback(vt,{timeout:120}):setTimeout(vt,0):B&&B()},"step");vt()},"paintList"),D=a(R=>{let B=R||r;B.querySelectorAll("span.button.switch:not([data-urppp-sw])").forEach(K=>{K.dataset.urpppSw="1",/_docu\b/.test(K.className)&&(K.classList.add("urppp-switch-leaf"),K.style.setProperty("display","none","important"))}),B.querySelectorAll("li > a").forEach(K=>F(K))},"paintScopeSync");E();try{D(r),r.dataset.urpppExpandClick||(r.dataset.urpppExpandClick="1",r.addEventListener("click",B=>{if(B.target.closest&&B.target.closest("span.button.switch")){let Lt=B.target.closest("span.button.switch"),qt=Lt&&Lt.parentElement;if(!qt||/_docu\b/.test(Lt.className))return;if(r.dataset.urpppUserExpanded="1",r.dataset.urpppBusy="1",window.__urpppPlanTreeObs)try{window.__urpppPlanTreeObs.disconnect()}catch{}setTimeout(()=>{D(qt),r.dataset.urpppBusy="0";let Ot=document.getElementById("tree_div")||r;if(window.__urpppPlanTreeObs&&Ot)try{window.__urpppPlanTreeObs.observe(Ot,{childList:!0,subtree:!0})}catch{}},0);return}let K=B.target&&B.target.closest?B.target.closest("li > a"):null;if(!K||!r.contains(K))return;let ut=K.parentElement;if(!ut)return;let vt=ut.querySelector(":scope > span.button.switch");if(!vt||/_docu\b/.test(vt.className)||vt.classList.contains("urppp-switch-leaf")||!K.classList.contains("urppp-expandable")&&!/_open|_close/.test(vt.className))return;if(B.preventDefault(),B.stopImmediatePropagation(),r.dataset.urpppUserExpanded="1",r.dataset.urpppBusy="1",window.__urpppPlanTreeObs)try{window.__urpppPlanTreeObs.disconnect()}catch{}vt.click(),D(ut),r.dataset.urpppBusy="0";let Et=document.getElementById("tree_div")||r;if(window.__urpppPlanTreeObs&&Et)try{window.__urpppPlanTreeObs.observe(Et,{childList:!0,subtree:!0})}catch{}},!0));let R=a((B,K)=>{let ut=document.getElementById(B);return!ut||ut.dataset.urpppBound==="1"?!1:(ut.dataset.urpppBound="1",ut.addEventListener("click",vt=>{vt.preventDefault(),vt.stopImmediatePropagation(),r.dataset.urpppUserExpanded="1",E();try{let Et=s();if(K){Et?Et.expandAll(!0):r.querySelectorAll('span.button.switch[class*="_close"]').forEach(qt=>{/_docu\b/.test(qt.className)||qt.click()});let Lt=r.querySelectorAll('li > a:not([data-urppp-node-done="1"])');J(Lt,M)}else{if(Et)try{Et.expandAll(!1)}catch{}l(),setTimeout(()=>{r.querySelector('span.button.switch[class*="_open"]:not([class*="docu"])')&&l(),M()},0)}}catch{K||l(),M()}},!0),!0)},"bindAll");R("expandAllBtn",!0),R("collapseAllBtn",!1),r.dataset.urpppAllBtnsRetry||(r.dataset.urpppAllBtnsRetry="1",setTimeout(()=>{R("expandAllBtn",!0),R("collapseAllBtn",!1)},300),setTimeout(()=>{R("expandAllBtn",!0),R("collapseAllBtn",!1)},1e3))}finally{requestAnimationFrame(()=>{requestAnimationFrame(M)})}}a(ne,"beautifyPlanTree");function pe(){if(!dr())try{let t=document.getElementById("soliderbox");if(t){t.style.setProperty("width","100%","important"),t.style.setProperty("max-width","720px","important"),t.style.setProperty("min-width","0","important"),t.classList.remove("container");let i=t.closest(".profile-info-row");i&&(i.style.setProperty("display","flex","important"),i.style.setProperty("align-items","center","important"),i.style.setProperty("width","100%","important"),i.style.setProperty("max-width","100%","important"));let s=t.closest(".profile-info-value");s&&(s.style.setProperty("width","auto","important"),s.style.setProperty("max-width","100%","important"),s.style.setProperty("flex","1 1 auto","important"),s.style.setProperty("min-width","0","important"))}let r=document.getElementById("mycoursetable");if(!r)return;let e=!!(window.matchMedia&&window.matchMedia("(max-width: 640px)").matches);r.classList.toggle("urppp-mobile-schedule-scroll",e),r.style.setProperty("position","relative","important"),r.style.setProperty("width","100%","important");let o=72;e||r.querySelectorAll("#courseTableBody tr, table tbody tr").forEach(i=>{let s=i.offsetHeight||0;s>o&&(o=s)}),o<56&&(o=72),r.querySelectorAll("div.class_div").forEach(i=>{let s=parseInt(i.getAttribute("classNum")||"1",10)||1,l=i.scrollHeight||0;if(l>0){let h=Math.ceil(l/s);o=e?Math.max(o,Math.min(h,88)):Math.max(o,h)}}),e?o=Math.min(Math.max(o,72),88):(o<64&&(o=72),o>160&&(o=120)),r.querySelectorAll("#courseTableBody tr, table tbody tr").forEach(i=>{i.style.setProperty("height",o+"px","important")}),r.querySelectorAll("td").forEach(i=>{let s=Array.from(i.querySelectorAll(":scope > div.class_div"));if(!s.length)return;i.style.setProperty("position","relative","important"),i.style.setProperty("vertical-align","top","important"),i.style.setProperty("overflow","visible","important");let l=i.getBoundingClientRect().width||i.offsetWidth||i.clientWidth||0,h=getComputedStyle(i),g=i.closest("table"),E=g?getComputedStyle(g):null,M=parseFloat(h.borderLeftWidth)||0,j=E&&E.borderCollapse==="collapse"?M/2:M,z=Math.max(1,s.length);s.forEach((F,J)=>{let D=parseInt(F.getAttribute("classNum")||"1",10)||1,R=sp(l,z,J,j),B=R.left,K=R.width;F.style.setProperty("position","absolute","important"),F.style.setProperty("top","0px","important"),F.style.setProperty("left",B+"px","important"),F.style.setProperty("right","auto","important"),F.style.setProperty("bottom","auto","important"),F.style.setProperty("transform","none","important"),F.style.setProperty("width",K+"px","important"),F.style.setProperty("max-width","none","important"),F.style.setProperty("height",o*D+"px","important"),F.style.setProperty("margin","0","important"),F.style.setProperty("box-sizing","border-box","important"),F.style.setProperty("z-index","2","important"),F.style.setProperty("overflow","hidden","important")})})}catch(t){console.warn("[URP++] week schedule fix failed",t)}}a(pe,"fixWeekScheduleLayout");function ya(){try{let t=typeof unsafeWindow<"u"?unsafeWindow:window;if(!t||t.__urpppDivBuildPatched||typeof t.divBuild!="function")return;t.__urpppDivBuildPatched=!0;let r=t.divBuild;t.__urpppOriginalDivBuild=r,t.divBuild=function(){try{pe()}catch{try{return r.apply(this,arguments)}catch{}}};try{t.divBuild._urppp=!0}catch{}}catch(t){console.warn("[URP++] patch divBuild failed",t)}}a(ya,"patchSiteDivBuild");let Br=null,$o=!1;function Mo(){let t=document.getElementById("mycoursetable")||document.getElementById("page-content-template")||document.body;if(Br&&Br.root===t&&t?.isConnected){pe();return}Br&&Br.disconnect(),Br=null;let r=!$o;$o=!0;let e=!1,o=a(()=>{if(!(e||dr())&&!(!document.getElementById("soliderbox")&&!document.getElementById("mycoursetable"))){e=!0;try{ya(),pe()}finally{setTimeout(()=>{e=!1},40)}}},"run");ya(),[0,50,150,400,1e3,2e3].forEach(l=>setTimeout(()=>{ya(),o()},l)),r&&window.addEventListener("resize",()=>{clearTimeout(window.__urpppWeekSchedResize),window.__urpppWeekSchedResize=setTimeout(o,120)});let i=a(l=>{if(!l||dr())return;let h=[];l.nodeType===1&&(l.matches&&l.matches("div.class_div")&&h.push(l),l.querySelectorAll&&l.querySelectorAll("div.class_div").forEach(g=>h.push(g))),h.forEach(g=>{let E=g.parentElement;E&&E.tagName==="TD"&&E.style.setProperty("position","relative","important"),g.style.setProperty("position","absolute","important"),g.style.setProperty("top","0px","important"),g.style.setProperty("left","0px","important"),g.style.setProperty("right","auto","important"),g.style.setProperty("transform","none","important"),g.style.setProperty("width","100%","important"),g.style.setProperty("margin","0","important"),g.style.setProperty("box-sizing","border-box","important")})},"pinNew"),s=new MutationObserver(l=>{if(dr())return;let h=!1;l.forEach(g=>{if(g.type==="childList"&&g.addedNodes.forEach(E=>{i(E),h=!0}),g.type==="attributes"&&g.attributeName==="style"&&g.target&&g.target.classList&&g.target.classList.contains("class_div")){let E=g.target,M=E.style.left||"",j=parseFloat(M);(!M||M==="auto"||Number.isFinite(j)&&j>200)&&(E.style.setProperty("left","0px","important"),E.style.setProperty("top","0px","important"),E.style.setProperty("position","absolute","important")),h=!0}}),h&&(clearTimeout(window.__urpppWeekSchedMut),window.__urpppWeekSchedMut=setTimeout(()=>{requestAnimationFrame(o)},16))});if(t){s.observe(t,{childList:!0,subtree:!0,attributes:!0,attributeFilter:["style","class"]});let l=null,h=0,g=0;if(t.id==="mycoursetable"&&typeof window.ResizeObserver=="function"){let E=t.getBoundingClientRect().width||0;l=new window.ResizeObserver(M=>{let j=M[0]?.contentRect?.width||t.getBoundingClientRect().width||0;!j||Math.abs(j-E)<.5||(E=j,h||(h=requestAnimationFrame(()=>{h=0,o()})),clearTimeout(g),g=setTimeout(o,80))}),l.observe(t)}Br={root:t,observer:s,disconnect(){s.disconnect(),l&&l.disconnect(),h&&cancelAnimationFrame(h),clearTimeout(g)}}}r&&document.addEventListener("mouseup",()=>{document.getElementById("soliderbox")&&(setTimeout(o,200),setTimeout(o,500))},!0)}a(Mo,"scheduleWeekScheduleFix");function Io(){try{let t=document.getElementById("curriculumInfo-divcon2");if(!t)return;let r=parseFloat(t.style.width||getComputedStyle(t).width||"0");if(!r||r<40)return;t.classList.add("urppp-curriculum-drawer");let e=t.querySelector(".modal-body");if(!e)return;let o=e.querySelector(":scope > .col-xs-12 > .row")||e.querySelector(".col-xs-12 > .row")||e.querySelector(".row");if(!o)return;o.classList.add("urppp-drawer-layout");let i=o.querySelector(":scope > .urppp-drawer-toolbar, :scope > p");i&&i.tagName==="P"&&i.classList.add("urppp-drawer-toolbar");let s=o.querySelector(":scope > .urppp-drawer-body"),l=o.querySelector(".urppp-drawer-left"),h=o.querySelector(".urppp-drawer-right");s||(s=document.createElement("div"),s.className="urppp-drawer-body"),l||(l=document.createElement("div"),l.className="urppp-drawer-left"),h||(h=document.createElement("div"),h.className="urppp-drawer-right"),s.contains(l)||s.appendChild(l),s.contains(h)||s.appendChild(h),s.parentElement!==o&&(i&&i.parentElement===o?o.insertBefore(s,i.nextSibling):o.appendChild(s)),i&&o.firstElementChild!==i&&o.insertBefore(i,o.firstElementChild);let g=o.querySelector("#treeDemo, .ztree")||t.querySelector("#treeDemo, .ztree"),E=null;if(g){E=g.closest(".col-xs-6, .col-sm-6, .widget-box")||g.parentElement;let J=g.closest(".col-xs-6, .col-sm-6");J&&(E=J)}let M=["fajh","xnxq","kz","kc","kcfa"],j=M.map(J=>document.getElementById(J)).filter(J=>J&&t.contains(J));E&&E.parentElement!==l&&l.appendChild(E),Array.from(l.children).forEach(J=>{(M.includes(J.id)||J.id&&M.includes(J.id)||J!==E&&J.querySelector&&!J.querySelector("#treeDemo, .ztree")&&J.classList&&J.classList.contains("col-xs-6"))&&h.appendChild(J)}),M.forEach(J=>{let D=document.getElementById(J);!D||!t.contains(D)||(D.parentElement!==h&&h.appendChild(D),D.style.setProperty("width","100%","important"),D.style.setProperty("max-width","100%","important"),D.style.setProperty("float","none","important"),D.style.setProperty("margin","0","important"),D.style.setProperty("padding","0","important"),D.style.setProperty("box-sizing","border-box","important"),D.style.display!=="none"&&getComputedStyle(D).display!=="none"&&D.style.setProperty("display","block","important"))});let z=document.getElementById("fajh");z&&t.contains(z)&&(z.parentElement!==h&&h.appendChild(z),(!z.innerHTML||!z.innerHTML.trim())&&!z.querySelector(".urppp-drawer-skeleton, .profile-user-info, .widget-box")&&(z.innerHTML=["<div class='widget-box transparent urppp-drawer-skeleton'>","  <div class='widget-header widget-header-small'>","    <h4 class='widget-title smaller grey'>方案计划信息</h4>","  </div>","</div>","<div class='self profile-user-info profile-user-info-striped urppp-drawer-skeleton-card'>","  <div class='profile-info-row'><div class='profile-info-name'>加载中</div><div class='profile-info-value'>正在获取方案信息…</div></div>","</div>"].join(""),z.style.setProperty("display","block","important"),z.dataset.urpppSkeleton="1"),z.dataset.urpppSkeleton==="1"&&z.querySelector(".profile-info-value")&&/方案名称|计划名称|年级|院系/.test(z.textContent||"")&&(delete z.dataset.urpppSkeleton,z.querySelectorAll(".urppp-drawer-skeleton, .urppp-drawer-skeleton-card").forEach(D=>D.remove())),z.innerHTML&&z.innerHTML.trim()&&z.style.display==="none"&&(z.dataset.urpppSkeleton==="1"||z.querySelector(".profile-user-info"))&&z.style.setProperty("display","block","important")),h.style.setProperty("min-height","240px","important"),l.style.setProperty("min-height","240px","important"),E&&(E.style.setProperty("width","100%","important"),E.style.setProperty("max-width","100%","important"),E.style.setProperty("float","none","important"),E.style.setProperty("margin","0","important"),E.style.setProperty("padding","0","important"),E.style.setProperty("border","none","important"),E.style.setProperty("box-sizing","border-box","important"));let F=l.querySelector(".widget-box");F&&(F.style.setProperty("width","100%","important"),F.style.setProperty("margin","0","important"),F.style.setProperty("border",Ir(),"important"),F.style.setProperty("border-radius","12px","important"),F.style.setProperty("overflow","hidden","important"),F.style.setProperty("background","var(--surface)","important")),t.querySelectorAll(".profile-info-row").forEach(J=>{J.classList.remove("urppp-query-row","urppp-dual-pair"),J.style.setProperty("display","grid","important"),J.style.setProperty("grid-template-columns","112px minmax(0,1fr)","important"),J.style.setProperty("width","100%","important"),Array.from(J.children).forEach(D=>{D.classList&&(D.style.setProperty("float","none","important"),D.style.setProperty("margin-left","0","important"),D.style.setProperty("width","auto","important"),D.style.setProperty("max-width","none","important"))})}),t.querySelectorAll(".profile-user-info, .profile-user-info-striped").forEach(J=>{J.classList.remove("urppp-query-form");try{qe(J)}catch{}J.querySelectorAll(".profile-info-value, .profile-info-value span, span.editable").forEach(D=>{D.style.setProperty("color","var(--text)","important"),D.style.setProperty("opacity","1","important"),D.style.setProperty("visibility","visible","important")}),J.style.setProperty("border-radius","12px","important"),J.style.setProperty("overflow","hidden","important"),J.style.setProperty("width","100%","important"),J.style.setProperty("max-width","100%","important"),J.style.setProperty("display","block","important"),J.style.setProperty("box-sizing","border-box","important")})}catch(t){console.warn("[URP++] curriculum drawer beautify failed",t)}}a(Io,"beautifyCurriculumDrawer");function Fi(){if(window.__urpppCurriculumDrawerBound)return;window.__urpppCurriculumDrawerBound=!0;let t=a(()=>Io(),"run");[0,50,150,350,800,1600].forEach(o=>setTimeout(t,o));let r=new MutationObserver(o=>{o.some(s=>!!(s.type==="childList"||s.type==="attributes"&&s.target&&(s.target.id==="curriculumInfo-divcon2"||s.target.id==="fajh")))&&(clearTimeout(window.__urpppCurriculumDrawerTimer),window.__urpppCurriculumDrawerTimer=setTimeout(()=>requestAnimationFrame(t),16))}),e=document.getElementById("curriculumInfo-divcon2");e&&r.observe(e,{childList:!0,subtree:!0,attributes:!0,attributeFilter:["style","class"]}),document.addEventListener("click",o=>{if(!document.getElementById("curriculumInfo-divcon2"))return;let i=o.target&&o.target.closest?o.target.closest("a,button,span,div"):null,s=(i&&i.textContent||"").replace(/\s+/g,"");(/培养方案|与我相关|方案计划|自动化培养/.test(s)||i&&i.closest&&i.closest("#curriculumInfo-divcon2"))&&(setTimeout(t,0),setTimeout(t,50),setTimeout(t,150),setTimeout(t,400))},!0)}a(Fi,"scheduleCurriculumDrawerBeautify");let{scheduleScrubTableInlineBg:No,scrubTableHeaderInlineBg:Di}=Lp({isNativePdfIsolationActive:dr}),{disarmNoticeTableHover:ji,pinNoticeRowSurface:Bo,scrubNoticeInlineBg:Fo,stripMistakenNoticeTable:Do}=Mp({getCurrentTheme:Qt});function va(){try{let t=document.querySelector("h4.header, h3.header, h4, h3, .breadcrumb, .page-header");return Ep({pathname:location.pathname,href:location.href,title:document.title,headingText:t?.textContent||""})}catch{return!1}}a(va,"isNoticePageContext");function Oi(t){return ao(t,{noticePage:va()})}a(Oi,"isNoticeListTable");function wa(t){return Cp(t,{noticePage:va()})}a(wa,"isBusinessDataTable");let jo,{bindNoticeHoverScrub:Hi,scheduleBeautifyNoticeTables:Oo}=$p({beautifyNoticeTables:a(t=>jo(t),"beautifyNoticeTables"),pinNoticeRowSurface:Bo});({beautifyNoticeTables:jo}=Ip({isNativePdfIsolationActive:dr,bindNoticeHoverScrub:Hi,scrubNoticeInlineBg:Fo,stripMistakenNoticeTable:Do,disarmNoticeTableHover:ji,pinNoticeRowSurface:Bo,isBusinessDataTable:wa,isNoticeListTable:Oi,isNoticePageContext:va,isNoticeBulletText:eo}));let{wrapTables:Ho,bindTableWrapObserver:Ro}=Pp({isNativePdfIsolationActive:dr,isBusinessDataTable:wa});function ie(){try{document.querySelectorAll(".modal").forEach(r=>{if(!r||!r.style)return;r.style.getPropertyPriority("display")==="important"&&r.style.removeProperty("display"),r.classList.contains("in")||r.classList.contains("show")?r.style.display==="none"&&r.style.removeProperty("display"):(r.style.display==="block"||getComputedStyle(r).display!=="none")&&(r.style.setProperty("display","none","important"),setTimeout(()=>{try{!r.classList.contains("in")&&!r.classList.contains("show")&&(r.style.getPropertyPriority("display")==="important"&&r.style.removeProperty("display"),r.style.display="none")}catch{}},0))}),document.querySelector(".modal.in, .modal.show")||(document.querySelectorAll(".modal-backdrop").forEach(r=>{try{r.parentElement&&r.parentElement.removeChild(r)}catch{}}),document.body&&(document.body.classList.remove("modal-open"),document.body.style.removeProperty("padding-right")))}catch{}}a(ie,"cleanupStuckModals");function Ri(){if(window.__urpppModalOpenPatched)return;window.__urpppModalOpenPatched=!0;let t=a(s=>{!s||!s.style||(s.style.getPropertyPriority("display")==="important"&&s.style.removeProperty("display"),s.style.getPropertyPriority("opacity")==="important"&&s.style.removeProperty("opacity"),s.style.getPropertyPriority("pointer-events")==="important"&&s.style.removeProperty("pointer-events"),s.style.getPropertyPriority("visibility")==="important"&&s.style.removeProperty("visibility"))},"unlock"),r=a(s=>{if(!(!s||!s.classList))try{s.classList.remove("in","show"),s.setAttribute("aria-hidden","true"),s.style.removeProperty("display"),s.style.setProperty("display","none","important"),setTimeout(()=>{try{!s.classList.contains("in")&&!s.classList.contains("show")&&(s.style.getPropertyPriority("display")==="important"&&s.style.removeProperty("display"),s.style.display="none")}catch{}},30)}catch{}},"forceHide"),e=a(()=>{document.querySelectorAll(".modal-backdrop").forEach(s=>{try{s.parentElement&&s.parentElement.removeChild(s)}catch{}}),document.body&&(document.body.classList.remove("modal-open"),document.body.style.removeProperty("padding-right"),document.body.style.removeProperty("overflow"))},"clearBackdrops"),o=a(s=>{if(s){if(s.classList&&s.classList.contains("modal-backdrop")&&(s=document.querySelector(".modal.in, .modal.show")||s),!s||!s.classList||!s.classList.contains("modal")){e();return}t(s),r(s),e();try{let l=typeof Le=="function"&&Le()||typeof unsafeWindow<"u"&&(unsafeWindow.jQuery||unsafeWindow.$)||window.jQuery||window.$;if(l&&l.fn&&typeof l.fn.modal=="function"){try{l(s).trigger("hide.bs.modal")}catch{}try{l(s).modal("hide")}catch{}try{l(s).trigger("hidden.bs.modal")}catch{}}}catch{}setTimeout(()=>{r(s),document.querySelector(".modal.in, .modal.show")||e();try{ie()}catch{}},0)}},"hideModalEl");document.addEventListener("show.bs.modal",s=>{let l=s.target;if(!(!l||!l.classList||!l.classList.contains("modal"))){t(l),l.style.display==="none"&&l.style.removeProperty("display");try{l.getAttribute("data-backdrop")==="static"&&l.setAttribute("data-backdrop","true"),l.dataset&&(l.dataset.backdrop="true")}catch{}}},!0),document.addEventListener("hide.bs.modal",s=>{let l=s.target;!l||!l.classList||!l.classList.contains("modal")||t(l)},!0),document.addEventListener("hidden.bs.modal",s=>{let l=s.target;!l||!l.classList||!l.classList.contains("modal")||(r(l),document.querySelector(".modal.in, .modal.show")||(document.querySelectorAll(".modal-backdrop").forEach(h=>{try{h.parentElement&&h.parentElement.removeChild(h)}catch{}}),document.body&&(document.body.classList.remove("modal-open"),document.body.style.removeProperty("padding-right"))))},!0);let i=a(s=>{let l=s.target;if(!l||!l.closest||l.closest(".modal-dialog, .modal-content, .modal-header, .modal-body, .modal-footer")&&!l.closest('[data-dismiss="modal"], .modal .close, .modal [aria-label="Close"]'))return;if(l.classList&&l.classList.contains("modal-backdrop")){let M=document.querySelector(".modal.in, .modal.show")||document.querySelector('.modal[style*="display: block"], .modal[style*="display:block"]');M?(s.preventDefault(),s.stopPropagation(),o(M)):(s.preventDefault(),e(),ie());return}let h=null;if(l.classList&&l.classList.contains("modal")?h=l:h=l.closest(".modal.in, .modal.show, .modal"),!h||!h.classList.contains("modal")||!(h.classList.contains("in")||h.classList.contains("show")||getComputedStyle(h).display!=="none"))return;let E=h.querySelector(".modal-dialog");if(E){let M=E.getBoundingClientRect(),j=s.clientX,z=s.clientY;if(j>=M.left&&j<=M.right&&z>=M.top&&z<=M.bottom&&!l.closest('[data-dismiss="modal"], .modal .close, .modal [aria-label="Close"]'))return}else if(l.closest(".modal-content"))return;s.preventDefault(),s.stopPropagation(),o(h)},"onBlankClose");document.addEventListener("pointerdown",i,!0),document.addEventListener("mousedown",i,!0),document.addEventListener("click",i,!0),document.addEventListener("click",s=>{let l=s.target&&s.target.closest?s.target.closest('[data-dismiss="modal"], .modal .close, .modal [aria-label="Close"]'):null;if(!l)return;let h=l.closest(".modal");h&&(s.preventDefault(),s.stopPropagation(),o(h)),setTimeout(()=>{try{ie()}catch{}},50),setTimeout(()=>{try{ie()}catch{}},220)},!0),document.addEventListener("click",s=>{let l=s.target&&s.target.closest?s.target.closest("a,button,td,span,div,i"):null;if(!l)return;["curriculumInfo-divcon","curriculumInfo-divcon1","curriculumInfo-divcon2","calssInfo-divcon","classroomInfo-divcon","billContainer"].forEach(g=>{let E=document.getElementById(g);E&&(t(E),E.style.opacity==="0"&&E.style.removeProperty("opacity"),E.style.pointerEvents==="none"&&E.style.removeProperty("pointer-events"))}),document.querySelectorAll(".modal").forEach(g=>t(g));let h=l.getAttribute&&(l.getAttribute("data-target")||l.getAttribute("href")||"");if(h&&h.charAt(0)==="#"){let g=document.querySelector(h);g&&t(g)}},!0)}a(Ri,"patchModalOpenPath");let Fr=null,ka=0;function Aa(){if(dr())return;let t=document.getElementById("courseTable");t&&t.querySelectorAll("td").forEach(r=>{let e=r.style.backgroundColor;if(!e||!e.includes("rgba"))return;let o=e.match(/rgba\((\d+),\s*(\d+),\s*(\d+),\s*[\d.]+\)/);o&&(r.style.backgroundColor=`rgba(${o[1]},${o[2]},${o[3]},0.5)`)})}a(Aa,"applyCourseTableOpacity");function Uo(){let t=document.getElementById("mycoursetable")||document.getElementById("courseTable");if(Fr&&Fr.root===t&&t?.isConnected){Aa();return}if(clearTimeout(ka),Fr&&Fr.observer.disconnect(),Fr=null,!t)return;let r=new MutationObserver(()=>{clearTimeout(ka),ka=setTimeout(Aa,60)});r.observe(t,{childList:!0,subtree:!0,attributes:!0,attributeFilter:["style"]}),Fr={root:t,observer:r},Aa()}a(Uo,"bindCourseTableOpacityObserver");function Ui(){try{let z=Qt();document.documentElement.dataset.urpppTheme=z,document.documentElement.classList.remove("urppp-theme-default","urppp-theme-dark","urppp-theme-scu-red"),document.documentElement.classList.add("urppp-theme-"+z),document.body&&(document.body.dataset.urpppTheme=z,document.body.classList.toggle("urppp-dark",z==="dark"))}catch{}let t=document.getElementById("urppp-internal-style");t||(t=document.createElement("style"),t.id="urppp-internal-style",document.head.appendChild(t));{let z=t;z.textContent=Hp}let r=document.getElementById("urppp-table-beautify-style");r||(r=document.createElement("style"),r.id="urppp-table-beautify-style",document.head.appendChild(r)),r.textContent=Gp;let e=document.getElementById("urppp-navigation-style");e||(e=document.createElement("style"),e.id="urppp-navigation-style",document.head.appendChild(e)),e.textContent=Jp;let o=document.getElementById("urppp-dashboard-style");o||(o=document.createElement("style"),o.id="urppp-dashboard-style",document.head.appendChild(o)),o.textContent=Yp;let i=document.getElementById("urppp-schedule-card-style");i||(i=document.createElement("style"),i.id="urppp-schedule-card-style",document.head.appendChild(i)),i.textContent=Rp;let s=document.getElementById("urppp-mobile-style");s||(s=document.createElement("style"),s.id="urppp-mobile-style",document.head.appendChild(s)),s.textContent=Xp;try{rr()}catch{}ie(),Ri(),["curriculumInfo-divcon","curriculumInfo-divcon1","curriculumInfo-divcon2","calssInfo-divcon","classroomInfo-divcon"].forEach(z=>{let F=document.getElementById(z);!F||!F.style||(["display","opacity","pointer-events","visibility"].forEach(J=>{F.style.getPropertyPriority(J)==="important"&&F.style.removeProperty(J)}),F.style.opacity==="0"&&F.style.removeProperty("opacity"),F.style.pointerEvents==="none"&&F.style.removeProperty("pointer-events"))}),document.querySelectorAll(".modal").forEach(z=>{z.style&&z.style.getPropertyPriority("display")==="important"&&z.style.removeProperty("display")}),Ho(),Oo(),No(),setTimeout(()=>document.querySelectorAll("table").forEach(z=>{wa(z)&&Do(z)}),500),Mo(),pe(),Fi(),Io(),Ro();let l=document.querySelector(".page-content");l&&l.querySelectorAll(".widget-box").length>=4&&setTimeout(gs,500),ln(),kr(),g();function g(){let z="(max-width: 640px)",F=a(()=>!!(window.matchMedia&&window.matchMedia(z).matches),"isNarrow"),J=a((W,pt)=>{if(!(!W||!document.body)){if(pt){Object.hasOwn(W.dataset,"urpppDesktopSidebarMin")||(W.dataset.urpppDesktopSidebarMin=W.classList.contains("menu-min")?"1":"0",W.dataset.urpppDesktopBodyMin=document.body.classList.contains("menu-min")?"1":"0"),W.classList.remove("menu-min"),document.body.classList.remove("menu-min");return}Object.hasOwn(W.dataset,"urpppDesktopSidebarMin")&&(W.classList.toggle("menu-min",W.dataset.urpppDesktopSidebarMin==="1"),document.body.classList.toggle("menu-min",W.dataset.urpppDesktopBodyMin==="1"),delete W.dataset.urpppDesktopSidebarMin,delete W.dataset.urpppDesktopBodyMin)}},"syncMobileSidebarMode"),D=new WeakMap,R=a(W=>{let pt=D.get(W);pt&&cancelAnimationFrame(pt),D.delete(W)},"stopDrawerAnimation"),B=a((W,pt)=>{R(W);let lt=W.getBoundingClientRect(),xt=Math.max(lt.width,W.offsetWidth||0,260),gt=Math.max(-xt,Math.min(0,lt.left)),St=pt?0:-xt,Tt=Math.abs(St-gt),jt=Math.max(140,Math.round(260*Tt/xt)),cr=performance.now(),Rt=W.classList.contains("urppp-clean-sidebar"),or=Rt?"12030":"1200",Nt=Rt?"12030":"1030";W.style.setProperty("display","block","important"),W.style.setProperty("transition","none","important"),W.style.setProperty("visibility","visible","important"),W.style.setProperty("pointer-events",pt?"auto":"none","important"),W.style.setProperty("z-index",or,"important"),W.style.setProperty("transform",`translate3d(${gt}px, 0, 0)`,"important"),W.classList.toggle("urppp-drawer-closing",!pt),W.classList.add("display");let At=a(()=>{W.style.setProperty("transform",`translate3d(${St}px, 0, 0)`,"important"),pt?(W.classList.remove("urppp-drawer-closing"),W.style.setProperty("pointer-events","auto","important")):(W.classList.remove("display","urppp-drawer-closing"),W.style.setProperty("visibility","hidden","important"),W.style.setProperty("z-index",Nt,"important")),D.delete(W)},"finish");if(Tt<1){At();return}let hr=a(Mt=>{if(!W.isConnected){D.delete(W);return}let Ut=Math.min(1,(Mt-cr)/jt),Vn=Ut<.5?4*Ut*Ut*Ut:1-Math.pow(-2*Ut+2,3)/2,Re=gt+(St-gt)*Vn;if(W.style.setProperty("transform",`translate3d(${Re}px, 0, 0)`,"important"),Ut>=1){At();return}D.set(W,requestAnimationFrame(hr))},"step");D.set(W,requestAnimationFrame(hr))},"animateDrawer"),K=a((W,pt,lt)=>{if(W){B(W,lt),pt&&(pt.setAttribute("aria-expanded",lt?"true":"false"),pt.setAttribute("aria-label",lt?"关闭菜单":"打开菜单"));try{Or()}catch{}}},"setDrawerOpen"),ut=a(()=>{K(document.getElementById("sidebar"),document.getElementById("urppp-mobile-menu-button"),!1)},"closeDrawer"),vt=a(()=>{let pt=document.getElementById("urppp-mobile-search-panel")?.querySelector("#form-search");if(!pt)return;Object.entries({position:"relative",right:"auto",top:"auto",left:"auto",transform:"none",width:"100%","min-width":"0","max-width":"none",height:"36px",opacity:"1",margin:"0",overflow:"visible","z-index":"1"}).forEach(([xt,gt])=>pt.style.setProperty(xt,gt,"important")),[pt.querySelector("form"),pt.querySelector(".input-icon")].forEach(xt=>{xt&&Object.entries({display:"block",position:"relative",width:"100%","min-width":"0","max-width":"none",height:"36px",margin:"0",padding:"0","box-sizing":"border-box"}).forEach(([gt,St])=>xt.style.setProperty(gt,St,"important"))});let lt=pt.querySelector("#search-input");lt&&(lt.style.setProperty("display","block","important"),lt.style.setProperty("width","100%","important"),lt.style.setProperty("min-width","0","important"),lt.style.setProperty("max-width","none","important"),lt.style.setProperty("height","36px","important"),lt.style.setProperty("box-sizing","border-box","important"))},"syncMobileSearchLayout"),Et=a(()=>{let W=document.getElementById("form-search");if(!W||!W.__urpppMobileParent)return;let pt=W.__urpppMobileParent,lt=W.__urpppMobileNext;pt.isConnected&&(lt&&lt.parentElement===pt?pt.insertBefore(W,lt):pt.appendChild(W)),W.classList.remove("urppp-mobile-form-search"),W.dataset.open="0",W.removeAttribute("style"),delete W.__urpppMobileParent,delete W.__urpppMobileNext;try{dt()}catch{}},"restoreMobileSearch"),Lt=a(()=>{let W=document.querySelector("#navbar .menu-toggler");!W||W.dataset.urpppMobileHidden!=="1"||(W.style.removeProperty("display"),W.removeAttribute("aria-hidden"),W.dataset.urpppPreviousTabindex?W.setAttribute("tabindex",W.dataset.urpppPreviousTabindex):W.removeAttribute("tabindex"),delete W.dataset.urpppPreviousTabindex,delete W.dataset.urpppMobileHidden)},"restoreNativeMenuToggler"),qt=a(()=>{let W=document.getElementById("urppp-mobile-menu-button");if(!F())return W?.remove(),Lt(),null;if(W)return W;let pt=document.getElementById("navbar"),lt=document.getElementById("sidebar");if(!pt||!lt)return null;let xt=pt.querySelector(".menu-toggler");xt&&(xt.dataset.urpppMobileHidden="1",xt.dataset.urpppPreviousTabindex=xt.getAttribute("tabindex")||"",xt.style.setProperty("display","none","important"),xt.setAttribute("aria-hidden","true"),xt.setAttribute("tabindex","-1"));let gt=document.createElement("button");gt.type="button",gt.id="urppp-mobile-menu-button",gt.className="urppp-mobile-menu-button",gt.setAttribute("aria-label","打开菜单"),gt.setAttribute("aria-expanded","false");let St=pt.querySelector(".navbar-container")||pt;return St.insertBefore(gt,St.firstChild),gt},"ensureMenuToggler"),Ot=a(W=>{!W||W.dataset.urpppIconReady||(W.dataset.urpppIconReady="1",W.innerHTML=['<span class="urppp-menu-icon" aria-hidden="true">','<svg class="urppp-menu-icon-open" viewBox="0 0 24 24" focusable="false">','<path d="M5 8h14"></path><path d="M5 16h10"></path>',"</svg>",'<svg class="urppp-menu-icon-close" viewBox="0 0 24 24" focusable="false">','<path d="M7 7l10 10"></path><path d="M17 7 7 17"></path>',"</svg>","</span>"].join(""))},"ensureMenuButtonIcon"),Ht=a(()=>{let W=qt(),pt=document.getElementById("sidebar");W&&Ot(W),W&&pt&&!W.__urpppToggleHandler&&(W.setAttribute("aria-label","打开菜单"),W.setAttribute("aria-expanded",pt.classList.contains("display")?"true":"false"),W.__urpppToggleHandler=lt=>{lt.preventDefault(),lt.stopImmediatePropagation(),F()&&J(pt,!0);let xt=W.getAttribute("aria-expanded")!=="true";K(pt,W,xt)},W.addEventListener("click",W.__urpppToggleHandler,!0)),document.__urpppMobileDrawerOutsideBound||(document.__urpppMobileDrawerOutsideBound=!0,document.addEventListener("click",lt=>{if(!F()||!lt.target.closest)return;let xt=document.getElementById("sidebar");if(!xt||!xt.classList.contains("display"))return;let gt=document.getElementById("urppp-clean-root");gt&&gt.classList.contains("open")||lt.target.closest("#sidebar, #urppp-mobile-menu-button")||ut()},!0)),document.__urpppMobileRouteCloseBound||(document.__urpppMobileRouteCloseBound=!0,document.addEventListener("click",lt=>{if(!F()||!lt.target.closest)return;let xt=document.getElementById("urppp-clean-root");if(xt&&xt.classList.contains("open"))return;let gt=lt.target.closest("#sidebar a[href]");if(!gt)return;let St=String(gt.getAttribute("href")||"").trim();!St||St==="#"||St.startsWith("javascript")||ut()}))},"bindDrawerControls"),$t=a((W,pt)=>{let lt=W?W.cloneNode(!0):document.createElement("a");return lt.className="urppp-mobile-user-action",lt.removeAttribute("style"),lt.removeAttribute("id"),!W&&pt&&(lt.href=pt.href,pt.onclick&&lt.setAttribute("onclick",pt.onclick),lt.innerHTML='<i class="ace-icon fa '+pt.icon+'" aria-hidden="true"></i><span>'+pt.label+"</span>"),lt},"createActionLink"),ar=a((W,pt)=>{if(document.getElementById("urppp-mobile-user"))return;let lt=W.querySelector(":scope > li.light-blue")||Array.from(W.children).find(Ut=>Ut.querySelector&&Ut.querySelector(".nav-user-photo, .user-menu, .dropdown-menu")),xt=document.createElement("section");xt.id="urppp-mobile-user",xt.className="urppp-mobile-user";let gt=document.createElement("div");gt.className="urppp-mobile-user-identity";let St=lt?.querySelector(".nav-user-photo")||document.querySelector("#navbar .nav-user-photo"),Tt=St?St.cloneNode(!0):document.createElement("img");Tt.className="nav-user-photo",Tt.removeAttribute("style"),Tt.getAttribute("src")||Tt.setAttribute("src","/main/queryStudent/img"),Tt.setAttribute("data-urppp-private","avatar"),Tt.alt=St?.alt?.replace(/\s+/g," ").trim()||"用户头像";let jt=lt?.querySelector(".user-info")||document.querySelector("#navbar .user-info"),cr=document.createElement("span");cr.className="urppp-mobile-user-copy";let Rt=document.createElement("small");Rt.className="urppp-mobile-user-welcome",Rt.textContent="欢迎您，";let or=document.createElement("span");or.className="user-info urppp-user-name-value",or.setAttribute("data-urppp-private","name"),or.textContent=jt?.textContent?.replace(/^\s*欢迎您[，,]?\s*/g,"").replace(/\s+/g," ").trim()||St?.alt?.replace(/\s+/g," ").trim()||"我的账户",cr.append(Rt,or),gt.append(Tt,cr),xt.appendChild(gt);let Nt=document.createElement("div");Nt.className="urppp-mobile-user-actions";let At=lt?Array.from(lt.querySelectorAll(".user-menu a, .dropdown-menu a")):[],hr=[{label:"首页",href:"/",icon:"fa-home"},{label:"在线反馈",href:"/main/systemQuestion/index",icon:"fa-question-circle"},{label:"修改密码",href:"javascript:changePassword('/student/rollManagement/personalInfoUpdate/updatePassword')",icon:"fa-user"},{label:"注销",href:"/logout",icon:"fa-power-off"}];At.length?At.forEach(Ut=>Nt.appendChild($t(Ut))):hr.forEach(Ut=>Nt.appendChild($t(null,Ut))),xt.appendChild(Nt);let Mt=pt.querySelector(".urppp-sidebar-header");Mt&&Mt.nextSibling?pt.insertBefore(xt,Mt.nextSibling):Mt?pt.appendChild(xt):pt.insertBefore(xt,pt.firstChild);try{Vt(xt)}catch{}},"ensureMobileUser"),Jn=a((W,pt,lt,xt={})=>{if(!lt||document.getElementById("urppp-mobile-quick"))return;let gt=document.createElement("section");gt.id="urppp-mobile-quick",gt.className="urppp-mobile-quick",gt.innerHTML='<div class="urppp-mobile-quick-title">快捷功能</div>';let St=document.createElement("div");St.className="urppp-mobile-tool-row";let Tt=W.querySelector(':scope > li > a[href*="customerServiceCenter"]'),jt=Tt?Tt.cloneNode(!0):document.createElement("a");jt.className="urppp-mobile-tool-button urppp-mobile-help-button",jt.removeAttribute("style"),jt.removeAttribute("onclick"),jt.removeAttribute("data-toggle"),jt.removeAttribute("target"),jt.querySelectorAll("[style]").forEach(At=>At.removeAttribute("style"));let cr=String(jt.getAttribute("href")||"").trim();(!cr||cr==="#"||cr.startsWith("javascript"))&&(jt.href="/main/customerServiceCenter"),jt.querySelector("i")||(jt.innerHTML='<i class="ace-icon glyphicon glyphicon-headphones" aria-hidden="true"></i>'),jt.querySelectorAll("span").forEach(At=>At.remove()),jt.insertAdjacentHTML("beforeend","<span>帮助</span>"),St.appendChild(jt);let Rt=document.createElement("button");Rt.type="button",Rt.id="urppp-mobile-search-button",Rt.className="urppp-mobile-tool-button",Rt.setAttribute("aria-expanded","false"),Rt.innerHTML='<i class="ace-icon fa fa-search" aria-hidden="true"></i><span>搜索</span>',St.appendChild(Rt),gt.appendChild(St);let or=document.createElement("div");or.className="urppp-mobile-quick-links",Array.from(W.querySelectorAll(":scope > li > a")).forEach(At=>{let hr=At.closest("li");if(hr?.classList.contains("light-blue")||hr?.querySelector("#intellegenceUDiv, #form-search")||At===Tt||At.classList.contains("dropdown-toggle")||!At.getAttribute("href")&&!At.getAttribute("onclick"))return;let Mt=At.cloneNode(!0);Mt.className="urppp-mobile-quick-link",Mt.removeAttribute("style");let Ut=String(At.getAttribute("onclick")||"");if(/openWorkRestSchedule|open\w*Schedule/i.test(Ut)||Mt.removeAttribute("onclick"),xt.cleanMode){let Re=String(At.getAttribute("href")||"");(Re==="/holiday"||/holiday/i.test(Re)||/假期/.test(At.textContent||""))&&(Mt.removeAttribute("href"),Mt.removeAttribute("target"),Mt.style.cursor="default",Mt.style.pointerEvents="none")}or.appendChild(Mt)});let Nt=document.createElement("div");Nt.id="urppp-mobile-search-panel",Nt.className="urppp-mobile-search-panel",Nt.hidden=!0;{let At=document.getElementById("form-search");At&&(At.__urpppMobileParent||(At.__urpppMobileParent=At.parentElement,At.__urpppMobileNext=At.nextSibling),At.classList.add("urppp-mobile-form-search"),At.dataset.open="0",Nt.appendChild(At),vt())}gt.appendChild(Nt),or.children.length&&gt.appendChild(or),Rt.addEventListener("click",At=>{if(At.preventDefault(),At.stopPropagation(),Nt.hidden){vt();let Mt=Nt.querySelector("#form-search");Mt&&(Mt.dataset.open="0",Mt.style.setProperty("pointer-events","auto","important"),Mt.style.setProperty("opacity","1","important"),Mt.style.setProperty("width","100%","important"),Mt.style.setProperty("min-width","0","important")),Nt.hidden=!1,Nt.classList.add("open"),setTimeout(()=>Nt.querySelector("#search-input")?.focus(),30),Rt.setAttribute("aria-expanded","true")}else Nt.hidden=!0,Nt.classList.remove("open"),Rt.setAttribute("aria-expanded","false")}),pt.insertBefore(gt,lt)},"ensureMobileQuick"),Cr=a(()=>{let W=F(),pt=document.querySelector("#navbar .navbar-buttons .ace-nav"),lt=document.getElementById("sidebar"),xt=document.getElementById("urppp-menus");if(lt&&J(lt,W),Ht(),!W){let gt=document.documentElement.classList.contains("urppp-clean-open");gt||Et(),gt||(document.getElementById("urppp-mobile-quick")?.remove(),document.getElementById("urppp-mobile-user")?.remove());let St=document.getElementById("urppp-nav-clean"),Tt=document.getElementById("urppp-nav-theme");St&&Tt&&St.parentElement!==Tt&&Tt.appendChild(St),Tt&&Tt.style.setProperty("display","inline-flex","important");return}if(!(!pt||!lt)){try{let gt=document.getElementById("urppp-nav-clean"),St=document.querySelector("#navbar .navbar-header"),Tt=document.getElementById("urppp-nav-theme");gt&&St&&gt.parentElement!==St&&St.appendChild(gt),Tt&&Tt.style.setProperty("display","inline-flex","important"),document.getElementById("urppp-nav-cal")?.remove()}catch{}ar(pt,lt),Jn(pt,lt,xt),vt()}},"apply");window.__urpppRefreshMobileNavbar=Cr,window.__urpppCloseMobileDrawer=ut,window.__urpppSetDrawerOpen=(W,pt,lt)=>{K(W,pt,lt)},window.__urpppStopDrawerAnimation=W=>{W&&R(W)},window.__urpppInjectCleanSidebarSections=W=>{let pt=document.querySelector("#navbar .navbar-buttons .ace-nav")||document.querySelector("#navbar .ace-nav"),lt=document.getElementById("urppp-menus");if(!pt||!W)return;try{ar(pt,W)}catch{}let xt=document.getElementById("urppp-mobile-quick");if(xt){let gt=xt.querySelector("#urppp-mobile-search-panel");if(gt&&gt.querySelector("#form-search"))try{Et()}catch{}xt.remove()}try{Jn(pt,W,lt,{cleanMode:!0})}catch{}};try{Cr()}catch{}if(setTimeout(Cr,300),setTimeout(Cr,900),setTimeout(Cr,1800),window.matchMedia){let W=window.matchMedia(z),pt=a(()=>Cr(),"onChange");typeof W.addEventListener=="function"?W.addEventListener("change",pt):typeof W.addListener=="function"&&W.addListener(pt)}try{window.__urpppMobileNavbarObserver&&window.__urpppMobileNavbarObserver.disconnect();let W=0,pt=new MutationObserver(()=>{clearTimeout(W),W=setTimeout(()=>{try{Cr()}catch{}},40)}),lt=document.getElementById("navbar"),xt=document.getElementById("sidebar");lt&&pt.observe(lt,{childList:!0,subtree:!0}),xt&&pt.observe(xt,{childList:!0}),window.__urpppMobileNavbarObserver=pt}catch{}}a(g,"setupMobileNavbar");let M=!!(window.matchMedia&&window.matchMedia("(max-width: 991px)").matches)?"8px 8px 24px":"16px 64px 40px";if(document.querySelectorAll(".page-content, #page-content-template").forEach(z=>{z.style.setProperty("padding",M,"important"),z.style.setProperty("box-sizing","border-box","important")}),ze(),ma(),Te(),Nr(),ga(),setTimeout(()=>{Nr(),ga()},300),setTimeout(()=>{Nr(),ga()},1e3),Lo(),zo(),Po(),mr(),ha(),oe(),qo(),setTimeout(()=>{mr(),oe()},200),setTimeout(()=>{mr(),oe()},800),setTimeout(ma,350),setTimeout(ma,1e3),ne(),setTimeout(()=>ne(),400),!window.__urpppPlanTreeObs){let z=0;window.__urpppPlanTreeObs=new MutationObserver(()=>{let J=document.getElementById("treeDemo");!J||J.dataset.urpppBusy==="1"||J.querySelector('li > a:not([data-urppp-node-done="1"])')&&(clearTimeout(z),z=setTimeout(()=>ne(),220))});let F=document.getElementById("tree_div")||document.getElementById("treeDemo");F&&window.__urpppPlanTreeObs.observe(F,{childList:!0,subtree:!0})}window.__urpppWrsBound||(window.__urpppWrsBound=!0,document.addEventListener("shown.bs.modal",z=>{z.target&&(z.target.id==="work_rest_schedule_modal"||z.target.querySelector?.("#work_rest_schedule_modal"))&&setTimeout(fa,30)},!0),document.addEventListener("click",z=>{let F=z.target&&z.target.closest?z.target.closest("a,button"):null;if(!F)return;let J=F.getAttribute("onclick")||"",D=(F.textContent||"").trim();(J.includes("openWorkRestSchedule")||D.includes("作息时间表"))&&(setTimeout(fa,80),setTimeout(fa,300))},!0)),Pe(),st(),dt(),xa();let j=a(()=>{ze(),Te(),Pe()},"layoutWave");setTimeout(j,200),setTimeout(j,800),window.__urpppLoadBound||(window.__urpppLoadBound=!0,window.addEventListener("load",()=>{st(),dt(),et(),xa(),Pe(),ze(),Te()})),setTimeout(()=>{document.body.classList.add("urppp-ready"),wt()},600),console.log("[URP++] style applied apple-leaning");try{bindScheduleHoverNearCursor()}catch{}Uo()}a(Ui,"beautifyInternal");function Wi(t){if(!t)return;let r=t.querySelector("#urppp-set-brutal-palettes");if(!r)return;let e=vo();r.innerHTML="",Q.filter(o=>o.id!==V).forEach(o=>{let i=document.createElement("button");i.type="button",i.className="urppp-set-scheme"+(o.id===e.id?" ac":""),i.dataset.palette=o.id,i.innerHTML=['<div class="urppp-set-scheme-preview">','  <span style="background:#000"></span>','  <span style="background:'+o.accent+'"></span>','  <span style="background:'+o.secondary+'"></span>',"</div>",'<div class="urppp-set-scheme-meta">',"  <strong>"+o.name+"</strong>","  <em>"+o.desc+"</em>","</div>"].join(""),i.addEventListener("click",()=>ko(o.id,{select:!0})),r.appendChild(i)})}a(Wi,"renderBrutalPaletteCards");let $e=Ap({getPrivacySettings:yr,setPrivacySettings:la,getCustomIdentity:qr,setCustomIdentity:mo,applyDisplay:a(()=>Vt(document),"applyDisplay"),refreshCleanDisplay:Fa,finishActiveDirectEdit:a(t=>{br?.__finish&&br.__finish(t)},"finishActiveDirectEdit")}),Gi=$e.sync,vc=$e.collect,wc=$e.setStatus,Ji=$e.bind,Sa=wp({document,getSettings:Ee,setSettings:ho,validateMapping:Pr,defaultMapping:xe,getRecoveryMessage:a(()=>rt,"getRecoveryMessage")}),kc=Sa.setStatus,Vi=Sa.sync,Yi=Sa.bind;function Dt(){let t=document.getElementById("urppp-settings-panel");if(!t)return;let r=Yt()||mt,e=Lr(),o=Qt(),i=Kt(),s=Zt(),l=$r(s),h=Mr(s),g=yo(s),E={};t.querySelectorAll(".urppp-set-mode").forEach(z=>{E[z.dataset.theme]=ee(z.dataset.theme,s)}),_p(t,{seed:r,currentTheme:o,followSystem:i,skinId:s,darkSupported:l,dynamicSupported:h,fixedPalettes:g,followUseDynamic:Ce(),cleanDefault:pa(),cleanAnalysis:ia()?"direct":"tab",appleEdge:xr(),autoUpdate:sa(),modeAvailability:E}),g&&Wi(t);try{Gi(t)}catch{}try{Vi(t)}catch{}try{window.__urpppCleanMode&&typeof window.__urpppCleanMode.refreshRender=="function"&&window.__urpppCleanMode.refreshRender()}catch{}let M=t.querySelector("#urppp-set-presets");M&&(M.innerHTML="",na().forEach(z=>{let F=document.createElement("button");F.type="button",F.className="urppp-set-swatch"+(z.toLowerCase()===r.toLowerCase()?" ac":""),F.title=z,F.style.background=z,F.addEventListener("click",()=>{GM_setValue(T,z),Kt()?Wt(vr(),{system:!0}):Wt("scu-red",{manual:!0}),Dt()}),M.appendChild(F)}));let j=t.querySelector("#urppp-set-schemes");j&&(j.innerHTML="",zr(r).forEach(z=>{let F=document.createElement("button");F.type="button",F.className="urppp-set-scheme"+(z.id===e?" ac":""),F.dataset.scheme=z.id,F.innerHTML=['<div class="urppp-set-scheme-preview">','  <span style="background:'+z.bg+'"></span>','  <span style="background:'+z.surface+";border-color:"+z.border+'"></span>','  <span style="background:'+z.primary+'"></span>',"</div>",'<div class="urppp-set-scheme-meta">',"  <strong>"+z.name+"</strong>","  <em>"+z.desc+"</em>","</div>"].join(""),F.addEventListener("click",()=>{oa(z.id),GM_setValue(T,r),Kt()?Wt(vr(),{system:!0}):Wt("scu-red",{manual:!0}),Dt()}),j.appendChild(F)}));try{is(t)}catch(z){try{console.warn("[URP++] renderSkinCards",z)}catch{}}try{let z=t.querySelector(".urppp-about-ver, #urppp-about-ver");z&&(z.textContent="SCU URP++ v"+p,z.tagName==="A"&&(z.setAttribute("href",n.repo),z.setAttribute("target","_blank"),z.setAttribute("rel","noopener noreferrer")))}catch{}try{Vo(t)}catch{}}a(Dt,"syncSettingsPanelUI");let Wo=yp({document,ensurePanel:Yo,syncPanel:Dt,refreshUpdateStatus:sn}),Qi=Sp({document,theme:{isModeAvailable:ee,apply:Wt,supportsDark:$r,supportsDynamic:Mr,getFollowSystem:Kt,setFollowSystem:Ae,resolveFollowTheme:vr,getCurrent:Qt,getFollowDynamic:Ce,setFollowDynamic:da,syncNavbar:ft},preferences:{getCleanDefault:pa,setCleanDefault:Ci,getCleanAnalysis:a(()=>ia()?"direct":"tab","getCleanAnalysis"),setCleanAnalysis:Pi,getAppleEdge:xr,setAppleEdge:zi,applySkin:rr,getAutoUpdate:sa,setAutoUpdate:Li,checkUpdates:$a},accent:{normalize:Gt,setAccent:a(t=>GM_setValue(T,t),"setAccent"),savePreset:Ei,getScheme:Lr,setScheme:oa,listSchemePreviews:zr},syncPanel:Dt}),_t=to({GM:{getValue:typeof GM_getValue=="function"?GM_getValue:null,setValue:typeof GM_setValue=="function"?GM_setValue:null,xmlHttp:typeof GM_xmlhttpRequest=="function"?GM_xmlhttpRequest:null,addStyle:typeof GM_addStyle=="function"?GM_addStyle:null},doc:document,hostInfo:{version:p},uiDeps:{openSubpanel:a(t=>{t==="plugin-store"&&_a("plugin")},"openSubpanel")}});a((function(){let r=a(()=>{try{_t.bootFromCache("assist")}catch{}},"run");document.readyState==="loading"?document.addEventListener("DOMContentLoaded",r):r()}),"bootstrapPlugins")();function Go(){return Wo.open()}a(Go,"openSettingsPanel");function Jo(){Wo.close()}a(Jo,"closeSettingsPanel");let Me="data:image/png;base64,";function Vo(t){let r=t&&t.querySelector?t.querySelector("#urppp-about-logo"):document.getElementById("urppp-about-logo");r&&(r.getAttribute("src")!==Me&&r.setAttribute("src",Me),r.removeAttribute("referrerpolicy"),r.alt="SCU URP++",r.style.maxWidth="100%",r.style.height="auto",r.style.display="block")}a(Vo,"ensureAboutLogo");function Yo(){if(document.getElementById("urppp-settings-panel"))return;ys();try{rr()}catch{}let t=document.createElement("div");t.id="urppp-settings-mask",t.addEventListener("click",Jo);let r=document.createElement("div");r.id="urppp-settings-panel",r.setAttribute("role","dialog"),r.setAttribute("aria-label","URP++ 设置");let e=Me;r.innerHTML=vp({logoData:Me,repositoryUrl:n.repo,version:p}),document.documentElement.appendChild(t),document.documentElement.appendChild(r),xp(r),r.querySelector("#urppp-set-close").addEventListener("click",Jo);try{Ji(r)}catch(i){console.warn("[URP++] privacy settings",i)}try{Yi(r)}catch(i){console.warn("[URP++] JSON settings",i)}try{Vo(r)}catch{}let o=r.querySelector("#urppp-about-logo");o&&!o.__urpppFallback&&(o.__urpppFallback=!0,o.addEventListener("error",()=>{o.dataset.fallback!=="1"&&(o.dataset.fallback="1",o.src=e)})),Qi.bind(r);try{_t.renderAssistUi(r.querySelector("#urppp-set-assist-slot"))}catch(i){console.warn("[URP++] plugin manager",i)}}a(Yo,"ensureSettingsPanel");function _a(t){let r=document.getElementById("urppp-settings-panel");if(!r)return;let e=document.getElementById("urppp-store-subpanel");e||(e=document.createElement("div"),e.id="urppp-store-subpanel",e.className="urppp-store-subpanel",e.innerHTML=`
        ...
        <div class="urppp-store-sub-body" id="urppp-store-sub-body"></div>`,r.appendChild(e),e.querySelector("#urppp-store-sub-back").onclick=Xi);let o=e.querySelector("#urppp-store-sub-title"),i=e.querySelector("#urppp-store-sub-body");o.textContent=t==="theme"?"主题商店":"插件商店",i.innerHTML="",t==="theme"?as(i):en(i),e.classList.add("open")}a(_a,"openStoreSubPanel");function Xi(){let t=document.getElementById("urppp-store-subpanel");if(!t)return;t.classList.remove("open");let r=t.querySelector("#urppp-store-sub-body");r&&(r.innerHTML="")}a(Xi,"closeStoreSubPanel");function Qo(t){t.querySelectorAll(".urppp-store-tab").forEach(r=>{r.addEventListener("click",()=>{t.querySelectorAll(".urppp-store-tab").forEach(o=>o.className="urppp-store-tab"),r.className="urppp-store-tab ac",t.querySelectorAll(".urppp-store-pane").forEach(o=>o.style.display="none");let e=t.querySelector('.urppp-store-pane[data-pane="'+r.dataset.tab+'"]');e&&(e.style.display="")})})}a(Qo,"bindStoreTabs");function Xo(t){Array.isArray(t)&&t.forEach(r=>{if(!r||!r.cardCss||!r.id)return;let e=document.getElementById("urppp-store-card-css-"+r.id);e||(e=document.createElement("style"),e.id="urppp-store-card-css-"+r.id,(document.head||document.documentElement).appendChild(e)),e.textContent!==r.cardCss&&(e.textContent=r.cardCss)})}a(Xo,"ensureStoreCardStyles");function Ki(t,r){let e=(u.find(s=>s.id===t.id)||{}).repo,o=t.repo||e,i=o?`<button type="button" class="urppp-set-btn ghost" style="position:absolute;left:12px;bottom:12px" data-repo="${at(o)}">仓库</button>`:"";return`<div class="urppp-skin-card" data-skin="${at(t.id)}">
      ...
      <button type="button" class="urppp-skin-apply" data-store-theme="${at(t.id)}"${r?" disabled":""}>${r?"已安装":"下载"}</button>
      ${i}
    </div>`}a(Ki,"themeStoreCard");async function Ea(t){let r=t.querySelector('[data-pane="download"]');if(!r)return;let e=[];try{e=(await Dr()).filter(o=>o.type==="theme"&&!Tr(o.id))}catch{}if(!e.length){r.innerHTML='<div class="urppp-store-empty"><p class="urppp-store-empty-title">暂无待下载主题</p><p class="urppp-store-sub">已安装的主题不会再显示在这里。</p></div>';return}Xo(e),r.innerHTML=`<div class="urppp-store-theme-grid">${e.map(o=>Ki(o,!1)).join("")}</div>`,r.querySelectorAll("[data-store-theme]").forEach(o=>{o.addEventListener("click",()=>Zi(o.dataset.storeTheme,o))}),r.querySelectorAll("[data-repo]").forEach(o=>o.addEventListener("click",()=>{try{window.open(o.dataset.repo,"_blank","noopener")}catch{}}))}a(Ea,"fetchCatalogThemes");async function Zi(t,r){if(!r||r.disabled)return;let e=(await Dr()).find(s=>s.id===t);if(!e||!Array.isArray(e.entry)||!e.entry.length)return;r.disabled=!0,r.textContent="下载中…";let o="";for(let s of e.entry)try{let l=await fetch(s,{cache:"no-store"});if(l.ok){o=await l.text();break}}catch{}if(!o){r.textContent="下载失败",setTimeout(()=>{r.textContent="下载",r.disabled=!1},1400);return}try{GM_setValue("urppp_theme_css_"+t,o)}catch{}try{fo(t).textContent=o}catch{}r.textContent="已安装",r.disabled=!0;let i=r.closest&&r.closest(".urppp-store-inline");if(i){try{let s=i.querySelector("#urppp-theme-manage");s&&await Ca(s)}catch{}try{Ea(i)}catch{}}try{Dt()}catch{}}a(Zi,"downloadStoreTheme");function ts(t,r){let o=t.installed!==!1?"":`<button type="button" class="urppp-set-btn ghost" style="position:absolute;right:96px;bottom:12px" data-theme-del="${at(t.id)}">删除</button>`,i=r&&r.repo||t.repo,s=i?`<button type="button" class="urppp-set-btn ghost" style="position:absolute;left:12px;bottom:12px" data-repo="${at(i)}">仓库</button>`:"",l=r&&r.downloads!=null?`<span class="urppp-store-dl">↓ ${at(String(r.downloads))}</span>`:"",h=Zt()===t.id;return`<div class="urppp-skin-card${h?" is-active":""}" data-skin="${at(t.id)}">
      <div class="urppp-skin-name">${at(t.name)}</div>
      <div class="urppp-skin-meta">${at(r&&r.author||"")}${r&&r.author&&t.version?" · ":""}v${at(t.version||"")}${l?" · "+l:""}</div>
      ...
      ${o}${s}
    </div>`}a(ts,"themeManageCardHtml");async function Ca(t){if(!t)return;let r=[];try{r=await Dr()}catch{}let e=u.filter(o=>o.installed!==!1||Tr(o.id));if(!e.length){t.innerHTML='<div class="urppp-store-empty"><p class="urppp-store-empty-title">暂无已装主题</p></div>';return}Xo(e.map(o=>r.find(i=>i.id===o.id))),t.innerHTML=`<div class="urppp-store-theme-grid">${e.map(o=>ts(o,r.find(i=>i.id===o.id))).join("")}</div>`,t.querySelectorAll("[data-theme-use]").forEach(o=>o.addEventListener("click",()=>{if(Ao(o.dataset.themeUse)){try{Dt()}catch{}t.querySelectorAll(".urppp-skin-card").forEach(i=>{let s=i.dataset.skin,l=i.querySelector(".urppp-skin-apply"),h=Zt()===s;i.classList.toggle("is-active",h),l&&(l.classList.toggle("is-current",h),l.disabled=h,l.textContent=h?"使用中":"使用")})}})),t.querySelectorAll("[data-theme-del]").forEach(o=>o.addEventListener("click",()=>{let i=o.dataset.themeDel;try{GM_setValue("urppp_theme_css_"+i,"")}catch{}Ti(i);try{Zt()===i&&(GM_setValue(d,"apple"),rr(),Wt("default",{manual:!0}))}catch{}try{Dt()}catch{}let s=t.closest(".urppp-store-inline");if(s){try{Ca(t)}catch{}try{Ea(s)}catch{}}})),t.querySelectorAll("[data-repo]").forEach(o=>o.addEventListener("click",()=>{try{window.open(o.dataset.repo,"_blank","noopener")}catch{}}))}a(Ca,"fetchThemeManage");function Ko(){return`<div class="urppp-store-settings">
    ...
    </div>`}a(Ko,"storeManageSettingsHtml");let rs=["https://raw.githubusercontent.com/chaolan2019/URP-plusplus-Repository/main/catalog.json","https://cdn.jsdelivr.net/gh/chaolan2019/URP-plusplus-Repository@main/catalog.json","https://gh-proxy.com/https://raw.githubusercontent.com/chaolan2019/URP-plusplus-Repository/main/catalog.json"];async function Dr(){let t=a((e,o)=>Promise.race([e,new Promise((i,s)=>setTimeout(()=>s(new Error("timeout")),o))]),"withTimeout"),r=await Promise.allSettled(rs.map(e=>t(fetch(e,{cache:"no-store"}),5e3)));for(let e of r)if(e.status==="fulfilled"&&e.value&&e.value.ok)try{let o=await e.value.json();if(o&&Array.isArray(o.items))return o.items}catch{}return[]}a(Dr,"fetchCatalogList");function Zo(t,r){let e=String(t||"0").split(".").map(Number),o=String(r||"0").split(".").map(Number);for(let i=0;i<Math.max(e.length,o.length);i+=1){let s=e[i]||0,l=o[i]||0;if(s!==l)return s>l}return!1}a(Zo,"versionGt");function es(t,r){let e=0;return r.forEach(o=>{if(!o.id)return;let i=t.querySelector('[data-theme-use="'+o.id+'"]');i&&Zo(o.version,u.find(l=>l.id===o.id)&&u.find(l=>l.id===o.id).version)&&(tn(i.closest(".urppp-skin-card"),"主题"),e+=1);let s=t.querySelector('[data-plugin-id="'+o.id+'"]');if(s){let l=_t&&_t.api&&_t.api.get&&_t.api.get(o.id);l&&Zo(o.version,l.version)&&(tn(s.closest(".urppp-store-item"),"插件"),e+=1)}}),e}a(es,"applyStoreUpdateBadges");function tn(t,r){if(!t||t.querySelector(".urppp-store-update"))return;let e=t.querySelector(".urppp-store-ops");if(!e)return;let o=document.createElement("button");o.type="button",o.className="urppp-set-btn urppp-store-update",o.textContent="有新更新",o.addEventListener("click",()=>{try{o.textContent="更新中…"}catch{}}),e.appendChild(o)}a(tn,"addUpdateBadge");function rn(t){let r=t.querySelector("[data-store-auto-update]"),e=t.querySelector("[data-store-check-update]");if(!r||!e)return;let o=GM_getValue("urppp_store_auto_update",!1),i=a(()=>{r.textContent="自动检测更新："+(o?"开":"关")},"sync");i(),r.addEventListener("click",()=>{o=!o,GM_setValue("urppp_store_auto_update",o),i()}),e.addEventListener("click",async()=>{e.disabled=!0;let s=e.textContent;e.textContent="检查中…";try{let l=await Dr(),h=es(t,l);e.textContent=h?"发现更新":"已是最新"}catch{e.textContent="检查失败"}setTimeout(()=>{e.textContent=s,e.disabled=!1},1600)})}a(rn,"bindStoreManageSettings");function as(t){t.innerHTML=`
      <div class="urppp-store-inline">
        <div class="urppp-store-tabs">
        ...
        <div class="urppp-store-body">
          ...
          <div class="urppp-store-pane" data-pane="manage" style="display:none">${Ko()}<div class="urppp-store-bd"><div id="urppp-theme-manage"><div class="urppp-store-empty"><p>加载中…</p></div></div></div></div>
        </div>
      </div>`,Qo(t),rn(t),Ea(t),Ca(t.querySelector("#urppp-theme-manage"))}a(as,"renderThemeStoreBody");function os(t){let r=t.downloads!=null?`${at(String(t.downloads))}`:"",e=t.repo?`<button type="button" class="urppp-store-repo" data-repo="${at(t.repo)}">仓库</button>`:"";return`<div class="urppp-skin-card" data-plugin-card="${at(t.id)}">
      <div class="urppp-skin-name">${at(t.name||t.id)}</div>
      <div class="urppp-skin-meta">${at(t.author||"")}${t.author&&t.version?" · ":""}v${at(t.version||"")}${r?" · ↓"+r:""}${e?" · "+e:""}</div>
      <p class="urppp-skin-desc">${at(t.description||"")}</p>
      <button type="button" class="urppp-skin-apply" data-plugin-apply="${at(t.id)}">安装</button>
    </div>`}a(os,"pluginStoreCard");async function ns(t){if(!t)return;let r=[];try{r=(await Dr()).filter(e=>e.type==="plugin"&&!(_t&&_t.api&&_t.api.isEnabled&&_t.api.isEnabled(e.id)))}catch{}if(!r.length){t.innerHTML='<div class="urppp-store-empty"><p class="urppp-store-empty-title">暂无待下载插件</p><p class="urppp-store-sub">已安装的插件不会再显示在这里。</p></div>';return}t.innerHTML=`<div class="urppp-store-theme-grid">${r.map(e=>os(e)).join("")}</div>`,t.querySelectorAll("[data-plugin-apply]").forEach(e=>e.addEventListener("click",async()=>{e.disabled=!0;let o=e.textContent;e.textContent="下载中…";try{_t&&_t.api&&_t.api.install&&await _t.api.install(e.dataset.pluginApply,null),e.textContent="已安装";try{Dt()}catch{}}catch{e.textContent="失败"}setTimeout(()=>{e.textContent=o,e.disabled=!1},1200)})),t.querySelectorAll("[data-repo]").forEach(e=>e.addEventListener("click",()=>{try{window.open(e.dataset.repo,"_blank","noopener")}catch{}}))}a(ns,"fetchCatalogPlugins");async function ps(t){if(!t)return;let r=[];try{r=await Dr()}catch{}let e=_t&&_t.api&&_t.api.list&&_t.api.list()||[];if(!e.length){t.innerHTML='<div class="urppp-store-empty"><p class="urppp-store-empty-title">暂无插件</p><p class="urppp-store-sub">已装载的插件会显示在这里。</p></div>';return}t.innerHTML=e.map(o=>{let i=r.find(h=>h.id===o.id),s=i&&i.downloads!=null?`<span class="urppp-store-dl">↓ ${at(String(i.downloads))}</span>`:"",l=o.repo||i&&i.repo?`<button type="button" class="urppp-set-btn ghost" data-repo="${at(o.repo||i.repo)}">仓库</button>`:"";return`<div class="urppp-store-item">
      ...
      <div class="urppp-store-inline">
        ...
        <div class="urppp-store-body">
          <div class="urppp-store-pane" data-pane="download"><div class="urppp-store-empty"><p class="urppp-store-empty-title">加载中…</p></div></div>
          <div class="urppp-store-pane" data-pane="manage" style="display:none">${Ko()}<div class="urppp-store-bd" id="urppp-plugin-manage"><div class="urppp-store-empty"><p>加载中…</p></div></div></div>
        </div>
      </div>`,Qo(t),rn(t),ns(t.querySelector('[data-pane="download"]')),ps(t.querySelector("#urppp-plugin-manage"))}a(en,"renderPluginStoreBody");function is(t){if(!t)return;let r=t.querySelector("#urppp-theme-store");r&&!r.dataset.bound&&(r.dataset.bound="1",r.addEventListener("click",()=>_a("theme")));let e=t.querySelector("#urppp-skin-list");if(!e)return;let o=Zt();if(e.innerHTML="",!u||!u.length){e.innerHTML='<p class="urppp-set-tip">暂无可用风格</p>';return}u.filter(i=>i.installed!==!1||Tr(i.id)).forEach(i=>{let s=document.createElement("div");s.className="urppp-skin-card"+(i.id===o?" is-active":""),s.dataset.skin=i.id;let l=document.createElement("button");l.type="button",l.className="urppp-skin-apply";let h=i.installed!==!1||Tr(i.id);h?i.id===o&&i.ready?(l.classList.add("is-current"),l.textContent="使用中",l.disabled=!0):l.textContent="应用主题":(l.classList.add("is-disabled"),l.textContent="去下载"),l.addEventListener("click",g=>{if(g.preventDefault(),g.stopPropagation(),!h){_a("theme");return}if(!(i.id===o&&i.ready)&&Ao(i.id)){Dt();try{window.__urpppCleanMode&&window.__urpppCleanMode.inject&&window.__urpppCleanMode.inject()}catch{}}}),s.innerHTML=['<div class="urppp-skin-name"></div>','<p class="urppp-skin-desc"></p>'].join(""),s.querySelector(".urppp-skin-name").textContent=i.name,s.querySelector(".urppp-skin-desc").textContent=i.desc,s.appendChild(l),e.appendChild(s)})}a(is,"renderSkinCards");let jr=[],Pa=!1;function ss(t,r,e){let o=typeof AbortController=="function"?new AbortController:null,i=o?setTimeout(()=>o.abort(),e):null;return fetch(t,{cache:"no-store",headers:r,signal:o?o.signal:void 0}).then(s=>{if(!s.ok)throw new Error("HTTP "+s.status);return s.text()}).finally(()=>{i&&clearTimeout(i)})}a(ss,"fetchWithTimeout");function ls(t,r){return new Promise((e,o)=>{try{GM_xmlhttpRequest({method:"GET",url:t,timeout:12e3,headers:r,onload:a(i=>{i.status>=200&&i.status<400?e(i.responseText||""):o(new Error("HTTP "+i.status))},"onload"),onerror:a(()=>o(new Error("network error")),"onerror"),ontimeout:a(()=>o(new Error("timeout")),"ontimeout")})}catch(i){o(i)}})}a(ls,"gmRequestForUpdate");function cs(t,r){let e={"Cache-Control":"no-cache"};return r&&r.range&&(e.Range=r.range),ss(t,e,12e3).catch(o=>{if(typeof GM_xmlhttpRequest=="function")return ls(t,e);throw o})}a(cs,"fetchTextForUpdate");async function za(t,r,e=1e3){let o=[],i=t[0],s=t.slice(1),l=a(z=>cs(z,r).then(F=>({url:z,text:F})).catch(F=>(o.push((z.split("/")[2]||z)+": "+(F&&F.message||F)),null)),"grab"),h=l(i),g=new Promise(z=>setTimeout(()=>z("__TIMEOUT__"),e)),E=await Promise.race([h,g]);if(E!=="__TIMEOUT__"){if(E&&E.text&&E.text.length>0)return E.text;let F=(await Promise.all(s.map(l))).find(J=>J&&J.text&&J.text.length>0);if(F)return F.text;throw new Error("所有更新源均不可用（"+o.join("; ")+"）")}let M=Promise.all(s.map(l)).then(z=>{let F=z.find(J=>J&&J.text&&J.text.length>0);if(F)return F.text;throw new Error("所有更新源均不可用（"+o.join("; ")+"）")}),j=h.then(z=>{if(z&&z.text&&z.text.length>0)return z.text;throw new Error("主源内容无效")}).catch(()=>new Promise(()=>{}));return Promise.race([j,M])}a(za,"fetchFirstAvailable");function wr(t,r){let e=document.getElementById("urppp-set-update-status");e&&(e.dataset.locked=t?"1":"",e.innerHTML=t||"",e.style.color=r==="err"?"#b91c1c":r==="ok"?"#15803d":"var(--text-muted)")}a(wr,"setUpdateStatus");async function La(){let t=p,r="",e=!1,o="";try{let s=await za(n.sourceUrls(n.versionJson)),l=JSON.parse(s);r=String(l&&l.version||"").trim(),l&&String(l.prevVersion||"").trim()===t&&(e=!0),l&&typeof l.changelog=="string"&&l.changelog.trim()&&(o=l.changelog)}catch{}if(!r){let s=await za(n.sourceUrls("urppp.user.js"),{range:"bytes=0-2048"});r=Qa(s)}if(!r)throw new Error("无法解析远程主插件版本");let i=ge(r,t);return{id:"main",name:"主插件",local:t,remote:r,status:i>0?"update":i===0?"latest":"ahead",updateUrl:n.mainRaw,pageUrl:n.greasySearch,changelogMd:e?o:""}}a(La,"checkMainUpdate");function an(t,r,e){let o=String(t||"").replace(/\r\n/g,`
`);if(!o.trim())return"";let i=/^##\s*\[?v?([0-9]+(?:\.[0-9]+){0,3}[\w\-]*)\]?[^\n]*$/gim,s=[],l;for(;(l=i.exec(o))!==null;)s.push({ver:l[1],index:l.index,headEnd:i.lastIndex});if(!s.length)return"";for(let g=0;g<s.length;g++){let E=g+1<s.length?s[g+1].index:o.length;s[g].body=o.slice(s[g].index,E).trim()}let h=[];for(let g of s)ge(g.ver,e)>0||ge(g.ver,r)<=0||h.push(g.body);return h.join(`

`).trim()}a(an,"extractChangelogRange");function on(){let t=document.getElementById("urppp-update-toast-style");t&&t.remove();let r=document.createElement("style");r.id="urppp-update-toast-style",r.textContent=`
      #urppp-update-toast{
    ...
    `,document.documentElement.appendChild(r)}a(on,"ensureUpdateToastStyles");function ds(t){let r=String(t||"").replace(/\r\n/g,`
`).trim();if(!r)return'<p class="uuc-meta">暂无更新日志</p>';let e=a(h=>{let g=at(h);return g=g.replace(/`([^`]+)`/g,"<code>$1</code>"),g=g.replace(/\*\*([^*]+)\*\*/g,"<strong>$1</strong>"),g=g.replace(/\[([^\]]+)\]\((https?:\/\/[^)\s]+)\)/g,'<a href="$2" target="_blank" rel="noopener noreferrer">$1</a>'),g=g.replace(/(^|[^"'>])(https?:\/\/[^\s<]+)/g,'$1<a href="$2" target="_blank" rel="noopener noreferrer">$2</a>'),g},"inline"),o=r.split(`
`),i=[],s=!1,l=a(()=>{s&&(i.push("</ul>"),s=!1)},"closeList");for(let h=0;h<o.length;h++){let E=o[h].replace(/\s+$/,"");if(!E.trim()){l();continue}let M=E.match(/^(#{2,3})\s+(.+)$/);if(M){l();let z=M[1].length,F=M[2];i.push(z===2?`<h2>${e(F)}</h2>`:`<h3>${e(F)}</h3>`);continue}let j=E.match(/^[-*]\s+(.+)$/);if(j){s||(i.push("<ul>"),s=!0),i.push(`<li>${e(j[1])}</li>`);continue}l(),i.push(`<p>${e(E)}</p>`)}return l(),i.join("")||'<p class="uuc-meta">暂无更新日志</p>'}a(ds,"renderChangelogMarkdown");function nn(t){let r=t||document.getElementById("urppp-update-toast");if(!r||!r.classList.contains("open")){r&&r.classList.remove("open","closing");return}if(r.__closing)return;r.__closing=!0,r.classList.add("closing"),r.classList.remove("open");let e=a(()=>{r.classList.remove("closing"),r.__closing=!1,r.removeEventListener("transitionend",o)},"done"),o=a(i=>{i&&i.target!==r||i&&i.propertyName&&i.propertyName!=="opacity"&&i.propertyName!=="transform"||e()},"onEnd");r.addEventListener("transitionend",o),setTimeout(e,380)}a(nn,"hideUpdateToast");function us(t){let r=t||document.getElementById("urppp-update-changelog");if(!r||!r.classList.contains("open")&&!r.classList.contains("closing")||r.__closing)return;r.__closing=!0,r.classList.add("closing"),r.classList.remove("open");let e=a(()=>{r.classList.remove("closing"),r.__closing=!1,r.removeEventListener("transitionend",o)},"done"),o=a(i=>{i&&i.target!==r||i&&i.propertyName&&i.propertyName!=="opacity"&&i.propertyName!=="background-color"&&i.propertyName!=="background"||e()},"onEnd");r.addEventListener("transitionend",o),setTimeout(e,360)}a(us,"hideChangelogModal");function pn(t,r){on();let e=document.getElementById("urppp-update-changelog");e||(e=document.createElement("div"),e.id="urppp-update-changelog",e.innerHTML=`
        ...
        </div>`,e.addEventListener("click",o=>{(o.target===e||o.target&&o.target.getAttribute&&o.target.getAttribute("data-close")==="1")&&us(e)}),document.documentElement.appendChild(e)),e.querySelector("h3").textContent=t||"更新日志",e.querySelector(".uuc-body").innerHTML=r||'<p class="uuc-meta">暂无更新日志</p>',e.__closing=!1,e.classList.remove("open","closing"),e.offsetWidth,requestAnimationFrame(()=>{requestAnimationFrame(()=>e.classList.add("open"))})}a(pn,"openChangelogModal");function qa(t){on();let r=document.getElementById("urppp-update-toast");r||(r=document.createElement("div"),r.id="urppp-update-toast",r.innerHTML=`
        ...
        </div>`,r.querySelector(".uut-close").addEventListener("click",()=>nn(r)),r.addEventListener("click",async e=>{let o=e.target&&e.target.closest?e.target.closest("[data-act]"):null;if(!o)return;let i=o.getAttribute("data-act"),s=r.__pack||{};if(i==="later"){nn(r);return}if(i==="go"){let l=s.updateUrl||n.mainRaw;try{window.open(l,"_blank","noopener,noreferrer")}catch{location.href=l}return}if(i==="log"){o.disabled=!0,o.textContent="加载中…";try{let l=s.changelogMd;l||(l=await za(n.sourceUrls("CHANGELOG.md")),s.changelogMd=l);let h=an(l,s.local,s.remote),g=h?ds(h):'<p class="uuc-meta">未找到区间日志。</p><p><a href="'+n.changelogPage+'" target="_blank" rel="noopener noreferrer">打开完整 CHANGELOG</a></p>';pn("更新日志 "+s.local+" → "+s.remote,g)}catch(l){pn("更新日志","<p>加载失败："+at(l&&l.message||l)+'</p><p><a href="'+n.changelogPage+'" target="_blank" rel="noopener noreferrer">打开 GitHub CHANGELOG</a></p>')}finally{o.disabled=!1,o.textContent="更新日志"}}}),document.documentElement.appendChild(r)),r.__pack=t||{},r.querySelector(".uut-title").textContent="发现新版本 "+(t&&t.remote||""),r.querySelector(".uut-sub").textContent="当前 "+(t&&t.local||"")+" · 主插件可更新",r.__closing=!1,r.classList.remove("open","closing"),r.offsetWidth,requestAnimationFrame(()=>{requestAnimationFrame(()=>r.classList.add("open"))})}a(qa,"showUpdateToast");async function Ta(){if(sa()&&!window.__urpppAutoUpdateTried){window.__urpppAutoUpdateTried=!0;try{let t=await La();t&&t.status==="update"&&qa(t);let r=await ms();if(r)try{console.log("[URP++] 辅助插件热更新到",r.version)}catch{}}catch(t){try{console.debug("[URP++] auto update check failed",t)}catch{}}}}a(Ta,"maybeAutoCheckUpdate");function ms(){let t=(window.__urpppUpdateCheckers||jr||[]).find(r=>r&&r.id==="assist");return!t||typeof t.check!="function"?Promise.resolve(null):Promise.resolve().then(()=>t.check()).then(r=>r&&r.status==="update"?_t.update("assist"):null).catch(()=>null)}a(ms,"hotUpdateAssist");async function $a(){if(Pa)return;Pa=!0;let t=document.getElementById("urppp-set-check-update");t&&(t.disabled=!0,t.textContent="检查中…"),wr("正在从多源检查更新…");try{let r=[La()];(jr||[]).forEach(g=>{g&&typeof g.check=="function"&&r.push(Promise.resolve().then(()=>g.check()).then(E=>E||{id:g.id||"extra",name:g.name||"扩展",status:"err",message:"无结果"}).catch(E=>({id:g.id||"extra",name:g.name||"扩展",status:"err",message:String(E&&E.message||E)})))});let e=await Promise.all(r),o=e.map(g=>{if(!g)return"";if(g.status==="err")return`• <b>${at(g.name||g.id)}</b>：检查失败（${at(g.message||"unknown")}）`;if(g.status==="update"){let E="";if(g.id==="assist"&&_t&&_t.loaded("assist"))E=' <a class="urppp-update-relaunch" href="javascript:void(0)" data-urppp-relaunch="assist" rel="nofollow">重新装载</a>';else{let M=g.updateUrl?` <a href="${at(g.updateUrl)}" target="_blank" rel="noopener noreferrer">打开更新源</a>`:"",j=g.pageUrl?` <a href="${at(g.pageUrl)}" target="_blank" rel="noopener noreferrer">Greasy Fork</a>`:"";E=M+j}return`• <b>${at(g.name)}</b>：发现新版本 <b>${at(g.remote)}</b>（当前 ${at(g.local)}）${E}`}return g.status==="ahead"?`• <b>${at(g.name)}</b>：本地 ${at(g.local)} 新于远程 ${at(g.remote)}`:`• <b>${at(g.name)}</b>：已是最新（${at(g.local)}）`}).filter(Boolean),i=e.some(g=>g&&g.status==="update"),s=e.some(g=>g&&g.status==="err");wr(`${i?"检查完成：发现更新":s?"检查完成：部分失败":"检查完成：全部最新"}<br>${o.join("<br>")}<br><span style="opacity:.85">仓库：<a href="${n.repo}" target="_blank" rel="noopener noreferrer">SCU-URP-plusplus</a></span>`,s?"err":"ok");let h=document.querySelector('#urppp-set-update-status .urppp-update-relaunch[data-urppp-relaunch="assist"]');h&&h.addEventListener("click",()=>{try{wr("正在重新装载辅助插件…",""),_t.install("assist").then(()=>{wr("辅助插件已重新装载，刷新页面后生效。","ok")}).catch(g=>{wr("重新装载失败："+(g&&g.message?g.message:g),"err")})}catch(g){wr("重新装载失败："+(g&&g.message?g.message:g),"err")}})}catch(r){wr("检查失败："+at(r&&r.message||r),"err")}finally{Pa=!1,t&&(t.disabled=!1,t.textContent="检查更新")}}a($a,"checkForUpdates");function sn(){let t=document.getElementById("urppp-set-update-status");if(!t||t.dataset.locked==="1")return;let r="当前主插件："+p,e=t.getAttribute("data-assist-version")||"";e&&(r+="；辅助插件："+e),t.textContent=r,t.style.color="var(--text-muted)"}a(sn,"refreshUpdateStatusHint");function bs(t){if(!t||typeof t.check!="function")return!1;let r=String(t.id||t.name||"").trim();if(!r)return!1;let e=jr.findIndex(i=>i&&i.id===r),o={id:r,name:t.name||r,check:t.check,localVersion:t.localVersion||""};e>=0?jr[e]=o:jr.push(o);try{let i=document.getElementById("urppp-set-update-status");i&&o.localVersion&&r==="assist"&&i.setAttribute("data-assist-version",String(o.localVersion))}catch{}try{sn()}catch{}return!0}a(bs,"registerUpdateChecker");function hs(){let t={version:p,urls:n,check:$a,checkMain:La,registerChecker:bs,compareVersions:ge,parseUserscriptVersion:Qa,extractChangelogRange:an,showUpdateToast:qa,maybeAutoCheckUpdate:Ta,listCheckers:a(()=>jr.slice(),"listCheckers")};try{window.__urpppUpdate=t}catch{}try{typeof unsafeWindow<"u"&&unsafeWindow&&(unsafeWindow.__urpppUpdate=t)}catch{}return t}a(hs,"publishUpdateApi"),hs();let{rebuildSidebarCompletely:ln,syncMobileContentOffset:Or,syncSidebarUnderNavbar:kr}=Ai({}),{rebuildDashboard:gs}=mi({deps:{statCardPrivacyMarkup:ws}}),fs="urppp-clean-open",Ma={100:4,99:4,98:4,97:4,96:4,95:4,94:3.9,93:3.8,92:3.7,91:3.6,90:3.5,89:3.4,88:3.3,87:3.2,86:3.1,85:3,84:2.9,83:2.8,82:2.7,81:2.6,80:2.5,79:2.4,78:2.3,77:2.2,76:2.1,75:2,74:1.9,73:1.8,72:1.7,71:1.6,70:1.5,69:1.4,68:1.3,67:1.2,66:1.1,65:1,64:.9,63:.8,62:.7,61:.6,60:.5};function ir(t){if(t==null||t==="")return!1;let r=String(t).trim();if(!r)return!1;if(/未评估|未评教|待评估|待评教/.test(r))return!0;let e=Number(r);return!Number.isNaN(e)&&e<0}a(ir,"isUnevaluatedScore");function Ie(t){if(t==null||t==="")return!1;let r=Number(t);return!Number.isNaN(r)&&r>=0&&r<=5}a(Ie,"isValidOfficialGpa");function Ne(t){let r=String(t||"").trim();if(!r)return"";let e=r.match(/[\u4e00-\u9fffA-Za-z0-9]/);return e?e[0]:r.charAt(0)}a(Ne,"firstContentChar");function Ia(t,r){let e=String(t||""),o=Number(r)||0;return!e||o<=0||o>e.length?!1:e.charAt(o-1)==="1"}a(Ia,"weekBitmapActive");function Hr(t){if(t==null||t==="")return null;let r=String(t).trim();if(!r||ir(r)||/^免修$|^通过$|^取消$|^缓考$|^旷考$|^缺考$/.test(r))return null;if(/^A\+$/i.test(r)||/^A$/i.test(r))return 4;if(/^A-$/i.test(r))return 3.7;if(/^B\+$/i.test(r))return 3.3;if(/^B$/i.test(r))return 3;if(/^B-$/i.test(r))return 2.7;if(/^C\+$/i.test(r))return 2.3;if(/^C$/i.test(r))return 2;if(/^C-$/i.test(r))return 1.7;if(/^D$/i.test(r))return 1.3;if(/^F$/i.test(r))return 0;if(/优秀/.test(r))return 4;if(/良好/.test(r))return 3;if(/中等/.test(r))return 2;if(/及格/.test(r)&&!/不及格/.test(r))return 1;if(/不及格|不合格|不通过/.test(r))return 0;if(/合格/.test(r))return 1;let e=parseFloat(r.replace(/[^\d.]/g,""));if(Number.isNaN(e)||e<0)return null;let o=Math.round(e);return o<60?0:o>100?4:Ma[o]!=null?Ma[o]:Ma[Math.max(60,Math.min(100,Math.floor(e)))]||0}a(Hr,"scoreToGpa");function Rr(t){let r=String(t||"").trim();if(!r||ir(r))return null;if(/优秀/.test(r))return 95;if(/良好/.test(r))return 85;if(/中等/.test(r))return 75;if(/及格/.test(r)&&!/不及格/.test(r))return 65;if(/不及格|不合格|不通过/.test(r))return 0;if(/合格/.test(r))return 70;if(/^A/i.test(r))return 95;if(/^B/i.test(r))return 85;if(/^C/i.test(r))return 75;if(/^D/i.test(r))return 65;if(/^F/i.test(r))return 0;let e=parseFloat(r.replace(/[^\d.]/g,""));return Number.isNaN(e)||e<0?null:e}a(Rr,"scoreToNumber");function Ar(t){return Math.round((Number(t)||0)*100)/100}a(Ar,"round2");function cn(t){return/必修/.test(String(t||""))}a(cn,"isRequiredAttr");function er(t){let r=0,e=0,o=0,i=0,s=0,l=0,h=0,g=0;return(t||[]).forEach(E=>{if(E&&(E.unevaluated||ir(E.score)))return;let M=Number(E.credit)||0,j=Rr(E.score),z=Ie(E.officialGpa)?Number(E.officialGpa):Hr(E.score);j==null||M<=0||(r+=M,e+=j*M,z!=null&&(o+=z*M,i+=M),E.required&&(s+=M,l+=j*M,z!=null&&(h+=z*M,g+=M)))}),{totalCredit:Ar(r),avgScore:Ar(r?e/r:0),avgGpa:Ar(i?o/i:0),requiredCredit:Ar(s),requiredGpa:Ar(g?h/g:0),requiredAvg:Ar(s?l/s:0),count:(t||[]).length}}a(er,"summarizeCourses");function Na(t){let r=String(t||"");return/^https?:\/\//i.test(r)?r:r.startsWith("//")?location.protocol+r:r.startsWith("/")?location.origin+r:location.origin+"/"+r.replace(/^\.\//,"")}a(Na,"absUrl");function Jt(t,r){let e=Na(t),o=r&&r.method||"GET",i=r&&r.data||null;return new Promise((s,l)=>{let h=a((g,E)=>g?s(E):l(new Error(E||"fetch failed")),"done");try{if(typeof GM_xmlhttpRequest=="function"){GM_xmlhttpRequest({method:o,url:e,data:i||void 0,headers:r&&r.headers?r.headers:{},withCredentials:!0,onload:a(g=>{g.status>=200&&g.status<400?h(!0,g.responseText||""):h(!1,"HTTP "+g.status)},"onload"),onerror:a(()=>h(!1,"network error"),"onerror")});return}}catch{}fetch(e,{method:o,credentials:"include",cache:"no-store",headers:r&&r.headers?r.headers:{},body:i||void 0}).then(g=>{if(!g.ok)throw new Error("HTTP "+g.status);return g.text()}).then(g=>h(!0,g)).catch(g=>h(!1,g&&g.message))})}a(Jt,"fetchText");function Be(t){return new DOMParser().parseFromString(String(t||""),"text/html")}a(Be,"parseHtml");function dn(){if(document.getElementById("urppp-feature-style"))return;let t=document.createElement("style");t.id="urppp-feature-style",t.textContent=Op,(document.head||document.documentElement).appendChild(t)}a(dn,"ensureFeatureStyles");function xs(){if(document.getElementById("urppp-schedule-export-style"))return;let t=document.createElement("style");t.id="urppp-schedule-export-style",t.textContent=Up,(document.head||document.documentElement).appendChild(t)}a(xs,"ensureScheduleExportStyles");function ys(){if(document.getElementById("urppp-settings-style"))return;let t=document.createElement("style");t.id="urppp-settings-style",t.textContent=Wp,(document.head||document.documentElement).appendChild(t)}a(ys,"ensureSettingsStyles");function un(t){let e=(t&&t.querySelector?t:document).querySelector("#navbar .user-info, .ace-nav .user-info, .user-info");if(!e)return null;let o=e.querySelector(".urppp-user-name-value");if(o)return o;let i=e.cloneNode(!0);i.querySelectorAll("small, i, img, b, .badge").forEach(h=>h.remove());let s=(i.textContent||"").replace(/^\s*欢迎您[，,]?\s*/g,"").replace(/\s+/g," ").trim();Array.from(e.childNodes).forEach(h=>{h.nodeType===Node.TEXT_NODE&&h.textContent.trim()&&h.remove()});let l=document.createElement("span");return l.className="urppp-user-name-value",l.textContent=s||"同学",l.__urpppOriginalText=l.textContent,e.appendChild(l),l}a(un,"ensureNavNameTarget");function Fe(t){let r=String(t||"").replace(/[\s:：]/g,"");return r?/姓名|英文姓名|姓名拼音/.test(r)?"name":/学号|证件|身份证|护照|证书编号|考生号|录取号|学籍号/.test(r)?"identity":/学院|院系|专业|班级|年级|主修方案|培养方案|专业方向|分流方向|毕业中学/.test(r)?"organization":/电话|手机|电子邮件|邮箱|QQ|地址|家长|个人主页|出生日期|入学日期|乘车区间|性别|籍贯|民族|政治面貌|国籍|户口|户籍|生源|出生地|健康|宗教|血型|婚姻|联系人|家庭/.test(r)?"contact":/绩点|GPA/.test(r)?"gpa":/学分/.test(r)?"credit":/成绩|分数|高考总分|均分|平均分|必修平均|课程门数|及格课程|不及格课程|待修读课程|已修读课程/.test(r)?"grade":/课表|日程安排/.test(r)?"schedule":"":""}a(Fe,"classifyPrivacyLabel");function vs(t,r){let e=String(t||"")+" "+String(r||"");return/绩点|GPA/.test(e)?"majorGpa":/主修为|培养方案|方案/.test(e)?"majorPlan":/尚不及格|未及格/.test(e)?"failedCourses":/待修读课程/.test(e)?"remainingCourses":/已修读课程/.test(e)?"completedCourses":""}a(vs,"classifyHomeDataKey");function De(t,r,e){let o=r?` data-urppp-edit-key="${r}"`:"";return`<span class="urppp-private-value" data-urppp-private="${t}"${o}>${e}</span>`}a(De,"homePrivateValueSpan");function ws(t,r){let e=at(t),o=at(r),i=vs(t,r),l={completedCourses:"other",failedCourses:"other",majorGpa:"gpa",majorPlan:"organization",remainingCourses:"other"}[i]||Fe(String(t||"")+" "+String(r||""));if(l==="organization")return r?{valueHtml:e,labelHtml:De("organization",i,o)}:{valueHtml:De("organization",i,e),labelHtml:o};if(!["grade","gpa","credit","other"].includes(l))return{valueHtml:e,labelHtml:o};let h=String(r||"").match(/-?\d+(?:\.\d+)?/);if(!(/^-?\d+(?:\.\d+)?$/.test(String(t||"").trim())||/^(优秀|良好|中等|及格|不及格|合格|不合格)$/.test(String(t||"").trim()))&&h){let E=h.index||0,M=String(r).slice(0,E),j=String(r).slice(E+h[0].length);return{valueHtml:e,labelHtml:`${at(M)}${De(l,i,at(h[0]))}${at(j)}`}}return{valueHtml:De(l,i,e),labelHtml:o}}a(ws,"statCardPrivacyMarkup");function Sr(t,r){if(!t||t.mode==="off")return"";if(t.mode==="one")return t.mask||ve;if(r==="name")return"";let e=t.fields&&t.fields[r];return!e||!e.enabled?"":String(e.replacement||t.mask||ve)}a(Sr,"privacyReplacement");function se(t,r){if(!(!t||!r)&&!(t.querySelector&&t.querySelector("input,select,textarea,button"))){if(!t.classList.contains("urppp-private-text")){let e=getComputedStyle(t).fontSize;e&&e!=="0px"&&t.style.setProperty("--urppp-private-font-size",e)}t.classList.add("urppp-private-text"),t.setAttribute("data-urppp-private-mask",r)}}a(se,"markPrivateText");function mn(t,r){if(!t||!t.parentElement)return;let e=t.parentElement;t.classList.add("urppp-private-avatar"),e.classList.add("urppp-private-avatar-host"),e.setAttribute("data-urppp-private-mask",r||ve);let o=t.getBoundingClientRect();e.style.setProperty("--urppp-avatar-left",t.offsetLeft+"px"),e.style.setProperty("--urppp-avatar-top",t.offsetTop+"px"),e.style.setProperty("--urppp-avatar-width",Math.max(1,o.width)+"px"),e.style.setProperty("--urppp-avatar-height",Math.max(1,o.height)+"px"),e.style.setProperty("--urppp-avatar-radius",getComputedStyle(t).borderRadius||"50%")}a(mn,"markPrivateAvatar");function ks(t,r){if(!t||!r)return;let e=t.matches("table")&&t.closest(".table-responsive, .urppp-table-wrap")||t;e.classList.add("urppp-private-block"),e.setAttribute("data-urppp-private-mask",r)}a(ks,"markPrivateBlock");function As(t,r){if(!(!t||!U[r])){if(!t.hasAttribute("data-urppp-direct-tabindex")){let e=t.getAttribute("tabindex");t.setAttribute("data-urppp-direct-tabindex",e??"__none__"),t.__urpppDirectTitle=t.getAttribute("title"),t.__urpppDirectAriaLabel=t.getAttribute("aria-label")}t.classList.add("urppp-direct-editable"),t.setAttribute("tabindex","0"),t.setAttribute("data-urppp-edit-key",r),t.setAttribute("aria-label","修改"+U[r]+"显示值"),t.title="点击修改显示值"}}a(As,"markDirectEditable");let br=null;function bn(t){let r=t&&t.getAttribute("data-urppp-edit-key");if(!r||!U[r])return;br&&br.__finish&&br.__finish(!1);let e=yr();if(e.mode!=="custom"||!e.directEdit.enabled)return;let i=String(e.directEdit.values[r]||"")||t.getAttribute("data-urppp-private-mask")||String(t.textContent||"").trim(),s=t.getBoundingClientRect(),l=t.parentElement?.getBoundingClientRect(),h=s.height>=8||!l?s:{left:s.left,top:l.top,width:Math.max(s.width,40),height:l.height},g=document.createElement("input"),E=getComputedStyle(t),M=Math.min(Math.max(h.width+64,140),Math.max(140,window.innerWidth-24)),j=Math.min(Math.max(12,h.left),Math.max(12,window.innerWidth-M-12)),z=Math.min(Math.max(12,h.top+(h.height-36)/2),Math.max(12,window.innerHeight-48));g.type="text",g.maxLength=80,g.className="urppp-direct-edit-input",g.value=i,g.setAttribute("aria-label","修改"+U[r]+"显示值"),g.style.left=j+"px",g.style.top=z+"px",g.style.setProperty("--urppp-direct-edit-width",M+"px"),g.style.fontFamily=E.fontFamily,g.style.fontSize=(window.innerWidth<=520?16:Math.min(18,Math.max(13,parseFloat(E.fontSize)||14)))+"px";let F=!1,J=a(D=>{if(F||(F=!0,g.remove(),br===g&&(br=null),D))return;let R=yr();R.mode!=="custom"||!R.directEdit.enabled||(R.directEdit.values[r]=String(g.value||"").trim().slice(0,80),la(R),Vt(document),Oa(R.directEdit.values[r]?"显示值已更新":"已恢复分类设置"))},"finish");g.__finish=J,g.addEventListener("click",D=>D.stopPropagation()),g.addEventListener("blur",()=>J(!1)),g.addEventListener("keydown",D=>{D.key==="Enter"&&(D.preventDefault(),J(!1)),D.key==="Escape"&&(D.preventDefault(),J(!0))}),document.documentElement.appendChild(g),br=g,g.focus(),g.select()}a(bn,"openDirectEditInput");function Ss(){document.__urpppDirectEditBound||(document.__urpppDirectEditBound=!0,document.addEventListener("click",t=>{let r=t.target?.closest?.(".urppp-direct-editable");r&&(t.preventDefault(),t.stopPropagation(),bn(r))},!0),document.addEventListener("keydown",t=>{if(!["Enter"," "].includes(t.key))return;let r=t.target?.closest?.(".urppp-direct-editable");r&&(t.preventDefault(),t.stopPropagation(),bn(r))},!0))}a(Ss,"bindDirectEditInteraction");function _s(t){let r=t&&t.querySelectorAll?t:document;r.querySelectorAll(".urppp-direct-editable").forEach(e=>{let o=e.getAttribute("data-urppp-direct-tabindex");e.classList.remove("urppp-direct-editable"),e.removeAttribute("data-urppp-direct-tabindex"),o==="__none__"?e.removeAttribute("tabindex"):o!=null&&e.setAttribute("tabindex",o),e.__urpppDirectTitle==null?e.removeAttribute("title"):e.setAttribute("title",e.__urpppDirectTitle),e.__urpppDirectAriaLabel==null?e.removeAttribute("aria-label"):e.setAttribute("aria-label",e.__urpppDirectAriaLabel),delete e.__urpppDirectTitle,delete e.__urpppDirectAriaLabel}),r.querySelectorAll(".urppp-private-text").forEach(e=>{e.classList.remove("urppp-private-text"),e.removeAttribute("data-urppp-private-mask"),e.style.removeProperty("--urppp-private-font-size")}),r.querySelectorAll(".urppp-private-avatar").forEach(e=>e.classList.remove("urppp-private-avatar")),r.querySelectorAll(".urppp-private-avatar-host").forEach(e=>{e.classList.remove("urppp-private-avatar-host"),e.removeAttribute("data-urppp-private-mask"),["--urppp-avatar-left","--urppp-avatar-top","--urppp-avatar-width","--urppp-avatar-height","--urppp-avatar-radius"].forEach(o=>e.style.removeProperty(o))}),r.querySelectorAll(".urppp-private-avatar-block").forEach(e=>{e.classList.remove("urppp-private-avatar-block"),e.removeAttribute("data-urppp-private-mask")}),r.querySelectorAll(".urppp-private-block").forEach(e=>{e.classList.remove("urppp-private-block"),e.removeAttribute("data-urppp-private-mask")})}a(_s,"clearPrivacyDisplay");function hn(t,r,e){if(!t||t.matches?.("input,select,textarea,button")||t.querySelector?.("input,select,textarea,button"))return;if(t.__urpppOriginalText==null){if(!r)return;t.__urpppOriginalText=t.textContent||""}let o=r&&e?e:t.__urpppOriginalText;t.textContent!==o&&(t.textContent=o)}a(hn,"applyCustomText");function Es(t){let r=t&&t.querySelectorAll?t:document,e=qr(),i=r.querySelector?.(".urppp-user-name-value")||(e.nameEnabled?un(r):null);hn(i,e.nameEnabled,e.name),r.querySelectorAll(".profile-info-row").forEach(h=>{let g=h.querySelector(".profile-info-name"),E=h.querySelector(".profile-info-value");!g||!E||String(g.textContent||"").replace(/[\s:：]/g,"")!=="姓名"||hn(E,e.nameEnabled,e.name)});let s=te(e.avatar),l=e.avatarEnabled&&!!s;r.querySelectorAll("#navbar img.nav-user-photo, #urppp-mobile-user img.nav-user-photo, img#avatar, .profile-picture img").forEach(h=>{let g=h.getAttribute("src")||"";g&&g!==h.__urpppAppliedCustomSrc&&(h.__urpppOriginalSrc=g),l?(h.__urpppOriginalSrc==null&&(h.__urpppOriginalSrc=g),g!==s&&h.setAttribute("src",s),h.__urpppAppliedCustomSrc=s):h.__urpppAppliedCustomSrc!=null&&(h.__urpppOriginalSrc&&h.setAttribute("src",h.__urpppOriginalSrc),delete h.__urpppAppliedCustomSrc)})}a(Es,"applyCustomIdentityDisplay");function Cs(t,r){t.querySelectorAll(".profile-info-row").forEach(e=>{let o=e.querySelector(".profile-info-name, th, label"),i=e.querySelector(".profile-info-value, td:last-child");if(!o||!i||o===i)return;let s=Fe(o.textContent),l=Sr(r,s);l&&se(i,l)})}a(Cs,"applyProfilePrivacy");function Ps(t,r){t.querySelectorAll("table").forEach(e=>{let o=Array.from(e.querySelectorAll("thead th, thead td, tr:first-child th, tr:first-child td"));if(!o.length)return;let i=o.map(s=>{let l=Fe(s.textContent);return["grade","gpa","credit"].includes(l)?l:""});i.some(Boolean)&&e.querySelectorAll("tbody tr").forEach(s=>{let l=s.querySelectorAll("td");i.forEach((h,g)=>{let E=Sr(r,h);h&&E&&se(l[g],E)})})})}a(Ps,"applyScoreTablePrivacy");function zs(t){let r=t&&t.querySelectorAll?t:document,e=yr();if(e.mode==="off")return;let o=Sr(e,"name"),i=Sr(e,"avatar"),s=Sr(e,"schedule"),l=o?un(r):r.querySelector?.(".urppp-user-name-value");o&&se(l,o),[["#courseNum, #coursePas, #xy_kcms","other"],["#gpa","gpa"],["#bottom","organization"]].forEach(([E,M])=>{let j=Sr(e,M);j&&r.querySelectorAll(E).forEach(z=>se(z,j))}),Ps(r,e);let g=e.mode==="custom"&&e.directEdit.enabled;if(r.querySelectorAll("[data-urppp-private]").forEach(E=>{let M=E.getAttribute("data-urppp-private"),j=E.getAttribute("data-urppp-edit-key"),F=(g&&j?String(e.directEdit.values[j]||"").trim():"")||Sr(e,M);!["avatar","schedule"].includes(M)&&F&&se(E,F),g&&j&&As(E,j)}),g&&Ss(),Cs(r,e),i&&(r.querySelectorAll('[data-urppp-private="avatar"]').forEach(E=>{let M=E.matches("img")?E:E.querySelector("img");M?mn(M,i):(E.classList.add("urppp-private-avatar-block"),E.setAttribute("data-urppp-private-mask",i))}),r.querySelectorAll("#navbar img.nav-user-photo, img#avatar, .profile-picture img, .uc-avatar img").forEach(E=>mn(E,i))),s){let E=Array.from(r.querySelectorAll('[data-urppp-private="schedule"], #main-calendar, #courseTable'));E.filter(M=>!E.some(j=>j!==M&&j.contains(M))).forEach(M=>ks(M,s))}}a(zs,"applyPrivacyDisplay");let Ba=0,sr=[];function gn(){let t=yr(),r=qr();return t.mode!=="off"||r.nameEnabled||r.avatarEnabled}a(gn,"personalDisplayIsEnabled");function Ls(){sr=sr.filter(({root:t})=>t&&t.isConnected),sr.forEach(({root:t,observer:r})=>r.observe(t,{childList:!0,subtree:!0}))}a(Ls,"resumePersonalDisplayObservers");function Vt(t){let r=t||document;sr.forEach(({observer:e})=>e.disconnect());try{dn()}catch{}try{_s(r)}catch{}try{Es(r)}catch(e){console.warn("[URP++] custom identity",e)}try{zs(r)}catch(e){console.warn("[URP++] privacy",e)}gn()?(Ls(),Ts()):(clearTimeout(Ba),sr=[])}a(Vt,"applyPersonalDisplay");function qs(t){clearTimeout(Ba),Ba=setTimeout(()=>Vt(t||document),140)}a(qs,"schedulePersonalDisplay");function Fa(){try{nt&&nt.open&&Yr()}catch{}}a(Fa,"refreshCleanPersonalDisplay");function Ts(){if(!gn()){sr.forEach(({observer:t})=>t.disconnect()),sr=[];return}[document.getElementById("navbar"),document.getElementById("page-content-template"),document.getElementById("urppp-clean-root")].filter(Boolean).forEach(t=>{if(sr.some(e=>e.root===t))return;let r=new MutationObserver(()=>qs(document));sr.push({root:t,observer:r}),r.observe(t,{childList:!0,subtree:!0})})}a(Ts,"bindPersonalDisplayObservers");function $s(t){let r=Object.assign({},t||{}),e=qr();e.nameEnabled&&e.name&&(r.name=e.name);let o=te(e.avatar);return e.avatarEnabled&&o&&(r.avatar=o),r}a($s,"personalizedProfile");let fn="/student/courseSelect/thisSemesterCurriculum/ajaxStudentSchedule/callback",Ms="/student/courseSelect/thisSemesterCurriculum/callback",Is="/student/courseSelect/thisSemesterCurriculum/index";async function Ns(){let t=document.querySelector("#planCode, #zxjxjhh");if(t&&t.value&&t.value!=="no")return String(t.value);try{let r=new URLSearchParams(location.search),e=r.get("planCode")||r.get("zxjxjhh");if(e)return e}catch{}if(nt&&nt.schedule&&nt.schedule.exportData){let r=nt.schedule.exportData.semester&&nt.schedule.exportData.semester.planCode;if(r)return r}if(/\/student\/courseSelect\/courseSelectResult\//.test(location.pathname))try{let r=await Jt(Ms),e=JSON.parse(r),o=Zr(e);if(o)return o}catch{}return""}a(Ns,"resolveSchedulePlanCode");async function xn(t){let r=await Ns(),e=r?{method:"POST",data:"planCode="+encodeURIComponent(r),headers:{"Content-Type":"application/x-www-form-urlencoded; charset=UTF-8"}}:null,o=await Jt(fn,e),i;try{i=JSON.parse(o)}catch{throw new Error("课表接口返回了非 JSON 内容，请刷新教务页面后重试")}r||(r=Zr(i)),(!i.jcsjbs||!i.jcsjbs.length)&&r&&(i=JSON.parse(await Jt(fn,{method:"POST",data:"planCode="+encodeURIComponent(r),headers:{"Content-Type":"application/x-www-form-urlencoded; charset=UTF-8"}})));let s=vn(i,r,t);if(!s.courses.length)throw new Error("没有读取到可导出的课表数据");return s}a(xn,"loadScheduleExportData");function Da(t){return String(t||"学生课表").replace(/[\\/:*?"<>|]+/g,"-").replace(/\s+/g,"").slice(0,80)||"学生课表"}a(Da,"safeScheduleFilename");function ja(t,r){let e=URL.createObjectURL(t),o=document.createElement("a");o.href=e,o.download=r,o.style.display="none",document.body.appendChild(o),o.click(),o.remove(),setTimeout(()=>URL.revokeObjectURL(e),1200)}a(ja,"downloadBlob");function Bs(t){let r=Ge(t),e=Ee(),o=e.enabled?Ve(r,e.mapping):Je(r),i=JSON.stringify(o,null,2)+`
`;return ja(new Blob([i],{type:"application/json;charset=utf-8"}),Da(t.semester.label)+".json"),Object.assign({customFormat:e.enabled},r.stats)}a(Bs,"exportScheduleJson");function yn(t){let e=(Array.from(document.querySelectorAll(".span_bbzx")).map(l=>l.textContent||"").join(" ")+" "+(document.querySelector("#navbar")?.textContent||"")).replace(/\s+/g," ").match(/(\d{4})-(\d{4})\s*(春|秋).*?第\s*(\d{1,2})\s*周/);if(!e)return"";let o=e[3]==="秋"?"1":"2";if(t&&!String(t).startsWith(e[1]+"-"+e[2]+"-"+o))return"";let i=Number(e[4]);if(i<1||i>30)return"";let s=Za(new Date);return s.setDate(s.getDate()-(i-1)*7),ye(s)}a(yn,"deriveCurrentSemesterMonday");function vn(t,r,e){let o=r||Zr(t),i=yn(o)||ca()[o]||"";return ip(t,o,e,{firstMonday:i})}a(vn,"normalizeScheduleDataForPage");function Fs(t){let r=t.semester.planCode,e=ca()[r],o=yn(r);return o?(bo(r,o),Promise.resolve(o)):Kr(e)?Promise.resolve(e):new Promise((i,s)=>{document.querySelector('.urppp-dialog-mask[data-dialog="schedule-date"]')?.remove();let l=document.createElement("div");l.className="urppp-dialog-mask",l.dataset.dialog="schedule-date",l.innerHTML=`<div class="urppp-dialog" role="dialog" aria-modal="true"><h3>确认第一教学周周一</h3><p>${at(t.semester.label)}没有可可靠推导的起始日期。该日期决定 ICS 中每节课的实际日历时间；预填值仅为估算，请对照校历核对。</p><input type="date" value="${at(e||ep(r))}"><div class="urppp-dialog-actions"><button type="button" class="urppp-set-btn ghost" data-action="cancel">取消</button><button type="button" class="urppp-set-btn" data-action="ok">确认并导出</button></div></div>`,document.documentElement.appendChild(l);let h=a((g,E)=>{l.remove(),g?s(g):i(E)},"close");l.querySelector('[data-action="cancel"]').addEventListener("click",()=>h(new Error("已取消导出"))),l.querySelector('[data-action="ok"]').addEventListener("click",()=>{let g=l.querySelector("input").value;Kr(g)&&(bo(r,g),h(null,g))}),l.addEventListener("click",g=>{g.target===l&&h(new Error("已取消导出"))})})}a(Fs,"requestScheduleFirstMonday");async function Ds(t){let r=await Fs(t),e=op(t,r);return ja(new Blob([e],{type:"text/calendar;charset=utf-8"}),Da(t.semester.label)+".ics"),np(t)}a(Ds,"exportScheduleIcs");let js={apple:"类 Apple",flat:"极简扁平",organic:"自然有机",brutal:"新野兽派",editorial:"编辑杂志",neu:"新拟物"};function lr(t,r,e){if(typeof document>"u")return Gt(r)||"#000000";let o=document.createElement("span");o.style.cssText="position:fixed;left:-9999px;visibility:hidden;color:var("+t+","+r+")",(document.body||document.documentElement).appendChild(o);let i=getComputedStyle(o).color;o.remove();let s=String(i||"").match(/[\d.]+/g)?.map(Number)||[];if(s.length>=3){let l=he(s[0],s[1],s[2]),h=s.length>3?Math.max(0,Math.min(1,s[3])):1;return h<1?Bt(e||r,l,h):l}return Gt(i)||Gt(r)||"#000000"}a(lr,"resolvedScheduleImageColor");function wn(){let t=Qt(),r=Zt(),e=t==="dark",o=e?{bg:"#000000",surface:"#1C1C1E",input:"#2C2C2E",text:"#F5F5F7",secondary:"#A1A1A6",muted:"#8E8E93",border:"#38383A",primary:"#0A84FF"}:{bg:"#F5F5F7",surface:"#FFFFFF",input:"#F5F5F7",text:"#1D1D1F",secondary:"#6E6E73",muted:"#86868B",border:"#D2D2D7",primary:"#0071E3"},i={bg:lr("--bg",o.bg),surface:lr(r==="neu"?"--neu-base":"--surface",o.surface),input:lr("--input-bg",o.input),text:lr("--text",o.text),secondary:lr("--text-secondary",o.secondary),muted:lr("--text-muted",o.muted),border:lr("--border",o.border,lr(r==="neu"?"--neu-base":"--surface",o.surface)),primary:lr("--primary",o.primary)},s={apple:{frameRadius:24,headerRadius:13,gridRadius:10,cardRadius:12,frameStroke:1,cardStroke:1,shadow:"soft"},flat:{frameRadius:0,headerRadius:0,gridRadius:0,cardRadius:0,frameStroke:2,cardStroke:2,shadow:"none"},organic:{frameRadius:30,headerRadius:18,gridRadius:14,cardRadius:18,frameStroke:1,cardStroke:1,shadow:"warm"},brutal:{frameRadius:0,headerRadius:0,gridRadius:0,cardRadius:0,frameStroke:3,cardStroke:3,shadow:"hard"},editorial:{frameRadius:0,headerRadius:0,gridRadius:0,cardRadius:0,frameStroke:1,cardStroke:1,shadow:"none",serif:!0},neu:{frameRadius:22,headerRadius:14,gridRadius:10,cardRadius:14,frameStroke:0,cardStroke:0,shadow:"neu"}};return{id:t,skin:r,dark:e,label:(js[r]||r)+" · "+(Ct[t]&&Ct[t].name||t),colors:i,shape:s[r]||s.apple}}a(wn,"currentScheduleImageTheme");function kn(t,r){return gp(t,r||wn())}a(kn,"buildScheduleSvg");function Os(t){return new Promise((r,e)=>{let o=new Blob([t.svg],{type:"image/svg+xml;charset=utf-8"}),i=URL.createObjectURL(o),s=new Image;s.onload=()=>{try{let h=Math.min(2,Math.sqrt(15e6/(t.width*t.height))),g=document.createElement("canvas");g.width=Math.floor(t.width*h),g.height=Math.floor(t.height*h);let E=g.getContext("2d");E.scale(g.width/t.width,g.height/t.height),E.fillStyle=t.background||"#F8FAFC",E.fillRect(0,0,t.width,t.height),E.drawImage(s,0,0,t.width,t.height),g.toBlob(M=>M?r(M):e(new Error("无法生成课表图片")),"image/png")}catch(l){e(l)}finally{URL.revokeObjectURL(i)}},s.onerror=()=>{URL.revokeObjectURL(i),e(new Error("课表图片渲染失败"))},s.src=i})}a(Os,"svgToPngBlob");async function Hs(t){let r=await Os(kn(t));ja(r,Da(t.semester.label)+".png")}a(Hs,"exportSchedulePng");function Oa(t,r){document.getElementById("urppp-feature-toast")?.remove();let e=document.createElement("div");e.id="urppp-feature-toast",e.textContent=String(t||""),e.className=r?"error":"",document.documentElement.appendChild(e),requestAnimationFrame(()=>e.classList.add("open")),setTimeout(()=>{e.classList.remove("open"),setTimeout(()=>e.remove(),220)},r?4200:2400)}a(Oa,"showFeatureToast");let Ha=fp({document,window,ensureStyles:xs,loadData:xn,exportJson:Bs,exportIcs:Ds,exportPng:Hs,showToast:Oa,nativePageUrl:Is,navigate:a(t=>{location.href=t},"navigate"),logger:console});function Rs(t,r,e,o){return Ha.run(t,r,e,o)}a(Rs,"runScheduleExport");function Us(t){return Ha.createMenu(t)}a(Us,"createScheduleExportMenu");function Ws(t){if(t){try{t.stage.remove()}catch{}try{document.getElementById("urppp-pdf-reset-style")?.remove()}catch{}}}a(Ws,"disposeNativePdfCapture");function Gs(){window.__urpppPdfDiagnose||(window.__urpppPdfDiagnose=async()=>{let t={time:new Date().toISOString()},r=document.getElementById("mycoursetable"),e=document.getElementById("page-content-template");t.host=!!r,t.pageSource=!!e,t.hostCards=r?r.querySelectorAll("div.class_div").length:-1,t.hostHasCourseTable=r?!!r.querySelector("#courseTable"):!1,t.hostHasCourseTableBody=r?!!r.querySelector("#courseTableBody"):!1,t.hostTableId=r&&r.querySelector("table")?r.querySelector("table").id:"none";try{let i=Bp(r);t.stage="ok",t.stageCards=i.target.querySelectorAll(".urppp-pdf-card").length,t.stageTableId=i.target.querySelector("table")?i.target.querySelector("table").id:"none",Ws(i)}catch(i){t.stage="failed",t.stageError=i&&i.message||String(i)}let o=typeof unsafeWindow<"u"?unsafeWindow:window;return t.deps={dollar:typeof o.$,loadFileList:typeof(o.Import&&o.Import.LoadFileList),back:typeof o.back,html2canvas:typeof o.html2canvas,originalDivBuild:typeof o.__urpppOriginalDivBuild},t})}a(Gs,"bindNativePdfDiagnose");function Js(t){return t?(Gs(),async()=>{let r=document.getElementById("urppp-settings-panel"),e=document.getElementById("urppp-settings-mask");r&&r.classList.contains("open")&&r.classList.remove("open"),e&&e.classList.contains("open")&&e.classList.remove("open");try{await jp(t,{document,page:typeof unsafeWindow<"u"?unsafeWindow:window,onAfterRestore:pe})}catch(o){console.warn("[URP++] isolated native PDF export failed",o),Oa("原生 PDF 隔离导出失败："+(o&&o.message||String(o))+"，请重试",!0)}}):null}a(Js,"pagePdfExportHandler");function Ra(t=location){return/\/(?:student\/courseSelect\/(?:thisSemesterCurriculum|courseSelectResult|calendarSemesterCurriculum)|student\/personalSenate\/giveLessonInfo\/thisSemesterSchedule)\//.test(t.pathname)}a(Ra,"isPersonalSchedulePage");function Vs(t=location){return/\/student\/integratedQuery\/scoreQuery\/[^/]+\/index$/.test(t.pathname)}a(Vs,"isScoreQueryPage");function Ua(){if(!Ra())return;let t=document.querySelector("#h4_id1")?.closest("h4")||document.querySelector("h4.header"),r=t?.querySelector(".right_top_oper")||document.querySelector("#mainDIV .right_top_oper, .page-content .right_top_oper"),e=Array.from((r||document).querySelectorAll("button, a")),o=a(l=>[l.textContent,l.getAttribute("title"),l.getAttribute("onclick")].filter(Boolean).join(" ").replace(/\s+/g," "),"signatureOf");if(e.forEach(l=>{/打印.*课表|\bdy\s*\(/i.test(o(l))&&l.setAttribute("data-urppp-native-print-source","1")}),document.getElementById("urppp-native-schedule-export"))return;let i=e.find(l=>/导出.*(?:课表|PDF)|exportTableToPdf|\bdc\s*\(/i.test(o(l))),s=Us({source:"native",pdfHandler:Js(i)});if(s.id="urppp-native-schedule-export",i&&i.parentElement){i.__urpppNativeExportState||(i.__urpppNativeExportState={display:i.style.getPropertyValue("display"),displayPriority:i.style.getPropertyPriority("display"),ariaHidden:i.getAttribute("aria-hidden"),tabIndex:i.getAttribute("tabindex")}),i.setAttribute("data-urppp-native-export-source","1"),i.style.setProperty("display","none","important"),i.setAttribute("aria-hidden","true"),i.setAttribute("tabindex","-1"),i.parentElement.insertBefore(s,i.nextSibling);return}if(r)r.appendChild(s);else if(t)t.appendChild(s);else{let l=document.getElementById("page-content-template")||document.querySelector(".page-content");if(l){let h=document.createElement("div");h.className="urppp-export-fallback",h.appendChild(s),l.prepend(h)}}}a(Ua,"patchNativeScheduleExport");let Ur=null,je=0;function Wa(){clearTimeout(je),je=0,Ur&&Ur.observer.disconnect(),Ur=null}a(Wa,"disconnectNativeScheduleExportObserver");function Ys(){if(!Ra()){Wa();return}let t=document.getElementById("page-content-template")||document.querySelector(".page-content")||document.body;if(!t||Ur&&Ur.root===t&&t.isConnected)return;Wa();let r=new MutationObserver(()=>{clearTimeout(je),je=setTimeout(()=>Ua(),80)});r.observe(t,{childList:!0,subtree:!0}),Ur={root:t,observer:r}}a(Ys,"bindNativeScheduleExportObserver");function An(t,r,e){e===null?t.removeAttribute(r):t.setAttribute(r,e)}a(An,"restoreOptionalAttribute");function Qs(t=document){let r=t&&t.querySelectorAll?t:document,e=r.matches?.("#urppp-native-schedule-export")?r:r.querySelector("#urppp-native-schedule-export");if(e){let o=e.closest(".urppp-export-fallback");e.remove(),o&&!o.children.length&&o.remove()}r.querySelectorAll("[data-urppp-native-export-source]").forEach(o=>{let i=o.__urpppNativeExportState;i&&(i.display?o.style.setProperty("display",i.display,i.displayPriority):o.style.removeProperty("display"),An(o,"aria-hidden",i.ariaHidden),An(o,"tabindex",i.tabIndex)),o.removeAttribute("data-urppp-native-export-source");try{delete o.__urpppNativeExportState}catch{}}),r.querySelectorAll("[data-urppp-native-print-source]").forEach(o=>{o.removeAttribute("data-urppp-native-print-source")})}a(Qs,"removeNativeScheduleExport");let Sn=wi({deps:{styles:Qp,loadScores:Fn,loadProfile:_n,scoreToNumber:Rr,scoreToGpa:Hr,getInsertHost:a(()=>document.querySelector(".page-content")||document.getElementById("page-content-template")||null,"getInsertHost"),shouldAutoExpand:a(()=>{let t=/[?&]urppp=sa(?:&|$)/.test(window.location.search);if(t)try{history.replaceState(null,"",window.location.pathname+window.location.hash)}catch{}return t},"shouldAutoExpand")}}),Xs=Yn([Ue({id:"schedule-export",matches:a(t=>Ra(t.location),"matches"),mount:a(()=>{Ua(),Ys()},"mount"),unmount:a(t=>{Wa(),Qs(t?.lifecycleKey)},"unmount")}),Ue({id:"score-analysis",matches:a(t=>Vs(t.location),"matches"),mount:a(()=>{try{Sn.mount()}catch(t){console.warn("[URP++] score analysis mount",t)}},"mount"),unmount:a(()=>{try{Sn.unmount()}catch{}},"unmount")})]);function Oe(){let t=document.getElementById("page-content-template")||document.querySelector(".page-content")||document.body;return Xs.refresh({document,location,window,lifecycleKey:t})}a(Oe,"refreshRouteFeatures");function Ks(t){Ha.bindHosts(t)}a(Ks,"bindScheduleExportHosts");function le(t){return String(t||"").replace(/\u00a0/g," ").replace(/\s+/g," ").replace(/^[\s:：]+|[\s:：]+$/g,"").trim()}a(le,"normalizeProfileValue");function He(t,r){if(!t||!t.querySelectorAll)return"";let e=(r||[]).map(i=>le(i).replace(/[：:]/g,"")),o=t.querySelectorAll(".profile-info-row, tr");for(let i=0;i<o.length;i++){let s=o[i],l=s.querySelector(".profile-info-name, th, label"),h=s.querySelector(".profile-info-value, td:last-child");if(!l||!h||l===h)continue;let g=le(l.textContent).replace(/[：:]/g,"");if(!e.some(M=>g===M||g.endsWith(M)))continue;let E=le(h.textContent);if(E&&E!=="—"&&E!=="-")return E}return""}a(He,"readLabeledProfileValue");function Wr(t){return le(t).replace(/^主修为\s*/,"").replace(/培养方案概况.*$/,"").replace(/…+/g,"").split(/主修必修GPA|GPA算法|已修读|尚不及格|本学期/)[0].trim()}a(Wr,"cleanMajorPlanName");function Zs(t){let r={majorPlan:"",majorGpa:""};return!t||!t.querySelectorAll||t.querySelectorAll(".infobox, .widget-box, .urppp-stat-card").forEach(e=>{let o=(e.innerText||e.textContent||"").trim(),i=le(o);if(/主修必修GPA/.test(i)){let s=i.match(/(-?\d+(?:\.\d+)?)\s*主修必修GPA/)||i.match(/主修必修GPA[^\d-]{0,20}(-?\d+(?:\.\d+)?)/);if(s){let l=Number(s[1]),h=Number(r.majorGpa);Number.isFinite(l)&&l>=0&&l<=5&&(!r.majorGpa||h===0||l>0)&&(r.majorGpa=s[1])}}if(/主修为|培养方案/.test(i)){let s=i.match(/([\u4e00-\u9fa5A-Za-z0-9（）()·+\-]{2,60}(?:培养方案|教学计划))/)||i.match(/^(.{2,60}?)\s*主修为/)||i.match(/主修为\s*(.{2,60})$/),l=Wr(s&&s[1]);if(l&&!/GPA|已修读|尚不及格|本学期/.test(l)){let h=/培养方案|教学计划/.test(l);(!r.majorPlan||h)&&(r.majorPlan=l)}}}),r}a(Zs,"extractAcademicOverview");async function _n(){let t={name:"",avatar:"",majorPlan:"",majorGpa:"",studentId:""};try{let e=document.querySelector("#navbar .user-info, .ace-nav .user-info, .user-info");if(e){let s=e.querySelector(".urppp-user-name-value"),l=s&&s.__urpppOriginalText;l&&(t.name=String(l).trim());let h=(e.innerText||e.textContent||"").replace(/\s+/g," ").trim(),g=t.name?null:h.match(/欢迎您[，,]\s*([\u4e00-\u9fa5·]{2,12})/);if(!t.name&&!g){let E=e.cloneNode(!0);E.querySelectorAll("small, i, img, b, .badge").forEach(j=>j.remove());let M=(E.textContent||"").replace(/\s+/g," ").trim();M=M.replace(/^欢迎您[，,]\s*/g,"").replace(/\d{8,}/g,"").trim(),g=M.match(/([\u4e00-\u9fa5·]{2,12})/)}g&&g[1]&&!/欢迎|同学|首页|反馈|密码|注销/.test(g[1])&&(t.name=g[1])}let o=document.querySelector("#navbar img.nav-user-photo, .ace-nav img.nav-user-photo");o&&(t.avatar=o.__urpppOriginalSrc||o.src||o.getAttribute("src")||"");let i=Zs(document);t.majorPlan=i.majorPlan,t.majorGpa=i.majorGpa}catch{}try{let e=await Jt("/student/rollManagement/rollInfo/index"),o=Be(e),i=o.body&&(o.body.innerText||o.body.textContent)||"";if(!t.name&&(t.name=He(o,["姓名"]),!t.name)){let g=i.match(/姓名\s*[：:]?\s*([\u4e00-\u9fa5·]{2,20})/);g&&(t.name=g[1].trim())}let s=He(o,["主修方案名称"]),l=He(o,["专业"]);t.studentId=He(o,["学号"]),s?t.majorPlan=Wr(s):!t.majorPlan&&l&&(t.majorPlan=Wr(l));let h=o.querySelector('.profile-picture img, img#avatar, img[src*="photo" i], img[src*="Photo"]');if(h&&h.getAttribute("src")&&!t.avatar){let g=h.getAttribute("src");t.avatar=/^https?:/i.test(g)?g:Na(g)}}catch{}let r=Number(t.majorGpa);return t.name||(t.name="同学"),t.majorPlan||(t.majorPlan="主修方案"),(!Number.isFinite(r)||r<=0||r>5)&&(t.majorGpa="—"),t}a(_n,"loadProfile");let En=["周日","周一","周二","周三","周四","周五","周六"];function Ga(t){let r=[],e=t.querySelector("#courseTableBody")||t.querySelector("#courseTable tbody");if(!e)return r;e.querySelectorAll("td[id]").forEach(i=>{let s=String(i.id||"").match(/^(\d+)_(\d+)$/);if(!s)return;let l=parseInt(s[1],10),h=parseInt(s[2],10),g=l===7?0:l,E=i.querySelectorAll('.class_div, .div_style, div[class*="div-kcb"]'),M=E.length?E:[];if(!M.length&&(i.textContent||"").trim()){let j=(i.textContent||"").replace(/\s+/g," ").trim();j&&r.push({name:j.slice(0,40),teacher:"",place:"",week:"",day:g,section:h});return}M.forEach(j=>{let z=Array.from(j.querySelectorAll("p")).map(K=>(K.textContent||"").trim()).filter(Boolean),F=(j.querySelector(".p-kcm-1, .p-kcm")||{}).textContent||z[0]||"",J=(j.querySelector('.p-jxl-1, [class*="jxl"]')||{}).textContent||"",D=z.find((K,ut)=>ut>0&&!/周|节/.test(K)&&K!==J)||"",R=z.find(K=>/周/.test(K))||"",B=String(F).replace(/_\d+\s*$/,"").trim();!B||B.length<2||r.push({name:B,teacher:String(D).trim(),place:String(J||"").trim(),week:String(R).trim(),day:g,section:h})})});let o=new Set;return r.filter(i=>{let s=[i.day,i.section,i.name,i.place].join("|");return o.has(s)?!1:(o.add(s),!0)})}a(Ga,"parseScheduleFromDoc");let Cn="urppp_term_week_v1";function Gr(t){let r=Number(t)||0;if(r<1||r>30)return 0;nt._termWeek=r,nt._termWeekResolved=!0;try{GM_setValue(Cn,r)}catch{}return r}a(Gr,"rememberTermWeek");function ce(){if(nt&&nt._termWeek>=1)return nt._termWeekResolved=!0,nt._termWeek;try{let t=Number(GM_getValue(Cn,0))||0;if(t>=1&&t<=30)return Gr(t)}catch{}return 0}a(ce,"readRememberedTermWeek");function de(t){let r=String(t||"").replace(/\s+/g," ");if(!r)return 0;let e=[/(?:\d{4}\s*[-–]\s*\d{4}).{0,40}?第\s*(\d{1,2})\s*周/,/20\d{2}.{0,40}?第\s*(\d{1,2})\s*周/,/(?:春|秋|夏|冬)\s*第\s*(\d{1,2})\s*周/,/第\s*(\d{1,2})\s*周\s*(?:星期|周[一二三四五六日天])/];for(let o=0;o<e.length;o++){let i=r.match(e[o]);if(i){let s=parseInt(i[1],10);if(s>=1&&s<=30)return s}}return 0}a(de,"extractTermWeekFromText");function _r(){if(nt._termWeekResolved&&nt._termWeek>=1&&nt._termWeek<=30)return nt._termWeek;try{let t=[document.querySelector("#navbar"),document.querySelector(".navbar-fixed-top"),document.querySelector(".navbar"),document.querySelector("#navbar .navbar-header"),document.querySelector("#navbar .navbar-buttons"),document.querySelector(".ace-nav"),document.querySelector("#breadcrumbs"),document.querySelector("#page-content-header"),document.querySelector(".page-header"),document.querySelector("header")].filter(Boolean);for(let l=0;l<t.length;l++){let h=t[l],g=de(h.innerText||h.textContent||"")||de(h.innerHTML||"");if(g)return Gr(g)}let r=document.documentElement&&document.documentElement.innerHTML||"",e=de(r);if(e)return Gr(e);let o=document.body&&document.body.innerText||"",i=de(o);if(i)return Gr(i);let s=ce();if(s)return s}catch{}return 0}a(_r,"getCurrentWeekNumber");let Jr=null;function tl(){let t=new Date,r=a(e=>String(e).padStart(2,"0"),"p");return`${t.getFullYear()}-${r(t.getMonth()+1)}-${r(t.getDate())}`}a(tl,"calTodayStr");function rl(t,r){let e=new Date(`${t}T00:00:00`);e.setDate(e.getDate()+r);let o=a(i=>String(i).padStart(2,"0"),"p");return`${e.getFullYear()}-${o(e.getMonth()+1)}-${o(e.getDate())}`}a(rl,"calAddDays");function ue(t){if(Jr)return Jr;let r=t||tl();return r>="2027-02-06"&&r<=rl("2027-02-06",6)?"springfestival":r>="2027-01-18"&&r<"2027-03-01"?"winter":r>="2027-07-04"&&r<"2027-08-31"||r>="2026-07-04"&&r<"2026-08-31"?"summer":"term"}a(ue,"calVacation");function el(){let t='<svg viewBox="0 0 52 190"><path d="M26 0v16" stroke="#c8102e" stroke-width="3"/><rect x="16" y="16" width="20" height="8" rx="4" fill="#c8102e"/><ellipse cx="26" cy="62" rx="22" ry="30" fill="#e63946"/><path d="M26 26v72M14 34q12 12 0 24M38 34q-12 12 0 24" stroke="#ffd75e" stroke-width="1.4" fill="none"/><path d="M14 92h24M17 98h18M20 104h12" stroke="#ffd75e" stroke-width="2.4" stroke-linecap="round"/></svg>';return`<div id="urppp-festive-decor" aria-hidden="true"><div class="ufd ufd-left">${t}</div><div class="ufd ufd-right">${t}</div></div>`}a(el,"festiveDecorHtml");function Pn(){let t=typeof document<"u"?document:null;if(!t)return;let r=ue()==="springfestival",e=t.getElementById("urppp-festive-decor");r&&!e?t.documentElement.insertAdjacentHTML("beforeend",el()):!r&&e&&e.remove()}a(Pn,"syncFestiveDecor");function zn(t){Jr=t==="summer"||t==="winter"||t==="springfestival"||t==="term"?t:null,Jr&&Jr!=="term"&&(nt.weekLocked=!1,nt.viewWeek=0);try{Pn()}catch{}try{typeof Yr=="function"&&Yr()}catch{}return Jr}a(zn,"setCalendarPhase");function al(){return ue()}a(al,"getCalendarPhase");function Ln(){if(ue()!=="term")return nt.weekLocked?(!nt.viewWeek||nt.viewWeek<0)&&(nt.viewWeek=0):nt.viewWeek=0,nt.viewWeek;let t=_r()||ce()||0;return nt.weekLocked?(!nt.viewWeek||nt.viewWeek<1)&&(nt.viewWeek=t>=1?t:1):t>=1?nt.viewWeek=t:(!nt.viewWeek||nt.viewWeek<1)&&(nt.viewWeek=1),!nt.weekLocked&&t>1&&nt.viewWeek===1&&(nt.viewWeek=t),nt.viewWeek}a(Ln,"getViewWeekNumber");async function ol(){let t=_r();if(t>=1)return t;try{let r=await Jt("/index");if(t=de(r),t)return Gr(t)}catch{}try{let r=new Date,e=r.getFullYear()+"-"+String(r.getMonth()+1).padStart(2,"0")+"-"+String(r.getDate()).padStart(2,"0"),o="xqh=03&jxlh=302&jslx=&jasm=&zwFrom=&zwTo=&searchDate="+encodeURIComponent(e),i=await Jt("/student/teachingResources/classroomUseStatus/jasInfo",{method:"POST",data:o,headers:{"Content-Type":"application/x-www-form-urlencoded; charset=UTF-8","X-Requested-With":"XMLHttpRequest"}}),s=JSON.parse(i),l=Number(s&&s.jxzc);if(l>=1&&l<=30)return Gr(l)}catch{}return ce()||0}a(ol,"ensureTermWeekResolved");function nl(t){let r=_r()||20;return(t||[]).forEach(e=>{let o=String(e.classWeek||"");o.length>r&&(r=o.length);let i=String(e.week||"").match(/(\d{1,2})\s*[-~至到]\s*(\d{1,2})/);i&&(r=Math.max(r,parseInt(i[2],10)||0));let s=String(e.week||"").match(/\d{1,2}/g);s&&s.forEach(l=>{r=Math.max(r,parseInt(l,10)||0)})}),Math.min(Math.max(r,1),30)}a(nl,"inferMaxWeek");function qn(t,r){if(!r||!t)return!1;let e=String(t);return e.length>=r?e.charAt(r-1)==="1":!1}a(qn,"weekBitActive");let Tn=["#3B82F6","#10B981","#F59E0B","#EF4444","#8B5CF6","#06B6D4","#EC4899","#84CC16","#F97316","#6366F1"];function $n(t){let r=0,e=String(t||"");for(let o=0;o<e.length;o++)r=r*31+e.charCodeAt(o)>>>0;return Tn[r%Tn.length]}a($n,"courseColor");function pl(t){let r=[],e=_r();(t&&t.xkxx||[]).forEach(s=>{Object.keys(s||{}).forEach(l=>{let h=s[l];if(!h)return;let g=h.courseName||h.englishCourseName||l,E=h.attendClassTeacher||"";(h.timeAndPlaceList||[]).forEach(j=>{let z=Number(j.classDay)||0,F=z===7?0:z,J=Number(j.classSessions)||1,D=Math.max(1,Number(j.continuingSession)||1),R=[j.campusName,j.teachingBuildingName,j.classroomName].filter(Boolean).join(""),B=j.weekDescription||h.skzcs||"",K=qn(j.classWeek,e)||e&&B.indexOf(String(e))>=0;r.push({name:String(g).trim(),teacher:String(E).trim(),place:String(R).trim(),week:String(B).trim(),classWeek:String(j.classWeek||""),day:F,section:J,span:D,thisWeek:!!K,color:$n(g)})})})});let i=new Set;return r.filter(s=>{let l=[s.day,s.section,s.span,s.name,s.place,s.week].join("|");return i.has(l)?!1:(i.add(l),!0)})}a(pl,"parseScheduleFromJson");async function il(){try{let t=await Jt("/student/courseSelect/thisSemesterCurriculum/ajaxStudentSchedule/callback"),r=[],e=null;try{e=JSON.parse(t);let i=Number(e&&(e.jxzc||e.zc||e.currentWeek));i>=1&&i<=30&&(nt._termWeek=Math.max(nt._termWeek||0,i),nt.weekLocked||(nt.viewWeek=nt._termWeek)),r=pl(e)}catch{r=Ga(Be(t))}r.length||(r=Ga(document));let o=e?vn(e,Zr(e),"clean"):null;return{courses:r,exportData:o,rawOk:r.length>0,error:r.length?"":"课表 JSON 无 timeAndPlaceList"}}catch(t){try{let r=Ga(document);if(r.length)return{courses:r,rawOk:!0,error:""}}catch{}return{courses:[],rawOk:!1,error:String(t&&t.message||t)}}}a(il,"loadSchedule");function sl(t,r){let e=String(t||""),o=new RegExp(`url\\s*=\\s*["']([^"']*`+r+`[^"']*)["']`,"i"),i=e.match(o);if(i&&i[1])return i[1];let s=new RegExp(`(\\/student\\/integratedQuery\\/scoreQuery\\/[^"'\\s]+`+r+")","i"),l=e.match(s);return l?l[1]:""}a(sl,"extractScoreCallback");function ll(t){let r=[];return(t&&t.lnList||[]).forEach(o=>{let i=o.cjlx||o.cjbh||o.famc||o.zxjxjhh||"成绩",s=[];(o.cjList||[]).forEach(l=>{let h=l.courseName||l.englishCourseName||"";if(!h)return;let g=l.cj!=null&&l.cj!==""?String(l.cj):"";!g&&l.courseScore!=null&&(g=String(l.courseScore)),!g&&l.gradeName&&(g=String(l.gradeName)),!g&&l.zscj!=null&&(g=String(l.zscj));let E=l.courseAttributeName||l.xkcsxmc||"",M=parseFloat(l.credit)||0,j=l.id&&(l.id.courseNumber||l.id.kch_zj)||"",z=l.id&&(l.id.coureSequenceNumber||l.id.courseSequenceNumber||l.id.kxh)||l.classNo||"",F=l.gradePointScore!=null?Number(l.gradePointScore):null,J=ir(g)||ir(l.gradeName)||F!=null&&F<0,D=J?"未评估":g;s.push({code:j,seq:String(z||""),name:h,attr:E,credit:M,score:D,unevaluated:J,required:cn(E),officialGpa:Ie(F)?F:null,evalUrl:""})}),s.length&&r.push({title:String(i).slice(0,100),courses:s,summary:er(s),meta:{zxf:o.zxf,tgms:o.tgms,zms:o.zms,famc:o.famc}})}),r}a(ll,"parseScoreJson");async function Mn(t,r){let e=await Jt(t),o=In(Be(e));if(o.length)return o;let i=sl(e,r);if(!i)return[];let s=await Jt(i);try{let l=JSON.parse(s);o=ll(l).map(h=>(h.summary=er(h.courses),h))}catch{o=In(Be(s))}return o}a(Mn,"loadScoreByIndex");function In(t){let r=[];return t.querySelectorAll("table").forEach(e=>{let o=Array.from(e.tHead&&e.tHead.rows[0]?e.tHead.rows[0].cells:e.rows[0]&&e.rows[0].cells||[]).map(M=>(M.textContent||"").replace(/\s+/g,""));if(!o.length)return;let i=o.join("|");if(!/课程名/.test(i)||!/成绩/.test(i))return;let s={code:o.findIndex(M=>M==="课程号"),name:o.findIndex(M=>M==="课程名"),attr:o.findIndex(M=>/课程属性|属性/.test(M)),credit:o.findIndex(M=>M==="学分"),score:o.findIndex(M=>M==="成绩")};if(s.name<0||s.score<0)return;let l="成绩",h=e.previousElementSibling;for(let M=0;M<8&&h;M++,h=h.previousElementSibling)if(/^H[1-4]$/.test(h.tagName)||h.classList&&h.classList.contains("header")){l=(h.textContent||"").replace(/\s+/g," ").trim();break}let g=[],E=e.tBodies.length?e.tBodies[0].rows:Array.from(e.rows).slice(1);Array.from(E).forEach(M=>{let j=Array.from(M.cells||M.querySelectorAll("td"));if(j.length<4)return;let z=a(B=>B>=0&&j[B]?(j[B].textContent||"").replace(/\s+/g," ").trim():"","get"),F=z(s.name),J=z(s.score);if(!F||!J||/课程名|序号/.test(F))return;let D=z(s.attr),R=ir(J);g.push({code:z(s.code),name:F,attr:D,credit:parseFloat(z(s.credit))||0,score:R?"未评估":J,unevaluated:R,required:cn(D),officialGpa:null,evalUrl:""})}),g.length&&r.push({title:l.slice(0,100),courses:g,summary:er(g)})}),r}a(In,"parseScoreTables");function me(t){return Wr(t&&t.meta&&t.meta.famc||t&&t.title||"")}a(me,"schemePlanName");function Nn(t,r){if(!t||!t.length)return 0;let e=Wr(r),o=t.findIndex(l=>{let h=me(l);return/培养方案/.test(h)&&!/微专业|辅修|双学位/.test(h)});if(o>=0&&(!e||me(t[o]).includes(e.slice(0,4)))||e&&(o=t.findIndex(l=>{let h=me(l);return h.includes(e.replace(/培养方案.*/,"培养方案"))||e.includes(h.slice(0,4))||h.includes(e.slice(0,4))}),o>=0))return o;let i=0,s=-1;return t.forEach((l,h)=>{if(/微专业|辅修/.test(me(l)))return;let g=(l.courses||[]).length;g>s&&(s=g,i=h)}),i}a(Nn,"pickMajorSchemeIndex");async function cl(){let t={};try{let r=await Jt("/student/teachingAssessment/evaluation/queryAll",{method:"POST",data:"pageNum=1&pageSize=200&flag=kt",headers:{"Content-Type":"application/x-www-form-urlencoded; charset=UTF-8","X-Requested-With":"XMLHttpRequest"}}),e;try{e=JSON.parse(r)}catch{e=null}(e&&e.data&&e.data.records||[]).forEach(i=>{let s=String(i.KCH||"").trim();if(!s)return;let l=String(i.SFPG)==="1",h=String(i.KTID||"").trim();if(!t[s]){t[s]={ktid:h,kxh:String(i.KXH||""),kcm:i.KCM||"",done:l,pending:l?0:1,total:1,url:!l&&h?"/student/teachingEvaluation/newEvaluation/evaluation/"+h:"/student/teachingEvaluation/newEvaluation/index"};return}t[s].total+=1,l||(t[s].pending+=1,t[s].done=!1,h&&(t[s].ktid=h,t[s].url="/student/teachingEvaluation/newEvaluation/evaluation/"+h))}),Object.keys(t).forEach(i=>{let s=t[i];s.done=!(s.pending>0)})}catch(r){console.warn("[URP++] evaluation map",r)}return t}a(cl,"loadEvaluationMap");function dl(t){if(!t)return!1;if(t.officialGpa!=null&&Ie(t.officialGpa))return!0;let r=t.score;return r==null||r===""||ir(r)?!1:Rr(r)!=null||Hr(r)!=null?!0:!/未评估|未评教|待评估|待评教/.test(String(r))}a(dl,"hasDisplayableScore");function ul(t,r){if(!t||!r)return t;let e=a(o=>(o||[]).forEach(i=>{if(!i||!i.code)return;let s=r[i.code];if(s){if(dl(i)){i.unevaluated=!1,s.done?i.evalUrl=i.evalUrl||"":i.evalUrl=s.url||"/student/teachingEvaluation/newEvaluation/index";return}s.done||(i.unevaluated=!0,i.evalUrl=s.url||"/student/teachingEvaluation/newEvaluation/index",(!i.score||i.score===""||ir(i.score))&&(i.score="未评估"))}}),"apply");return(t.passing||[]).forEach(o=>e(o.courses)),(t.schemes||[]).forEach(o=>e(o.courses)),t}a(ul,"attachEvaluationLinks");function Bn(t){return t&&(t.passing&&t.passing[0]&&(t.passing[0].summary=er(t.passing[0].courses)),t.schemes=(t.schemes||[]).map(r=>(r.summary=er(r.courses),r)),t)}a(Bn,"refreshScoreSummaries");async function ml(t){if(!t||t.evaluationLoading)return t;t.evaluationLoading=!0;try{let r=await cl();return ul(t,r),t.evalMap=r,t.evaluationReady=!0,Bn(t)}finally{t.evaluationLoading=!1}}a(ml,"enrichScoresWithEvaluation");function bl(){if(!nt.scores||!nt.scores.schemes)return;let t=nt.scores.schemes,r=nt.profile&&nt.profile.majorPlan,e=Nn(t,r);nt.scores.majorIdx=e,nt._schemeUserSelected||(nt.activeSchemeIdx=e,nt._schemeInited=!0);let o=t[e];if(!o||!nt.profile)return;let i=me(o),s=Wr(nt.profile.majorPlan);/培养方案|教学计划/.test(i)&&(!/培养方案|教学计划/.test(s)||s==="主修方案")&&(nt.profile.majorPlan=i);let l=o.summary||{},h=Number(l.requiredCredit),g=Number(l.requiredGpa),E=Number(nt.profile.majorGpa);h>0&&Number.isFinite(g)&&g>=0&&g<=5&&(!Number.isFinite(E)||E<=0)&&(nt.profile.majorGpa=String(Ar(g)))}a(bl,"reconcileProfileAndScores");let Vr=null;async function Fn(t){return t&&(Vr=null),Vr&&!Vr.error||(Vr=await hl()),Vr}a(Fn,"loadScores");async function hl(){let t={passing:[],schemes:[],error:"",majorIdx:0,evaluationReady:!1,evaluationLoading:!1};try{let[r,e]=await Promise.all([Mn("/student/integratedQuery/scoreQuery/allPassingScores/index","allPassingScores/callback"),Mn("/student/integratedQuery/scoreQuery/schemeScores/index","schemeScores/callback")]),o=[];r.forEach(i=>i.courses.forEach(s=>{o.push(Object.assign({term:i.title},s))})),t.passing=[{title:"全部及格成绩",courses:o,summary:er(o),groups:r}],t.schemes=e,!t.schemes.length&&o.length&&(t.schemes=[{title:"方案成绩",courses:o,summary:er(o)}]),Bn(t),t.majorIdx=Nn(t.schemes,nt.profile&&nt.profile.majorPlan),!o.length&&!t.schemes.length&&(t.error="成绩 callback 无数据")}catch(r){t.error=String(r&&r.message||r)}return t}a(hl,"loadScoresImpl");function Er(t){if(!t)return[];let r=String(t).trim();if(!r)return[];r=r.replace(/^['"]|['"]$/g,"");try{return JSON.parse(r)}catch{}try{return JSON.parse(r.replace(/&quot;/g,'"').replace(/&#34;/g,'"'))}catch{}return[]}a(Er,"parseJsonArrayLoose");function Dn(t,r){let e=t.indexOf(r);if(e<0)return"";let o=t.indexOf("[",e);if(o<0)return"";let i=0;for(let s=o;s<t.length&&s<o+3e5;s++){let l=t[s];if(l==="[")i++;else if(l==="]"&&(i--,i===0))return t.slice(o,s+1)}return""}a(Dn,"extractBalancedArray");async function gl(){let t=await Jt("/student/teachingResources/classroomUseStatus/index");if(/欢迎登录|name=["']j_username["']|loginEn/i.test(t)&&!/jxlList|teachingBuildingName|classroomUseStatus/i.test(t))throw new Error("登录已失效，请刷新页面后重试");let r=[],e=[];try{let s=(t.match(/id=["']xqList["'][^>]*value=["']([^"']*)["']/i)||t.match(/name=["']xqList["'][^>]*value=["']([^"']*)["']/i)||[])[1],l=(t.match(/id=["']jxlList["'][^>]*value=["']([^"']*)["']/i)||t.match(/name=["']jxlList["'][^>]*value=["']([^"']*)["']/i)||[])[1];if(s&&(r=Er(s)),l&&(e=Er(l)),!r.length){let h=t.match(/(?:var\s+)?xqList\s*=\s*(\[[\s\S]*?\])\s*;/);h&&(r=Er(h[1]))}if(!e.length){let h=t.match(/(?:var\s+)?jxlList\s*=\s*(\[[\s\S]*?\])\s*;/);h&&(e=Er(h[1]))}if(!e.length){let h=Dn(t,"teachingBuildingName");h&&(e=Er(h))}if(!r.length){let h=Dn(t,"campusName");h&&(r=Er(h))}}catch(s){console.warn("[URP++] classroom json parse",s)}if(!e.length){let s=[{campusNumber:"01",campusName:"望江"},{campusNumber:"02",campusName:"华西"},{campusNumber:"03",campusName:"江安"}];r=s;let l=[];for(let h of s)try{let g=await Jt("/student/teachingResources/classroomCurriculum/"+h.campusNumber+"/teachingBuildingJson");Er(g).forEach(M=>{l.push({id:{campusNumber:h.campusNumber,teachingBuildingNumber:String(M.id&&M.id.teachingBuildingNumber||M.teachingBuildingNumber||"")},teachingBuildingName:M.teachingBuildingName||M.name||""})})}catch(g){console.warn("[URP++] building json",h.campusNumber,g)}e=l}r.length||(r=[{campusNumber:"01",campusName:"望江"},{campusNumber:"02",campusName:"华西"},{campusNumber:"03",campusName:"江安"}]);let o=r.map(s=>({campus:s.campusName||s.campusNumber,campusNumber:String(s.campusNumber||s.id&&s.id.campusNumber||""),buildings:[]}));e.forEach(s=>{let l=String(s.id&&s.id.campusNumber||s.campusNumber||""),h=String(s.id&&s.id.teachingBuildingNumber||s.teachingBuildingNumber||""),g=s.teachingBuildingName||s.name||h;if(!l||!h||!g)return;let E=o.find(j=>j.campusNumber===l);E||(E={campus:l,campusNumber:l,buildings:[]},o.push(E));let M="/student/teachingResources/classroomUseStatus/"+l+"/"+h+"/"+encodeURI(encodeURI(E.campus||l))+"/"+encodeURI(encodeURI(g));E.buildings.push({name:g,path:M,campusNumber:l,buildingNumber:h})});let i=o.filter(s=>s.buildings.length);if(!i.length)throw new Error("未解析到教学楼，请刷新后重试");return i}a(gl,"loadClassroomCatalog");function be(t){let r=String(t&&t.occupancymoduleId||""),e={"06":"有课","07":"考试",14:"实验",room:"借用"};if(e[r])return e[r];if(t&&t.remark){let o=String(t.remark).trim();if(o)return o}return"占用"}a(be,"occupancyTypeLabel");function fl(t){if(t&&t.contentName)return String(t.contentName).trim();if(t&&t.remark){let r=String(t.remark).trim();if(r)return r}return be(t)}a(fl,"occupancyReason");async function xl(t,r,e,o){let i=new URLSearchParams({planNumber:String(t||""),campusNumber:String(r||""),teachingBuildingNumber:String(e||""),classroomNumber:String(o||"")}),s=await Jt("/student/teachingResources/classroomCurriculum/searchCurriculum/callback?"+i.toString());try{let l=JSON.parse(s);return Array.isArray(l)?l.length&&Array.isArray(l[0])?l[0]:l.filter(h=>h&&typeof h=="object"&&(h.kcm||h.id&&h.id.kch)):l&&Array.isArray(l.list)?l.list:[]}catch{return[]}}a(xl,"fetchClassroomCurriculum");function yl(t,r,e){let o=t||[],i=Number(r.xq)||0,s=Number(r.start)||0,l=Number(e)||0,h=[];return o.forEach(g=>{let E=g.id||{},M=Number(E.skxq!=null?E.skxq:g.skxq)||0,j=Number(E.skjc!=null?E.skjc:g.skjc)||0,z=Math.max(1,Number(g.cxjc)||1),F=E.skzc||g.skzc||"";i&&M&&i!==M||s&&(s<j||s>=j+z)||l&&F&&!Ia(F,l)||h.push(g)}),h.length?(h.sort((g,E)=>{let M=Ia(g.id&&g.id.skzc||g.skzc,l)?0:1,j=Ia(E.id&&E.id.skzc||E.skzc,l)?0:1;return M-j}),h[0]):null}a(yl,"matchCurriculumCourse");async function vl(t,r,e){if(!t||!t.rooms||!t.rooms.length)return t;let o=String(r.campusNumber||""),i=String(r.buildingNumber||""),s=e||t.planNumber||"";if(!o||!i||!s)return t;let l=t.rooms.filter(z=>(z.slots||[]).some(F=>F.busy)),h={},g=a(async z=>{if(h[z])return h[z];try{h[z]=await xl(s,o,i,z)}catch{h[z]=[]}return h[z]},"queue"),E=4,M=0,j=new Array(Math.min(E,Math.max(l.length,1))).fill(0).map(async()=>{for(;M<l.length;){let z=M++,F=l[z],J=await g(F.name);(F.slots||[]).forEach(D=>{if(!D.busy)return;let R={xq:D.detail&&D.detail.xq||D.xq||0,start:D.section,week:t.jxzc};D.detail&&D.detail.xq!=null&&(R.xq=D.detail.xq);let B=yl(J,R,t.jxzc);if(B&&B.kcm){let K=String(B.kcm).trim();D.contentName=K,D.reason=K,D.displayChar=Ne(K),D.detail&&(D.detail.contentName=K,D.detail.reason=K,D.detail.teacher=B.jsm||"",D.detail.weeks=B.zcsm||"",D.detail.courseNo=B.id&&B.id.kch||"",D.detail.typeLabel=be({occupancymoduleId:D.module}))}else D.displayChar=Ne(D.reason||"占用"),D.detail&&(D.detail.typeLabel=be({occupancymoduleId:D.module}))})}});return await Promise.all(j),t}a(vl,"enrichOccupancyWithCurriculum");function wl(t){return t==="有课"?"kind-course":t==="考试"?"kind-exam":t==="实验"?"kind-lab":t==="借用"?"kind-borrow":"kind-busy"}a(wl,"occupancyKindClass");async function kl(t){let r="",e="",o="",i="";if(t&&typeof t=="object")r=String(t.campusNumber||""),e=String(t.buildingNumber||""),o=t.name||"",i=t.path||"";else{i=String(t||"");let B=i.match(/classroomUseStatus\/(\d+)\/(\d+)\//);B&&(r=B[1],e=B[2])}if(!r||!e)throw new Error("缺少校区/楼栋编号");let s=Number(t&&t.dateOffset!=null?t.dateOffset:nt.roomDateOffset)||0,l=Al(jn(new Date,s)),h="xqh="+encodeURIComponent(r)+"&jxlh="+encodeURIComponent(e)+"&jslx=&jasm=&zwFrom=&zwTo=&searchDate="+encodeURIComponent(l),g=await new Promise((B,K)=>{let ut=Na("/student/teachingResources/classroomUseStatus/jasInfo");typeof GM_xmlhttpRequest=="function"?GM_xmlhttpRequest({method:"POST",url:ut,data:h,withCredentials:!0,headers:{"Content-Type":"application/x-www-form-urlencoded; charset=UTF-8","X-Requested-With":"XMLHttpRequest"},onload:a(vt=>vt.status>=200&&vt.status<400?B(vt.responseText||""):K(new Error("HTTP "+vt.status)),"onload"),onerror:a(()=>K(new Error("network")),"onerror")}):fetch(ut,{method:"POST",credentials:"include",headers:{"Content-Type":"application/x-www-form-urlencoded; charset=UTF-8","X-Requested-With":"XMLHttpRequest"},body:h}).then(vt=>vt.text()).then(B).catch(K)}),E;try{E=JSON.parse(g)}catch{throw new Error("jasInfo 非 JSON")}let M=(E.classrooms||[]).map(B=>{let K=B.classroomName||B.id&&B.id.classroomNumber||"",ut=B.placeNum||"",vt=B.remark||"",Et=[];for(let Lt=1;Lt<=12;Lt++)Et.push({section:Lt,busy:!1});return{name:K,seats:ut,type:vt,slots:Et,map:{}}}),j={};M.forEach(B=>{j[B.name]=B}),(E.classroomTime||[]).forEach(B=>{let K=B.id||{},ut=K.classroomNumber||"",vt=Number(K.sessionstart)||1,Et=Math.max(1,Number(B.continuingsession)||1),Lt=j[ut];if(!Lt)return;let qt=be(B),Ot=fl(B);for(let Ht=vt;Ht<vt+Et&&Ht<=12;Ht++){let $t=Lt.slots.find(ar=>ar.section===Ht);$t&&($t.busy=!0,$t.kind=B.timestatenumber||B.occupancymoduleId||"",$t.module=B.occupancymoduleId||"",$t.reason=Ot,$t.typeLabel=qt,$t.displayChar=Ne(Ot),$t.xq=K.xq,$t.weekBitmap=K.week||"",$t.detail={room:ut,section:Ht,start:vt,span:Et,reason:Ot,typeLabel:qt,week:K.week||"",xq:K.xq||"",state:B.timestatenumber||"",module:B.occupancymoduleId||""})}});let z="";try{let B=E.jhZxjxjhb;typeof B=="string"&&/\d{4}-\d{4}-\d-\d/.test(B)?z=B:B&&typeof B=="object"&&(z=String(B.zxjxjhh||B.jhxnxq||B.executiveEducationPlanNumber||B.planNumber||""))}catch{}if(!z&&E.classrooms&&E.classrooms[0]&&E.classrooms[0].id&&(z=E.classrooms[0].id.executiveEducationPlanNumber||""),E.jxzc!=null&&Number(E.jxzc)>=1){let B=Number(E.jxzc);nt._termWeek=Math.max(nt._termWeek||0,B),nt.weekLocked||(nt.viewWeek=nt._termWeek)}let F=["日","一","二","三","四","五","六"],J=Sl(E.date||l)||jn(new Date,s),D=E.week!=null?Number(E.week):J.getDay(),R=s===1?"明天":s===2?"后天":"今天";return{rooms:M,dateLabel:(E.date||l)+"（周"+(F[D]||D)+" · "+R+"）",jxzc:E.jxzc,planNumber:z,week:E.week!=null?E.week:D,searchDate:E.date||l,dateOffset:s}}a(kl,"loadBuildingOccupancy");function jn(t,r){let e=new Date(t.getFullYear(),t.getMonth(),t.getDate());return e.setDate(e.getDate()+(Number(r)||0)),e}a(jn,"addDays");function Al(t){return t.getFullYear()+"-"+String(t.getMonth()+1).padStart(2,"0")+"-"+String(t.getDate()).padStart(2,"0")}a(Al,"formatLocalDate");function Sl(t){let r=String(t||"").match(/(\d{4})-(\d{1,2})-(\d{1,2})/);return r?new Date(Number(r[1]),Number(r[2])-1,Number(r[3])):null}a(Sl,"parseLocalDate");let On={clean:'<svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round"><path d="M4 7h16M4 12h11M4 17h14"/></svg>',exit:'<svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round"><path d="M9 6H6a2 2 0 0 0-2 2v8a2 2 0 0 0 2 2h3"/><path d="M14 12H8"/><path d="m14 8 4 4-4 4"/></svg>',refresh:'<svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round"><path d="M20 12a8 8 0 1 1-2.2-5.5"/><path d="M20 4v5h-5"/></svg>',schedule:'<svg viewBox="0 0 24 24" width="22" height="22" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round"><rect x="3.5" y="5" width="17" height="15" rx="2"/><path d="M3.5 10h17M8 3.5v4M16 3.5v4"/></svg>',score:'<svg viewBox="0 0 24 24" width="26" height="26" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round"><path d="M7 3.5h10v17H7z"/><path d="M10 8h4M10 12h4M10 16h3"/></svg>',room:'<svg viewBox="0 0 24 24" width="26" height="26" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round"><path d="M4 20V8l8-4 8 4v12"/><path d="M9 20v-7h6v7"/><path d="M9 10h.01M15 10h.01"/></svg>',eval:'<svg viewBox="0 0 24 24" width="22" height="22" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round"><path d="M8 7h11M8 12h11M8 17h8"/><path d="M5 7h.01M5 12h.01M5 17h.01"/></svg>',plan:'<svg viewBox="0 0 24 24" width="22" height="22" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round"><path d="M7 3.5h8l3 3V20.5H7z"/><path d="M15 3.5V7h3M10 12h5M10 16h5"/></svg>',apply:'<svg viewBox="0 0 24 24" width="22" height="22" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round"><rect x="6" y="3.5" width="12" height="17" rx="2"/><path d="M9 8h6M9 12h6M9 16h4"/></svg>',home:'<svg viewBox="0 0 24 24" width="26" height="26" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round"><path d="m4 11 8-7 8 7"/><path d="M7 10.5V20h10v-9.5"/></svg>',more:'<svg viewBox="0 0 24 24" width="26" height="26" fill="none" stroke="currentColor" stroke-width="1.7"><circle cx="6" cy="12" r="1.6"/><circle cx="12" cy="12" r="1.6"/><circle cx="18" cy="12" r="1.6"/></svg>',close:'<svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round"><path d="M6 6l12 12M18 6 6 18"/></svg>'};function Hn(t){return On[t]||On.more}a(Hn,"ico");let nt=Kp();function _l(){if(document.getElementById("urppp-clean-style"))return;let t=document.createElement("style");t.id="urppp-clean-style",t.textContent=Vp,(document.head||document.documentElement).appendChild(t)}a(_l,"ensureStyle");let El=ra({deps:{scoreToNumber:Rr,scoreToGpa:Hr}}),{metricHtml:Cl,occupancyHtml:Pl,render:Yr,renderScheduleBoard:Ac,roomPickerHtml:zl,scheduleRender:Ll}=ci({state:nt,deps:{DIRECT_EDIT_LABELS:U,DAY_NAMES:En,analyzeScores:a(t=>El.analyzeScores(t),"analyzeScores"),applyPersonalDisplay:Vt,bandsChartSvg:aa,bindUI:a(t=>Tl(t),"bindUI"),classifyPrivacyLabel:Fe,courseColor:$n,ensureRoot:a(()=>Un(),"ensureRoot"),escapeHtml:at,firstContentChar:Ne,getViewWeekNumber:Ln,ico:Hn,isCleanAnalysisDirect:ia,occupancyKindClass:wl,occupancyTypeLabel:be,personalizedProfile:$s,scoreChartLayout:a(()=>{try{return window.matchMedia&&window.matchMedia("(max-width: 900px)").matches?{variant:"mobile"}:null}catch{return null}},"scoreChartLayout"),scoreToNumber:Rr,summarizeCourses:er,trendChartSvg:ea,weekBitActive:qn,calVacation:ue,setCalendarPhase:zn}}),{ensureRoomCatalogLoaded:Rn,loadAll:ql}=ti({state:nt,deps:{ensureTermWeekResolved:ol,enrichScoresWithEvaluation:ml,getCurrentWeekNumber:_r,loadClassroomCatalog:gl,loadProfile:_n,loadSchedule:il,loadScores:Fn,readRememberedTermWeek:ce,reconcileProfileAndScores:bl,render:Yr,scheduleRender:Ll}}),{bindUI:Tl,closeModal:$l,getRoomHost:Sc,openModal:_c,openRoomModal:Ec,openScoreModal:Cc,showBuilding:Pc}=di({state:nt,deps:{DAY_NAMES:En,applyPersonalDisplay:Vt,bindScheduleExportHosts:Ks,closeCleanMode:a(()=>Il(),"closeCleanMode"),ensureRoomCatalogLoaded:Rn,enrichOccupancyWithCurriculum:vl,ensureRoot:a(()=>Un(),"ensureRoot"),escapeHtml:at,fetchText:Jt,getCurrentWeekNumber:_r,getViewWeekNumber:Ln,inferMaxWeek:nl,isUnevaluatedScore:ir,isValidOfficialGpa:Ie,loadBuildingOccupancy:kl,metricHtml:Cl,occupancyHtml:Pl,render:Yr,rootEl:a(()=>Nl(),"rootEl"),roomPickerHtml:zl,scoreToGpa:Hr,scoreToNumber:Rr,summarizeCourses:er,summarizeCoursesPreferOfficial:er}}),{cleanModeApi:Ml,closeCleanMode:Il,ensureRoot:Un,injectCleanEntry:zc,openCleanMode:Lc,rootEl:Nl}=ui({state:nt,deps:{CLEAN_FLAG:fs,applySkinAttr:rr,closeModal:$l,ensureRoomCatalogLoaded:Rn,ensureStyle:_l,getCurrentWeekNumber:_r,getSkin:Zt,handleThemeDotClick:ct,ico:Hn,injectCleanSidebarSections:a(t=>{try{window.__urpppInjectCleanSidebarSections?.(t)}catch{}},"injectCleanSidebarSections"),refreshMobileNavbar:a(()=>{try{window.__urpppRefreshMobileNavbar?.()}catch{}},"refreshMobileNavbar"),setDrawerOpen:a((t,r,e)=>{try{window.__urpppSetDrawerOpen?.(t,r,e)}catch{}},"setDrawerOpen"),stopDrawerAnimation:a(t=>{try{window.__urpppStopDrawerAnimation?.(t)}catch{}},"stopDrawerAnimation"),isHomePage:go,loadAll:ql,openSettingsPanel:Go,readRememberedTermWeek:ce,refreshCleanPersonalDisplay:Fa,render:Yr,scoreToGpa:Hr,summarizeCourses:er,syncNavbarThemeUI:ft,syncSettingsPanelUI:Dt,syncThemeDotGroup:Z}});window.__urpppCleanMode=Ml;function Ja(){if(!document.body){setTimeout(Ja,10);return}if(xo(),Wt(Qt()),document.addEventListener("focusin",r=>{let e=r.target;if(!e||!e.matches||!e.matches(".chosen-search input"))return;let o=[],i=e.parentElement;for(;i;){let s=i.scrollTop,l=i.scrollLeft;(s||l||i.scrollHeight>i.clientHeight||i.scrollWidth>i.clientWidth)&&o.push({el:i,top:s,left:l}),i=i.parentElement}requestAnimationFrame(()=>{o.forEach(s=>{s.el.scrollTop=s.top,s.el.scrollLeft=s.left})})},!0),!!document.getElementById("formContent")&&!!document.querySelector(".form-signin"))So();else{Ui();try{dn()}catch{}try{Oe()}catch(r){console.warn("[URP++] route feature refresh",r)}try{Vt(document)}catch{}try{Pn()}catch{}[350,900,1800].forEach(r=>setTimeout(()=>{try{Oe()}catch{}try{Vt(document)}catch{}},r));try{window.__urpppCleanMode&&window.__urpppCleanMode.inject()}catch{}[400,1200,2500].forEach(r=>setTimeout(()=>{try{window.__urpppCleanMode&&window.__urpppCleanMode.inject()}catch{}},r));try{pa()&&go()&&window.__urpppCleanMode&&setTimeout(()=>{try{window.__urpppCleanMode.open(!1)}catch{}},700)}catch{}}}if(a(Ja,"init"),!window.__urpppSidebarSyncBound){window.__urpppSidebarSyncBound=!0,window.addEventListener("resize",()=>{clearTimeout(window.__urpppSidebarSyncTimer),window.__urpppSidebarSyncTimer=setTimeout(kr,50)}),window.addEventListener("load",()=>{kr(),Or(),setTimeout(kr,100),setTimeout(kr,400)}),document.addEventListener("click",r=>{r.target&&r.target.closest&&r.target.closest("#menu-toggler, .menu-toggler, .navbar-toggle, .urppp-sidebar-toggle, .sidebar-collapse, #sidebar-collapse")&&(setTimeout(Or,0),setTimeout(Or,50),setTimeout(Or,200))},!0);let t=document.getElementById("sidebar");t&&!t.__urpppMarginObs&&(t.__urpppMarginObs=new MutationObserver(()=>{clearTimeout(window.__urpppMarginObsTimer),window.__urpppMarginObsTimer=setTimeout(Or,30)}),t.__urpppMarginObs.observe(t,{attributes:!0,attributeFilter:["class","style"]}))}function Wn(){if(window.__urpppRouteWatchBound)return;window.__urpppRouteWatchBound=!0;let t=0,r=a(()=>{try{let i=!!(window.matchMedia&&window.matchMedia("(max-width: 640px)").matches),s=!!(document.getElementById("urppp-clean-root")&&document.getElementById("urppp-clean-root").classList.contains("open"));i&&!s&&window.__urpppCloseMobileDrawer&&window.__urpppCloseMobileDrawer()}catch{}clearTimeout(t),t=setTimeout(()=>{if(nt._termWeekResolved=!1,!!document.getElementById("sidebar")){kr(),ln(),st(),kr();try{window.__urpppRefreshMobileNavbar&&window.__urpppRefreshMobileNavbar()}catch{}dt(),[250,700].forEach(s=>setTimeout(()=>{try{window.__urpppRefreshMobileNavbar&&window.__urpppRefreshMobileNavbar()}catch{}},s)),xa(),Ho(),Ro(),Mo(),Uo(),Oo(),No(),document.querySelectorAll(".page-content, #page-content-template").forEach(s=>{let l=!!(window.matchMedia&&window.matchMedia("(max-width: 991px)").matches);s.style.setProperty("padding",l?"8px 8px 24px":"16px 64px 40px","important"),s.style.setProperty("box-sizing","border-box","important")}),ze(),Te(),Nr(),setTimeout(Nr,300),setTimeout(Nr,1e3),Po(),mr(),oe(),qo(),setTimeout(oe,300),ne(),setTimeout(()=>ne(),500),Pe(),Lo();try{Oe()}catch{}try{Vt(document)}catch{}setTimeout(()=>{try{Oe()}catch{}try{Vt(document)}catch{}},500)}},100)},"run");window.addEventListener("popstate",r),window.addEventListener("hashchange",r);let e=history.pushState,o=history.replaceState;history.pushState=function(...i){let s=e.apply(this,i);return r(),s},history.replaceState=function(...i){let s=o.apply(this,i);return r(),s}}a(Wn,"watchRouteChanges");let Qr=typeof unsafeWindow<"u"?unsafeWindow:window;Qr.__urpppDebug=Qr.__urpppDebug||{},Qr.__urpppDebug.setCalendarPhase=t=>zn(t),Qr.__urpppDebug.getCalendarPhase=()=>al(),Qr.__urpppDebug.calVacation=t=>ue(t),Qr.urppp={version:p,showLogo(t){let r=document.querySelector("#urppp-brand .ub-logo");r&&r.classList.toggle("show",t)},theme:{apply:a(t=>{Wt(t)},"apply"),setAccent:_i,getAccent:Yt,getCurrent:Qt,list:a(()=>Object.entries(Ct).map(([t,r])=>({name:t,displayName:r.name,current:t===Qt()})),"list")},update:{check:$a,auto:Ta,showToast:qa},privacy:{get:yr,set(t){return la(t),Vt(document),yr()},apply:a(()=>Vt(document),"apply"),identity:{get:qr,set(t){return mo(t),Vt(document),Fa(),qr()}}},scheduleExport:{load:a(()=>xn("api"),"load"),run:a(t=>Rs(t,"api",null,null),"run"),patch:Ua,image:{theme:wn,build:a((t,r)=>kn(t,r),"build")},jsonFormat:{get:Ee,set:ho,validate:Pr,build(t,r){let e=Ge(t);if(r)return Ve(e,Pr(r));let o=Ee();return o.enabled?Ve(e,o.mapping):Je(e)},buildDefault(t){return Je(Ge(t))}}}};function Gn(){setTimeout(()=>{try{Ta()}catch{}},1800)}a(Gn,"scheduleAutoUpdateCheck");try{xo()}catch{}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",()=>{Ja(),Wn(),Gn()}):(Ja(),Wn(),Gn())})();})();
