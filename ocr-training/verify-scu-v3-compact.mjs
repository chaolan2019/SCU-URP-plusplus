...
function recognizeFull(model, feat) {
...
function recognizeCompact(model, feat) {
  const bytes = Buffer.from(model.tpl, 'base64');
  const step = 49;
  let best = '?', bestD = Infinity;
  for (let i = 0; i < model.chars.length; i++) {
    const t = [];
    for (let j = 0; j < step; j++) t.push(bytes[i * step + j] / 255);
    const d = dist(feat, t);
    if (d < bestD) { bestD = d; best = model.chars[i]; }
  }
  return best;
}

const fullModel = JSON.parse(readFileSync(path.join(ROOT, 'model', 'scu-id-v3.json'), 'utf8'));
const compactModel = JSON.parse(readFileSync(path.join(ROOT, 'model', 'scu-id-v3-compact.json'), 'utf8'));
...
let total = 0, hitFull = 0, hitCompact = 0, mismatch = 0;
for (const [id, label] of Object.entries(cleanLabels)) {
  ...
  if (feats.length !== 4) continue;
  let tFull = '', tCompact = '';
  for (const feat of feats) {
    tFull += recognizeFull(fullModel, feat);
    tCompact += recognizeCompact(compactModel, feat);
  }
  total++;
  if (tFull === label) hitFull++;
  if (tCompact === label) hitCompact++;
  if (tFull !== tCompact) mismatch++;
}

console.log(`v3 全精度 vs v3 compact（${total} 张）:`);
console.log(`全精度: ${(hitFull / total * 100).toFixed(2)}% (${hitFull}/${total})`);
console.log(`compact: ${(hitCompact / total * 100).toFixed(2)}% (${hitCompact}/${total})`);
console.log(`预测不一致: ${mismatch} 张 ${mismatch === 0 ? '✓ 无损' : '✗ 有差异'}`);
