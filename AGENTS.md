...
UPR++ 是四川大学 URP 教务系统的油猴脚本（Tampermonkey 用户脚本），对整个教务系统进行全面的 UI 美化与交互优化。采用自建主题系统（简约白 · 深邃暗 · 川大红），支持实时切换。
...
```
E:\Coder-WorkSpace\UPR++\
├── uprpp.user.js          # 主脚本（唯一运行文件）
├── AGENTS.md              # 本文档
└── docs/                  # 截图、笔记等辅助材料
```
...
主题存储在 `GM_getValue('uprpp_theme_v3')`，登录页底部有主题切换点。

**强调色**：`GM_getValue('uprpp_accent_v1')` 可自定义主题色，通过 `applyAccent(hex)` 设置。
...
完全替换 ACE 原生的 `#menus`，生成 Hanako 风格的 `#uprpp-menus`。
...
### 7. 全站 CSS（`uprpp-internal-style`）

**注入位置**：`beautifyInternal()`，仅注入一次（通过 `uprpp-internal-style` id 检测）
...
6. **全站美化 CSS 范围**：当前 `uprpp-internal-style` 覆盖较为全面，但某些页面特有组件（如分页下拉 `.dataTables_length`）未完全覆盖。
