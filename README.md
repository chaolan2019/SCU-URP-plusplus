...
- **默认视觉**：简约白 / 深邃暗 按 **Apple 风格**精修（系统灰底、链接蓝、大圆角、轻阴影、胶囊按钮）；动态配色仍可自选种子色
- **三套配色模式**：简约白 / 深邃暗 / 动态配色；可跟随系统浅色/深色
- **界面风格（主题选择）**：设置内预览多套风格卡片（Apple / 极简扁平 / 自然有机 / 新野兽派 / 编辑杂志 / 新拟物）；当前仅 Apple 可应用，其余开发中
- **清爽模式**（首页）：
  ...
  - 手机底栏：首页 / 成绩 / 教室 / 其他
- **设置面板（四选项卡）**：
  - **主题设置**：配色模式、跟随系统、种子色与配色方案（默认打开此页）
  - **主题选择**：界面风格大卡片
  - **系统设置**：默认进入清爽、自动/手动检查更新；辅助插件入口挂载于此
  - **关于**：Logo、版本与作者
...
- 顶栏主题圆点切换配色模式；齿轮打开设置
- 设置默认进入 **主题设置**：跟随系统、动态种子色与方案
- **主题选择** 可预览多套界面风格（仅 Apple 已启用）
- **系统设置** 中可开「默认进入清爽模式」「自动检测更新」，并管理辅助插件
- **关于** 查看版本与作者
...
- 默认皮肤向 Apple Soft Utility 收敛；后续多风格以 Skin Token 扩展（见 `Design/`）
...
1. 读取 `urppp.user.js` 的 `@version`（如 `1.0.8`）
2. 创建 tag `v1.0.8`
3. 发布 GitHub Release  
