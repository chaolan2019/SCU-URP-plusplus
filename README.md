<div align="center">
  ...
  <p><strong>主脚本 v1.6.0</strong> · 辅助插件 v1.4.0</p>
...
4. **更新检测与跨域授权提示**：开启「自动检测更新」后，首次加载教务页面会静默检查新版本；点击「检查更新」或首次触发更新检测时，Tampermonkey 可能弹出跨域访问确认（更新源为 `github.com` / `cdn.jsdelivr.net` / `gh-proxy.com` 三个域名）。允许一次后 Tampermonkey 会记住授权，之后不再提示。若希望完全免提示，可关闭「自动检测更新」，或在 Tampermonkey 的脚本设置 → 连接中预授权上述域名。
